...
public class CILicenseUsageResourceImpl implements CILicenseUsageResource {
  ...
  @NGAccessControlCheck(resourceType = LICENSE, permission = VIEW_LICENSE_PERMISSION)
  public ResponseDTO<Page<ActiveDevelopersDTO>> ciLicenseUsage(@AccountIdentifier String accountIdentifier, int page,
      int size, List<String> sort, long currentTsInMs, CIDevelopersFilterParams filterParams) {
  ...
  @NGAccessControlCheck(resourceType = LICENSE, permission = VIEW_LICENSE_PERMISSION)
  public Response downloadActiveDevelopersCSVReport(@AccountIdentifier String accountIdentifier, long currentTsInMs) {
    currentTsInMs = fixOptionalCurrentTs(currentTsInMs);
    File file = ciLicenseUsageService.getLicenseUsageCSVReport(
        accountIdentifier, ModuleType.CI, currentTsInMs, Optional.empty());
  ...
  @NGAccessControlCheck(resourceType = LICENSE, permission = VIEW_LICENSE_PERMISSION)
  public ResponseDTO<Set<String>> listActiveDevelopers(
      @AccountIdentifier String accountIdentifier, long currentTsInMs) {
  ...
  public ResponseDTO<CILicenseHistoryDTO> getLicenseHistoryUsage(
      @AccountIdentifier String accountIdentifier, CILicenseType licenseType, CIDevelopersFilterParams filterParams) {
    return ResponseDTO.newResponse(
        ciLicenseUsageService.getLicenseHistoryUsage(accountIdentifier, licenseType, filterParams));
  }

  @NGAccessControlCheck(resourceType = LICENSE, permission = VIEW_LICENSE_PERMISSION)
  public ResponseDTO<CILicense> getCILicense(@AccountIdentifier String accountIdentifier) {
    return ResponseDTO.newResponse(ciLicenseUsageService.getCILicense(accountIdentifier));
  }

...
if [[ "" != "$VM_GAR_PUSH_IMAGE" ]]; then
  export VM_GAR_PUSH_IMAGE; yq -i '.ciExecutionServiceConfig.stepConfig.vmImageConfig.buildAndPushGAR=env(VM_GAR_PUSH_IMAGE)' $CONFIG_FILE
fi

if [[ "" != "$VM_ACR_PUSH_IMAGE" ]]; then
  export VM_ACR_PUSH_IMAGE; yq -i '.ciExecutionServiceConfig.stepConfig.vmImageConfig.buildAndPushACR=env(VM_ACR_PUSH_IMAGE)' $CONFIG_FILE
fi
...
if [[ "" != "$SHOULD_DEREGISTER_WITH_PMS" ]]; then
  export SHOULD_DEREGISTER_WITH_PMS; yq -i '.shouldDeregisterWithPMS=env(SHOULD_DEREGISTER_WITH_PMS)' $CONFIG_FILE
fi
...
replace_key_value featureFlagConfig.fmeConfig.splitToken "$SPLIT_TOKEN"

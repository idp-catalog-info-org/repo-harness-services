...
public class K8InitializeTaskUtilsHelper {
  ...
  private static final String HAR_REGISTRY_REF = "registry";
  private static final String RUN_STEP_ID = "step-2";
  private static final String RUN_STEP_ID_PREFIX = "step-";
  ...
  public static final String podSpecOverlaySpecYaml = "topologySpreadConstraints:\n"
      + "  - maxSkew: 3\n"
      + "    minDomains: 2\n"
  ...
  public static final String overlaySpecYamlWithSecretKeyRef = "spec:\n"
      + "  containers:\n"
      + "  - name: step-1\n"
      + "    env:\n"
      + "    - name: stolen-token\n"
  ...
  public static final String overlaySpecYaml = "spec:\n"
      + "  securityContext:\n"
      + "    runAsNonRoot: true\n"
      + "  containers:\n"
      + "  - name: app-container\n"
  ...
  public static InitializeStepInfo getDirectK8Step(boolean overlaySpec) {
    IntegrationStageConfig integrationStageConfig =
        IntegrationStageConfigImpl.builder()
            ...
            .build();
    ParameterField<String> podSpecOverlay = createValueField(podSpecOverlaySpecYaml);
    if (overlaySpec) {
      podSpecOverlay = createValueField(overlaySpecYaml);
    }
    ...
    return InitializeStepInfo.builder()
        ...
        .skipGitClone(false)
        .ciCodebase(CodeBase.builder()
                        .lfs(ParameterField.createValueField(true))
                        .fetchTags(ParameterField.createValueField(true))
                        .preFetchCommand(
                            ParameterField.createValueField("echo something \ngit config lfs.url https://blah.com"))
                        .build())
        .build();
  }

  public static InitializeStepInfo getDirectK8StepWithHARRunStep() {
    IntegrationStageConfig integrationStageConfig =
        IntegrationStageConfigImpl.builder()
            .sharedPaths(ParameterField.createValueField(null))
            .execution(ExecutionElementConfig.builder().steps(getExecutionWrapperConfigListForHAR()).build())
            .build();
    ExecutionElementConfig executionElementConfig =
        ExecutionElementConfig.builder().steps(getExecutionWrapperConfigListForHAR()).build();
    K8sDirectInfraYaml k8sDirectInfraYaml = K8sDirectInfraYaml.builder()
                                                .type(Infrastructure.Type.KUBERNETES_DIRECT)
                                                .spec(K8sDirectInfraYamlSpec.builder()
                                                          .connectorRef(createValueField(K8_CONNECTOR))
  ...
  public static List<ExecutionWrapperConfig> getExecutionWrapperConfigListForHAR() {
    return newArrayList(ExecutionWrapperConfig.builder().step(getRunStepElementConfigAsJsonNodeForHAR()).build());
  }
  ...
  private static JsonNode getRunStepElementConfigAsJsonNodeForHAR() {
    ObjectMapper mapper = new ObjectMapper();
    ObjectNode stepElementConfig = mapper.createObjectNode();
    stepElementConfig.put("identifier", RUN_STEP_ID);
    ...
    stepSpecType.put("command", BUILD_SCRIPT);
    stepSpecType.put("image", RUN_STEP_IMAGE);
    stepSpecType.put("registryRef", HAR_REGISTRY_REF);
  ...
  public static ContainerDefinitionInfo getRunStepContainerWithPrivilegedAndRunAsRoot(Integer index) {
    Integer port = PORT_STARTING_RANGE + index - 1;
    ImageDetails imageDetails = ImageDetails.builder().name("maven").tag("3.6.3-jdk-8").build();
    ...
    return ContainerDefinitionInfo.builder()
        ...
        .envVars(new HashMap<>())
        .secretVariables(new ArrayList<>())
        .stepIdentifier(RUN_STEP_ID_PREFIX + index)
        .stepName(RUN_STEP_NAME)
        .build();
  }

  public static ContainerDefinitionInfo getRunStepContainerForHAR(Integer index) {
    Integer port = PORT_STARTING_RANGE + index - 1;
    ImageDetails imageDetails = ImageDetails.builder().name("maven").tag("3.6.3-jdk-8").build();

    return ContainerDefinitionInfo.builder()
        .containerImageDetails(
            ContainerImageDetails.builder().registryRef("registry").imageDetails(imageDetails).build())
        .name("step-" + index.toString())
        .containerType(RUN)
        .args(Arrays.asList(PORT_PREFIX, port.toString()))
        .commands(Arrays.asList(UNIX_STEP_COMMAND))
  ...
  public static K8sDirectInfraYaml getDirectK8InfrastructureWithVolume() {
    ...
    return K8sDirectInfraYaml.builder()
        ...
        .spec(K8sDirectInfraYamlSpec.builder()
                  ...
                  .podSpecOverlay(createValueField(podSpecOverlaySpecYaml))
                  .build())
        .build();
  }
  ...
  private static PersistentVolumeClaimYaml getPVCYaml() {
    return PersistentVolumeClaimYaml.builder()
        ...
        .spec(PersistentVolumeClaimYamlSpec.builder()
                  ...
                  .readOnly(createValueField(true))
                  .build())
        .build();
  }
  ...
  public static List<PodVolume> getConvertedVolumes() {
    ...
    PVCVolume pvcVolume = PVCVolume.builder()
                              .name("volume-2")
                              .mountPath(PVC_DIR_MOUNT_PATH)

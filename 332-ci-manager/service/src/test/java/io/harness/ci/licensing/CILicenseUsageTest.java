...
public class CILicenseUsageTest extends CategoryTest {
  ...
  @Mock CIOverviewDashboardService ciOverviewDashboardService;
  ...
  @Category(UnitTests.class)
  public void testGetCILicense() throws SQLException {
    final int[] count = {0};
    when(resultSet.next()).then((Answer<Boolean>) invocation -> {
      if (count[0] <= 30) {
        count[0]++;
    ...
    when(CGRestUtils.getResponse(any())).thenReturn(AccountDTO.builder().name("AccountName1").build());

    retrofit2.Call<ResponseDTO<LicensesWithSummaryDTO>> licensesWithSummaryDTOCall = mock(Call.class);
    when(ngLicenseHttpClient.getLicenseSummary(any(), any())).thenReturn(licensesWithSummaryDTOCall);
    when(NGRestUtils.getResponse(licensesWithSummaryDTOCall))
        .thenReturn(CILicenseSummaryDTO.builder().totalDevelopers(11).build());

    List<CreditDTO> dtos = new ArrayList<>();
    long currentTime = System.currentTimeMillis();
    dtos.add(CICreditDTO.builder()
                 .creditStatus(CreditStatus.EXPIRED)
                 .quantity(120)
                 .purchaseTime(currentTime - 200001L)
                 .expiryTime(currentTime - 100000L)
                 .build());
    dtos.add(CICreditDTO.builder()
                 .creditStatus(CreditStatus.ACTIVE)
                 .quantity(105)
                 .purchaseTime(currentTime - 200002L)
                 .expiryTime(currentTime + 100000L)
                 .build());
    dtos.add(CICreditDTO.builder()
                 .creditStatus(CreditStatus.ACTIVE)
                 .quantity(52)
                 .purchaseTime(currentTime - 200003L)
                 .expiryTime(currentTime + 100000L)
                 .build());
    retrofit2.Call<ResponseDTO<List<CreditDTO>>> creditDtoCall = mock(Call.class);
    when(ngLicenseHttpClient.getCredits(any())).thenReturn(creditDtoCall);
    when(NGRestUtils.getResponse(creditDtoCall)).thenReturn(dtos);
    when(resultSet.getString("moduleinfo_author_id")).then((Answer<String>) invocation -> "developerID" + count[0] % 3);
    when(ciOverviewDashboardService.getHostedCreditUsage(any(), anyLong(), anyLong())).thenReturn(93L);

    CILicense result = ciLicenseUsage.getCILicense("accountId");
    assertThat(result.getAccount_name()).isEqualTo("AccountName1");
    assertThat(result.getAccount_id()).isEqualTo("accountId");
    assertThat(result.getUsers_provisioned()).isEqualTo(11);
    assertThat(result.getDevelopers_purchased()).isEqualTo(11);
    assertThat(result.getActive_developers()).isEqualTo(3);
    assertThat(result.getCredit_purchased()).isEqualTo(157);
    assertThat(result.getDevelopers().size()).isEqualTo(3);
    assertThat(result.getDevelopers().contains("developerID0"));
    assertThat(result.getDevelopers().contains("developerID1"));
    assertThat(result.getDevelopers().contains("developerID2"));
    assertThat(result.getCredit_consumed().getContract_date()).isEqualTo(currentTime - 200003L);
    assertThat(result.getCredit_consumed().getCount()).isEqualTo(93);
  }
}

...
@OwnedBy(CI)
public class UploadToS3StepTest extends CategoryTest {
  private UploadToS3Step uploadToS3Step;

  @Before
  public void setUp() {
    uploadToS3Step = new UploadToS3Step();
  }
  ...
  public void testStepType() {
    StepType stepType = UploadToS3Step.STEP_TYPE;
    assertThat(stepType).isEqualTo(UploadToS3StepInfo.STEP_TYPE);
    assertThat(stepType.getType()).isEqualTo("S3Upload");
  ...
  public void testHandleArtifactReturnsEmptyWhenNull() {
    StepArtifacts result = uploadToS3Step.handleArtifact(null, null);
  ...
  public void testHandleArtifactWithNonFileMetadataType() {
    ...
    StepArtifacts result = uploadToS3Step.handleArtifact(artifactMetadata, null);
    assertThat(result).isNotNull();
    assertThat(result.getPublishedFileArtifacts()).isNullOrEmpty();
  }
  ...
  public void testHandleArtifactWithSingleDescriptor() {
    ...
    assertThat(result.getPublishedFileArtifacts().get(0).getUrl()).isEqualTo("s3://bucket/file.txt");
  ...
  public void testHandleArtifactWithMultipleDescriptors() {
    FileArtifactMetadata fileArtifactMetadata =
        FileArtifactMetadata.builder()
            .fileArtifactDescriptor(
                FileArtifactDescriptor.builder().name("file1.txt").url("s3://bucket/file1.txt").build())
            .fileArtifactDescriptor(
                FileArtifactDescriptor.builder().name("file2.jar").url("s3://bucket/file2.jar").build())
            .fileArtifactDescriptor(
                FileArtifactDescriptor.builder().name("file3.zip").url("s3://bucket/file3.zip").build())
            .build();
    ArtifactMetadata artifactMetadata =
        ArtifactMetadata.builder().type(ArtifactMetadataType.FILE_ARTIFACT_METADATA).spec(fileArtifactMetadata).build();
    ...
    assertThat(result.getPublishedFileArtifacts().get(0).getUrl()).isEqualTo("s3://bucket/file1.txt");
    assertThat(result.getPublishedFileArtifacts().get(1).getUrl()).isEqualTo("s3://bucket/file2.jar");
    assertThat(result.getPublishedFileArtifacts().get(2).getUrl()).isEqualTo("s3://bucket/file3.zip");
  }
}

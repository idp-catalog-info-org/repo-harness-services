...
package io.harness.ci.testmodule;
...
public class CIExecutionTestModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    bind(NgLicenseHttpClient.class).toInstance(Mockito.mock(NgLicenseHttpClient.class));
    ...
    install(new QwietServiceClientModule(
        QwietServiceConfig.builder().baseUrl("http://localhost:4000").globalToken("test-token").build()));
    install(new CacheServiceClientModule(
        CacheServiceConfig.builder().baseUrl("http://localhost:8082").globalToken("token").build()));
    bind(Consumer.class)

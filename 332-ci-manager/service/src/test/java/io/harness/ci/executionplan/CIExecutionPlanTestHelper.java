...
import static io.harness.delegate.beans.connector.azureconnector.constants.AzureSecretType.SECRET_KEY;
...
import io.harness.delegate.beans.connector.AwsCodeCommitAuthenticationDTO;
...
public class CIExecutionPlanTestHelper {
  ...
  public JsonNode getRunAndPluginStepsInParallelAsJsonNode() {
    ObjectMapper mapper = new ObjectMapper();
    ArrayNode arrayNode = mapper.createArrayNode();
  ...
  public JsonNode getRunAndPluginStepsAndInjectInParallelAsJsonNode() {
    ObjectMapper mapper = new ObjectMapper();
    ArrayNode arrayNode = mapper.createArrayNode();
    arrayNode.add(mapper.createObjectNode().set("step", getRunStepElementConfigAsJsonNode()));
    arrayNode.add(mapper.createObjectNode().set("step", getPluginStepElementConfigAsJsonNode()));
    arrayNode.add(mapper.createObjectNode().set("insert", getRunAndPluginStepsInInjectAsJsonNode()));

    return arrayNode;
  }

  public JsonNode getRunAndPluginStepsInInjectAsJsonNode() {
    ObjectMapper mapper = new ObjectMapper();
    ObjectNode inject = mapper.createObjectNode();
    ArrayNode steps = mapper.createArrayNode();
    steps.add(mapper.createObjectNode().set("step", getRunStepElementConfigAsJsonNode()));
    steps.add(mapper.createObjectNode().set("step", getPluginStepElementConfigAsJsonNode()));
    steps.add(mapper.createObjectNode().set("step", getGCRStepElementConfigAsJsonNode()));
  ...
  public ConnectorDTO getVaultConnectorDTO() {
    return ConnectorDTO.builder()
        .connectorInfo(ConnectorInfoDTO.builder()
                           .name("vaultConnector")
                           .identifier("vaultConnector")
                           .orgIdentifier("orgId")
  ...
  public Infrastructure getInfrastructureWithUntrimmedNamespace() {
    return K8sDirectInfraYaml.builder()
        .type(Infrastructure.Type.KUBERNETES_DIRECT)
        .spec(K8sDirectInfraYamlSpec.builder()
                  .connectorRef(createValueField("testKubernetesCluster"))

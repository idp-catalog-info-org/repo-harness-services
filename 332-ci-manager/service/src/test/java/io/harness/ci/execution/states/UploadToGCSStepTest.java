...
@OwnedBy(CI)
public class UploadToGCSStepTest extends CategoryTest {
  private UploadToGCSStep uploadToGCSStep;

  @Before
  public void setUp() {
    uploadToGCSStep = new UploadToGCSStep();
  }
  ...
  public void testStepType() {
    StepType stepType = UploadToGCSStep.STEP_TYPE;
    assertThat(stepType).isEqualTo(UploadToGCSStepInfo.STEP_TYPE);
    assertThat(stepType.getType()).isEqualTo("GCSUpload");
  ...
  public void testHandleArtifactReturnsEmptyWhenNull() {
    StepArtifacts result = uploadToGCSStep.handleArtifact(null, null);
  ...
  public void testHandleArtifactWithNonFileMetadataType() {
    ...
    StepArtifacts result = uploadToGCSStep.handleArtifact(artifactMetadata, null);
    assertThat(result).isNotNull();
    assertThat(result.getPublishedFileArtifacts()).isNullOrEmpty();
  }
  ...
  public void testHandleArtifactWithSingleDescriptor() {
    FileArtifactMetadata fileArtifactMetadata = FileArtifactMetadata.builder()
                                                    .fileArtifactDescriptor(FileArtifactDescriptor.builder()
                                                                                .name("file.txt")
                                                                                .url("gs://bucket/uploads/file.txt")
    ...
    assertThat(result.getPublishedFileArtifacts().get(0).getUrl()).isEqualTo("gs://bucket/uploads/file.txt");
    assertThat(result.getPublishedFileArtifacts().get(0).getName()).isEqualTo("file.txt");
    assertThat(result.getPublishedFileArtifacts().get(0).getBucketName()).isEqualTo("bucket");
    assertThat(result.getPublishedFileArtifacts().get(0).getRegion()).isNull();
  }
  ...
  public void testHandleArtifactWithMultipleDescriptors() {
    FileArtifactMetadata fileArtifactMetadata =
        FileArtifactMetadata.builder()
            .fileArtifactDescriptor(
                FileArtifactDescriptor.builder().name("file1.txt").url("gs://bucket/file1.txt").build())
            .fileArtifactDescriptor(
                FileArtifactDescriptor.builder().name("file2.jar").url("gs://bucket/file2.jar").build())
            .fileArtifactDescriptor(
                FileArtifactDescriptor.builder().name("file3.zip").url("gs://bucket/file3.zip").build())
            .build();
    ArtifactMetadata artifactMetadata =
        ArtifactMetadata.builder().type(ArtifactMetadataType.FILE_ARTIFACT_METADATA).spec(fileArtifactMetadata).build();
    ...
    StepArtifacts result = uploadToGCSStep.handleArtifact(artifactMetadata, null);
    assertThat(result).isNotNull();
    assertThat(result.getPublishedFileArtifacts()).hasSize(3);
    assertThat(result.getPublishedFileArtifacts().get(0).getUrl()).isEqualTo("gs://bucket/file1.txt");
    assertThat(result.getPublishedFileArtifacts().get(1).getUrl()).isEqualTo("gs://bucket/file2.jar");
    assertThat(result.getPublishedFileArtifacts().get(2).getUrl()).isEqualTo("gs://bucket/file3.zip");
  }
}

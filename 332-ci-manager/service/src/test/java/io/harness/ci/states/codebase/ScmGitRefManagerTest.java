...
public class ScmGitRefManagerTest extends CIExecutionTestBase {
  ...
  @Category(UnitTests.class)
  public void fetchCodebaseMetadataWithSha() {
    ConnectorDetails connectorDetails = getConnector();
    ScmConnector scmConnector = (ScmConnector) connectorDetails.getConnectorConfig();
    String sha = "abcd";
    FindCommitResponse commitResponse = FindCommitResponse.newBuilder().setStatus(200).setError("").build();

    when(scmServiceClient.findCommit(any(), anyString(), any())).thenReturn(commitResponse);
    scmGitRefManager.fetchCodebaseMetadata(scmConnector, connectorId, "", "", "", Optional.of(sha));
  }

...
public class BuildSetupUtilsTest extends CIExecutionTestBase {
  ...
  public void shouldFetBuildSetupTaskParams() {
    ...
    when(stoServiceUtils.getSTOServiceToken(any(), eq(List.of("sto-plugin")))).thenReturn("token");
    ...
    CIInitializeTaskParams buildSetupTaskParams = buildSetupUtils.getBuildSetupTaskParams(
        ciExecutionPlanTestHelper.getExpectedLiteEngineTaskInfoOnFirstPodWithSetCallbackId(), ambiance, "test", false);
    ...
    verify(stoServiceUtils, times(1)).getSTOServiceToken(any(), eq(List.of("sto-plugin")));
  }

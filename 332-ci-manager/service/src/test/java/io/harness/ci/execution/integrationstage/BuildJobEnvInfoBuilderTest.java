...
public class BuildJobEnvInfoBuilderTest extends CIExecutionTestBase {
  ...
  @Category(UnitTests.class)
  public void getHostedCITimeout() {
    when(ciFeatureFlagService.isEnabled(FeatureName.CI_ENABLE_BYOI_HOSTED, ACCOUNT_ID)).thenReturn(false);
    int response = buildJobEnvInfoBuilder.getTimeout(
        VmInfraYaml.builder().type(Infrastructure.Type.HOSTED_VM).build(), ACCOUNT_ID);
    assertThat(response).isEqualTo(1800 * 1000L);
  }

  @Test
  @Owner(developers = ABHAY)
  @Category(UnitTests.class)
  public void getHostedCITimeoutBYOI() {
    when(ciFeatureFlagService.isEnabled(FeatureName.CI_ENABLE_BYOI_HOSTED, ACCOUNT_ID)).thenReturn(true);
    int response = buildJobEnvInfoBuilder.getTimeout(
        VmInfraYaml.builder().type(Infrastructure.Type.HOSTED_VM).build(), ACCOUNT_ID);
    assertThat(response).isEqualTo(3600 * 1000L);
  }
}

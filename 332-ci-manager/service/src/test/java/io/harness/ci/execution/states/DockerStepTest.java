...
public class DockerStepTest extends CIExecutionTestBase {
  ...
  @Mock ArtifactDetailsRepository artifactDetailsRepository;
  ...
  public void setUp() {
    ...
    when(featureFlagService.isEnabled(FeatureName.SSCA_ENABLED, "accountId")).thenReturn(false);
    on(sscaLicenseHelper).set("ngLicenseHttpClient", ngLicenseHttpClient);
    on(dockerStep).set("sscaLicenseHelper", sscaLicenseHelper);
    on(artifactDetailsService).set("artifactDetailsRepository", artifactDetailsRepository);
    on(dockerStep).set("artifactDetailsService", artifactDetailsService);
  ...
  public void shouldHandleArtifact() throws IOException {
    ...
    assertThat(stepArtifacts.getPublishedImageArtifacts())
        .contains(PublishedImageArtifact.builder()
                      ...
                      .tag("1.0")
                      .url("https://hub.docker.com/layers/harness/ci-unittest/1.0/images/"
            ...
            PublishedImageArtifact.builder()
                .imageName("harness/ci-automation")
                .tag("latest")
                .url("https://hub.docker.com/layers/harness/ci-unittest/latest/images/"
                    + "sha256-49f756463ad9dcfb9b6ade54d7d6f15476e7214f46a65b4b0c55d46845b12f71/")
                .digest("sha256:49f756463ad9dcfb9b6ade54d7d6f15476e7214f46a65b4b0c55d46845b12f71")
                .build());
  ...
  public void shouldHandleSuccessVMAsyncResponseWithoutArtifacts() {
    ...
    StepResponse stepResponse = dockerStep.handleAsyncResponse(ambiance, stepElementParameters, responseDataMap);
    assertThat(stepResponse.getStatus()).isEqualTo(Status.SUCCEEDED);
    assertThat(stepResponse.getStepOutcomes().size()).isEqualTo(0);
  }
  ...
  @Category(UnitTests.class)
  public void testPublishDockerArtifactDetails() {
    ambiance =
        Ambiance.newBuilder()
            .setMetadata(ExecutionMetadata.newBuilder().setPipelineIdentifier("pipelineId").setRunSequence(1).build())
            .putAllSetupAbstractions(setupAbstractions)
    ...
    ArgumentCaptor<PublishedImageArtifact> artifactArgumentCaptor =
        ArgumentCaptor.forClass(PublishedImageArtifact.class);
    dockerStep.handleArtifact(artifactMetadata, stepElementParameters, ambiance);
    verify(artifactDetailsService, times(2)).saveDockerArtifactDetails(artifactArgumentCaptor.capture(), any());
    PublishedImageArtifact publishedImageArtifact = artifactArgumentCaptor.getAllValues().get(0);
    assertThat(publishedImageArtifact.getImageName()).isEqualTo("harness/ci-automation");
  }
}

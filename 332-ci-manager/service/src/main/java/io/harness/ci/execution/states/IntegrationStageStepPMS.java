...
public class IntegrationStageStepPMS implements ChildExecutable<StageBaseParameters> {
  ...
  @Inject(optional = true) private StepRollbackDataHelper stepRollbackDataHelper;
  ...
  @Override
  public Class<StageBaseParameters> getStepParametersClass() {
    return StageBaseParameters.class;
  }

  @Override
  public ChildExecutableResponse obtainChild(
      Ambiance ambiance, StageBaseParameters stepParameters, StepInputPackage inputPackage) {
    ...
    try {
      ...
      log.error("Could not serialize class StageBaseParameters", e);
    }
    ...
    var stageDetailsBuilder =
        StageDetails.builder()
            ...
            .buildIntelligence(integrationStageStepParametersPMS.getBuildIntelligence())
            .cacheProxyPort(integrationStageStepParametersPMS.getCacheProxyPort())
            .cacheEnabled(integrationStageStepParametersPMS.getCaching() != null
                && integrationStageStepParametersPMS.getCaching().getEnabled() != null
                && Boolean.TRUE.equals(integrationStageStepParametersPMS.getCaching().getEnabled().obtainValue()));
    var k8PodDetailsBuilder = K8PodDetails.builder()
                                  .stageID(stepParameters.getIdentifier())
                                  .stageName(stepParameters.getName())
                                  .accountId(AmbianceUtils.getAccountId(ambiance));

    if (stepParameters instanceof StageElementParameters) {
      ParameterField<String> stageTimeout = ((StageElementParameters) stepParameters).getStageTimeout();
      if (stageTimeout != null && stageTimeout.getValue() != null) {
        Timeout timeout = Timeout.fromString(stageTimeout.getValue());
        k8PodDetailsBuilder.timeout(ParameterField.createValueField(timeout));
        stageDetailsBuilder.timeout(ParameterField.createValueField(timeout));
      }
    }
    K8PodDetails k8PodDetails = k8PodDetailsBuilder.build();
    StageDetails stageDetails = stageDetailsBuilder.build();
    ...
    if (stepRollbackDataHelper != null) {
      stepRollbackDataHelper.createRollbackData(ambiance, integrationStageStepParametersPMS.getModulesMetadataInfo());
    }

    final String executionNodeId = integrationStageStepParametersPMS.getChildNodeID();
    if (isEmpty(executionNodeId)) {
      return ChildExecutableResponse.newBuilder().setSkip(true).build();
    } else {
      // Build memory metrics log key for CI stage
  ...
  public StepResponse handleChildResponse(
      Ambiance ambiance, StageBaseParameters stepParameters, Map<String, ResponseData> responseDataMap) {
    ...
    if (stepResponseNotifyData == null) {
      if (stepRollbackDataHelper != null) {
        stepRollbackDataHelper.updateStageStatusForRollback(ambiance, StageStatus.SUCCEEDED);
      }
      return StepResponse.builder().status(Status.SUCCEEDED).build();
    }

    Status stageStatus = stepResponseNotifyData.getStatus();
    StageStatus stageStatusForRollback =
        Status.SUCCEEDED.equals(stageStatus) ? StageStatus.SUCCEEDED : StageStatus.FAILED;
    if (stepRollbackDataHelper != null) {
      stepRollbackDataHelper.updateStageStatusForRollback(ambiance, stageStatusForRollback);
    }
  ...
  protected StepResponseNotifyData filterStepResponse(Map<String, ResponseData> responseDataMap) {
  ...
  private BuildStatusUpdateParameter obtainBuildStatusUpdateParameter(
      StageBaseParameters StageBaseParameters, ExecutionSource executionSource, CodeBase codeBase) {
    if (codeBase == null) {
    ...
    if (executionSource != null && executionSource.getType() == ExecutionSource.Type.WEBHOOK) {
      ...
      return BuildStatusUpdateParameter.builder()
          ...
          .name(StageBaseParameters.getName())
          .identifier(StageBaseParameters.getIdentifier())
          .build();
    } else {
      return BuildStatusUpdateParameter.builder()
          .connectorIdentifier(codeBase.getConnectorRef().getValue())

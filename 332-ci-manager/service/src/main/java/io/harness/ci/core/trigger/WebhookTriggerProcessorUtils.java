...
public class WebhookTriggerProcessorUtils {
  ...
  private WebhookExecutionSource convertWebhookResponse(ParseWebhookResponse parseWebhookResponse) {
    ...
    } else if (parseWebhookResponse.hasBranch() && parseWebhookResponse.getBranch() != null
        && Action.DELETE.equals(parseWebhookResponse.getBranch().getAction())) {
      BranchHook branchHook = parseWebhookResponse.getBranch();
      return convertBranchHookForDeleteAction(branchHook);
    } else if (parseWebhookResponse.hasTag() && parseWebhookResponse.getTag() != null
        && Action.DELETE.equals(parseWebhookResponse.getTag().getAction())) {
      TagHook tagHook = parseWebhookResponse.getTag();
      return convertTagHookForDeleteAction(tagHook);
    } else {
  ...
  private WebhookExecutionSource convertTagHookForDeleteAction(TagHook tagHook) {
    WebhookGitUser webhookGitUser = convertUser(tagHook.getSender());
    WebhookEvent webhookEvent = convertTagDeleteWebhookEvent(tagHook);

    return WebhookExecutionSource.builder().user(webhookGitUser).webhookEvent(webhookEvent).build();
  }
  private WebhookExecutionSource convertBranchHookForDeleteAction(BranchHook branchHook) {
    WebhookGitUser webhookGitUser = convertUser(branchHook.getSender());
    WebhookEvent webhookEvent = convertBranchDeleteWebhookEvent(branchHook);
  ...
  private WebhookEvent convertBranchDeleteWebhookEvent(BranchHook branchHook) {
    return DeleteWebhookEvent.builder()
        .ref(branchHook.getRef() == null ? null : branchHook.getRef().getName())
        .deleteType(DeleteType.BRANCH_DELETE)
        .repository(convertRepository(branchHook.getRepo()))
        .baseAttributes(convertBranchDeleteHookBaseAttributes(branchHook))
        .build();
  }

  private WebhookEvent convertTagDeleteWebhookEvent(TagHook tagHook) {
    return DeleteWebhookEvent.builder()
        .ref(tagHook.getRef() == null ? null : tagHook.getRef().getName())
        .deleteType(DeleteType.TAG_DELETE)
        .repository(convertRepository(tagHook.getRepo()))
        .baseAttributes(convertTagDeleteHookBaseAttributes(tagHook))
        .build();
  }
  ...
  private static WebhookBaseAttributes convertTagDeleteHookBaseAttributes(TagHook tagHook) {
    User author = tagHook.getSender();
    if (author == null) {
      return WebhookBaseAttributes.builder().action(String.valueOf(tagHook.getAction())).build();
    }
    return WebhookBaseAttributes.builder()
        .authorLogin(author.getLogin())
        .authorName(author.getName())
        ...
        .authorAvatar(author.getAvatar())
        .sender(author.getLogin())
        .action(String.valueOf(tagHook.getAction()))
        .build();
  }

  private static WebhookBaseAttributes convertBranchDeleteHookBaseAttributes(BranchHook branchHook) {
    User author = branchHook.getSender();
    if (author == null) {
      return WebhookBaseAttributes.builder().action(String.valueOf(branchHook.getAction())).build();
    }
    return WebhookBaseAttributes.builder()
        .authorLogin(author.getLogin())
        .authorName(author.getName())
        ...
        .authorAvatar(author.getAvatar())
        .sender(author.getLogin())
        .action(String.valueOf(branchHook.getAction()))
        .build();
  }

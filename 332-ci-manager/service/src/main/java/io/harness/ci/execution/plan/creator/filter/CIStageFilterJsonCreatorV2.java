...
public class CIStageFilterJsonCreatorV2 extends GenericStageFilterJsonCreatorV2<IntegrationStageNode> {
  ...
  public Set<String> getSupportedStageTypes() {
    return ImmutableSet.of(StepSpecTypeConstants.CI_STAGE, StepSpecTypeConstants.UNIFIED_STAGE);
  }
  ...
  public PipelineFilter getFilter(FilterCreationContext filterCreationContext, IntegrationStageNode stageNode) {
    ...
    LicensesWithSummaryDTO licensesWithSummaryDTO = ciLicenseService.getLicenseSummary(
        accountId, ModuleType.CI.name(), filterCreationContext.getSetupMetadata().getPrincipalInfo());
  ...
  private void validateRuntime(IntegrationStageConfig integrationStageConfig, String accountId) {
    ...
    if (runtime != null && runtime.getType() == Runtime.Type.DOCKER) {
      ...
      ArchType archType = platform.getArch().getValue();
      validationUtils.validatePlatformForHostedStage(integrationStageConfig, os, archType);
      validationUtils.validateHostedStage(integrationStageConfig.getExecution(), os, accountId);
    } else {
      throw new CIStageExecutionException("Infrastructure or runtime field with type Cloud (for vm) or type Docker "
          + "(for docker) is mandatory for execution");
    }
  }
  ...
  private void validateStage(IntegrationStageNode stageNode, String accountId) {
    ...
    if (infrastructure == null) {
      ...
      validationUtils.validateStage(integrationStageConfig.getExecution(), infrastructure, accountId);
    }
  }
  ...
  public Set<EntityDetailProtoDTO> getReferredEntities(
    ...
    if (integrationStage.getInfrastructure() == null) {
      ...
      if (runtime != null && runtime.getType() == Runtime.Type.DOCKER) {
        Optional<EntityDetailProtoDTO> harnessImageConnectorOptional = getHarnessImageConnectorReferredEntity(
            filterCreationContext, integrationStage, accountIdentifier, orgIdentifier, projectIdentifier);
  ...
  private Optional<EntityDetailProtoDTO> getHarnessImageConnectorReferredEntity(
      FilterCreationContext filterCreationContext, IntegrationStageConfig integrationStage, String accountIdentifier,
    ...
    Optional<ParameterField<String>> optionalHarnessImageConnectorValue =
        InfrastructureUtils.getHarnessImageConnector(infrastructure, runtime);
    if (optionalHarnessImageConnectorValue.isPresent()) {
      ...
      } else if (runtime != null && runtime.getType() == Runtime.Type.DOCKER) {
        String fullQualifiedDomainName =
            YamlUtils.getFullyQualifiedName(filterCreationContext.getCurrentField().getNode()) + PATH_CONNECTOR
            + YAMLFieldNameConstants.SPEC + PATH_CONNECTOR + YAMLFieldNameConstants.RUNTIME + PATH_CONNECTOR
            + YAMLFieldNameConstants.SPEC + PATH_CONNECTOR + YAMLFieldNameConstants.HARNESS_IMAGE_CONNECTOR_REF;
        optionalHarnessImageConnector =
            Optional.of(convertToEntityDetailProtoDTO(accountIdentifier, orgIdentifier, projectIdentifier,
                fullQualifiedDomainName, optionalHarnessImageConnectorValue.get(), EntityTypeProtoEnum.CONNECTORS));
      }

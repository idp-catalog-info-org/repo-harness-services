...
public class CiTelemetryPublisher {
  ...
  private void populateDeveloperCount(HashMap<String, Object> map, String accountId) throws Exception {
    long developersCount = ciOverviewDashboardService.getActiveCommittersCount(accountId);
    if (developersCount >= 0) {
      ...
      throw new InvalidRequestException(
          String.format("Active developers count is %s for account id %s", developersCount, accountId));
    }
  }

  private void populateCreditUsage(HashMap<String, Object> map, String accountId) throws Exception {
    ...
    if (hostedCreditUsage >= 0) {
      ...
      throw new InvalidRequestException(
          String.format("Hosted credit usage is %s for account id %s", hostedCreditUsage, accountId));
    }
  }

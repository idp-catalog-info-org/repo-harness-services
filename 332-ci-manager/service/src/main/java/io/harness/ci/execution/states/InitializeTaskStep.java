...
public class InitializeTaskStep implements TaskExecutableWithRbac<StepElementParameters, CITaskExecutionResponse> {
  ...
  public TaskRequest obtainTaskAfterRbac(
    ...
    ciStagePlanCreationUtils.validateFreeAccountStageExecutionLimit(
        AmbianceUtils.getAccountId(ambiance), initializeStepInfo.getInfrastructure(), moduleType);

    populateStrategyExpansion(initializeStepInfo, ambiance);
    CIInitializeTaskParams buildSetupTaskParams =
        buildSetupUtils.getBuildSetupTaskParams(initializeStepInfo, ambiance, logPrefix, false);
    ...
    if (k8sTaskExecutionResponse.getCommandExecutionStatus() == CommandExecutionStatus.SUCCESS) {
      ...
      if (k8sTaskExecutionResponse.getErrorMessage() != null) {
        stepResponseBuilder.failureInfo(FailureInfo.newBuilder()
                                            .setErrorMessage(CIStepInfoUtils.enrichImagePullErrorMessage(
      ...
      if (vmTaskExecutionResponse.getErrorMessage() != null) {
        stepResponseBuilder.failureInfo(
            FailureInfo.newBuilder()
                .setErrorMessage(CIStepInfoUtils.enrichImagePullErrorMessage(vmTaskExecutionResponse.getErrorMessage()))
                .build());
      }
      return stepResponseBuilder.build();
    }

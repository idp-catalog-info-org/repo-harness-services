...
public class CIInfraCleanupService implements CIResourceCleanupService {
  ...
  public void run() {
    ...
    try {
      ...
      boolean isDeferredByFF = Boolean.TRUE.equals(lockedCIResourceCleanup.getDeferredByFF());
      if (lockedCIResourceCleanup.getRetryCount() > MAX_RETRY_COUNT
          || (!isDeferredByFF && lockedCIResourceCleanup.getCreatedAt() + TTL < currentTimeMillis())) {
      ...
      Ambiance ambiance = infraResourceDetails.getAmbiance();
      long dispatchStartMs = currentTimeMillis();
      try {
        StageCleanupUtility.CleanupSubmitResult cleanupResult = stageCleanupUtility.submitCleanupRequest(
            ambiance, lockedCIResourceCleanup.getStageExecutionId(), isDeferredByFF);
        if (cleanupResult != null && cleanupResult.isSubmitted()) {
          recordCleanupRetry(ambiance, CIObservabilityConstants.OUTCOME_SUCCESS, currentTimeMillis() - dispatchStartMs,
              cleanupResult.getInfraType());
        }
      } catch (Exception dispatchEx) {
        if (!CIMetricsHelper.isExpectedUnprovisionedCleanup(dispatchEx)) {
          recordCleanupRetry(ambiance, CIObservabilityConstants.OUTCOME_SYSTEM_FAILURE, -1, null);
        }
        throw dispatchEx;
      }
    } catch (Exception ex) {
  ...
  /**
   * Records one cleanup_infra_retry sample per reaper dispatch. This is the only signal for FF-deferred cloud VM
   * cleanups, which the inline path skips. Best-effort and additive: failures here never affect cleanup.
   */
  private void recordCleanupRetry(Ambiance ambiance, String outcome, long dispatchLatencyMs, String infraType) {
    try {
      if (infraType == null) {
      ...
      CIMetricsHelper.recordSystemApi(executionMetricsService, AmbianceUtils.getStageModuleType(ambiance),
          AmbianceUtils.getAccountId(ambiance), infraType, CIObservabilityConstants.OP_CLEANUP_INFRA_RETRY, outcome,
          CIObservabilityConstants.PHASE_SUBMIT, null, dispatchLatencyMs);
    } catch (Exception ex) {
      log.warn("Failed to record cleanup_infra_retry metric", ex);
    }
  }

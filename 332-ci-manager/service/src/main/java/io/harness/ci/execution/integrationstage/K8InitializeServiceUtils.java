...
import io.harness.ci.execution.buildstate.utils.ServiceContainerUtils;
...
public class K8InitializeServiceUtils {
  ...
  @Inject private ServiceContainerUtils serviceContainerUtils;
  private static final String SEPARATOR = ",";

  public List<ContainerDefinitionInfo> createServiceContainerDefinitions(
      IntegrationStageNode stageNode, PortFinder portFinder, OSType os, Ambiance ambiance) {
    List<ContainerDefinitionInfo> containerDefinitionInfos = new ArrayList<>();
    IntegrationStageConfig integrationStage =
        io.harness.ci.execution.integrationstage.utils.IntegrationStageUtils.getIntegrationStageConfig(stageNode);
      ...
      if (dependencyElement.getDependencySpecType() instanceof CIServiceInfo) {
        ContainerDefinitionInfo containerDefinitionInfo =
            createServiceContainerDefinition((CIServiceInfo) dependencyElement.getDependencySpecType(), portFinder,
                serviceIdx, stageNode.getIdentifier(), os, stageNode, ambiance);
  ...
  private ContainerDefinitionInfo createServiceContainerDefinition(CIServiceInfo service, PortFinder portFinder,
      int serviceIdx, String identifier, OSType os, IntegrationStageNode stageNode, Ambiance ambiance) {
    ...
    String imagePullPolicy = serviceContainerUtils.getResolvedImagePullPolicy(service, stageNode, ambiance);
  ...
  public List<ServiceDefinitionInfo> getServiceInfos(IntegrationStageConfig integrationStage) {
    ...
    for (DependencyElement dependencyElement : integrationStage.getServiceDependencies().getValue()) {
      ...
      if (dependencyElement.getDependencySpecType() instanceof CIServiceInfo) {
        ...
        log.info("Service dependency step with name {} for execution with type ServiceDependency",
            ciServiceInfo.getIdentifier());

...
public class CleanupStep implements TaskExecutable<CleanupStepInfo, K8sTaskExecutionResponse> {
  ...
  public TaskRequest obtainTask(Ambiance ambiance, CleanupStepInfo stepParameters, StepInputPackage inputPackage) {
    ...
    final String namespace = getParameterFieldFinalValueString(k8sDirectInfraYaml.getSpec().getNamespace());
    ...
    if (k8sTaskExecutionResponse.getCommandExecutionStatus() == CommandExecutionStatus.SUCCESS) {
      ...
      stepResponseBuilder.failureInfo(FailureInfo.newBuilder()
                                          .addFailureData(CIStepInfoUtils.getDefaultCIFailureDataInfo(
                                              k8sTaskExecutionResponse.getErrorMessage(), ambiance))
                                          .setErrorMessage(emptyIfNull(k8sTaskExecutionResponse.getErrorMessage()))
                                          .build());

...
import io.harness.ci.pipeline.executions.beans.CIBuildDeleteHook;
...
public class CIModuleInfoMapper {
  ...
  public static CIWebhookInfoDTO getCIBuildResponseDTO(ExecutionSource executionSource) throws InternalError {
    ...
    if (executionSource != null) {
      if (executionSource.getType() == ExecutionSource.Type.WEBHOOK) {
        ...
        } else if (webhookExecutionSource.getWebhookEvent().getType() == WebhookEvent.Type.DELETE) {
          DeleteWebhookEvent deleteWebhookEvent = (DeleteWebhookEvent) webhookExecutionSource.getWebhookEvent();
          ciWebhookInfoDTO.setDelete(convertDelete(deleteWebhookEvent));
          ciWebhookInfoDTO.setEvent(DELETE);
        }
      }
    }
  ...
  private static CIBuildDeleteHook convertDelete(DeleteWebhookEvent delete) {
    return CIBuildDeleteHook.builder().ref(delete.getRef()).build();
  }

...
public class CIAccountValidationServiceImpl implements CIAccountValidationService {
  ...
  @Override
  public boolean isAccountValidForExecution(String accountId, String moduleType) {
    Integer trustLevel = AccountTrustLevel.BASIC_USER;
    try {
      LicensesWithSummaryDTO ciLicense = ciLicenseService.getLicenseSummary(accountId, moduleType);
      if (ciLicense == null) {
      ...
      if (ciLicense != null) {
        if (ciLicense.getEdition() == Edition.ENTERPRISE || ciLicense.getEdition() == Edition.TEAM
            || ciLicense.getEdition() == Edition.ESSENTIALS || ciLicense.getLicenseType() == LicenseType.PAID) {
          return true;
        }
      }
  ...
  @Override
  public long getMaxBuildPerDay(String accountId, String moduleType) {
    int trustLevel = obtainTrustLevel(accountId, moduleType);
  ...
  @Override
  public long getMaxCreditsPerMonth(String accountId, String moduleType) {
    int trustLevel = obtainTrustLevel(accountId, moduleType);
  ...
  private Integer obtainTrustLevel(String accountId, String moduleType) {

...
public class CIStagePlanCreationUtils {
  ...
  static final String NOT_VERIFIED_ERROR =
      "We apologize, but your account is not verified for Harness Cloud. To resolve this issue, please use your work "
      + "email or contact support to request account verification: support@harness.io";
  ...
  public StageElementParametersBuilder getStageParameters(IntegrationStageNode stageNode) {
    ...
    stageBuilder.identities(stageNode.getIdentities());
  ...
  public void validateFreeAccountStageExecutionLimit(
      String accountId, Infrastructure infrastructure, String moduleType) {
    if (isHostedInfra(infrastructure)) {
      LicensesWithSummaryDTO licensesWithSummaryDTO = ciLicenseService.getLicenseSummary(accountId, moduleType);
      ...
      if (licensesWithSummaryDTO != null && licensesWithSummaryDTO.getEdition() == Edition.FREE) {
        ...
        if (accountExecutionMetadata.isPresent()) {
          enforceCreditsCount(accountId, moduleType);
        }
      }
    }
  }
  ...
  public void validateFreeAccountStageExecutionLimit(
      boolean isFreePlan, String accountId, Infrastructure infrastructure, String moduleType) {
    if (!isFreePlan) {
      return;
    }
    if (isHostedInfra(infrastructure)) {
  ...
  private void enforceCreditsCount(String accountId, String moduleType) {
    ...
    try {
      ...
      long monthlyLimit = validationService.getMaxCreditsPerMonth(accountId, moduleType);
      ...
      log.error("Exception occurred while fetching credits usage ", e);
    }
  }

  private void enforceBuildsCount(
      String accountId, Optional<CIAccountExecutionMetadata> accountExecutionMetadata, String moduleType) {
    ...
    if (countPerDay != null) {
      long maxBuildsPerDay = validationService.getMaxBuildPerDay(accountId, moduleType);

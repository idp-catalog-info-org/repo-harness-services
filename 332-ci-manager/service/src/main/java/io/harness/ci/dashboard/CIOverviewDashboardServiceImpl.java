...
public class CIOverviewDashboardServiceImpl implements CIOverviewDashboardService {
  @Inject @Nullable TimeScaleDBService timeScaleDBService;
  ...
  private static final List<String> failedList = Arrays.asList(ExecutionStatus.FAILED.name(),
      ExecutionStatus.ABORTED.name(), ExecutionStatus.EXPIRED.name(), ExecutionStatus.ERRORED.name());

  private static final List<String> activeStatusList = Arrays.asList(ExecutionStatus.RUNNING.name(),
  ...
  public long getActiveCommittersCount(String accountId) throws SQLException {
    return getActiveCommittersCountExecutionSummaryTable(accountId);
  }

  public long getActiveCommittersCountExecutionSummaryTable(String accountId) throws SQLException {
    long timestamp = System.currentTimeMillis();
    long totalTries = 0;
    String query = "select count(distinct moduleinfo_author_id) from " + tableName
        + (" where accountid=? and moduleinfo_type ='CI' and moduleinfo_author_id is not null and "
            + "moduleinfo_is_private=true and (trigger_type='")
  ...
  public long getHostedCreditUsage(String accountId, long startInterval, long endInterval) throws SQLException {
    ...
    long totalTries = 0;
    String query = "select sum(ceil(cputime/60000.0)*buildmultiplier) from " + tableNameStageSummary
  ...
  @Override
  public UsageDataDTO getActiveCommitters(String accountId, long timestamp) {
    return getActiveCommittersExecutionSummaryTable(accountId, timestamp);
  }

  public UsageDataDTO getActiveCommittersExecutionSummaryTable(String accountId, long timestamp) {
    long totalTries = 0;
    ...
    while (totalTries <= MAX_RETRY_COUNT) {
      ...
      try (Connection connection = timeScaleDBService.getDBConnection();
        ...
        statement.setLong(3, timestamp - 30 * DAY_IN_MS);
        resultSet = statement.executeQuery();
        return buildUsageDataDTOActiveCommitters(resultSet);
      } catch (SQLException ex) {
        log.error("Caught SQL Exception:" + ex.getMessage());
      } finally {
        DBUtils.close(resultSet);
  ...
  private UsageDataDTO buildUsageDataDTOActiveCommitters(ResultSet resultSet) throws SQLException {
  ...
  public StatusAndTime queryCalculatorForStatusAndTime(String accountId,
    ...
    String uniqueIds = orgProjectIdentifiers.stream()
                           .map(OrgProjectIdentifier::getUniqueId)
                           .distinct()
                           .map(u -> String.format("'%s'", u))
                           .collect(Collectors.joining(","));

    if (ciFeatureFlagService.isEnabled(CI_USE_UNIQUE_PARENT_ID_FOR_QUERY, accountId) && isNotEmpty(uniqueIds)) {
      selectStatusQuery += " and parent_unique_id IN (" + uniqueIds + ") and startts>=? and startts<?;";
    } else {
  ...
  public DashboardBuildsHealthInfo getDashBoardBuildHealthInfoWithRate(String accountId, String orgId, String projectId,
    ...
    endInterval = endInterval + DAY_IN_MS;
    String uniqueId = scopeInfoHelper.getParentUniqueId(accountId, orgId, projectId);
    OrgProjectIdentifier orgProjectIdentifier =
        OrgProjectIdentifier.builder().orgIdentifier(orgId).projectIdentifier(projectId).uniqueId(uniqueId).build();
  ...
  public DashboardBuildExecutionInfo getBuildExecutionBetweenIntervals(
    ...
    List<OrgProjectIdentifier> orgProjectIdentifiers =
        getOrgProjectIdentifierList(orgProjectIdentifierList, orgId, projectId, uniqueId);
  ...
  public List<BuildFailureInfo> queryCalculatorBuildFailureInfo(
      String accountId, String orgId, String projectId, long limit, long startInterval, long endInterval) {
    ...
    String parentUniqueId = getParentUniqueId(accountId, orgId, projectId);
    String selectStatusQuery =
        "select name, pipelineidentifier, moduleinfo_branch_name, moduleinfo_branch_commit_message, moduleinfo_event, "
        + "moduleinfo_repository, planexecutionid, source_branch, moduleinfo_branch_commit_id, moduleinfo_author_id, "
        + "author_avatar, startts, trigger_type, endts, status, id  from " + tableName
        + (" where accountid=? and orgidentifier=? and projectidentifier=? and startts>=? and startts<? and status in "
            + "(");
    if (!isEmpty(parentUniqueId)) {
      selectStatusQuery = "select name, pipelineidentifier, moduleinfo_branch_name, moduleinfo_branch_commit_message, "
          + "moduleinfo_event, moduleinfo_repository, planexecutionid, source_branch, moduleinfo_branch_commit_id, "
          + "moduleinfo_author_id, author_avatar, startts, trigger_type, endts, status, id  from " + tableName
          + " where parent_unique_id=? and startts>=? and startts<? and status in (";
    }
    ...
    while (totalTries <= MAX_RETRY_COUNT) {
      ...
      try (Connection connection = timeScaleDBService.getDBConnection();
        ...
        setPrepareStatement(accountId, orgId, projectId, startInterval, endInterval, limit, statement, parentUniqueId);
        resultSet = statement.executeQuery();
        while (resultSet != null && resultSet.next()) {
  ...
  public List<BuildActiveInfo> queryCalculatorBuildActiveInfo(
    ...
    String parentUniqueId = getParentUniqueId(accountId, orgId, projectId);
    String selectStatusQuery =
        "select name, pipelineidentifier, moduleinfo_branch_name, planexecutionid, moduleinfo_branch_commit_message, "
        + "moduleinfo_branch_commit_id, source_branch, moduleinfo_author_id, author_avatar, moduleinfo_event, "
        + "moduleinfo_repository, startts, status, trigger_type, id   from " + tableName
        + " where accountid=? and orgidentifier=? and projectidentifier=? and startts>=? and startts<? and ";
    if (!isEmpty(parentUniqueId)) {
      selectStatusQuery =
          "select name, pipelineidentifier, moduleinfo_branch_name, planexecutionid, moduleinfo_branch_commit_message, "
          + "moduleinfo_branch_commit_id, source_branch, moduleinfo_author_id, author_avatar, moduleinfo_event, "
          + "moduleinfo_repository, startts, status, trigger_type, id   from " + tableName
          + " where parent_unique_id=? and startts>=? and startts<? and ";
    }
    StringBuilder totalBuildSqlBuilder = new StringBuilder(2048);
  ...
  private String getParentUniqueId(String accountId, String orgId, String projectId) {
    boolean useParentUniqueIdQuery = ciFeatureFlagService.isEnabled(CI_USE_UNIQUE_PARENT_ID_FOR_QUERY, accountId);
    if (!useParentUniqueIdQuery) {
      return null;
    }
    return scopeInfoHelper.getParentUniqueId(accountId, orgId, projectId);
  }
  ...
  public List<BuildFailureInfo> getDashboardBuildFailureInfo(
    ...
    return queryCalculatorBuildFailureInfo(accountId, orgId, projectId, days, startInterval, endInterval);
  }

  @Override
  public List<BuildActiveInfo> getDashboardBuildActiveInfo(
      String accountId, String orgId, String projectId, long days, long startInterval, long endInterval) {
    return queryCalculatorBuildActiveInfo(accountId, orgId, projectId, days, startInterval, endInterval);
  }
  ...
  public RepositoryInformation queryRepositoryCalculator(
    ...
    String parentUniqueId = getParentUniqueId(accountId, orgId, projectId);
    String selectStatusQuery = "select moduleinfo_repository, status, startts, endts, "
        + "moduleinfo_branch_commit_message, moduleinfo_author_id, author_avatar  from " + tableName
        + (" where accountid=? and orgidentifier=? and projectidentifier=? and moduleinfo_repository IS NOT NULL and "
            + "startts>=? and startts<?;");
    if (!isEmpty(parentUniqueId)) {
      selectStatusQuery = "select moduleinfo_repository, status, startts, endts, moduleinfo_branch_commit_message, "
          + "moduleinfo_author_id, author_avatar  from " + tableName
          + " where parent_unique_id=? and moduleinfo_repository IS NOT NULL and startts>=? and startts<?;";
    }
    ...
    while (totalTries <= MAX_RETRY_COUNT) {
      ...
      try (Connection connection = timeScaleDBService.getDBConnection();
           PreparedStatement statement = connection.prepareStatement(selectStatusQuery)) {
        setPrepareStatement(accountId, orgId, projectId, previousStartInterval, endInterval, statement, parentUniqueId);
  ...
  public CIUsageResult getCIUsageResult(String accountId, long timestamp) {
    return CIUsageResult.builder()
        ...
        .timestamp(timestamp)
        .module("CI")
        .activeCommitters(getActiveCommitters(accountId, timestamp))
        .build();
  }
  ...
  private void setPrepareStatement(String accountId, String orgId, String projectId, long startInterval,
      long endInterval, PreparedStatement preparedStatement, String parentUniqueId) throws SQLException {
    if (!isEmpty(parentUniqueId)) {
      preparedStatement.setString(1, parentUniqueId);
      preparedStatement.setLong(2, startInterval);
      preparedStatement.setLong(3, endInterval);
      return;
    }
    preparedStatement.setString(1, accountId);
    preparedStatement.setString(2, orgId);
    preparedStatement.setString(3, projectId);
    preparedStatement.setLong(4, startInterval);
    preparedStatement.setLong(5, endInterval);
  }

  private void setPrepareStatement(String accountId, String orgId, String projectId, long startInterval,
      long endInterval, long limit, PreparedStatement preparedStatement, String parentUniqueId) throws SQLException {
    if (!isEmpty(parentUniqueId)) {
      preparedStatement.setString(1, parentUniqueId);
      preparedStatement.setLong(2, startInterval);
      preparedStatement.setLong(3, endInterval);
    ...
    preparedStatement.setLong(4, startInterval);
    preparedStatement.setLong(5, endInterval);
    preparedStatement.setLong(6, limit);
  }
    ...
    return emptyIfNull(listOfAccessibleProject)
        ...
        .map(projectDTO
            -> OrgProjectIdentifier.builder()
                   ...
                   .uniqueId(projectDTO.getUniqueId())
                   .build())
        .collect(Collectors.toList());
  }

  private List<OrgProjectIdentifier> getOrgProjectIdentifierList(List<OrgProjectIdentifier> orgProjectIdentifierList,
      String orgIdentifier, String projectIdentifier, String uniqueId) {
    if (isNotEmpty(orgIdentifier) && isNotEmpty(projectIdentifier)) {
      OrgProjectIdentifier orgProjectIdentifier = OrgProjectIdentifier.builder()
                                                      .orgIdentifier(orgIdentifier)
                                                      .projectIdentifier(projectIdentifier)
                                                      .uniqueId(uniqueId)
                                                      .build();
      boolean exists = orgProjectIdentifierList.stream().anyMatch(
          opi -> orgIdentifier.equals(opi.getOrgIdentifier()) && projectIdentifier.equals(opi.getProjectIdentifier()));

...
public class ActiveDevelopersSyncHandler implements MongoPersistenceIterator.Handler<IteratorEntity> {
  private PersistenceIteratorFactory persistenceIteratorFactory;
  private MongoTemplate mongoTemplate;
  private IDPModuleLicenseUsage idpModuleLicenseUsage;
  private static final String ACTIVE_DEVELOPERS_SYNC_HANDLER = "ActiveDevelopersSyncHandler";

  @Override
  public void handle(IteratorEntity entity) {
    log.info("Active developers sync iterator started");
    try {
      idpModuleLicenseUsage.populateActiveDevelopersDataForSync();
    } catch (Exception ex) {
      log.error("Error in running active developers iterator. Error = {}", ex.getMessage(), ex);
    }
    log.info("Active developers iterator completed");
  }

  public void registerIterators(IteratorConfig iteratorConfig) {
    persistenceIteratorFactory.createPumpIteratorWithDedicatedThreadPool(
        PersistenceIteratorFactory.PumpExecutorOptions.builder()
            .name(ACTIVE_DEVELOPERS_SYNC_HANDLER)
            .poolSize(iteratorConfig.getThreadPoolCount())
            .interval(ofSeconds(10))
            .build(),
        ActiveDevelopersSyncHandler.class,
        MongoPersistenceIterator.<IteratorEntity, SpringFilterExpander>builder()
            .clazz(IteratorEntity.class)
            .filterExpander(
                query -> query.addCriteria(where(IteratorEntity.IteratorsKeys.name).is(ACTIVE_DEVELOPERS_SYNC_HANDLER)))
            .fieldName(IteratorEntity.IteratorsKeys.nextIteration)
            .targetInterval(ofSeconds(iteratorConfig.getTargetIntervalInSeconds()))
            .acceptableExecutionTime(ofSeconds(150))

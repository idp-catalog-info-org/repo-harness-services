...
public class IDPTelemetryRecordsHandler implements MongoPersistenceIterator.Handler<IteratorEntity> {
  private PersistenceIteratorFactory persistenceIteratorFactory;
  private MongoTemplate mongoTemplate;
  private IDPTelemetryPublisher publisher;
  private static final String IDP_TELEMETRY_RECORDS_HANDLER = "IDPTelemetryRecordsHandler";

  @Override
  public void handle(IteratorEntity entity) {
    try {
      log.info("IDPTelemetryRecordsHandler started");
      publisher.recordTelemetry();
      log.info("IDPTelemetryRecordsHandler completed");
    } catch (Exception e) {
      log.error("Exception in IDPTelemetryRecordsHandler. Error = {}", e.getMessage(), e);
    }
  }
  ...
  public void registerIterators(IteratorConfig iteratorConfig) {
    persistenceIteratorFactory.createPumpIteratorWithDedicatedThreadPool(
        PersistenceIteratorFactory.PumpExecutorOptions.builder()
            .name(IDP_TELEMETRY_RECORDS_HANDLER)
            .poolSize(iteratorConfig.getThreadPoolCount())
            .interval(ofSeconds(10))
            .build(),
        IDPTelemetryRecordsHandler.class,
        MongoPersistenceIterator.<IteratorEntity, SpringFilterExpander>builder()
            .clazz(IteratorEntity.class)
            .filterExpander(
                query -> query.addCriteria(where(IteratorEntity.IteratorsKeys.name).is(IDP_TELEMETRY_RECORDS_HANDLER)))

...
@OwnedBy(IDP)
@Singleton
public class AppConfigsKafkaConsumer extends CdcKafkaAvroConsumer {
  @Inject
  public AppConfigsKafkaConsumer(AppConfigsCdcMessageHandler messageHandler,
  ...
  private AppConfigsKafkaConsumer(CdcKafkaConsumerConfig consumerConfig, KafkaBaseConfig kafkaBaseConfig,
      AppConfigsCdcMessageHandler messageHandler, ConsumerMaintenanceListener consumerMaintenanceListener,
      FeatureFlagService featureFlagService) {
    super(consumerConfig.getTopic(), CdcKafkaConstants.APP_CONFIGS_CONSUMER_GROUP, kafkaBaseConfig, messageHandler,
        consumerMaintenanceListener, consumerConfig.getMaxPollRecords(), featureFlagService,
        FeatureName.IDP_CDC_KAFKA_APP_CONFIGS, consumerConfig.isUseJsonValueFormat());
  }
    ...
    return cdcKafkaConfig.getConsumer(CdcKafkaConfig.APP_CONFIGS_CONSUMER)
        .orElseThrow(()
                         -> new IllegalStateException(
                             "Missing CDC Kafka consumer config for '" + CdcKafkaConfig.APP_CONFIGS_CONSUMER + "'"));
  }
}

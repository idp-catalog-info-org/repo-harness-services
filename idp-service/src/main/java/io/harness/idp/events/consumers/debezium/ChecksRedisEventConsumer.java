...
public class ChecksRedisEventConsumer extends DebeziumAbstractRedisConsumer implements IdpRedisConsumer {
  private final FeatureFlagService featureFlagService;
  private final boolean redisShortCircuit;

  @Inject
  public ChecksRedisEventConsumer(@Named(IDP_CHECKS_REDIS_EVENT_CONSUMER) Consumer redisConsumer,
      QueueController queueController, ChecksChangeEventHandler eventHandler,
  ...
  protected boolean processMessage(Message message) {
    try {
      if (redisShortCircuit && featureFlagService.isGlobalEnabled(FeatureName.IDP_CDC_KAFKA_CHECKS)) {
        return true;
      }
    } catch (Exception e) {
      log.warn(
          "Failed to evaluate FF {}, falling back to Redis processing", FeatureName.IDP_CDC_KAFKA_CHECKS.name(), e);
    }

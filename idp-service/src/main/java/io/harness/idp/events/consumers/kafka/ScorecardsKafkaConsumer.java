...
@OwnedBy(IDP)
@Singleton
public class ScorecardsKafkaConsumer extends CdcKafkaAvroConsumer {
  @Inject
  public ScorecardsKafkaConsumer(ScorecardsCdcMessageHandler messageHandler,
  ...
  private ScorecardsKafkaConsumer(CdcKafkaConsumerConfig consumerConfig, KafkaBaseConfig kafkaBaseConfig,
      ScorecardsCdcMessageHandler messageHandler, ConsumerMaintenanceListener consumerMaintenanceListener,
      FeatureFlagService featureFlagService) {
    super(consumerConfig.getTopic(), CdcKafkaConstants.SCORECARDS_CONSUMER_GROUP, kafkaBaseConfig, messageHandler,
        consumerMaintenanceListener, consumerConfig.getMaxPollRecords(), featureFlagService,
        FeatureName.IDP_CDC_KAFKA_SCORECARDS, consumerConfig.isUseJsonValueFormat());
  }

  private static CdcKafkaConsumerConfig resolveConsumerConfig(CdcKafkaConfig cdcKafkaConfig) {
    return cdcKafkaConfig.getConsumer(CdcKafkaConfig.SCORECARDS_CONSUMER)
        .orElseThrow(()
                         -> new IllegalStateException(
                             "Missing CDC Kafka consumer config for '" + CdcKafkaConfig.SCORECARDS_CONSUMER + "'"));
  }
}

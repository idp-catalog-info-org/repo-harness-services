...
public class ModuleLicensesRedisEventConsumer extends DebeziumAbstractRedisConsumer implements IdpRedisConsumer {
  ...
  protected boolean processMessage(Message message) {
    try {
      if (redisShortCircuit && featureFlagService.isGlobalEnabled(FeatureName.IDP_CDC_KAFKA_MODULE_LICENSES)) {
        return true;
      }
    } catch (Exception e) {
      log.warn("Failed to evaluate FF {}, falling back to Redis processing",
          FeatureName.IDP_CDC_KAFKA_MODULE_LICENSES.name(), e);
    }

...
public class CatalogRedisEventConsumer extends DebeziumAbstractRedisConsumer implements IdpRedisConsumer {
  private final FeatureFlagService featureFlagService;
  private final boolean redisShortCircuit;

  @Inject
  public CatalogRedisEventConsumer(@Named(IDP_CATALOG_REDIS_EVENT_CONSUMER) Consumer redisConsumer,
      QueueController queueController, CatalogChangeEventHandler eventHandler,
  ...
  // Short-circuits Redis processing only when BOTH the FF is ON (Kafka consumer is actively
  // processing) AND redisShortCircuit=true (operator confirmed Kafka is healthy and Redis can stop).
  // During FF transitions both consumers may briefly process the same event — handlers must be idempotent.
  @Override
  protected boolean processMessage(Message message) {
    try {
      if (redisShortCircuit && featureFlagService.isGlobalEnabled(FeatureName.IDP_CDC_KAFKA_CATALOG)) {
        return true;
      }
    } catch (Exception e) {
      log.warn(
          "Failed to evaluate FF {}, falling back to Redis processing", FeatureName.IDP_CDC_KAFKA_CATALOG.name(), e);
    }

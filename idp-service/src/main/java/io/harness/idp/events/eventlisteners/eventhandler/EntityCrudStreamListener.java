  ...
  public boolean handleMessage(Message message) {
    ...
    try (AutoLogContext ignore = new NgEventLogContext(messageId, OVERRIDE_ERROR)) {
      ...
      if (eventMessageHandler != null) {
        AcquiredLock lock;
        try {
          if (eventMessageHandler.getClass().equals(AccountMessageHandler.class)) {
            AccountEntityChangeDTO accountEntityChangeDTO =
                AccountEntityChangeDTO.parseFrom(message.getMessage().getData());
            entityChangeDTO = EntityChangeDTO.newBuilder()
                                  .setAccountIdentifier(StringValue.of(accountEntityChangeDTO.getAccountId()))
                                  .build();
          } else if (eventMessageHandler.getClass().equals(ProjectMessageHandler.class)) {
            ProjectEntityChangeDTO projectEntityChangeDTO =
                ProjectEntityChangeDTO.parseFrom(message.getMessage().getData());
            entityChangeDTO = EntityChangeDTO.newBuilder()
                                  .setAccountIdentifier(StringValue.of(projectEntityChangeDTO.getAccountIdentifier()))
                                  .setIdentifier(StringValue.of(projectEntityChangeDTO.getIdentifier()))
                                  .build();
          } else if (eventMessageHandler.getClass().equals(OrganizationMessageHandler.class)) {
            ...
            entityChangeDTO =
                EntityChangeDTO.newBuilder()
                    .setAccountIdentifier(StringValue.of(organizationEntityChangeDTO.getAccountIdentifier()))
                    .setIdentifier(StringValue.of(organizationEntityChangeDTO.getIdentifier()))
                    .build();
          } else {

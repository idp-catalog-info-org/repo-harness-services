...
@Slf4j
public abstract class DataSourceProviderV1 implements DataSourceProvider {
  ...
  protected DataSourceProviderV1(String identifier, DataPointService dataPointService,
  ...
  protected abstract Map<String, String> getAuthHeaders(String accountIdentifier, String configs, String host);
  ...
  protected Map<String, Map<String, Object>> processOut(String accountIdentifier, String identifier, Object entity,
      Map<String, String> replaceableHeaders, Map<String, String> possibleReplaceableRequestBodyPairs,
      Map<String, String> possibleReplaceableUrlPairs, List<DataFetchDTO> dataToFetch, boolean throughDelegate,
    ...
    for (Map.Entry<String, List<DataFetchDTO>> entry : dataToFetchByDsl.entrySet()) {
      ...
      log.debug("Fetching data for entity {} and DTOs {}", getEntityUUId(entity), dataToFetchForDsl);
      ...
      if (dataSourceLocation instanceof DataSourceLocationLoop) {
        try {
          Map<String, Object> response = dataSourceLocation.fetchData(accountIdentifier, entity,
              dataSourceLocationEntity, dataToFetchForDsl, replaceableHeaders, possibleReplaceableRequestBodyPairs,
              possibleReplaceableUrlPairs, config, throughDelegate, delegateSelectors);
          log.info(
              "Response for DSL in Process out - entity - {} dsl Identifier - {} rule identifiers - {} Response - {} ",
              getEntityUUId(entity), dslIdentifier, ruleIdentifiers, response);
          ...
          log.warn("Could not fetch data for entity - {} dsl identifier - {} rule identifiers - {}",
              getEntityUUId(entity), dslIdentifier, ruleIdentifiers, e);
        }
      } else {
        for (DataFetchDTO dataFetchDTO : dataToFetchForDsl) {
          try {
            Map<String, Object> response =
                dataSourceLocation.fetchData(accountIdentifier, entity, dataSourceLocationEntity,
                    Collections.singletonList(dataFetchDTO), replaceableHeaders, possibleReplaceableRequestBodyPairs,
                    possibleReplaceableUrlPairs, config, throughDelegate, delegateSelectors);
            log.info("Response for DSL in Process out - entity - {} dsl Identifier - {} rule identifiers - {} Response "
                    + "- {} ",
                getEntityUUId(entity), dslIdentifier, ruleIdentifiers, response);
    ...
    log.info("Aggregated data for entity with uuid {} : {}", getEntityUUId(entity), aggregatedData);
  ...
  private String getEntityUUId(Object entity) {
    if (entity instanceof CatalogEntity) {
      return CatalogUtils.entityRefV1((CatalogEntity) entity);
    } else {

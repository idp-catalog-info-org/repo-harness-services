...
import static io.harness.idp.scorecard.datapoints.constants.DataPoints.K8_LABEL_SELECTOR_ANNOTATION_ERROR;
...
public class KubernetesDsl implements DslDataProvider {
  ...
  public static final Map<String, String> WORKLOAD_API_PATHS =
      Map.of("deployment", "apis/apps/v1/deployments", "statefulset", "apis/apps/v1/statefulsets");

  public static final Map<String, String> WORKLOAD_WITH_NAMESPACE_API_PATHS = Map.of(
      "deployment", "apis/apps/v1/namespaces/%s/deployments", "statefulset", "apis/apps/v1/namespaces/%s/statefulsets");
  ...
  public Map<String, Object> getDslData(String accountIdentifier, Object config) {
    ...
    if (isEmpty(labelSelector)) {
      returnData.put(ERROR_MESSAGE_KEY, K8_LABEL_SELECTOR_ANNOTATION_ERROR);
      return returnData;
    }
    ...
    for (ClusterConfig cluster : clusters) {
      ...
      DslClient client = dslClientFactory.getClient(accountIdentifier, uri.getHost(), false);
      ...
      for (Map.Entry<String, String> entry : paths.entrySet()) {
        try {
          ...
          Response response = client.call(accountIdentifier, requestDetails, Set.of(), null);

...
public class ScorecardDetailsMapper {
  ...
  public ScorecardDetailsResponse toDTO(ScorecardEntity scorecardEntity, Map<String, CheckEntity> checkEntityMap,
    ...
    details.setOnDemand(scorecardEntity.isOnDemand());
    details.setWeightageStrategy(scorecardEntity.getWeightageStrategy());
    details.setPercentage(null);
    if (tierAnalyticsEnabled) {
      details.setTierGroupIdentifier(scorecardEntity.getTierGroupIdentifier());
      if (scorecardEntity.getComponentCount() != null) {
        details.setComponents(scorecardEntity.getComponentCount());
        details.setTierAnalytics(ScorecardMapper.toTierAnalytics(scorecardEntity));
      }
    } else {
      details.setTierAnalytics(null);
      if (countAndPercentage != null) {
        details.setComponents(countAndPercentage.getCount());
        details.setPercentage((double) Math.round(countAndPercentage.getPercentage() * 100.0));
      }
    }
  ...
  public ScorecardEntity fromDTO(ScorecardDetailsRequest scorecardDetailsRequest, String accountIdentifier) {
    ...
    return ScorecardEntity.builder()
        ...
        .onDemand(details.isOnDemand())
        .weightageStrategy(details.getWeightageStrategy())
        .tierGroupIdentifier(details.getTierGroupIdentifier())
        .build();
  }
}

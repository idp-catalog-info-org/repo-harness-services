...
public class HarnessTestPassingInCi implements DslDataProvider {
  ...
  @Inject
  public HarnessTestPassingInCi(TIServiceUtils tiServiceUtils, TIServiceClient tiServiceClient,
      PipelineServiceClient pipelineServiceClient,
      PipelineTestSummaryReportResponseFactory pipelineTestSummaryReportResponseFactory,
      @Named("tiServiceConfig") TIServiceConfig tiServiceConfig) {
    this.tiServiceUtils = tiServiceUtils;
    this.tiServiceClient = tiServiceClient;
    this.pipelineServiceClient = pipelineServiceClient;
    this.pipelineTestSummaryReportResponseFactory = pipelineTestSummaryReportResponseFactory;
  ...
  public Map<String, Object> getDslData(String accountIdentifier, Object config) {
    ...
    try {
      ...
      log.error(String.format("Error in getting the ci pipeline info of test passing on ci check in account - %s, org "
    ...
    try {
      ...
      log.error(String.format("Error in getting the summary report info  from ti service of test passing in ci check "
                        + "account - %s, org - %s, project - %s, and pipeline - %s",
                    ciIdentifiers.get(DslConstants.CI_ACCOUNT_IDENTIFIER_KEY),
                    ciIdentifiers.get(DslConstants.CI_ORG_IDENTIFIER_KEY),
                    ciIdentifiers.get(DslConstants.CI_PROJECT_IDENTIFIER_KEY),
                    ciIdentifiers.get(DslConstants.CI_PIPELINE_IDENTIFIER_KEY)),

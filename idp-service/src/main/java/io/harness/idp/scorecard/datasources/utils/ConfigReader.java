...
public class ConfigReader {
  ...
  public Object getConfigValues(String accountIdentifier, String yaml, String keyExpression) {
    ...
    Object value;
    try {
      value = evaluator.evaluateExpression(keyExpression, ExpressionMode.RETURN_NULL_IF_UNRESOLVED);
    } catch (HintException | EngineExpressionEvaluationException e) {
      // JEXL 3.5 throws on missing property/variable instead of returning null. Honor the
      // RETURN_NULL_IF_UNRESOLVED contract by treating an unresolved expression as null here.
      log.info("Could not resolve expression - {}", keyExpression, e);
      return null;
    }
    if (value == null) {
  ...
  public String getDecryptedValue(String accountIdentifier, String envName) {
    ...
    if (envVariableOpt.isPresent() && envVariableOpt.get().getType().equals(BackstageEnvVariable.TypeEnum.SECRET)) {
      ...
      String decryptedValue = backstageEnvVariableService
                                  .getDecryptedValueAndLastModifiedTime(
                                      envName, secret.getHarnessSecretIdentifier(), accountIdentifier, null, null)
                                  .getFirst();

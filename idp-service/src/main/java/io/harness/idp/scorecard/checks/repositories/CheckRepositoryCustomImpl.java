...
public class CheckRepositoryCustomImpl implements CheckRepositoryCustom {
  ...
  @Override
  public List<String> findUniqueTags(String accountIdentifier, String searchTerm, int limit) {
    Criteria criteria =
        Criteria.where(CheckKeys.accountIdentifier).is(accountIdentifier).and(CheckKeys.isDeleted).is(false);

    List<AggregationOperation> operations = new ArrayList<>();
    operations.add(Aggregation.match(criteria));
    operations.add(Aggregation.unwind(CheckKeys.tags));

    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
      operations.add(Aggregation.match(
          Criteria.where(CheckKeys.tags).regex(".*" + CommonUtils.escapeRegexMetacharacters(searchTerm) + ".*", "i")));
    }
    operations.add(Aggregation.group(CheckKeys.tags));
    operations.add(Aggregation.limit(limit));
    operations.add(Aggregation.project().andExpression("_id").as("tag"));

    Aggregation aggregation = Aggregation.newAggregation(operations);
    AggregationResults<CheckTagResult> results =
        mongoTemplate.aggregate(aggregation, CheckEntity.class, CheckTagResult.class);

    List<String> tags = new ArrayList<>();
    results.getMappedResults().forEach(checkTagResult -> {
      String tag = checkTagResult.getTag();

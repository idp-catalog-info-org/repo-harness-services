...
public class AllowListServiceImpl implements AllowListService {
  ...
  private static final Pattern ALLOWLIST_URL_PATTERN =
      Pattern.compile("^(?:\\*\\.)?[a-zA-Z0-9-]+(\\.[a-zA-Z0-9-]+)*\\.[a-zA-Z]{2,}(:\\d{1,5}(-\\d{1,5})?)?$");
  ...
  @Inject
  AllowListServiceImpl(ConfigManagerService configManagerService,
  ...
  void validateRequest(List<HostInfo> hostInfoList) {
    for (HostInfo hostInfo : hostInfoList) {
      if (isEmpty(hostInfo.getHost())) {
        throw new InvalidRequestException("Host cannot be empty");
      }
      if (!isValidUrl(hostInfo.getHost())) {
        String errorMessage =
            String.format("Invalid host URL : [%s] . Please enter a valid host in one of the following formats: "
                    + "example.com, *.example.com, *.example.c.co, example.com:80, example.com:80-60 ",
                hostInfo.getHost());
  ...
  private void createOrUpdateAllowListAppConfig(String config, String accountIdentifier) throws Exception {
    ...
    if (!isValidSchema(config, schema).isEmpty()) {
    ...
    Failsafe.with(transactionRetryPolicy).get(() -> transactionTemplate.execute(status -> {
      ...
      configManagerService.saveUpdateAndMergeConfigForAccount(appConfig, accountIdentifier, ConfigType.BACKEND, false);
  ...
  private boolean isValidUrl(String url) {
    Matcher matcher = ALLOWLIST_URL_PATTERN.matcher(url);

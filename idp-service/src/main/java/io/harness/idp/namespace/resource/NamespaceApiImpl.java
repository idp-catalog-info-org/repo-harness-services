...
public class NamespaceApiImpl implements NamespaceApi {
  ...
  @Override
  public Response updateNamespaceInfo(@Valid NamespaceRequest body, String harnessAccount) {
    try {
      NamespaceInfo namespaceInfo = namespaceService.update(harnessAccount, body);
      return Response.status(Response.Status.OK).entity(namespaceInfo).build();
    } catch (Exception e) {
      log.error("Error in updating namespace for accountId - {}", harnessAccount, e);
      return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
          .entity(ResponseMessage.builder().message(e.getMessage()).build())
          .build();
    }

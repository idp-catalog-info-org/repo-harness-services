...
public class DelegateProxyApiImpl implements DelegateProxyApi {
  ...
  public Response forwardProxy(@Context UriInfo info, @Context javax.ws.rs.core.HttpHeaders headers,
    ...
    try (AutoLogContext ignore1 =
      ...
      try {
        backstageProxyRequest = OBJECT_MAPPER.readValue(body, BackstageProxyRequest.class);
      } catch (Exception err) {
      ...
      Long executionTimeoutInSeconds =
          (isEmpty(backstageProxyRequest.getHeaders())
              || isEmpty(backstageProxyRequest.getHeaders().get("executiontimeoutinseconds")))
          ? null
          : Long.valueOf(backstageProxyRequest.getHeaders().get("executiontimeoutinseconds"));

      HttpStepResponse httpResponse = delegateProxyRequestForwarder.forwardRequestToDelegate(accountIdentifier,
          backstageProxyRequest.getUrl(), headerList, backstageProxyRequest.getBody(),
          backstageProxyRequest.getMethod(), delegateSelectors, null, executionTimeoutInSeconds);
        ...
        for (String header : responseHeaders.split(HEADER_SEPARATOR)) {
          ...
          if (name.equals(CONTENT_TYPE_HEADER)
              && (value.equals(APPLICATION_GZIP_MEDIA_TYPE) || value.equals(APPLICATION_OCTET_STREAM_MEDIA_TYPE)
                  || value.equals(APPLICATION_TAR_GZ_STREAM_MEDIA_TYPE))) {
            isByteResponse = true;
          }

...
@Slf4j
public class IDPCapacityPoller implements Managed {
  @Inject(optional = true) CICapacityPollerUtils executionPollerUtils;
  ...
  public void start() {
    ExecutorService executorService = Executors.newSingleThreadExecutor(
        new ThreadFactoryBuilder()
            .setNameFormat(executionPollerUtils.getModuleName() + "-global-queue-poller")
            .build());
  ...
  public void run() {
    ...
    try {
      do {
        ...
        executionPollerUtils.readEventsFrameworkMessagesWithOrder();
      } while (!Thread.currentThread().isInterrupted() && !shouldStop.get());
    } catch (Exception ex) {
      log.error("hsqs Consumer unexpectedly stopped", ex);
    } finally {
      log.info("finished consuming messages for {} capacity task", executionPollerUtils.getModuleName());
    }
  }

...
import static io.harness.idp.configmanager.utils.ReservedEnvVariables.RESERVED_ENV_VARIABLES;
...
public class ConfigEnvVariablesServiceImpl implements ConfigEnvVariablesService {
  ...
  @Inject
  private ConfigEnvVariablesServiceImpl(ConfigEnvVariablesRepository configEnvVariablesRepository,
  ...
  @Override
  public List<BackstageEnvSecretVariable> insertConfigEnvVariables(
      AppConfig appConfig, String accountIdentifier, boolean skipReservedEnvVariableCheck) {
    ...
    List<String> errorMessagesForEnvVariables =
        getErrorMessagesForEnvVariables(appConfig, accountIdentifier, skipReservedEnvVariableCheck);
  ...
  @Override
  public List<BackstageEnvSecretVariable> updateConfigEnvVariables(
      AppConfig appConfig, String accountIdentifier, boolean skipReservedEnvVariableCheck) {
    ...
    List<BackstageEnvSecretVariable> newBackstageEnvVariables = new ArrayList<>(appConfig.getEnvVariables());
    ...
    newBackstageEnvVariables.removeIf(backstageEnvSecretVariable
        -> newlyAddedEnvSecretVariables.stream().anyMatch(
            backstageEnv -> backstageEnv.getEnvName().equals(backstageEnvSecretVariable.getEnvName())));
  ...
  private List<String> getErrorMessagesForEnvVariables(
      AppConfig appConfig, String accountIdentifier, boolean skipReservedEnvVariableCheck) {
    ...
    for (BackstageEnvSecretVariable backstageEnvSecretVariable : listEnvVariables) {
      ...
      if (!skipReservedEnvVariableCheck && isReservedEnvVariable(envName)) {
  ...
  private Boolean isReservedEnvVariable(String envVariableName) {
    for (Pattern reservedEnvVariable : RESERVED_ENV_VARIABLES) {
      if (reservedEnvVariable.matcher(envVariableName).matches()) {
        return true;
      }
    }
    return false;

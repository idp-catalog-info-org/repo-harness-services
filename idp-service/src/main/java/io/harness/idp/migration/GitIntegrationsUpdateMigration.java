...
public class GitIntegrationsUpdateMigration implements NGMigration {
  IntegrationEntityRepository integrationEntityRepository;
  GitIntegrationServiceImpl gitIntegrationService;

  @Override
  public void migrate() {
    log.info("Starting the migration for git integrations update sync.");
    ...
    integrationEntities.forEach(integrationEntity -> {
      if (integrationEntity.isManaged()) {
        return;
      }
      try {
        ...
        log.warn("Error in updating git integration for sync - Account {} Identifier {} Error {}",
            integrationEntity.getAccountIdentifier(), integrationEntity.getIdentifier(), ex.getMessage(), ex);
      }
    });

    log.info("Completed the migration for git integrations update sync.");
  }

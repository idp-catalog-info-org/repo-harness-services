...
public class GitIntegrationHarnessCodeRepoUrlMigration implements NGMigration {
  ...
  @Override
  public void migrate() {
    log.info("Starting the migration for correcting base url for HCR git integration if subdomain url is present.");
      ...
      try {
        ...
        integrationEntities.forEach(integrationEntity -> {
          if (integrationEntity.getParentType().equals(IntegrationEntity.ParentType.HARNESS_CODE_REPO)) {
            gitIntegrationService.updateDefaultConnectorLessManagedHarnessCodeRepoIntegration(accountIdentifier);
          }
        });
      } catch (Exception ex) {
        log.warn("Error in migration for correcting base url for HCR git integration if subdomain url is present for "
                + "account = {} Error = {}",
            accountIdentifier, ex.getMessage(), ex);
      }
    });

    log.info("Completed the migration for correcting base url for HCR git integration if subdomain url is present.");
  }
}

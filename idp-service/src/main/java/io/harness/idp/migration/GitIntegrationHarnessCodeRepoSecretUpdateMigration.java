...
public class GitIntegrationHarnessCodeRepoSecretUpdateMigration implements NGMigration {
  ...
  @Override
  public void migrate() {
    log.info("Starting the migration for updating secret for HCR git integration.");
      ...
      try {
        ...
        integrationEntities.forEach(integrationEntity -> {
          if (integrationEntity.getParentType().equals(IntegrationEntity.ParentType.HARNESS_CODE_REPO)) {
            gitIntegrationService.updateIdpGitIntegrationManagedHcrSecret(accountIdentifier, "dummy");
          }
        });
      } catch (Exception ex) {
        log.warn("Error in migration for updating secret for HCR git integration for account = {} Error = {}",
            accountIdentifier, ex.getMessage(), ex);
      }
    });

    log.info("Completed the migration for updating secret for HCR git integration.");
  }
}

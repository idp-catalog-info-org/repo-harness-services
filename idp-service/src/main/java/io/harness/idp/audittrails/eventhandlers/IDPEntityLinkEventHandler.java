...
@Slf4j
public class IDPEntityLinkEventHandler implements OutboxEventHandler {
  private static final ObjectMapper objectMapper = NG_DEFAULT_OBJECT_MAPPER;
  private final AuditClientService auditClientService;

  @Inject
  public IDPEntityLinkEventHandler(AuditClientService auditClientService) {
  ...
  public boolean handle(OutboxEvent outboxEvent) {
    try {
      switch (outboxEvent.getEventType()) {
        ...
        case ENTITY_LINK_UPDATED:
          return handleUpdateEvent(outboxEvent);
        case ENTITY_LINK_DELETED:
  ...
  private boolean handleCreateEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    EntityLinkCreateEvent event = objectMapper.readValue(outboxEvent.getEventData(), EntityLinkCreateEvent.class);
    AuditEntry auditEntry = AuditEntry.builder()
                                .action(Action.CREATE)
                                .module(ModuleType.IDP)
                                .newYaml(event.getNewEntityLinkJson())
  ...
  private boolean handleUpdateEvent(OutboxEvent outboxEvent) throws IOException {
    GlobalContext globalContext = outboxEvent.getGlobalContext();
    EntityLinkUpdateEvent event = objectMapper.readValue(outboxEvent.getEventData(), EntityLinkUpdateEvent.class);
    AuditEntry auditEntry = AuditEntry.builder()
                                .action(Action.UPDATE)
                                .module(ModuleType.IDP)
                                .oldYaml(event.getOldEntityLinkJson())
                                .newYaml(event.getNewEntityLinkJson())
                                .timestamp(outboxEvent.getCreatedAt())
                                .resource(ResourceDTO.fromResource(outboxEvent.getResource()))
                                .resourceScope(ResourceScopeDTO.fromResourceScope(outboxEvent.getResourceScope()))
  ...
  private boolean handleDeleteEvent(OutboxEvent outboxEvent) throws IOException {
    GlobalContext globalContext = outboxEvent.getGlobalContext();
    EntityLinkDeleteEvent event = objectMapper.readValue(outboxEvent.getEventData(), EntityLinkDeleteEvent.class);
    AuditEntry auditEntry = AuditEntry.builder()

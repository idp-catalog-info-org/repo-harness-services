...
public class AuthInfoServiceImplTest {
  ...
  public void testSaveGoogleAuthEnvVariables() throws Exception {
    String jsonString =
        "{\"auth\":{\"environment\":\"development\",\"providers\":{\"github\":{\"development\":{\"clientId\":\"${AUTH_"
        + "GOOGLE_CLIENT_ID}\",\"clientSecret\":\"${AUTH_GOOGLE_CLIENT_SECRET}\"}}}}}";
    ...
    when(ConfigManagerUtils.isValidSchema(any(), any())).thenReturn(new HashSet<>());
    when(configManagerService.saveUpdateAndMergeConfigForAccount(any(), any(), any(), anyBoolean()))
    ...
    verify(configManagerService, times(1)).saveUpdateAndMergeConfigForAccount(any(), any(), any(), anyBoolean());
    ...
    backstageEnvVariables =
        authInfoServiceImpl.saveAuthEnvVariables(GOOGLE_AUTH, buildGoogleAuthEnvVariables(), ACCOUNT_ID);
    assertEquals(backstageEnvVariables.size(), 2);
    verify(configManagerService, times(2)).saveUpdateAndMergeConfigForAccount(any(), any(), any(), anyBoolean());
    mockStatic.close();
  }
  ...
  public void testSaveGithubAuthEnvVariables() throws Exception {
    String jsonString =
        "{\"auth\":{\"environment\":\"development\",\"providers\":{\"github\":{\"development\":{\"clientId\":\"${AUTH_"
        + "GITHUB_CLIENT_ID}\",\"clientSecret\":\"${AUTH_GITHUB_CLIENT_SECRET}\"}}}}}";
    String development = "{\"development\":{\"clientId\":\"${AUTH_GITHUB_CLIENT_ID}\",\"clientSecret\":\"${AUTH_GITHUB_"
        + "CLIENT_SECRET}\"}}";
    JsonNode rootNode = objectMapper.readTree(jsonString);
    ...
    when(configManagerService.saveUpdateAndMergeConfigForAccount(any(), any(), any(), eq(false)))
        .thenReturn(new AppConfig());

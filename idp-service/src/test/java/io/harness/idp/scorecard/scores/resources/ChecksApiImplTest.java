...
public class ChecksApiImplTest extends CategoryTest {
  ...
  @Category(UnitTests.class)
  public void testGetCheckswithemptysort() {
    when(checkService.getChecksByAccountId(any(), any(), any(), any())).thenReturn(getPageCheckEntity());
    when(checkService.getCheckStatusByAccountIdAndIdentifiers(any(), any())).thenReturn(getCheckStatusEntityMap());
    when(idpCommonService.buildPageResponse(0, 10, 1, getCheckResponse()))
        .thenReturn(Response.ok().entity(getCheckResponse()).build());
    Response response = checksApiImpl.getChecks(true, ACCOUNT_ID, 0, 10, "data_source", null);
    assertEquals(Response.Status.OK.getStatusCode(), response.getStatus());
  }
  ...
  @Owner(developers = AGNIVA)
  @Category(UnitTests.class)
  public void testGetCheckswithSortSameNotAsAllowedProperties() {
    when(checkService.getChecksByAccountId(any(), any(), any(), any())).thenReturn(getPageCheckEntity());
    when(checkService.getCheckStatusByAccountIdAndIdentifiers(any(), any())).thenReturn(getCheckStatusEntityMap());
    when(idpCommonService.buildPageResponse(0, 10, 1, getCheckResponse()))
        .thenReturn(Response.ok().entity(getCheckResponse()).build());
    try {
      checksApiImpl.getChecks(true, ACCOUNT_ID, 0, 10, "notallowedproperties", null);
    } catch (InvalidArgumentsException e) {
      assertEquals("Invalid sort property: notallowedproperties", e.getMessage());
    }
  }

...
public class ScoresApiImplTest extends CategoryTest {
  ...
  @Mock ScorecardRecalibrateRequest scorecardRecalibrateRequest;
  ...
  private ScorecardRecalibrateRequest testRecalibrateRequest;

  @Before
  public void setUp() {
    MockitoAnnotations.openMocks(this);
    testRecalibrateRequest = new ScorecardRecalibrateRequest();
    ScorecardRecalibrateIdentifiers identifiers = new ScorecardRecalibrateIdentifiers();
    identifiers.setScorecardIdentifier(TEST_SCORECARD_IDENTIFIER);
    identifiers.setEntityIdentifier(TEST_ENTITY_IDENTIFIER);
    testRecalibrateRequest.setIdentifiers(identifiers);
  }
  @Test
  ...
  public void testGetAllScorecardSummary() {
    ...
    when(scoreService.getScoresSummaryForAnEntityV2(TEST_ACCOUNT_IDENTIFIER, TEST_ENTITY_IDENTIFIER))
  ...
  @Category(UnitTests.class)
  public void testGetScoresRecalibrateStatusSuccess() {
    ScorecardRecalibrateInfo recalibrateInfo = new ScorecardRecalibrateInfo();
    recalibrateInfo.setStartTime(123456789L);
    Response response = scoreApiImpl.getScoresRecalibrateStatus(testRecalibrateRequest, TEST_ACCOUNT_IDENTIFIER);
    assertEquals(Response.Status.OK.getStatusCode(), response.getStatus());
    assertTrue(response.getEntity() instanceof ScorecardRecalibrateResponseV2);
  }

  @Test
  @Owner(developers = AGNIVA)
  @Category(UnitTests.class)
  public void testGetScoresRecalibrateStatusError() {
    ScorecardRecalibrateRequest testRecalibrateRequest = new ScorecardRecalibrateRequest();
    ScorecardRecalibrateIdentifiers identifiers = new ScorecardRecalibrateIdentifiers();
    identifiers.setScorecardIdentifier("testScorecard");
    identifiers.setEntityIdentifier("testEntity");
    testRecalibrateRequest.setIdentifiers(identifiers);
    when(asyncScoreComputationService.getRecalibrateInfo(TEST_ACCOUNT_IDENTIFIER, "testScorecard", "testEntity"))
        .thenThrow(new RuntimeException(ERROR_MESSAGE_FOR_API_CALL));
    Response response = scoreApiImpl.getScoresRecalibrateStatus(testRecalibrateRequest, TEST_ACCOUNT_IDENTIFIER);
    assertEquals(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(), response.getStatus());
    assertEquals(ERROR_MESSAGE_FOR_API_CALL, ((ResponseMessage) response.getEntity()).getMessage());
  }
  ...
  private ScorecardRecalibrateRequest getTestScorecardRecalibrateRequest() {
    ScorecardRecalibrateIdentifiers identifiers = new ScorecardRecalibrateIdentifiers();
    identifiers.setScorecardIdentifier(TEST_SCORECARD_IDENTIFIER);
    identifiers.setEntityIdentifier(TEST_ENTITY_IDENTIFIER);

    ScorecardRecalibrateRequest request = new ScorecardRecalibrateRequest();

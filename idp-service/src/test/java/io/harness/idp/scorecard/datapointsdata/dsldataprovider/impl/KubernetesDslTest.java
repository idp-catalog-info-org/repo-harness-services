...
public class KubernetesDslTest extends CategoryTest {
  ...
  public void testGetDslData() {
    ...
    when(dslClientFactory.getClient(eq(TEST_ACCOUNT), anyString(), anyBoolean())).thenReturn(dslClient);
    when(dslClient.call(eq(TEST_ACCOUNT), any(ApiRequestDetails.class), anySet(), any()))
  ...
  @Owner(developers = VIGNESWARA)
  @Category(UnitTests.class)
  public void testGetDslDataLabelSelectorKeyError() {
    KubernetesConfig kubernetesConfig = new KubernetesConfig();
    ClusterConfig clusterConfig = new ClusterConfig();
    clusterConfig.setName(TEST_CLUSTER);
    clusterConfig.setUrl(TEST_URL);
    ...
    assertTrue(dslData.containsKey(ERROR_MESSAGE_KEY));
    assertEquals(K8_LABEL_SELECTOR_ANNOTATION_ERROR, dslData.get(ERROR_MESSAGE_KEY));
  }
}

...
public class GitlabMeanTimeToMergePRDslTest extends CategoryTest {
  AutoCloseable openMocks;
  @InjectMocks GitlabMeanTimeToMergePRDsl gitlabMeanTimeToMergePRDsl;
  ...
  public void testFetchDataForDefaultBranch() {
    ...
    Map<String, Object> data = gitlabMeanTimeToMergePRDsl.fetchData(ACCOUNT_ID, null, getDataSourceLocationEntity(),
  ...
  @Owner(developers = VIGNESWARA)
  @Category(UnitTests.class)
  public void testFetchDataForInvalidBranch() {
  ...
  public void testFetchDataForInvalidToken() {
    Response response = Response.status(401).entity("{\"errors\":[{\"message\":\"Invalid token\"}]}").build();
    when(directDslClient.call(any(), any(), anySet(), any())).thenReturn(response);
    Map<String, Object> data = gitlabMeanTimeToMergePRDsl.fetchData(ACCOUNT_ID, null, getDataSourceLocationEntity(),
        List.of(getDataFetchDTO(true)), replaceableHeaders(), possibleReplaceableRequestBody(),
        possibleReplaceableUrlBody(), getDataSourceConfig(), false, Set.of());
  ...
  private Map<String, String> possibleReplaceableUrlBody() {
    Map<String, String> pairs = new HashMap<>();
    pairs.put(API_BASE_URL, "gitlab.com/api/v4");
    return pairs;
  }

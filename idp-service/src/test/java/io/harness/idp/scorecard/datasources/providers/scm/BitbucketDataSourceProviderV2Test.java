...
public class BitbucketDataSourceProviderV2Test extends CategoryTest {
  private static final String ACCOUNT_ID = "test-account-123";
  private static final String CONFIGS = "bitbucket-configs";
  ...
  private static final String DATA_POINT_IDENTIFIER = "bitbucket.test.datapoint";
  private static final String CATALOG_DSL_IDENTIFIER = "bitbucket-catalog-dsl";
  private static final String HQL_DSL_IDENTIFIER = "bitbucket-hql-dsl";
  private static final String LEGACY_DSL_IDENTIFIER = "bitbucket-legacy-dsl";
  ...
  private static final String PROVIDER = "BitbucketCloud";
  private static final String DESCRIPTION_JEXL = "catalog.metadata.integration_properties.BitbucketCloud.description";
  private static final String DESCRIPTION_VALUE = "Harness IDP bitbucket repo";
  ...
  @Mock BitbucketProvider bitbucketProvider;
  ...
  BitbucketDataSourceProviderV2 bitbucketDataSourceProviderV2;
  ...
  public void setUp() {
    ...
    dataPointParserFactory = (identifier, dataSourceLocationType)
        -> identifier != null && identifier.startsWith("bitbucket.hql.") ? hqlDataPointParser : catalogDataPointParser;
    ...
    bitbucketDataSourceProviderV2 = new BitbucketDataSourceProviderV2(dataPointService, dataSourceLocationFactory,
        dataSourceLocationRepository, dataPointParserFactory, dataSourceRepository, bitbucketProvider);
  }
  ...
  public void testGetIdentifier() {
    assertEquals(BITBUCKET_IDENTIFIER, bitbucketDataSourceProviderV2.getIdentifier());
  }
  ...
  public void testFetchData_EmptyDataToFetch() {
    ...
    Map<String, Map<String, Object>> result =
        bitbucketDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, Collections.emptyList(), CONFIGS);

    assertNotNull(result);
    assertTrue(result.isEmpty());
    verify(bitbucketProvider, never()).fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS));
  }
  ...
  public void testFetchData_V2Catalog_SuccessfulFetch() {
    ...
    when(dataPointService.getDslDataPointsInfo(eq(ACCOUNT_ID), eq(BITBUCKET_IDENTIFIER), anyList()))
    ...
    Map<String, Map<String, Object>> result =
        bitbucketDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, Collections.singletonList(dataFetchDTO), CONFIGS);

    assertNotNull(result);
    assertTrue(result.containsKey(BITBUCKET_IDENTIFIER));
    Map<String, Object> dataPointInfo = (Map<String, Object>) result.get(BITBUCKET_IDENTIFIER).get(RULE_IDENTIFIER);
  ...
  public void testFetchData_V2Catalog_MultipleDataPoints() {
    Map<String, Object> bitbucket = new HashMap<>();
    bitbucket.put("description", DESCRIPTION_VALUE);
    bitbucket.put("stars", 42);
    CatalogEntity entity = createEnrichedCatalogEntity(PROVIDER, bitbucket);
    ...
    Map<String, Map<String, Object>> result =
        bitbucketDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(dataFetchDTO1, dataFetchDTO2), CONFIGS);

    assertNotNull(result);
    Map<String, Object> providerData = result.get(BITBUCKET_IDENTIFIER);
  ...
  public void testFetchData_V2Catalog_EntityNotCatalogEntity() {
    ...
    Map<String, Map<String, Object>> result = bitbucketDataSourceProviderV2.fetchData(
  ...
  public void testFetchData_V2Hql_SuccessfulFetch() {
    ...
    DataFetchDTO dataFetchDTO = DataFetchDTO.builder()
                                    ...
                                    .dataPoint(DataPointEntity.builder()
                                                   .identifier("bitbucket.hql.datapoint")
  ...
  @Owner(developers = SARABJYOT)
  @Category(UnitTests.class)
  public void testFetchData_LegacyOnly_DelegatesToBitbucketProvider() {
    CatalogEntity entity = createCatalogEntity(ENTITY_NAME, ENTITY_KIND);
    ...
    when(bitbucketProvider.fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS)))
        .thenReturn(Map.of(BITBUCKET_IDENTIFIER, Map.of(LEGACY_RULE, 42)));

    Map<String, Map<String, Object>> result =
        bitbucketDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, Collections.singletonList(legacyFetchDTO), CONFIGS);

    assertNotNull(result);
    assertEquals(42, result.get(BITBUCKET_IDENTIFIER).get(LEGACY_RULE));

    ArgumentCaptor<List<DataFetchDTO>> legacyCaptor = ArgumentCaptor.forClass(List.class);
    verify(bitbucketProvider).fetchData(eq(ACCOUNT_ID), eq(entity), legacyCaptor.capture(), eq(CONFIGS));
  ...
  public void testFetchData_MixedLegacyAndV2_MergesResults() {
    ...
    when(dataSourceLocationRepository.findByIdentifier(CATALOG_DSL_IDENTIFIER))
        .thenReturn(createCatalogLocationEntity(DESCRIPTION_JEXL));
    when(bitbucketProvider.fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS)))
        .thenReturn(Map.of(BITBUCKET_IDENTIFIER, new HashMap<>(Map.of(LEGACY_RULE, "legacy-value"))));

    Map<String, Map<String, Object>> result =
        bitbucketDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(legacyFetchDTO, v2FetchDTO), CONFIGS);
  ...
  public void testFetchData_MissingDslSkippedAmongValidOnes() {
    ...
    Map<String, Map<String, Object>> result =
        bitbucketDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(missingFetchDTO, v2FetchDTO), CONFIGS);

    assertEquals(1, result.get(BITBUCKET_IDENTIFIER).size());
    assertFalse(result.get(BITBUCKET_IDENTIFIER).containsKey("missing-rule"));
    Map<String, Object> dataPointInfo = (Map<String, Object>) result.get(BITBUCKET_IDENTIFIER).get(RULE_IDENTIFIER);
    assertEquals(DESCRIPTION_VALUE, dataPointInfo.get(DATA_POINT_VALUE_KEY));
  }
  ...
  private DataFetchDTO createDataFetchDTO(String ruleIdentifier) {
    DataPointEntity dataPointEntity =
        DataPointEntity.builder().identifier(DATA_POINT_IDENTIFIER).dataSourceIdentifier(BITBUCKET_IDENTIFIER).build();
  ...
  /**
   * Builds a catalog entity whose {@code BitbucketCloud} integration properties are stored under
   * {@code decorator._processed_data.metadata.integration_properties.BitbucketCloud}, mirroring how the integration

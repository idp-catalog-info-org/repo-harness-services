...
public class KubernetesDataSourceProviderV2Test extends CategoryTest {
  private static final String ACCOUNT_ID = "test-account-123";
  private static final String CONFIGS = "kubernetes-configs";
  ...
  private static final String DATA_POINT_IDENTIFIER = "kubernetes.test.datapoint";
  private static final String CATALOG_DSL_IDENTIFIER = "kubernetes-catalog-dsl";
  private static final String HQL_DSL_IDENTIFIER = "kubernetes-hql-dsl";
  private static final String LEGACY_DSL_IDENTIFIER = "kubernetes-legacy-dsl";
  ...
  private static final String DESCRIPTION_JEXL = "catalog.metadata.integration_properties.Kubernetes.description";
  private static final String DESCRIPTION_VALUE = "Payments service cluster workloads";
  ...
  @Mock KubernetesProvider kubernetesProvider;
  ...
  KubernetesDataSourceProviderV2 kubernetesDataSourceProviderV2;
  ...
  public void setUp() {
    ...
    dataPointParserFactory = (identifier, dataSourceLocationType)
        -> identifier != null && identifier.startsWith("kubernetes.hql.") ? hqlDataPointParser : catalogDataPointParser;
    ...
    kubernetesDataSourceProviderV2 = new KubernetesDataSourceProviderV2(dataPointService, dataSourceLocationFactory,
        dataSourceLocationRepository, dataPointParserFactory, dataSourceRepository, kubernetesProvider);
  }
  ...
  public void testGetIdentifier() {
    assertEquals(KUBERNETES_IDENTIFIER, kubernetesDataSourceProviderV2.getIdentifier());
  }
  ...
  public void testFetchData_EmptyDataToFetch() {
    ...
    Map<String, Map<String, Object>> result =
        kubernetesDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, Collections.emptyList(), CONFIGS);

    assertNotNull(result);
    assertTrue(result.isEmpty());
    verify(kubernetesProvider, never()).fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS));
  }
  ...
  public void testFetchData_V2Catalog_SuccessfulFetch() {
    ...
    when(dataPointService.getDslDataPointsInfo(eq(ACCOUNT_ID), eq(KUBERNETES_IDENTIFIER), anyList()))
    ...
    Map<String, Map<String, Object>> result =
        kubernetesDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, Collections.singletonList(dataFetchDTO), CONFIGS);

    assertNotNull(result);
    assertTrue(result.containsKey(KUBERNETES_IDENTIFIER));
    Map<String, Object> dataPointInfo = (Map<String, Object>) result.get(KUBERNETES_IDENTIFIER).get(RULE_IDENTIFIER);
  ...
  public void testFetchData_V2Catalog_MultipleDataPoints() {
    ...
    kubernetes.put("description", DESCRIPTION_VALUE);
    kubernetes.put("replicas", 3);
    CatalogEntity entity = createEnrichedCatalogEntity(PROVIDER, kubernetes);
    ...
    Map<String, Map<String, Object>> result =
        kubernetesDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(dataFetchDTO1, dataFetchDTO2), CONFIGS);

    assertNotNull(result);
    Map<String, Object> providerData = result.get(KUBERNETES_IDENTIFIER);
  ...
  public void testFetchData_V2Catalog_EntityNotCatalogEntity() {
    ...
    Map<String, Map<String, Object>> result = kubernetesDataSourceProviderV2.fetchData(
  ...
  public void testFetchData_V2Hql_SuccessfulFetch() {
    ...
    DataFetchDTO dataFetchDTO = DataFetchDTO.builder()
                                    ...
                                    .dataPoint(DataPointEntity.builder()
                                                   .identifier("kubernetes.hql.datapoint")
  ...
  @Owner(developers = SARABJYOT)
  @Category(UnitTests.class)
  public void testFetchData_LegacyOnly_DelegatesToKubernetesProvider() {
    CatalogEntity entity = createCatalogEntity(ENTITY_NAME, ENTITY_KIND);
    ...
    when(kubernetesProvider.fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS)))
        .thenReturn(Map.of(KUBERNETES_IDENTIFIER, Map.of(LEGACY_RULE, 42)));

    Map<String, Map<String, Object>> result = kubernetesDataSourceProviderV2.fetchData(
        ACCOUNT_ID, entity, Collections.singletonList(legacyFetchDTO), CONFIGS);

    assertNotNull(result);
    assertEquals(42, result.get(KUBERNETES_IDENTIFIER).get(LEGACY_RULE));

    ArgumentCaptor<List<DataFetchDTO>> legacyCaptor = ArgumentCaptor.forClass(List.class);
    verify(kubernetesProvider).fetchData(eq(ACCOUNT_ID), eq(entity), legacyCaptor.capture(), eq(CONFIGS));
  ...
  public void testFetchData_MixedLegacyAndV2_MergesResults() {
    ...
    when(dataSourceLocationRepository.findByIdentifier(CATALOG_DSL_IDENTIFIER))
        .thenReturn(createCatalogLocationEntity(DESCRIPTION_JEXL));
    when(kubernetesProvider.fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS)))
        .thenReturn(Map.of(KUBERNETES_IDENTIFIER, new HashMap<>(Map.of(LEGACY_RULE, "legacy-value"))));

    Map<String, Map<String, Object>> result =
        kubernetesDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(legacyFetchDTO, v2FetchDTO), CONFIGS);
  ...
  public void testFetchData_MissingDslSkippedAmongValidOnes() {
    ...
    Map<String, Map<String, Object>> result =
        kubernetesDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(missingFetchDTO, v2FetchDTO), CONFIGS);

    assertEquals(1, result.get(KUBERNETES_IDENTIFIER).size());
    assertFalse(result.get(KUBERNETES_IDENTIFIER).containsKey("missing-rule"));
    Map<String, Object> dataPointInfo = (Map<String, Object>) result.get(KUBERNETES_IDENTIFIER).get(RULE_IDENTIFIER);
    assertEquals(DESCRIPTION_VALUE, dataPointInfo.get(DATA_POINT_VALUE_KEY));
  }
  ...
  private DataFetchDTO createDataFetchDTO(String ruleIdentifier) {
    DataPointEntity dataPointEntity =
        DataPointEntity.builder().identifier(DATA_POINT_IDENTIFIER).dataSourceIdentifier(KUBERNETES_IDENTIFIER).build();
  ...
  /**
   * Builds a catalog entity whose {@code Kubernetes} integration properties are stored under
   * {@code decorator._processed_data.metadata.integration_properties.Kubernetes}, mirroring how the integration

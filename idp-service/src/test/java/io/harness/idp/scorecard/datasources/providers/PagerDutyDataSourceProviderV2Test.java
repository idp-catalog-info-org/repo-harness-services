...
public class PagerDutyDataSourceProviderV2Test extends CategoryTest {
  private static final String ACCOUNT_ID = "test-account-123";
  private static final String CONFIGS = "pagerduty-configs";
  ...
  private static final String DATA_POINT_IDENTIFIER = "pagerduty.test.datapoint";
  private static final String CATALOG_DSL_IDENTIFIER = "pagerduty-catalog-dsl";
  private static final String HQL_DSL_IDENTIFIER = "pagerduty-hql-dsl";
  private static final String LEGACY_DSL_IDENTIFIER = "pagerduty-legacy-dsl";
  ...
  private static final String PROVIDER = "PagerDuty";
  private static final String DESCRIPTION_JEXL = "catalog.metadata.integration_properties.PagerDuty.name";
  private static final String DESCRIPTION_VALUE = "Payments on-call service";
  ...
  @Mock PagerDutyProvider pagerDutyProvider;
  ...
  PagerDutyDataSourceProviderV2 pagerDutyDataSourceProviderV2;
  ...
  public void setUp() {
    ...
    dataPointParserFactory = (identifier, dataSourceLocationType)
        -> identifier != null && identifier.startsWith("pagerduty.hql.") ? hqlDataPointParser : catalogDataPointParser;
    ...
    pagerDutyDataSourceProviderV2 = new PagerDutyDataSourceProviderV2(dataPointService, dataSourceLocationFactory,
        dataSourceLocationRepository, dataPointParserFactory, dataSourceRepository, pagerDutyProvider);
  }
  ...
  public void testGetIdentifier() {
    assertEquals(PAGERDUTY_IDENTIFIER, pagerDutyDataSourceProviderV2.getIdentifier());
  }
  ...
  public void testFetchData_EmptyDataToFetch() {
    CatalogEntity entity = createEnrichedCatalogEntity(PROVIDER, Map.of("name", DESCRIPTION_VALUE));

    Map<String, Map<String, Object>> result =
        pagerDutyDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, Collections.emptyList(), CONFIGS);

    assertNotNull(result);
    assertTrue(result.isEmpty());
    verify(pagerDutyProvider, never()).fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS));
  }
  ...
  public void testFetchData_V2Catalog_SuccessfulFetch() {
    ...
    when(dataPointService.getDslDataPointsInfo(eq(ACCOUNT_ID), eq(PAGERDUTY_IDENTIFIER), anyList()))
    ...
    Map<String, Map<String, Object>> result =
        pagerDutyDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, Collections.singletonList(dataFetchDTO), CONFIGS);

    assertNotNull(result);
    assertTrue(result.containsKey(PAGERDUTY_IDENTIFIER));
    Map<String, Object> dataPointInfo = (Map<String, Object>) result.get(PAGERDUTY_IDENTIFIER).get(RULE_IDENTIFIER);
  ...
  public void testFetchData_V2Catalog_MultipleDataPoints() {
    Map<String, Object> pagerDuty = new HashMap<>();
    pagerDuty.put("name", DESCRIPTION_VALUE);
    pagerDuty.put("analyticsTotalIncidents", 12);
    CatalogEntity entity = createEnrichedCatalogEntity(PROVIDER, pagerDuty);
    ...
    Map<String, Map<String, Object>> result =
        pagerDutyDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(dataFetchDTO1, dataFetchDTO2), CONFIGS);

    assertNotNull(result);
    Map<String, Object> providerData = result.get(PAGERDUTY_IDENTIFIER);
  ...
  public void testFetchData_V2Catalog_EntityNotCatalogEntity() {
    ...
    Map<String, Map<String, Object>> result = pagerDutyDataSourceProviderV2.fetchData(
  ...
  public void testFetchData_V2Hql_SuccessfulFetch() {
    ...
    DataFetchDTO dataFetchDTO = DataFetchDTO.builder()
                                    ...
                                    .dataPoint(DataPointEntity.builder()
                                                   .identifier("pagerduty.hql.datapoint")
                                                   .type(DataPointEntity.Type.NUMBER)
                                                   .dataSourceIdentifier(PAGERDUTY_IDENTIFIER)
                                                   .build())
                                    .build();
  ...
  @Owner(developers = SARABJYOT)
  @Category(UnitTests.class)
  public void testFetchData_LegacyOnly_DelegatesToPagerDutyProvider() {
    CatalogEntity entity = createCatalogEntity(ENTITY_NAME, ENTITY_KIND);
    ...
    when(pagerDutyProvider.fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS)))
        .thenReturn(Map.of(PAGERDUTY_IDENTIFIER, Map.of(LEGACY_RULE, 42)));

    Map<String, Map<String, Object>> result =
        pagerDutyDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, Collections.singletonList(legacyFetchDTO), CONFIGS);

    assertNotNull(result);
    assertEquals(42, result.get(PAGERDUTY_IDENTIFIER).get(LEGACY_RULE));

    ArgumentCaptor<List<DataFetchDTO>> legacyCaptor = ArgumentCaptor.forClass(List.class);
    verify(pagerDutyProvider).fetchData(eq(ACCOUNT_ID), eq(entity), legacyCaptor.capture(), eq(CONFIGS));
  ...
  public void testFetchData_MixedLegacyAndV2_MergesResults() {
    ...
    when(dataSourceLocationRepository.findByIdentifier(CATALOG_DSL_IDENTIFIER))
        .thenReturn(createCatalogLocationEntity(DESCRIPTION_JEXL));
    when(pagerDutyProvider.fetchData(eq(ACCOUNT_ID), eq(entity), anyList(), eq(CONFIGS)))
        .thenReturn(Map.of(PAGERDUTY_IDENTIFIER, new HashMap<>(Map.of(LEGACY_RULE, "legacy-value"))));

    Map<String, Map<String, Object>> result =
        pagerDutyDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(legacyFetchDTO, v2FetchDTO), CONFIGS);
  ...
  public void testFetchData_MissingDslSkippedAmongValidOnes() {
    ...
    Map<String, Map<String, Object>> result =
        pagerDutyDataSourceProviderV2.fetchData(ACCOUNT_ID, entity, List.of(missingFetchDTO, v2FetchDTO), CONFIGS);

    assertEquals(1, result.get(PAGERDUTY_IDENTIFIER).size());
    assertFalse(result.get(PAGERDUTY_IDENTIFIER).containsKey("missing-rule"));
    Map<String, Object> dataPointInfo = (Map<String, Object>) result.get(PAGERDUTY_IDENTIFIER).get(RULE_IDENTIFIER);
    assertEquals(DESCRIPTION_VALUE, dataPointInfo.get(DATA_POINT_VALUE_KEY));
  }
  ...
  private DataFetchDTO createDataFetchDTO(String ruleIdentifier) {
    DataPointEntity dataPointEntity =
        DataPointEntity.builder().identifier(DATA_POINT_IDENTIFIER).dataSourceIdentifier(PAGERDUTY_IDENTIFIER).build();
  ...
  /**
   * Builds a catalog entity whose {@code PagerDuty} integration properties are stored under
   * {@code decorator._processed_data.metadata.integration_properties.PagerDuty}, mirroring how the integration

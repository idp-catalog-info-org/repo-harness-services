...
public class ProxyApiImplTest extends CategoryTest {
  ...
  private final String NG_MANAGER = "ng-manager";
  private final ProxyAllowListConfig.ServiceDefinitionConfig manager =
  ...
  private final ProxyAllowListConfig.ServiceDefinitionConfig ngManager =
      ProxyAllowListConfig.ServiceDefinitionConfig.builder()
          .proxyPath("v1/idp-proxy-service/ng-manager")
          .clientConfig(ServiceHttpClientConfig.builder().baseUrl(BASE_URL).build())
          .secret("secret")
          .allowList(Collections.singletonList("/v2/secrets/[^/]+/decrypt"))
          .shouldEncryptResponseList(Collections.singletonList("/v2/secrets/[^/]+/decrypt"))
          .build();
  private final Map<String, ProxyAllowListConfig.ServiceDefinitionConfig> serviceDefinitionConfigMap = new HashMap<>() {
    {
      put(SERVICE, manager);
      put(NG_MANAGER, ngManager);
    }
  };
  final ProxyAllowListConfig proxyAllowListConfig =
      ProxyAllowListConfig.builder().services(serviceDefinitionConfigMap).build();
  final OkHttpClientConnectionPoolConfig okHttpClientConnectionPoolConfig =
      OkHttpClientConnectionPoolConfig.builder().maxIdleConnections(5).keepAliveDuration(5).timeUnit("MINUTES").build();

  @Before
  public void setUp() {
    openMocks = MockitoAnnotations.openMocks(this);
    proxyApiImpl = Mockito.spy(new ProxyApiImpl(tokenGenerator, proxyAllowListConfig, okHttpClientConnectionPoolConfig,
        okHttpClientConnectionPoolConfig, "dummy"));
  }
  ...
  @Owner(developers = VIGNESWARA)
  @Category(UnitTests.class)
  public void testGetProxyNgManagerDecryptSecret() throws IOException {
    MultivaluedMap<String, String> headersMap = new MultivaluedHashMap<>();
    headersMap.put(TEST_HEADER_KEY, Collections.singletonList(TEST_HEADER_VALUE));
    MultivaluedMap<String, String> queryParamsMap = new MultivaluedHashMap<>();
    queryParamsMap.put(TEST_QUERY_PARAM_KEY, Collections.singletonList(TEST_QUERY_PARAM_VALUE));
    Request request = new Request.Builder().url(BASE_URL + NG_MANAGER).method("GET", null).build();
    Response responseSuccess =
        new Response.Builder()
            .code(200)
            .message("success")
    ...
    String DECRYPT_SECRET_PATH = "v1/idp-proxy-service/ng-manager/v2/secrets/IDP_BACKEND_SECRET/decrypt";

    when(clientConfig.getBaseUrl()).thenReturn(BASE_URL);
    when(uriInfo.getPath()).thenReturn(DECRYPT_SECRET_PATH);
    when(uriInfo.getQueryParameters()).thenReturn(queryParamsMap);
    when(headers.getRequestHeaders()).thenReturn(headersMap);
    when(proxyApiImpl.getOkHttpClient(NG_MANAGER)).thenReturn(client);
    when(client.newCall(any())).thenReturn(call);
    when(call.execute()).thenReturn(responseSuccess);

    javax.ws.rs.core.Response result =
        proxyApiImpl.getProxyService(uriInfo, headers, NG_MANAGER, BASE_URL + DECRYPT_SECRET_PATH, TEST_ACCOUNT);

    assertEquals(javax.ws.rs.core.Response.Status.OK.getStatusCode(), result.getStatus());
    assertNotSame("Response from NG Manager", result.getEntity().toString());
  }

...
public class IdpEntityCrudStreamProducerTest extends CategoryTest {
  ...
  public static final String SCORECARD_IDENTIFIER = "example-scorecard";
  public static final String ENTITY_IDENTIFIER = "example-entity";
  ...
  @Category(UnitTests.class)
  public void testPublishAsyncScoreComputationChangeEventToRedisSuccessful() throws EventsFrameworkDownException {
    String eventId = "test-event-id";
    when(eventProducer.send(any(Message.class))).thenReturn(eventId);

    boolean result = streamProducer.publishAsyncScoreComputationChangeEventToRedis(
        ACCOUNT_IDENTIFIER, SCORECARD_IDENTIFIER, ENTITY_IDENTIFIER);
    ...
    Message sentMessage = messageCaptor.getValue();
    assertEquals(ACCOUNT_IDENTIFIER, sentMessage.getMetadataOrDefault("accountId", ""));
    assertEquals(ASYNC_SCORE_COMPUTATION_ENTITY,
        sentMessage.getMetadataOrDefault(EventsFrameworkMetadataConstants.ENTITY_TYPE, ""));
    assertEquals("create", sentMessage.getMetadataOrDefault(EventsFrameworkMetadataConstants.ACTION, ""));
    ByteString expectedPayload = EntityChangeDTO.newBuilder()
                                     .setAccountIdentifier(StringValue.of(ACCOUNT_IDENTIFIER))
                                     .putMetadata("scorecardIdentifier", SCORECARD_IDENTIFIER)
                                     .putMetadata("entityIdentifier", ENTITY_IDENTIFIER)
                                     .build()
                                     .toByteString();
    assertEquals(expectedPayload, sentMessage.getData());
  }
  ...
  @Owner(developers = SATHISH)
  @Category(UnitTests.class)
  public void testPublishAsyncScoreComputationChangeEventToRedisFailure() throws EventsFrameworkDownException {
    when(eventProducer.send(any(Message.class))).thenThrow(EventsFrameworkDownException.class);

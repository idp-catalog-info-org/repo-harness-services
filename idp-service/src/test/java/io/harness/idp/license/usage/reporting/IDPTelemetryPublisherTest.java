...
public class IDPTelemetryPublisherTest extends CategoryTest {
  ...
  final List<String> allIDPAccountIds = List.of(TEST_ACCOUNT_IDENTIFIER);
    ...
    HashMap<String, Object> map = new HashMap<>() {
      {
        ...
        put("idp_license_active_developers", 100);
      }
    };

    when(namespaceService.getAccountIds()).thenReturn(allIDPAccountIds);
    when(idpTelemetrySentStatusRepository.findByAccountId(TEST_ACCOUNT_IDENTIFIER))
        .thenReturn(Optional.of(IDPTelemetrySentStatus.builder()
                                    .accountId(TEST_ACCOUNT_IDENTIFIER)
                                    .lastSent(System.currentTimeMillis() - 85700000)
                                    .build()));
    when(idpLicenseUsage.getLicenseUsage(anyString(), any(), anyLong(), any())).thenReturn(idpLicenseUsageDTO);
    doNothing()

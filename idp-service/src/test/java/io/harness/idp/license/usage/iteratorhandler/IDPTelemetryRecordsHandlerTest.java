...
public class IDPTelemetryRecordsHandlerTest extends CategoryTest {
  AutoCloseable openMocks;
  @InjectMocks private IDPTelemetryRecordsHandler handler;
  @Mock private IDPTelemetryPublisher publisher;
  ...
  public void testHandle() {
    ...
    verify(publisher).recordTelemetry();
  }
  ...
  public void testHandleThrowsException() {
    willAnswer(invocation -> { throw new Exception("Exception Throw"); }).given(publisher).recordTelemetry();
    handler.handle(IteratorEntity.builder().build());
  }
  ...
  public void testRegisterIterators() {
    ...
    verify(persistenceIteratorFactory, times(1))
        .createPumpIteratorWithDedicatedThreadPool(any(), eq(IDPTelemetryRecordsHandler.class), any());
  }

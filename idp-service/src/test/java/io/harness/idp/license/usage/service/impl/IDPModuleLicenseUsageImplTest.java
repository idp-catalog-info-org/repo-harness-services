...
public class IDPModuleLicenseUsageImplTest extends CategoryTest {
  ...
  @Category(UnitTests.class)
  public void testGetHistoryTrendV2Daily() {
    IDPLicenseDateUsageParams idpLicenseDateUsageParams = IDPLicenseDateUsageParams.builder()
                                                              .fromDate("2023-10-23")
                                                              .toDate("2023-10-26")
                                                              .reportType(LicenseDateUsageReportType.DAILY)
    ...
    Set<String> uniqueUserIds1 = new HashSet<>();
    uniqueUserIds1.add("user1");
    uniqueUserIds1.add("user2");

    Set<String> uniqueUserIds2 = new HashSet<>();
    uniqueUserIds2.add("user2");
    uniqueUserIds2.add("user3");

    List<ActiveDevelopersDailyCountEntity> activeDevelopersDailyCountEntities = new ArrayList<>();
    activeDevelopersDailyCountEntities.add(ActiveDevelopersDailyCountEntity.builder()
                                               .accountIdentifier(TEST_ACCOUNT_IDENTIFIER)
                                               .dateInStringFormat("2023-10-23")
                                               .dateInDateFormat(TEST_DATE_IN_DATE_FORMAT)
                                               .count(2)
                                               .uniqueUserIdentifiers(uniqueUserIds1)
                                               .build());
    activeDevelopersDailyCountEntities.add(ActiveDevelopersDailyCountEntity.builder()
                                               .accountIdentifier(TEST_ACCOUNT_IDENTIFIER)
                                               .dateInStringFormat("2023-10-24")
                                               .dateInDateFormat(TEST_DATE_IN_DATE_FORMAT)
                                               .count(2)
                                               .uniqueUserIdentifiers(uniqueUserIds2)
                                               .build());

    when(activeDevelopersDailyCountRepository.findByAccountIdentifierAndDateInDateFormatBetween(any(), any(), any()))
        .thenReturn(activeDevelopersDailyCountEntities);

    ActiveDevelopersTrendResponse response =
        idpModuleLicenseUsage.getHistoryTrendV2(TEST_ACCOUNT_IDENTIFIER, idpLicenseDateUsageParams);

    assertNotNull(response);
    assertNotNull(response.getActiveDevelopersTrendList());
    assertEquals(2, response.getActiveDevelopersTrendList().size());
    assertEquals(2, response.getPeak());
    assertEquals(3, response.getTotalUniqueDevelopers()); // user1, user2, user3
    assertEquals(2, response.getActiveDevelopersTrendList().get(0).getCount());
    assertEquals("2023-10-23", response.getActiveDevelopersTrendList().get(0).getDate());
  }

  @Test
  @Owner(developers = KOTA_KARTHIK)
  @Category(UnitTests.class)
  public void testGetHistoryTrendV2Monthly() {
    IDPLicenseDateUsageParams idpLicenseDateUsageParams = IDPLicenseDateUsageParams.builder()
                                                              .fromDate("2023-09-01")
                                                              .toDate("2023-10-31")
                                                              .reportType(LicenseDateUsageReportType.MONTHLY)
    ...
    uniqueUserIds2.add("user2");
    uniqueUserIds2.add("user3");
    uniqueUserIds2.add("user4");

    List<ActiveDevelopersDailyCountEntity> activeDevelopersDailyCountEntities = new ArrayList<>();
    activeDevelopersDailyCountEntities.add(ActiveDevelopersDailyCountEntity.builder()
                                               .accountIdentifier(TEST_ACCOUNT_IDENTIFIER)
                                               .dateInStringFormat("2023-09-15")
                                               .dateInDateFormat(new Date(123, 8, 15))
                                               .count(2)
                                               .uniqueUserIdentifiers(uniqueUserIds1)
                                               .build());
    activeDevelopersDailyCountEntities.add(ActiveDevelopersDailyCountEntity.builder()
                                               .accountIdentifier(TEST_ACCOUNT_IDENTIFIER)
                                               .dateInStringFormat("2023-10-20")
                                               .dateInDateFormat(new Date(123, 9, 20))
                                               .count(3)
                                               .uniqueUserIdentifiers(uniqueUserIds2)
                                               .build());
    ...
    assertNotNull(response.getActiveDevelopersTrendList());
    assertEquals(2, response.getActiveDevelopersTrendList().size());
    assertEquals(3, response.getPeak());
    assertEquals(4, response.getTotalUniqueDevelopers());
  }

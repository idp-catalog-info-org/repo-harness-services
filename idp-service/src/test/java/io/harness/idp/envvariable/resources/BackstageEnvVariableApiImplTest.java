...
public class BackstageEnvVariableApiImplTest extends CategoryTest {
  ...
  @Category(UnitTests.class)
  public void testReloadBackstageEnvVariables() {
    doNothing().when(idpCommonService).checkUserAuthorization();
    doNothing().when(backstageEnvVariableService).reloadSecrets(TEST_ACCOUNT_IDENTIFIER, TEST_NAMESPACE);
    Response response =
        backstageEnvVariableApiImpl.reloadBackstageEnvVariables(TEST_NAMESPACE, TEST_ACCOUNT_IDENTIFIER);
    assertEquals(Response.Status.NO_CONTENT.getStatusCode(), response.getStatus());
  }
  ...
  public void testResolveBackstageEnvVariables() {
    ...
    when(backstageEnvVariableService.resolveEnvVariables(TEST_ACCOUNT_IDENTIFIER, TEST_NAMESPACE))

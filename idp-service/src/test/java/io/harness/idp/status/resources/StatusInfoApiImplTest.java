...
public class StatusInfoApiImplTest {
  ...
  @Category(UnitTests.class)
  public void testGetStatusByTypeInvalidRequestException() {
    type = StatusType.ONBOARDING.toString();
    when(statusInfoService.findByAccountIdentifierAndType(ACCOUNT_ID, type))
    ...
    assertEquals("Invalid request", responseMessage.getMessage());
  }

  @Test
  @Owner(developers = AGNIVA)
  @Category(UnitTests.class)
  public void testSaveStatusInfoByTypeInvalidRequestException() {
    type = StatusType.ONBOARDING.toString();
    StatusInfo statusInfo = initializeStatusInfo();
    StatusInfoRequest statusInfoRequest = new StatusInfoRequest();
    statusInfoRequest.setStatus(statusInfo);

    when(statusInfoService.save(statusInfo, ACCOUNT_ID, type))
        .thenThrow(new InvalidRequestException("Invalid request"));

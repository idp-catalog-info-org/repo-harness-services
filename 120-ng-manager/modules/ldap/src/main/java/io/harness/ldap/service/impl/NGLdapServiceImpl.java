...
public class NGLdapServiceImpl implements NGLdapService {
  ...
  public LdapTestResponse validateLdapConnectionSettings(
      ScopeInfo scopeInfo, software.wings.beans.sso.LdapSettingsDTO settings) {
    log.info("Validate ldap connection in NG LDAP case for account: {}, organization: {}, project: {} with ldap "
            + "settings id {}",
    ...
    NGLdapDelegateTaskParameters parameters =
        getNgLdapDelegateTaskParameters(scopeInfo.getAccountIdentifier(), settings);

    DelegateResponseData delegateResponseData =
        getDelegateResponseData(scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(),
            scopeInfo.getProjectIdentifier(), parameters, NG_LDAP_TEST_CONN_SETTINGS, parameters.getLdapSettings());
  ...
  public LdapTestResponse validateLdapUserSettings(
      ScopeInfo scopeInfo, software.wings.beans.sso.LdapSettingsDTO settings) {
    log.info("Validate ldap user query in NG LDAP case for account: {}, organization: {}, project: {} with ldap "
            + "settings id {}",
        scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(), scopeInfo.getProjectIdentifier(),
        settings.getUuid());
    NGLdapDelegateTaskParameters parameters =
    ...
    DelegateResponseData delegateResponseData =
        getDelegateResponseData(scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(),
            scopeInfo.getProjectIdentifier(), parameters, NG_LDAP_TEST_USER_SETTINGS, parameters.getLdapSettings());
  ...
  public LdapTestResponse validateLdapGroupSettings(
      ScopeInfo scopeInfo, software.wings.beans.sso.LdapSettingsDTO settings) {
    log.info("NGLDAP: Validate ldap group query in NG LDAP case for account: {}, organization: {}, project: {} with "
            + "ldap settings id {}",
        scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(), scopeInfo.getProjectIdentifier(),
        settings.getUuid());
    NGLdapDelegateTaskParameters parameters =
    ...
    DelegateResponseData delegateResponseData =
        getDelegateResponseData(scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(),
            scopeInfo.getProjectIdentifier(), parameters, NG_LDAP_TEST_GROUP_SETTINGS, parameters.getLdapSettings());
  ...
  @Override
  public Collection<LdapGroupResponse> searchLdapGroupsByName(ScopeInfo scopeInfo, String ldapId, String name) {
    ...
    LdapSettingsWithEncryptedDataDetail settingsWithEncryptedDataDetail =
        getLdapSettingsWithEncryptedDataInternal(scopeInfo.getAccountIdentifier());
    ...
    DelegateResponseData delegateResponseData =
        getDelegateResponseData(scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(),
            scopeInfo.getProjectIdentifier(), parameters, NG_LDAP_SEARCH_GROUPS, ldapSettings);
    ...
    log.info("NGLDAP:Received delegate response for searchLdapGroupsByName in NG LDAP for account: {}",
        scopeInfo.getAccountIdentifier());
    return groupSearchResponse.getLdapListGroupsResponses();
  }

  @Override
  public void syncUserGroupsJob(ScopeInfo scopeInfo) {
    LdapSettingsWithEncryptedDataDetail settingsWithEncryptedDataDetail =
        getLdapSettingsWithEncryptedDataInternal(scopeInfo.getAccountIdentifier());

    if (checkAndLogIfLDAPAuthorizationIsEnabled(scopeInfo, settingsWithEncryptedDataDetail)) {
      return;
    }
    ...
    List<UserGroup> userGroupsToSync = userGroupService.getUserGroupsBySsoId(
        scopeInfo.getAccountIdentifier(), settingsWithEncryptedDataDetail.getLdapSettings().getUuid());
    syncUserGroupsJobInternal(scopeInfo, settingsWithEncryptedDataDetail, userGroupsToSync);
  }
  ...
  public LdapResponse testLDAPLogin(ScopeInfo scopeInfo, String email, String password) {
    ...
    LdapSettingsWithEncryptedDataAndPasswordDetail withEncryptedDataAndPasswordDetail = getResponse(
        managerClient.getLdapSettingsAndEncryptedPassword(scopeInfo.getAccountIdentifier(), authenticationRequest));
    ...
    DelegateResponseData delegateResponseData = getDelegateResponseData(scopeInfo.getAccountIdentifier(),
        scopeInfo.getOrgIdentifier(), scopeInfo.getProjectIdentifier(), taskParameters, NG_LDAP_TEST_AUTHENTICATION,
        taskParameters.getLdapSettings());
  ...
  @Override
  public void syncAUserGroupJob(ScopeInfo scopeInfo, String userGroupIdentifier) {
    LdapSettingsWithEncryptedDataDetail settingsWithEncryptedDataDetail =
        getLdapSettingsWithEncryptedDataInternal(scopeInfo.getAccountIdentifier());
    ...
    log.info("NGLDAP: Sync a user group with id {} for NG LDAP starting for account: {}, organization: {}, project: {}",
        userGroupIdentifier, scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(),
        scopeInfo.getProjectIdentifier());
    Optional<UserGroup> userGroup = userGroupService.get(scopeInfo, userGroupIdentifier);

    if (userGroup.isEmpty()) {
      log.warn("NGLDAP: User group with identifier {} not found to trigger LDAP sync in account: {}, organization: {}, "
              + "project: {}",
          userGroupIdentifier, scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(),
          scopeInfo.getProjectIdentifier());
      return;
  ...
  private boolean checkAndLogIfLDAPAuthorizationIsEnabled(
      ScopeInfo scopeInfo, LdapSettingsWithEncryptedDataDetail settingsWithEncryptedDataDetail) {
    if (null != settingsWithEncryptedDataDetail && null != settingsWithEncryptedDataDetail.getLdapSettings()
      ...
      log.info("NGLDAP: Sync user group is disabled at LDAP setting level for NG LDAP on account: {}, organization: "
              + "{}, project: {}. Skipping user group sync",
          scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(), scopeInfo.getProjectIdentifier());
      return true;
    }
    return false;
  ...
  private void syncUserGroupsJobInternal(ScopeInfo scopeInfo,
    ...
    if (isEmpty(userGroupsToSync)) {
      log.info("NGLDAP: No User groups to sync for acccountIdentifer:{}", scopeInfo.getAccountIdentifier());
      return;
    }
    ...
    for (UserGroup userGroup : userGroupsToSync) {
      try {
        ...
        DelegateResponseData delegateResponseData =
            getDelegateResponseData(userGroup.getAccountIdentifier(), ugScopeInfo.getOrgIdentifier(),
                ugScopeInfo.getProjectIdentifier(), parameters, NG_LDAP_GROUPS_SYNC, parameters.getLdapSettings());
        ...
        log.info("NGLDAP: Received delegate response for syncLdapGroupByDn in NG LDAP for group {} in account: {}",
            userGroup.getIdentifier(), scopeInfo.getAccountIdentifier());
        ...
        if (null != groupSearchResponse.getLdapGroupsResponse()) {
          if (isUserGroupSsoStateValid(ugScopeInfo, userGroup)) {
            ...
            log.warn("NGLDAP: userGroup {} sync is not happening for account {} due to invalid SSO state",
                userGroup.getIdentifier(), userGroup.getAccountIdentifier());
          }
        } else {
          log.warn("NGLDAP: No LDAP group response received in delegate response. Points to some error in delegate "
                  + "task execution for group: {} in account: {}",
              userGroup, scopeInfo.getAccountIdentifier());
        }
      } catch (Exception e) {
        log.error("NGLDAP: Sync error for user group {} and accountId {}", userGroup.getName(),
            scopeInfo.getAccountIdentifier(), e);
      }
    }

    ngLdapGroupSyncHelper.reconcileAllUserGroups(userGroupsToLdapGroupMap,
        settingsWithEncryptedDataDetail.getLdapSettings().getUuid(), scopeInfo.getAccountIdentifier());
  }
  ...
  private NGLdapDelegateTaskParameters getNgLdapDelegateTaskParameters(
      String accountIdentifier, software.wings.beans.sso.LdapSettingsDTO settings) {
    LdapSettingsWithEncryptedDataDetail settingsWithEncryptedDataDetail =
  ...
  private LdapSettingsWithEncryptedDataDetail getLdapSettingsWithEncryptedDataInternal(
      String accountIdentifier, software.wings.beans.sso.LdapSettingsDTO ldapSettings) {
  ...
  private DelegateResponseData executeDelegateSyncTask(DelegateTaskRequest delegateTaskRequest) {
    ...
    try {
      ...
      log.warn("NGLDAP: Error occurred while executing delegate task.", ex);
  ...
  Duration getLdapDelegateTaskResponseTimeout(
    ...
    log.info("NG_LDAP_DELEGATE_TASK_RESPONSE_TIMEOUT: Setting delegate tasks response timeout of {} seconds for task "
            + "type: {} in account: {}",

...
import static io.harness.NGConstants.NG_LDAP_CACHE_NAME;
...
public class LdapGroupSyncStreamConsumer extends RedisTraceConsumer {
  ...
  @Inject @Named(NG_LDAP_EXECUTOR) private ExecutorService executorService;
  @Inject @Named(NG_LDAP_CACHE_NAME) private Cache<String, Long> syncedAccountsCache;
  ...
  private void pollAndProcessMessages() {
    ...
    for (Message message : messages) {
      ...
      try {
        accountIdentifier = LdapGroupSyncDTO.parseFrom(message.getMessage().getData()).getAccountIdentifier();
        if (isNotEmpty(accountIdentifier)) {
          if (syncedAccountsCache.containsKey(accountIdentifier)) {
            log.debug(
                "Skipping ldap sync with messageId: {} for accountId: {} as already done within last 10 minutes at timestamp: {}",
                messageId, accountIdentifier, syncedAccountsCache.get(accountIdentifier));
            continue;
          } else {
            syncedAccountsCache.put(accountIdentifier, Instant.now().toEpochMilli());
          }
        }
      } catch (Exception e) {
        log.error(
            "Failed to query {} for messageId: {} accountId: {}", NG_LDAP_CACHE_NAME, messageId, accountIdentifier, e);
      }
      ...
      executorService.submit(() -> {
        try (AutoLogContext ignore = new MessageLogContext(message)) {
          ...
          log.error("[LdapGroupSyncStreamConsumer]: Exception occurred while processing event with messageId: {}",
              message.getId(), ex);
        }
      });
    }
    ...
    if (message.hasMessage()) {
      ...
      try {
        ...
        ScopeInfo scopeInfo = ScopeInfo.builder()
                                  .accountIdentifier(ldapGroupSyncDTO.getAccountIdentifier())
                                  .uniqueId(ldapGroupSyncDTO.getAccountIdentifier())

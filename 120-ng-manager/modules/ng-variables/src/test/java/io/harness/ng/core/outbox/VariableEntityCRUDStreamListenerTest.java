...
public class VariableEntityCRUDStreamListenerTest extends CategoryTest {
  ...
  public void testAccountDeleteEvent() {
    ...
    when(variableEntityCRUDEventHandler.deleteAssociatedVariables(idCaptor.capture())).thenReturn(true);
    boolean result = variableEntityCRUDStreamListener.handleMessage(message);
    verify(variableEntityCRUDEventHandler, times(1)).deleteAssociatedVariables(any());
    assertEquals(idCaptor.getValue().getAccountIdentifier(), accountId);
    assertTrue(result);
  }
  ...
  public void testAccountChangeEvent_withoutAnyActionSetInMetadata() {
    ...
    Message message = Message.newBuilder()
                          ...
                          .build();
    boolean result = variableEntityCRUDStreamListener.handleMessage(message);
    verify(variableEntityCRUDEventHandler, times(0)).deleteAssociatedVariables(any());
    assertTrue(result);
  }
  ...
  public void testProjectDeleteEvent() {
    ...
    Message message = Message.newBuilder()
                          ...
                          .build();
    final ArgumentCaptor<ScopeInfo> idCaptor = ArgumentCaptor.forClass(ScopeInfo.class);
    when(variableEntityCRUDEventHandler.deleteAssociatedVariables(any())).thenReturn(true);
    variableEntityCRUDStreamListener.handleMessage(message);
    verify(variableEntityCRUDEventHandler, times(1)).deleteAssociatedVariables(idCaptor.capture());

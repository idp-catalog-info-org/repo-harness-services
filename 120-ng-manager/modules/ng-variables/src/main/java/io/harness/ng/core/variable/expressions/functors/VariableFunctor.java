...
public class VariableFunctor implements SdkFunctor {
  ...
  public Object get(Ambiance ambiance, String... args) {
    log.debug("Fetching ng variables with args: {}", Arrays.asList(args));
  ...
  private Map<String, Object> getVariableValue(NGAccess ngAccess, ScopeLevel variableScopeLevel) {
    log.debug("Fetching variables of scope [{}]", variableScopeLevel);
    List<VariableDTO> variableDTOList;
    switch (variableScopeLevel) {
      case ACCOUNT:
        variableDTOList =
            variableService.list(scopeInfoService.getScopeInfo(ngAccess.getAccountIdentifier(), null, null));
        break;
      case ORGANIZATION:
        variableDTOList = variableService.list(
            scopeInfoService.getScopeInfo(ngAccess.getAccountIdentifier(), ngAccess.getOrgIdentifier(), null));
        break;
      case PROJECT:
        variableDTOList = variableService.list(scopeInfoService.getScopeInfo(
  ...
  private Object getVariableValue(NGAccess ngAccess, ScopeLevel scopeLevel, String identifier) {
    log.debug("Fetching variable [{}] in scope [{}]", identifier, scopeLevel);
    Optional<VariableResponseDTO> responseDTOOptional;
    switch (scopeLevel) {
      case ACCOUNT:
        responseDTOOptional =
            variableService.get(scopeInfoService.getScopeInfo(ngAccess.getAccountIdentifier(), null, null), identifier);
        break;
      case ORGANIZATION:
        ...
        break;
      case PROJECT:
        responseDTOOptional = variableService.get(scopeInfoService.getScopeInfo(ngAccess.getAccountIdentifier(),
  ...
  private void validateAccess(ScopeLevel ambianceScopeLevel, ScopeLevel variableScopeLevel) {
    log.debug("Validating scope bases access. Ambiance Scope: {} | Variable scope: {}", ambianceScopeLevel,
        variableScopeLevel);

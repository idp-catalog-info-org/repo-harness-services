...
public interface VariableResource {
  ...
  @POST
  @Path("list")
  @ApiOperation(value = "Get list of Variable", nickname = "listVariable")
  @Operation(operationId = "listVariable",
      summary = "Get list of  Variable by scope identifiers and variable identifiers.",
      responses =
      {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "default",
            description = "Returns the variable with the requested scope identifiers and variable identifier.")
  ...
  listV2(@Parameter(description = ACCOUNT_PARAM_MESSAGE, required = true) @QueryParam(
             ACCOUNT_KEY) @NotNull String accountIdentifier,
      @Parameter(description = ORG_PARAM_MESSAGE) @QueryParam(ORG_KEY) String orgIdentifier,
      @Parameter(description = PROJECT_PARAM_MESSAGE) @QueryParam(PROJECT_KEY) String projectIdentifier,
      @Parameter(description = "This would be used to filter Variables. Any Variable having the specified string in "
              + "its Name or ID would be filtered.") @QueryParam(SEARCH_TERM_KEY) String searchTerm,
      ...
      @RequestBody(description = "Specifies the variables to list") VariableListRequestDTO variableListRequestDTO,
  ...
  @Timed
  @ResponseMetered
  ResponseDTO<Boolean> delete(@Parameter(description = ACCOUNT_PARAM_MESSAGE, required = true) @QueryParam(
                                  ACCOUNT_KEY) @NotNull String accountIdentifier,
      @Parameter(description = ORG_PARAM_MESSAGE) @QueryParam(ORG_KEY) String orgIdentifier,
      @Parameter(description = PROJECT_PARAM_MESSAGE) @QueryParam(PROJECT_KEY) String projectIdentifier,
  ...
  @GET
  @Path("functor/{identifier}")
  @ApiOperation(hidden = true, value = "Get variable value for functor", nickname = "getVariableFunctorValue")
  @Operation(operationId = "getVariableFunctorValue",
      summary = "Internal API to get variable value or scope map for expression evaluation",
      responses =
      {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "default",
            description = "Returns variable value (for identifier) or map of variables (for scope 'account'/'org'). "
                + "Internal use only.")
      })
  @InternalApi
  @Hidden
  ...
  @ResponseMetered
  ResponseDTO<Object>
  getVariableFunctorValue(@Parameter(description = "Variable identifier or scope ('account'/'org')") @PathParam(
                              IDENTIFIER_KEY) @NotNull String identifier,
      @Parameter(description = ACCOUNT_PARAM_MESSAGE, required = true) @QueryParam(
          ACCOUNT_KEY) @NotNull String accountIdentifier,
      @Parameter(description = ORG_PARAM_MESSAGE) @QueryParam(ORG_KEY) String orgIdentifier,
      @Parameter(description = PROJECT_PARAM_MESSAGE) @QueryParam(PROJECT_KEY) String projectIdentifier,

...
public class CannyResource {
  ...
  @Produces({"application/json", "application/yaml"})
  @Consumes({"application/json", "application/yaml"})
  @ApiOperation(value = "Get a list of categories available for a board", nickname = "getCannyCategories")
  @Operation(operationId = "getCannyBoards", summary = "Get a list of categories available for a board in Canny",
      responses =
      {
        @io.swagger.v3.oas.annotations.responses.
        ApiResponse(responseCode = "default", description = "Get a list of categories available for a board in Canny")
      })
  public ResponseDTO<CannyCategoriesResponseDTO>
  getCategoriesList(@Parameter(description = ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
      "accountIdentifier") String accountIdentifier) {
    return ResponseDTO.newResponse(cannyClient.getCategories());
  }
  ...
  @ResponseMetered
  @Produces({"application/json", "application/yaml"})
  @ApiOperation(value = "create Canny Post for given user", nickname = "createCannyPostV2")
  @Operation(operationId = "createCannyPostV2", summary = "create Canny Post for given user",
      responses =
      {
        @io.swagger.v3.oas.annotations.responses.
  ...
  createCannyPostV2(@Parameter(description = ACCOUNT_PARAM_MESSAGE) @NotEmpty @QueryParam(
                        "accountIdentifier") String accountIdentifier,
      ...
      @Parameter(description = "boardId where post must be created") @NotEmpty @FormDataParam("boardId") String boardId,
      @Parameter(description = "categoryId where post must be created") @NotEmpty @FormDataParam(
          "categoryId") String categoryId) {
    return ResponseDTO.newResponse(cannyClient.createPostV2(email, name, title, details, boardId, categoryId));
  }

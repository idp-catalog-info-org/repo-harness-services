...
public class FreezeEvalutationResource {
  ...
  private final @Named("NON_PRIVILEGED") AccessControlClient accessControlClient;
  ...
  isGlobalFreezeActive(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
    ...
    List<FreezeSummaryResponseDTO> freezeSummaryResponseDTO =
        freezeEvaluateService.anyGlobalFreezeActive(accountId, orgId, projectId, scopeInfo);
  ...
  shouldDisableDeployment(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
      ...
      @Parameter(description = NGCommonEntityConstants.PROJECT_PARAM_MESSAGE) @QueryParam(
          NGCommonEntityConstants.PROJECT_KEY) @ProjectIdentifier String projectId,
      @Parameter(description = "pipeline identifier for the entity") @QueryParam(NGCommonEntityConstants.PIPELINE_KEY)
      String pipelineId, @Context ScopeInfo scopeInfo) {
    List<FreezeSummaryResponseDTO> freezeSummaryResponseDTO =
        freezeEvaluateService.getActiveFreezeEntities(accountId, orgId, projectId, pipelineId, scopeInfo);
    List<FreezeReference> freezeReferences = new LinkedList<>();
    freezeSummaryResponseDTO.forEach(freeze
  ...
  @POST
  @Path("/get-active-freeze-entities")
  @ApiOperation(value = "Get active global and manual freeze entities", nickname = "getActiveFreezeEntities")
  @Operation(operationId = "getActiveFreezeEntities", summary = "Get active global and manual freeze entities",
      responses =
      {
        @io.swagger.v3.oas.annotations.responses.
        ApiResponse(responseCode = "default", description = "Returns active freeze entities for the given scope")
      })
  @Hidden
  public ResponseDTO<ActiveFreezeEntitiesResponseDTO>
  getActiveFreezeEntities(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
                              NGCommonEntityConstants.ACCOUNT_KEY) @AccountIdentifier String accountIdentifier,
      @Parameter(description = NGCommonEntityConstants.ORG_PARAM_MESSAGE) @QueryParam(
          NGCommonEntityConstants.ORG_KEY) @OrgIdentifier String orgIdentifier,
      @Parameter(description = NGCommonEntityConstants.PROJECT_PARAM_MESSAGE) @QueryParam(
          NGCommonEntityConstants.PROJECT_KEY) @ProjectIdentifier String projectIdentifier,
      @RequestBody FreezeEntitiesRequestDTO freezeEntitiesRequestDTO, @Context ScopeInfo scopeInfo) {
    return ResponseDTO.newResponse(freezeEvaluateService.getActiveGlobalAndManualFreezeEntities(
        accountIdentifier, orgIdentifier, projectIdentifier, freezeEntitiesRequestDTO.getEntityMap(), scopeInfo));
  }
}

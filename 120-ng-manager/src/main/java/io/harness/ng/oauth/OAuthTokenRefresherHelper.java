...
public class OAuthTokenRefresherHelper {
  ...
  List<EncryptedDataDetail> getEncryptionDetails(DecryptableEntity decryptableEntity, ScopeInfo scopeInfo) {
  ...
  public SecretDTOV2 getSecretSecretValue(Scope scope, SecretRefData token, ScopeInfo scopeInfo) {
    SecretResponseWrapper tokenWrapper = null;
    ...
    tokenWrapper = ngSecretCrudService.get(scopeInfo, token.getIdentifier()).orElse(null);
  ...
  public void updateSecretSecretValue(Scope scope, SecretDTOV2 secretDTOV2, String newSecret) {
    ...
    ScopeInfo scopeInfo = scopeResolverService.getScopeInfo(
        scope.getAccountIdentifier(), secretDTOV2.getOrgIdentifier(), secretDTOV2.getProjectIdentifier());
    try {
      ngSecretCrudService.update(scopeInfo, secretDTOV2.getIdentifier(), secretDTOV2);
    } catch (Exception ex) {

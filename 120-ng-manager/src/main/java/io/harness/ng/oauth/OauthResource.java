...
public class OauthResource {
  ...
  public RestResponse<OauthAccessTokenResponseDTO> provisionCIResources(
    ...
    return new RestResponse<>(oauthSecretService.createSecrets(scopeInfo, scmProvider, accessToken,
        secretManagerIdentifier, isPrivateSecret, accessToken.getUserDetailsDTO()));
  }

  @POST
  @Path("create-access-token")
  @ApiOperation(value = "Create access token for oauth", nickname = "createAccessToken")
  public RestResponse<OAuthAccessTokenApiResponse> createAccessToken(
      @Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE, required = true) @NotBlank @QueryParam(
          NGCommonEntityConstants.ACCOUNT_KEY) String accountIdentifier,
      @Parameter(description = "state info", required = true) @Body OAuthCreateTokenAPIRequest stateInfo) {
    OAuthInfoParams infoParams = oauthSecretService.toStateInfoParams(stateInfo);
    UserDetailsDTO userDetailsDTO =
        UserDetailsDTO.builder().userEmail(stateInfo.getUserDetails().getUserEmail()).build();
    OauthAccessTokenResponseDTO responseDTO =
        oauthSecretService.createAccessTokenRequest(accountIdentifier, infoParams, userDetailsDTO);
    return new RestResponse<>(OAuthAccessTokenApiResponse.builder()
                                  .accessTokenRef(responseDTO.getAccessTokenRef())
                                  .refreshTokenRef(responseDTO.getRefreshTokenRef())
                                  .build());
  }
}

...
public abstract class AbstractSCMOAuthTokenRefresher<T extends UserSourceCodeManager>
    ...
    try (AutoLogContext autoLogContext = new TokenRefresherLogContext(entity.getAccountIdentifier(),
      ...
      try (NgManagerOpaContextGuard ignore = new NgManagerOpaContextGuard()) {
        ...
        try {
          ...
          log.error("Error from SCM for refreshing token clientID short:{}, client Secret short:{}", clientIdShort,
              clientSecretShort, e);
          return;
        }
  ...
  SecretDTOV2 getSecretSecretValue(T entity, SecretRefData token, ScopeInfo scopeInfo) {
    return oAuthTokenRefresherHelper.getSecretSecretValue(
        Scope.of(entity.getAccountIdentifier(), null, null), token, scopeInfo);
  }
  ...
  DecryptableEntity getOAuthDecryptedInternal(DecryptableEntity decryptableEntity, String accountIdentifier) {
    List<EncryptedDataDetail> encryptionDetails = oAuthTokenRefresherHelper.getEncryptionDetails(decryptableEntity,
        ScopeInfo.builder()

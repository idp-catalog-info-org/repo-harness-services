...
import io.harness.ng.core.event.handler.TerragruntConfigEntityCRUDEventHandler;
...
@Slf4j
@Singleton
public class TerragruntConfigEntityCRUDStreamListener implements MessageListener {
  private final TerragruntConfigEntityCRUDEventHandler terragruntConfigEntityCRUDEventHandler;
  private final PmsFeatureFlagHelper pmsFeatureFlagHelper;

  @Inject
  public TerragruntConfigEntityCRUDStreamListener(
      TerragruntConfigEntityCRUDEventHandler terragruntConfigEntityCRUDEventHandler,
      PmsFeatureFlagHelper pmsFeatureFlagHelper) {
    this.pmsFeatureFlagHelper = pmsFeatureFlagHelper;
    this.terragruntConfigEntityCRUDEventHandler = terragruntConfigEntityCRUDEventHandler;
  }
  ...
  private boolean processAccountChangeEvent(Message message) {
    ...
    return pmsFeatureFlagHelper.isEnabled(
               accountEntityChangeDTO.getAccountId(), FeatureName.PL_MOVE_PROJECTS_TERRAFORM_TERRAGRUNT_CONFIG)
        ? terragruntConfigEntityCRUDEventHandler.deleteAssociatedTerragruntConfigForScope(
              accountEntityChangeDTO.getAccountId())
        : terragruntConfigEntityCRUDEventHandler.deleteAssociatedTerragruntConfigForAccount(
              accountEntityChangeDTO.getAccountId());
  }
  ...
  private boolean processOrganizationChangeEvent(Message message) {
    ...
    return pmsFeatureFlagHelper.isEnabled(organizationEntityChangeDTO.getAccountIdentifier(),
               FeatureName.PL_MOVE_PROJECTS_TERRAFORM_TERRAGRUNT_CONFIG)
        ? terragruntConfigEntityCRUDEventHandler.deleteAssociatedTerragruntConfigForScope(
              organizationEntityChangeDTO.getUniqueId())
        : terragruntConfigEntityCRUDEventHandler.deleteAssociatedTerragruntConfigForOrganization(
  ...
  private boolean processProjectChangeEvent(Message message) {
    ...
    return pmsFeatureFlagHelper.isEnabled(
               projectEntityChangeDTO.getAccountIdentifier(), FeatureName.PL_MOVE_PROJECTS_TERRAFORM_TERRAGRUNT_CONFIG)
        ? terragruntConfigEntityCRUDEventHandler.deleteAssociatedTerragruntConfigForScope(
              projectEntityChangeDTO.getUniqueId())
        : terragruntConfigEntityCRUDEventHandler.deleteAssociatedTerragruntConfigForProject(

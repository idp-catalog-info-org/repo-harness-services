          ...
          try (HIterator<Environment> iterator = new HIterator<>(environmentQuery.fetch())) {
            for (Environment envEntity : iterator) {
              if (isBlank(envEntity.getYaml())) {
                try {
                  NGEnvironmentConfig ngEnvironmentConfig = EnvironmentMapper.toNGEnvironmentConfig(envEntity, null);
                  ...
                  log.info(String.format(DEBUG_LOG
                                   + ("Migration of populating yaml failed for envIdentifier: [%s], envName: [%s], "
                                       + "projectId: [%s], orgId: [%s], accountId: [%s]"),

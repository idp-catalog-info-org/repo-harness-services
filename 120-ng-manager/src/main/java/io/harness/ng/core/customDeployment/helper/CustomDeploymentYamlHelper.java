...
import static io.harness.ng.core.template.TemplateListType.ALL_TEMPLATE_TYPE;
...
import io.harness.cdng.infra.definition.config.InfrastructureDefinitionConfig.InfrastructureDefinitionConfigKeys;
import io.harness.cdng.infra.yaml.CustomDeploymentInfrastructure.CustomDeploymentInfrastructureKeys;
...
import io.harness.plancreator.customDeployment.StepTemplateRef.StepTemplateRefKeys;
...
public class CustomDeploymentYamlHelper {
  ...
  public Page<InfrastructureEntity> getFilteredInfraEntities(int page, int size, List<String> sort,
    ...
    List<String> versionLabels =
        resolveVersionLabels(accountId, orgIdentifier, projectIdentifier, deploymentTemplateIdentifier, versionLabel);
    List<InfrastructureEntity> entities =
        infraEntities.getContent()
            .stream()
            .filter(infra -> isInfraUsingDeploymentTemplate(deploymentTemplateIdentifier, versionLabels, infra))
  ...
  public Page<ServiceEntity> getFilteredServiceEntities(int page, int size, List<String> sort,
      String deploymentTemplateIdentifier, String versionLabel, Page<ServiceEntity> serviceEntities, String accountId,
    ...
    List<ServiceEntity> entities =
        serviceEntities.getContent()
            .stream()
            .filter(s -> isServiceUsingDeploymentTemplate(deploymentTemplateIdentifier, versionLabels, s))
            .collect(Collectors.toList());
    return new PageImpl<>(entities, PageUtils.getPageRequest(page, size, sort), entities.size());
  }
  ...
  public boolean isDeploymentTemplateService(
    ...
    if (!isNull(serviceEntity)) {
      ScopeInfo scopeInfo =
          scopeResolutionHelper.getScopeInfo(serviceEntity.getAccountId(), serviceEntity.getParentUniqueId());
      String yaml = serviceEntity.getYaml(scopeInfo);
      if (!isNull(yaml)) {
  ...
  public boolean isServiceUsingDeploymentTemplate(
      String deploymentTemplateIdentifier, List<String> versionLabels, ServiceEntity serviceEntity) {
    for (String versionLabel : versionLabels) {
      if (isDeploymentTemplateService(deploymentTemplateIdentifier, versionLabel, serviceEntity)) {
        return true;
      }
    }
    return false;
  ...
  private boolean isInfraUsingDeploymentTemplate(
      String deploymentTemplateIdentifier, List<String> versionLabels, InfrastructureEntity infrastructureEntity) {
    for (String versionLabel : versionLabels) {
      if (isDeploymentTemplateInfra(deploymentTemplateIdentifier, versionLabel, infrastructureEntity)) {
        return true;
      }
    }
    return false;
  ...
  private String getStableVersionOfTemplateByIdentifier(
      String accountId, String orgIdentifier, String projectIdentifier, String templateIdentifier) {
    PageResponse<TemplateMetadataSummaryResponseDTO> templateMetadataSummaryList;
    if (templateIdentifier.contains(ACCOUNT_IDENTIFIER)) {
      orgIdentifier = null;
      projectIdentifier = null;
      templateIdentifier = templateIdentifier.replace(ACCOUNT_IDENTIFIER, "");
    } else if (templateIdentifier.contains(ORG_IDENTIFIER)) {
      projectIdentifier = null;
      templateIdentifier = templateIdentifier.replace(ORG_IDENTIFIER, "");
    }
    templateMetadataSummaryList = NGRestUtils.getResponse(templateResourceClient.listTemplateMetadata(accountId,
        orgIdentifier, projectIdentifier, ALL_TEMPLATE_TYPE, 0, 1000, getTemplateIdentifierFilter(templateIdentifier)));
    List<TemplateMetadataSummaryResponseDTO> stableTemplate =
        templateMetadataSummaryList.getContent()
            .stream()
            .filter(templateMetadataSummary -> templateMetadataSummary.getStableTemplate())
            .collect(Collectors.toList());

    if (stableTemplate.isEmpty()) {
      return EMPTY;
    }
    return stableTemplate.get(0).getVersionLabel();
  }

  private TemplateFilterPropertiesDTO getTemplateIdentifierFilter(String templateIdentifier) {
    return TemplateFilterPropertiesDTO.builder()
  ...
  public List<String> resolveVersionLabels(String accountId, String orgIdentifier, String projectIdentifier,
      String deploymentTemplateIdentifier, String versionLabel) {
    List<String> versionLabels = new ArrayList<>();
    String stableVersion = getStableVersionOfTemplateByIdentifier(
        accountId, orgIdentifier, projectIdentifier, deploymentTemplateIdentifier);
    if (isEmpty(versionLabel) || versionLabel.equals(stableVersion)) {
      versionLabels.add(EMPTY);
      versionLabels.add(stableVersion);
    } else {
  ...
  public TemplateResponseDTO getScopedTemplateResponseDTO(
    ...
    return getScopedTemplateResponseDTO(accRef, orgRef, projectRef, templateRef, versionLabel, null);
  }

  public TemplateResponseDTO getScopedTemplateResponseDTO(
      String accRef, String orgRef, String projectRef, String templateRef, String versionLabel, String label) {
    ...
    if (templateRef.contains(ACCOUNT_IDENTIFIER)) {
      response = NGRestUtils.getResponse(templateResourceClient.get(
          templateRef.replace(ACCOUNT_IDENTIFIER, ""), accRef, null, null, label, versionLabel, false));
    } else if (templateRef.contains(ORG_IDENTIFIER)) {
      response = NGRestUtils.getResponse(templateResourceClient.get(
          templateRef.replace(ORG_IDENTIFIER, ""), accRef, orgRef, null, label, versionLabel, false));
    } else {
      response = NGRestUtils.getResponse(
          templateResourceClient.get(templateRef, accRef, orgRef, projectRef, label, versionLabel, false));
    }
    return response;
  }
  ...
  public List<EntityDetailProtoDTO> getReferencesFromYaml(
      String accountId, String orgId, String projectId, String parentUniqueId, String entityYaml) {
    List<EntityDetailProtoDTO> referredEntities = new ArrayList<>();
    Stack<String> path = new Stack<>();
    try {
      YamlField yaml = YamlUtils.getTopRootFieldInYaml(entityYaml);
      populateReferredEntitiesListForLeafNodes(
          accountId, orgId, projectId, parentUniqueId, referredEntities, yaml, path);
    } catch (Exception e) {
  ...
  private void populateReferredEntitiesListForLeafNodes(String accountId, String orgId, String projectId,
      String parentUniqueId, List<EntityDetailProtoDTO> referredEntities, YamlField yamlField, Stack<String> path) {
    path.push(yamlField.getName());
    if (yamlField.getNode().isArray()) {
      populateReferredEntitiesListForLeafNodesInArray(
          accountId, orgId, projectId, parentUniqueId, referredEntities, yamlField.getNode(), path);
    } else if (yamlField.getNode().isObject()) {
  ...
  private void populateReferredEntitiesListForLeafNodesInObject(String accountId, String orgId, String projectId,
      String parentUniqueId, List<EntityDetailProtoDTO> referredEntities, YamlNode yamlNode, Stack<String> path) {
    for (YamlField field : yamlNode.fields()) {
      if (!field.getNode().getCurrJsonNode().isValueNode()) {
        populateReferredEntitiesListForLeafNodes(
            accountId, orgId, projectId, parentUniqueId, referredEntities, field, path);
      }
    }
  }
    ...
    if (YamlUtils.checkIfNodeIsArrayWithPrimitiveTypes(yamlNode.getCurrJsonNode())) {
      if (!path.empty() && path.lastElement().equals(STEP_TEMPLATE_REFS)) {
        ...
        for (YamlNode arrayElement : yamlNode.asArray()) {
          ...
          EntityDetailProtoDTO referredEntity =
              getTemplateReferredEntity(accountId, orgId, projectId, parentUniqueId, arrayElement.asText());
      ...
      for (YamlNode arrayElement : yamlNode.asArray()) {
        if (isEmpty(arrayElement.getIdentifier()) && isNotEmpty(arrayElement.getArrayUniqueIdentifier())) {
          ...
          EntityDetailProtoDTO referredEntity =
              getConnectorReferredEntity(accountId, orgId, projectId, parentUniqueId, arrayElement, path);
          if (!isNull(referredEntity)) {
            referredEntities.add(referredEntity);
          }
        }
  ...
  private EntityDetailProtoDTO getConnectorReferredEntity(String accountId, String orgId, String projectId,
      String parentUniqueId, YamlNode arrayElement, Stack<String> path) {
    if (arrayElement.getField(YAMLFieldNameConstants.TYPE).getNode().asText().equalsIgnoreCase("connector")) {
      ...
      return buildConnectorEntityDetailProtoDTO(accountId, orgId, projectId, parentUniqueId, connectorRef, metadata);
    }
    return null;
  }

  private EntityDetailProtoDTO getTemplateReferredEntity(
      String accountId, String orgId, String projectId, String parentUniqueId, String templateRef) {
    if (isEmpty(templateRef)) {
      throw new InvalidRequestException("step template linked cannot have empty identifier");
    }
    return buildStableTemplateEntityDetailProtoDTO(accountId, orgId, projectId, parentUniqueId, templateRef);
  }

  private EntityDetailProtoDTO buildConnectorEntityDetailProtoDTO(String accountId, String orgId, String projectId,
      String parentUniqueId, String connectorRef, Map<String, String> metadata) {
    ...
    if (connectorRef.contains(ACCOUNT_IDENTIFIER)) {
      identifierRefProtoDTO.setScope(ScopeProtoEnum.ACCOUNT)
          ...
          .setParentUniqueId(StringValue.of(accountId));
      ...
      identifierRefProtoDTO.setScope(ScopeProtoEnum.ORG)
          ...
          .setParentUniqueId(StringValue.of(getOrgParentUniqueId(accountId, orgId, projectId, parentUniqueId)));
    } else {
      identifierRefProtoDTO.setScope(ScopeProtoEnum.PROJECT)
          ...
          .setParentUniqueId(StringValue.of(parentUniqueId));
    }
  ...
  private boolean isDeploymentTemplateInfra(
    ...
    YamlConfig yamlConfig = new YamlConfig(yaml);
    JsonNode infraNode = yamlConfig.getYamlMap().get(INFRASTRUCTURE_DEFINITION);
    if (!isNull(infraNode)) {
      JsonNode infraDefSpec = infraNode.get(InfrastructureDefinitionConfigKeys.spec);
      if (!isNull(infraDefSpec)) {
        JsonNode customDeploymentRef = infraDefSpec.get(CustomDeploymentInfrastructureKeys.customDeploymentRef);
        if (!isNull(customDeploymentRef)) {
          JsonNode ref = customDeploymentRef.get(StepTemplateRefKeys.templateRef);
          // If version label is null, filter infra based on customDeploymentRef
          if (versionLabel == null) {
            return ref.asText().equals(deploymentTemplateIdentifier);
          }
          JsonNode versionLabelNode = customDeploymentRef.get(StepTemplateRefKeys.versionLabel);
          String versionLabelRef = isNull(versionLabelNode) ? "" : versionLabelNode.asText();
          return ref.asText().equals(deploymentTemplateIdentifier) && versionLabelRef.equals(versionLabel);
        }
      }
  ...
  private String getOrgParentUniqueId(String accountId, String orgId, String projectId, String parentUniqueId) {
    if (orgId == null) {
      // this case should not happen but handling it just in case
      return accountId;
    }
    if (projectId == null && orgId != null) {
      // the parentUniqueId is org's uniqueId only. Simply return that
      return parentUniqueId;
    } else {
      // the parentUniqueId is project's uniqueId. Return org's by calculating using scopeInfo
      return scopeInfoService.getScopeInfo(accountId, orgId, null).getUniqueId();
    }
  }
}

...
import io.harness.cdng.gitops.beans.ClusterLinkAuditYamlDTO;
import io.harness.context.GlobalContext;
import io.harness.event.shared.MoveConfigAuditHandler;
...
import io.harness.ng.core.events.EnvironmentMoveConfigEvent;
...
public class EnvironmentOutboxEventHandler implements OutboxEventHandler, MoveConfigAuditHandler {
  ...
  private boolean handlerEnvironmentCreated(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = ScopeInfo.builder()
                              .accountIdentifier(environmentCreateEvent.getAccountIdentifier())
                              .orgIdentifier(environmentCreateEvent.getOrgIdentifier())
                              .projectIdentifier(environmentCreateEvent.getProjectIdentifier())
                              .uniqueId(environment.getParentUniqueId())
                              .build();
    AuditEntry auditEntry =
        AuditEntry.builder()
            ...
            .newYaml(isNotEmpty(environment.getYaml(scopeInfo))
                    ? environment.getYaml(scopeInfo)
                    : getYamlString(EnvironmentMapper.toNGEnvironmentConfig(environment, scopeInfo)))
            .build();
  ...
  private boolean handlerEnvironmentUpserted(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = ScopeInfo.builder()
                              .accountIdentifier(environmentUpsertEvent.getAccountIdentifier())
                              .orgIdentifier(environmentUpsertEvent.getOrgIdentifier())
                              .projectIdentifier(environmentUpsertEvent.getProjectIdentifier())
                              .uniqueId(environment.getParentUniqueId())
                              .build();
  ...
  private Map<String, String> updateYaml(OutboxEvent outboxEvent) throws IOException {
    ...
    switch (environmentUpdateEvent.getResourceType()) {
      ...
      case INFRASTRUCTURE:
        ...
        return yamls;
      case GITOPS_CLUSTER_LINK:
        return getAuditYamlDiffForClusterLink(environmentUpdateEvent);
      default:
        ScopeInfo newEnvScopeInfo =
            ScopeInfo.builder()
                .accountIdentifier(environmentUpdateEvent.getNewEnvironment().getAccountIdentifier())
                .orgIdentifier(environmentUpdateEvent.getNewEnvironment().getOrgIdentifier())
                .projectIdentifier(environmentUpdateEvent.getNewEnvironment().getProjectIdentifier())
                .uniqueId(environmentUpdateEvent.getNewEnvironment().getParentUniqueId())
                .build();
        ScopeInfo oldEnvScopeInfo =
            ScopeInfo.builder()
                .accountIdentifier(environmentUpdateEvent.getOldEnvironment().getAccountIdentifier())
                .orgIdentifier(environmentUpdateEvent.getOldEnvironment().getOrgIdentifier())
                .projectIdentifier(environmentUpdateEvent.getOldEnvironment().getProjectIdentifier())
                .uniqueId(environmentUpdateEvent.getOldEnvironment().getParentUniqueId())
                .build();

        yamls.put(OLD_YAML,
            isNotEmpty(environmentUpdateEvent.getOldEnvironment().getYaml(oldEnvScopeInfo))
                ? environmentUpdateEvent.getOldEnvironment().getYaml(oldEnvScopeInfo)
                : getYamlString(EnvironmentMapper.toNGEnvironmentConfig(
                      environmentUpdateEvent.getOldEnvironment(), oldEnvScopeInfo)));
        yamls.put(NEW_YAML,
            isNotEmpty(environmentUpdateEvent.getNewEnvironment().getYaml(newEnvScopeInfo))
                ? environmentUpdateEvent.getNewEnvironment().getYaml(newEnvScopeInfo)
                : getYamlString(EnvironmentMapper.toNGEnvironmentConfig(
                      environmentUpdateEvent.getNewEnvironment(), newEnvScopeInfo)));
        return yamls;
    }
  }
  ...
  @NotNull
  private Map<String, String> getAuditYamlDiffForClusterLink(EnvironmentUpdatedEvent environmentUpdateEvent) {
    Map<String, String> yamls = new HashMap<>();
    String clusterLinkYaml =
        getYamlString(ClusterLinkAuditYamlDTO.builder()
                          .clusterLinked(ClusterLinkAuditYamlDTO.ClusterLinkDetails.builder()
                                             .clusterRef(environmentUpdateEvent.getClusterRef())
                                             .agentIdentifier(environmentUpdateEvent.getAgentIdentifier())
                                             .envRef(environmentUpdateEvent.getEnvRef())
                                             .build())
                          .build());
    if (environmentUpdateEvent.getStatus() == EnvironmentUpdatedEvent.Status.CREATED) {
      yamls.put(NEW_YAML, clusterLinkYaml);
      yamls.put(OLD_YAML, EMPTY_YAML);
    } else if (environmentUpdateEvent.getStatus() == EnvironmentUpdatedEvent.Status.DELETED) {
      yamls.put(OLD_YAML, clusterLinkYaml);
      yamls.put(NEW_YAML, EMPTY_YAML);
    }
    return yamls;
  }
  ...
  private boolean handlerEnvironmentDeleted(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = ScopeInfo.builder()
                              .accountIdentifier(environmentDeleteEvent.getAccountIdentifier())
                              .orgIdentifier(environmentDeleteEvent.getOrgIdentifier())
                              .projectIdentifier(environmentDeleteEvent.getProjectIdentifier())
                              .uniqueId(environmentDeleteEvent.getEnvironment().getParentUniqueId())
                              .build();
    ...
    AuditEntry auditEntry =
        AuditEntry.builder()
            ...
            .resourceScope(ResourceScopeDTO.fromResourceScope(outboxEvent.getResourceScope()))
            .timestamp(outboxEvent.getCreatedAt())
            .oldYaml(isNotEmpty(environment.getYaml(scopeInfo))
                    ? environment.getYaml(scopeInfo)
                    : getYamlString(EnvironmentMapper.toNGEnvironmentConfig(environment, scopeInfo)))
            .build();
  ...
  private boolean handlerEnvironmentForceDeleted(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = ScopeInfo.builder()
                              .accountIdentifier(environmentForceDeleteEvent.getAccountIdentifier())
                              .orgIdentifier(environmentForceDeleteEvent.getOrgIdentifier())
                              .projectIdentifier(environmentForceDeleteEvent.getProjectIdentifier())
                              .uniqueId(environmentForceDeleteEvent.getEnvironment().getParentUniqueId())
                              .build();
  ...
  public boolean handle(OutboxEvent outboxEvent) {
    try {
      switch (outboxEvent.getEventType()) {
        ...
        case OutboxEventConstants.ENVIRONMENT_MOVED_TO_REMOTE:
          return handleMoveConfigAudit(outboxEvent);
  ...
  @Override
  public Class<? extends MoveConfigEvent> getMoveEventClass() {
    return EnvironmentMoveConfigEvent.class;
  }

  @Override
  public AuditClientService getAuditClientService() {

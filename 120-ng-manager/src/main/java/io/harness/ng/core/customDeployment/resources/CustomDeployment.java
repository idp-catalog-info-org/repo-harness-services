...
public class CustomDeployment {
  ...
  getInfraVariables(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
    ...
    TemplateResponseDTO response = NGRestUtils.getResponse(
        templateResourceClient.get(templateIdentifier, accountId, orgId, projectId, null, versionLabel, deleted));
  ...
  getEntityReferences(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
    ...
    List<EntityDetailProtoDTO> response = customDeploymentYamlHelper.getReferencesFromYaml(
        accountId, orgId, projectId, scopeInfo.getUniqueId(), customDeploymentYamlRequestDTO.getEntityYaml());
    return ResponseDTO.newResponse(response);
  }
  ...
  validateInfrastructure(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
    ...
    if (featureFlagHelperService.isEnabled(accountId, FeatureName.CDS_ENTITY_CRUD_RBAC)) {
      accessControlClient.checkForAccessOrThrow(ResourceScope.of(accountId, orgId, projectId),
          Resource.of(ENVIRONMENT, envIdentifier), ENVIRONMENT_VIEW_PERMISSION);
    }
    ScopeInfo scopeInfo = scopeResolverService.getScopeInfo(accountId, orgId, projectId);
    Optional<InfrastructureEntity> infraEntityOptional =
        infrastructureEntityService.get(accountId, orgId, projectId, scopeInfo, envIdentifier, infraIdentifier);
  ...
  getUpdatedYamlForInfra(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
    ...
    TemplateResponseDTO responseTemplate = customDeploymentYamlHelper.getScopedTemplateResponseDTO(accountId, orgId,
        projectId, customDeploymentRef.getTemplateRef(), customDeploymentRef.getVersionLabel(),
        customDeploymentRef.getLabel());

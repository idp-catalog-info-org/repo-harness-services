...
public class IPAllowlistConfigEventHandler implements OutboxEventHandler {
  ...
  private boolean handleIpAllowlistConfigCreateEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    if (ipAllowlistConfigCreateEvent.getIpAllowlistEntity() != null) {
      ipAllowlistConfig = toIPAllowlistConfig(ipAllowlistConfigCreateEvent.getIpAllowlistEntity());
    } else {
    ...
    AuditEntry auditEntry =
        AuditEntry.builder()
            ...
            .newYaml(getYamlString(IPAllowlistDTO.builder().ipAllowlistConfig(ipAllowlistConfig).build()))
  ...
  private boolean handleIpAllowlistConfigUpdateEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    if (ipAllowlistConfigUpdateEvent.getOldIpAllowlistEntity() == null) {
      oldIpAllowlistConfig = ipAllowlistConfigUpdateEvent.getOldIpAllowlistConfig();
      newIpAllowlistConfig = ipAllowlistConfigUpdateEvent.getNewIpAllowlistConfig();
    } else {
      oldIpAllowlistConfig = toIPAllowlistConfig(ipAllowlistConfigUpdateEvent.getOldIpAllowlistEntity());
      newIpAllowlistConfig = toIPAllowlistConfig(ipAllowlistConfigUpdateEvent.getNewIpAllowlistEntity());
    }
    AuditEntry auditEntry =
        AuditEntry.builder()
            .action(Action.UPDATE)
            .module(ModuleType.CORE)
            .oldYaml(getYamlString(IPAllowlistDTO.builder().ipAllowlistConfig(oldIpAllowlistConfig).build()))
            .newYaml(getYamlString(IPAllowlistDTO.builder().ipAllowlistConfig(newIpAllowlistConfig).build()))
            .timestamp(outboxEvent.getCreatedAt())
            .resource(ResourceDTO.fromResource(outboxEvent.getResource()))
            .resourceScope(ResourceScopeDTO.fromResourceScope(outboxEvent.getResourceScope()))
            .insertId(outboxEvent.getId())
  ...
  private boolean handleIpAllowlistConfigDeleteEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    IPAllowlistConfig ipAllowlistConfig;
    if (ipAllowlistConfigDeleteEvent.getIpAllowlistEntity() != null) {
      ipAllowlistConfig = toIPAllowlistConfig(ipAllowlistConfigDeleteEvent.getIpAllowlistEntity());
    } else {
    ...
    AuditEntry auditEntry =
        AuditEntry.builder()
            ...
            .module(ModuleType.CORE)
            .oldYaml(getYamlString(IPAllowlistDTO.builder().ipAllowlistConfig(ipAllowlistConfig).build()))
            .timestamp(outboxEvent.getCreatedAt())
            .resource(ResourceDTO.fromResource(outboxEvent.getResource()))
            .resourceScope(ResourceScopeDTO.fromResourceScope(outboxEvent.getResourceScope()))
            .insertId(outboxEvent.getId())

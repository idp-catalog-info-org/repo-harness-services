...
public class RancherClusterService {
  ...
  public RancherClusterListResponseDTO listClusters(String accountId, String orgId, String projectId,
    ...
    RancherListClustersTaskResponse taskResponse =
        executeRancherListClustersTask(taskParams, baseNGAccess, connectorRef);
    return RancherClusterListResponseDTO.builder()
        .clusters(taskResponse.getClusters())
        .clusterItems(taskResponse.getClusterItems())
        .build();
  }

  private RancherListClustersTaskResponse executeRancherListClustersTask(
      RancherTaskParams taskParams, BaseNGAccess baseNGAccess, IdentifierRef connectorRef) {
    DelegateResponseData delegateTaskResponse =
        clusterHelper.executeListClustersDelegateTask(taskParams, baseNGAccess, connectorRef.buildScopedIdentifier());

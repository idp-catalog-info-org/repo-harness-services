...
public class ApiKeyResource {
  ...
  createApiKey(@Parameter(description = ACCOUNT_PARAM_MESSAGE, required = true) @NotNull @QueryParam(
    ...
    try {
      ...
      log.error("Error Creating API key", ex);
      throw new InvalidArgumentsException(ex.getMessage(), USER_SRE);
    }
    apiKeyService.validateParentIdentifier(scopeInfo, apiKeyDTO.getApiKeyType(), apiKeyDTO.getParentIdentifier());
    ApiKeyDTO apiKey = apiKeyService.createApiKey(apiKeyDTO, scopeInfo);
  ...
  updateApiKey(@Valid ApiKeyDTO apiKeyDTO,
    ...
    try {
      ...
      log.error("Error Updating API key", ex);
      throw new InvalidArgumentsException(ex.getMessage(), USER_SRE);
    }
    apiKeyService.validateParentIdentifier(scopeInfo, apiKeyDTO.getApiKeyType(), apiKeyDTO.getParentIdentifier());
    ApiKeyDTO apiKey = apiKeyService.updateApiKey(apiKeyDTO, scopeInfo);
    return ResponseDTO.newResponse(apiKey);
  }
  ...
  deleteApiKey(
    ...
    boolean deleted = apiKeyService.deleteApiKey(scopeInfo, apiKeyType, parentIdentifier, identifier);
  ...
  listApiKeys(
      ...
      @Optional @QueryParam(IDENTIFIERS) List<String> identifiers, @Context ScopeInfo scopeInfo) {
    apiKeyService.validateParentIdentifier(scopeInfo, apiKeyType, parentIdentifier);
    List<ApiKeyDTO> apiKeyDTOs = apiKeyService.listApiKeys(scopeInfo, apiKeyType, parentIdentifier, identifiers);
  ...
  listAggregatedApiKeys(@Parameter(description = ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
      ...
      @Parameter(description = "This would be used to filter API keys. Any API key having the specified string in its "
    ...
    PageResponse<ApiKeyAggregateDTO> requestDTOS =
        apiKeyService.listAggregateApiKeys(scopeInfo, getPageRequest(pageRequest), filterDTO);
  ...
  getAggregatedApiKey(@Parameter(description = ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
      ...
      @Context ScopeInfo scopeInfo) {
    apiKeyService.validateParentIdentifier(scopeInfo, apiKeyType, parentIdentifier);
    ApiKeyAggregateDTO aggregateDTO =
        apiKeyService.getApiKeyAggregateDTO(scopeInfo, apiKeyType, parentIdentifier, identifier);

...
@Slf4j
public class ResetForceMigrationForGitXWebhooks implements NGMigration {
  @Inject private HPersistence persistence;

  private static final String DEBUG_LOG = "[ResetForceMigrationForGitXWebhooksStatus]: ";

  @Override
  public void migrate() {
    try {
      log.info(DEBUG_LOG + "Starting resetting of forceMigration field for GitXWebhook to false");
      ...
      persistence.update(gitXwebhookRerunMigrationQuery,
          persistence.createUpdateOperations(NGManagerUniqueIdParentIdMigrationStatus.class)
              ...
              .set(NGManagerUniqueIdParentIdMigrationStatusKeys.forceMigrateParentUniqueId, false));
    } catch (Exception e) {
      log.error(DEBUG_LOG + "Failed to reset forceMigration field for GitXWebhook", e);
    }
    log.info(DEBUG_LOG + ("Successfully reset forceMigrateParentUniqueId to false for GitXWebhook"));
  }

...
public class CleanupOrphanedApiKeysMigration implements NGMigration {
  @Inject private MongoTemplate mongoTemplate;

  private static final String DEBUG_LOG = "[CleanupOrphanedApiKeysMigration]: ";
  private static final int BATCH_SIZE = 100;
  private static final String COLLECTION = "ngApiKeys";
  ...
  @Override
  public void migrate() {
    log.info(DEBUG_LOG + "Starting cleanup of orphaned USER API keys with no account-level user membership");
    ...
    try (Stream<Document> stream = mongoTemplate.stream(query, Document.class, COLLECTION)) {
      ...
      while (iterator.hasNext()) {
        ...
        if (batch.size() == BATCH_SIZE || !iterator.hasNext()) {
          deletionCounter += deleteOrphanedKeysInBatch(batch);
          batch.clear();
        }
      }
    } catch (Exception e) {
  ...
  private int deleteOrphanedKeysInBatch(List<Document> batch) {
    List<Document> nullFieldKeys = new ArrayList<>();
    List<Document> validFieldKeys = new ArrayList<>();
    for (Document doc : batch) {
      if (doc.getString(FIELD_ACCOUNT_IDENTIFIER) == null || doc.getString(FIELD_PARENT_IDENTIFIER) == null) {
        nullFieldKeys.add(doc);
      } else {
        validFieldKeys.add(doc);
      }
    }

    int deleted = deleteKeys(nullFieldKeys);

    if (validFieldKeys.isEmpty()) {
      return deleted;
    }

    Set<String> accountIds =
        validFieldKeys.stream().map(d -> d.getString(FIELD_ACCOUNT_IDENTIFIER)).collect(Collectors.toSet());
    Set<String> userIds =
        validFieldKeys.stream().map(d -> d.getString(FIELD_PARENT_IDENTIFIER)).collect(Collectors.toSet());
    ...
    List<Document> toDelete = validFieldKeys.stream()
                                  .filter(d
    ...
    deleted += deleteKeys(toDelete);
    return deleted;
  }

  private int deleteKeys(List<Document> keys) {
    if (keys.isEmpty()) {
      return 0;
    }
    List<Object> ids = keys.stream().map(d -> d.get("_id")).filter(Objects::nonNull).collect(Collectors.toList());
    if (ids.isEmpty()) {
      return 0;
    }
    try {
      ...
      log.error("{} Failed to bulk delete {} API keys: {}", DEBUG_LOG, ids.size(), e.getMessage(), e);
      return 0;
    }
  }

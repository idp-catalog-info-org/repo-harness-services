...
import static io.harness.eventsframework.EventsFrameworkMetadataConstants.PERPETUAL_TASK_ENTITY_REFERENCE;
...
public class EntityCRUDStreamConsumer extends RedisTraceConsumer {
  ...
  public EntityCRUDStreamConsumer(@Named(ENTITY_CRUD) Consumer redisConsumer,
      ...
      @Named(CONNECTOR_ENTITY + "_ZOOM" + ENTITY_CRUD) MessageListener zoomConnectorEntityCRUDStreamListener,
      ...
      @Named(
          PERPETUAL_TASK_ENTITY_REFERENCE + ENTITY_CRUD) MessageListener perpetualTaskEntityReferenceCRUDStreamListener,
      ...
      @Named(GITOPS_AGENT_ENTITY + ENTITY_CRUD) MessageListener gitopsAgentInstancesCleanupProcessor,
      ...
      @Named(INSTANCE_NG + ENTITY_CRUD) MessageListener instanceNGCRUDStreamListener,
      @Named(DEFAULT_ENTITY + ENTITY_CRUD) MessageListener defaultEntityCleanUpListener,
      @Named(INSTANCE_SYNC + ENTITY_CRUD) MessageListener instanceSyncCrudStreamListener,
      @Named(DRIFT_DETECTION + ENTITY_CRUD) MessageListener driftDetectionCrudStreamListener,
    ...
    messageListenersList.add(zoomConnectorEntityCRUDStreamListener);
    ...
    messageListenersList.add(perpetualTaskEntityReferenceCRUDStreamListener);
    ...
    messageListenersList.add(gitopsAgentInstancesCleanupProcessor);
    ...
    messageListenersList.add(defaultEntityCleanUpListener);
    messageListenersList.add(instanceSyncCrudStreamListener);
    messageListenersList.add(driftDetectionCrudStreamListener);
  ...
  protected boolean processMessage(Message message) {
    ...
    messageListenersList.forEach(messageListener -> {
      try {
        ...
        log.error(
            "Error: {} listener encountered an error while processing the message: {}", messageListener, message, ex);
      }
    });

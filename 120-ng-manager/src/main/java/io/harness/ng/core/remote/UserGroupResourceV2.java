...
public class UserGroupResourceV2 {
  ...
  create(@Parameter(description = ACCOUNT_PARAM_MESSAGE, required = true) @NotNull @QueryParam(
      ...
      @Valid UserGroupRequestV2DTO userGroupDTO, @Context ScopeInfo scopeInfo) {
    ...
    try {
      savedDTO = userGroupService.createWithEvaluation(scopeInfo, UserGroupMapper.toV1(userGroupDTO, userIds));
      ...
      GovernanceMetadata governanceMetadata = ex.getMetadata() instanceof GovernanceMetadataErrorDTO
          ...
          : null;
      return ResponseDTO.newResponse(UserGroupResponseV2DTO.builder().governanceMetadata(governanceMetadata).build());
    }
    return ResponseDTO.newResponse(Long.toString(savedDTO.getVersion()),
        UserGroupMapper.toV2Response(savedDTO, userGroupService.getUserMetaData(userIds)));
  }
  ...
  update(@Parameter(description = ACCOUNT_PARAM_MESSAGE, required = true) @NotEmpty @QueryParam(
      ...
      @NotNull UserGroupRequestV2DTO userGroupDTO, @Context ScopeInfo scopeInfo) {
    ...
    if (isNotEmpty(userGroupDTO.getUsers())) {
      userIds = userGroupService.getUserIds(userGroupDTO.getUsers());
    }
    UserGroupDTO v1DTO = UserGroupMapper.toV1(userGroupDTO, userIds);
    UserGroupDTO savedDTO;
    try {
      savedDTO = userGroupService.updateWithEvaluation(scopeInfo, v1DTO);
    } catch (OPAPolicyEvaluationException ex) {
      GovernanceMetadata governanceMetadata = ex.getMetadata() instanceof GovernanceMetadataErrorDTO
          ? ((GovernanceMetadataErrorDTO) ex.getMetadata()).getGovernanceMetadata()
          : null;
    ...
    return ResponseDTO.newResponse(Long.toString(savedDTO.getVersion()),
        UserGroupMapper.toV2Response(savedDTO, userGroupService.getUserMetaData(savedDTO.getUsers())));
  }

  @GET
  @Path("{identifier}")
  @ApiOperation(value = "Get a User Group", nickname = "getUserGroupV2")
  @Operation(operationId = "getUserGroupV2", description = "Get a User Group in an account/org/project",
  ...
  get(@Parameter(description = ACCOUNT_PARAM_MESSAGE, required = true) @NotEmpty @QueryParam(
    ...
    if (userGroupOptional.isPresent()) {
      ...
      return ResponseDTO.newResponse(Long.toString(userGroupOptional.get().getVersion()),
          UserGroupMapper.toV2Response(
              toDTO(scopeInfo, userGroup), userGroupService.getUserMetaData(userGroup.getUsers())));
    } else {
  ...
  private static void validateScopes(ScopeInfo scopeInfo, UserGroupRequestV2DTO userGroupDTO) {

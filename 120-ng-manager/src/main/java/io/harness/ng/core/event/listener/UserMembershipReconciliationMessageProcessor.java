...
public class UserMembershipReconciliationMessageProcessor implements MessageListener {
  ...
  private boolean processAccountDeleteEvent(Message message) {
    ...
    return processScopeDelete(ScopeInfo.builder()
                                  .accountIdentifier(StringUtils.stripToNull(accountEntityChangeDTO.getAccountId()))
                                  .uniqueId(StringUtils.stripToNull(accountEntityChangeDTO.getAccountId()))
  ...
  private boolean processOrgDeleteEvent(Message message) {
    ...
    ScopeInfo scopeInfo =
        ScopeInfo.builder()
            .accountIdentifier(StringUtils.stripToNull(organizationEntityChangeDTO.getAccountIdentifier()))
            .orgIdentifier(StringUtils.stripToNull(organizationEntityChangeDTO.getIdentifier()))
            .uniqueId(StringUtils.stripToNull(organizationEntityChangeDTO.getUniqueId()))
            .scopeType(ScopeLevel.ORGANIZATION)
            .build();
    return processScopeDelete(scopeInfo);
  }
  ...
  private boolean processProjectDeleteEvent(Message message) {
    ...
    ScopeInfo scopeInfo = ScopeInfo.builder()
                              .accountIdentifier(StringUtils.stripToNull(projectEntityChangeDTO.getAccountIdentifier()))
                              .orgIdentifier(StringUtils.stripToNull(projectEntityChangeDTO.getOrgIdentifier()))
                              .projectIdentifier(StringUtils.stripToNull(projectEntityChangeDTO.getIdentifier()))
                              .uniqueId(StringUtils.stripToNull(projectEntityChangeDTO.getUniqueId()))
                              .scopeType(ScopeLevel.PROJECT)
                              .build();
    return processScopeDelete(scopeInfo);
  }

  private boolean processScopeDelete(ScopeInfo scopeInfo) {
    List<String> userIds = ngUserService.listUserIds(Scope.of(scopeInfo), scopeInfo);
    AtomicBoolean success = new AtomicBoolean(true);
    userIds.forEach(userId -> {
      if (!ngUserService.removeUserFromScope(
              userId, scopeInfo, UserMembershipUpdateSource.SYSTEM, NGRemoveUserFilter.STRICTLY_FORCE_REMOVE_USER)) {
        log.error("Delete operation failed for users with at scope [{}]", ScopeUtils.toString(scopeInfo));
        success.set(false);
      }
    });
    if (success.get()) {
      log.info("Successfully completed deletion for users at scope [{}]", ScopeUtils.toString(scopeInfo));
    }
    return success.get();
  }
}

...
public class AccountSetupListener implements MessageListener {
  ...
  private boolean processAccountUpdateEvent(AccountEntityChangeDTO accountEntityChangeDTO) {
    ...
    if (account.isNextGenEnabled()) {
      ...
      ngAccountSetupService.setupAccountForNG(
          accountEntityChangeDTO.getAccountId(), account.getEdition(), account.getPricingType());
    }
    return true;
  }
  ...
  private boolean processAccountDeleteEvent(AccountEntityChangeDTO accountEntityChangeDTO) {
    ...
    return processOrgDeletionForAccountDeleteEvent(accountIdentifier)
        && processNGAccessEntitiesDeletion(accountIdentifier);
  }

  private boolean processOrgDeletionForAccountDeleteEvent(String accountIdentifier) {
  ...
  private boolean processNGAccessEntitiesDeletion(String accountIdentifier) {
    Reflections reflections = new Reflections(IO_HARNESS);
    reflections.getSubTypesOf(NGAccountAccess.class).forEach(entity -> {
      if (containsField(entity, ACCOUNT_IDENTIFIER)) {
        deleteEntityDataUsingAccountId(accountIdentifier, entity, ACCOUNT_IDENTIFIER);
      } else if (containsField(entity, ACCOUNT_ID)) {
        deleteEntityDataUsingAccountId(accountIdentifier, entity, ACCOUNT_ID);
      }
    });
    return true;
  }

  public boolean deleteEntityDataUsingAccountId(
      String accountId, Class<? extends NGAccountAccess> entity, String field) {
    String collectionName = getCollectionName(entity);
    try {
      if (PersistentEntity.class.isAssignableFrom(entity)) {
        log.info("Deleting data in collection {} for accountId {}", collectionName, accountId);
        Query query = new Query(Criteria.where(field).is(accountId));
        mongoTemplate.remove(query, entity).getDeletedCount();
      }
    } catch (Exception e) {
      log.error("Exception while deleting data in collection {} for accountId {}", collectionName, accountId, e);
      return false;
    }
    return true;
  }
  ...
  private boolean containsField(Class<?> entityClass, String fieldName) {
    try {

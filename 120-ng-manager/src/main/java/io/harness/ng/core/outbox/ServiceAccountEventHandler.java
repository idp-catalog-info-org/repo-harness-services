...
public class ServiceAccountEventHandler implements OutboxEventHandler {
  ...
  private boolean handleServiceAccountCreateEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = getScopeInfo(serviceAccountCreateEvent.getServiceAccount());

    boolean publishedToRedis = publishEvent(accountIdentifier, orgIdentifier, projectIdentifier,
        outboxEvent.getResource().getIdentifier(), resource, EventsFrameworkMetadataConstants.CREATE_ACTION, scopeInfo);
  ...
  private boolean handleServiceAccountUpdateEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = getScopeInfo(serviceAccountUpdateEvent.getNewServiceAccount());
    boolean publishedToRedis = publishEvent(accountIdentifier, orgIdentifier, projectIdentifier,
        outboxEvent.getResource().getIdentifier(), resource, EventsFrameworkMetadataConstants.UPDATE_ACTION, scopeInfo);
    AuditEntry auditEntry =
        AuditEntry.builder()
  ...
  private boolean handleServiceAccountDeleteEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = getScopeInfo(serviceAccountDeleteEvent.getServiceAccount());
    boolean publishedToRedis = publishEvent(accountIdentifier, orgIdentifier, projectIdentifier,
        outboxEvent.getResource().getIdentifier(), resource, EventsFrameworkMetadataConstants.DELETE_ACTION, scopeInfo);
  ...
  private boolean publishEvent(String accountIdentifier, String orgIdentifier, String projectIdentifier,
      String identifier, Resource resource, String action, ScopeInfo scopeInfo) {
    try {
      eventProducer.send(
          Message.newBuilder()
  ...
  private ScopeInfo getScopeInfo(ServiceAccountDTO serviceAccountDTO) {
    return ScopeInfo.newBuilder()
        .setUniqueId(StringValue.of(serviceAccountDTO.getParentUniqueId()))
        .setScope(getScopeType(serviceAccountDTO))
        .build();
  }

  private ScopeProtoEnum getScopeType(ServiceAccountDTO serviceAccountDTO) {
    if (isNotEmpty(serviceAccountDTO.getOrgIdentifier())) {
      if (isEmpty(serviceAccountDTO.getProjectIdentifier())) {

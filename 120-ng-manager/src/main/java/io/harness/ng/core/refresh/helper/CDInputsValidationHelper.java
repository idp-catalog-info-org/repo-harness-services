...
public class CDInputsValidationHelper {
  ...
  @Inject ChainedPipelineScopeHelper chainedPipelineScopeHelper;
  ...
  private void validateInputsInObject(
    ...
    for (YamlField childYamlField : yamlNode.fields()) {
      ...
      if (serviceEntityService.isServiceField(fieldName, value)) {
        if (currentYamlNode.getField(YamlTypes.USE_FROM_STAGE) != null) {
          validateResolvedTemplatesYamlNodeForUseFromStageService(
              currentYamlNode, context.getResolvedTemplatesYamlNode());
        }
      ...
      EntityRefreshContext contextForChildren = context;
      if (chainedPipelineScopeHelper.isChainedPipelineStage(fieldName, value)) {
        contextForChildren = chainedPipelineScopeHelper.childScopedContext(value, context);
        if (contextForChildren == null) {
          continue;
        }
      }
      ...
      if (value.isArray() && !YamlUtils.checkIfNodeIsArrayWithPrimitiveTypes(value)) {
        // Value -> Array
        validateInputsInArray(childYamlField.getNode(), contextForChildren, inputsValidationResponse);
      } else if (value.isObject()) {
        // Value -> Object
        validateInputsInObject(childYamlField.getNode(), contextForChildren, inputsValidationResponse);
      }
    }
  }
  ...
  void validateServiceInputs(
    ...
    ScopeInfo scopeInfo = scopeResolutionHelper.getScopeInfo(context.getAccountId(), serviceEntity.getParentUniqueId());

    String serviceYaml = serviceEntity.fetchNonEmptyYaml(scopeInfo);
    ...
    YamlNode primaryManifestRefNode = YamlNodeUtils.goToPathUsingFqn(
        entityNode, "serviceInputs.serviceDefinition.spec.manifestConfigurations.primaryManifestRef");

    String primaryArtifactRefNodeValue = primaryArtifactRefNode == null ? null : primaryArtifactRefNode.asText();
    String primaryManifestRefNodeValue = primaryManifestRefNode == null ? null : primaryManifestRefNode.asText();

    String serviceRuntimeInputYaml = serviceEntityService.createServiceInputsYamlGivenPrimaryArtifactRefAndManifest(
        context.getAccountId(), serviceYaml, serviceRef, primaryArtifactRefNodeValue, primaryManifestRefNodeValue);
  ...
  private void validateResolvedTemplatesYamlNodeForUseFromStageService(
      YamlNode entityNode, YamlNode resolvedTemplatesYamlNode) {
    YamlNode stageYamlNodeInResolvedTemplatesYaml =
        environmentRefreshHelper.getCorrespondingStageNodeInResolvedTemplatesYaml(
            entityNode, resolvedTemplatesYamlNode);
    ...
    YamlField serviceRefNode = serviceNodeInResolvedTemplatesYaml.getField(YamlTypes.SERVICE_REF);
    if (serviceRefNode == null) {
      return;
    }
    String serviceRef = serviceRefNode.getNode().getCurrJsonNode().asText();
    YamlField useFromStage = serviceNodeInResolvedTemplatesYaml.getField(YamlTypes.USE_FROM_STAGE);
    if (useFromStage != null && !(NGExpressionUtils.matchesInputSetPattern(serviceRef))) {
      throw new InvalidRequestException(
          String.format("Service in stage [%s] cannot be propagated from it's previous stages if this service is "
                  + "already defined as a fixed or an expression value in the template or pipeline yaml.",
              stageYamlNodeInResolvedTemplatesYaml.getIdentifier()));
    }
  }
}

...
public class GitInfoPopulatorForConnector {
  ...
  @Inject
  public GitInfoPopulatorForConnector(
      @Named(CONNECTOR_DECORATOR_SERVICE) ConnectorService connectorService, ScopeInfoService scopeResolverService) {
  ...
  private List<ConnectorResponseDTO> getConnectorsList(
    ...
    try (GlobalContextManager.GlobalContextGuard guard = GlobalContextManager.ensureGlobalContextGuard()) {
      ...
      int size = 100;
      do {
        connectorPage = connectorService.list(scopeResolverService.getScopeInfo(accountId, orgId, projectId),
            ConnectorFilterPropertiesDTO.builder().connectorIdentifiers(connectorIdentifiers).build(), null, null, null,

...
public class ApiKeyEventHandler implements OutboxEventHandler {
  ...
  private boolean handleApiKeyCreateEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = getScopeInfo(apiKeyCreateEvent.getApiKey());
    boolean publishedToRedis = publishEvent(apiKeyCreateEvent.getApiKey(), CREATE_ACTION, scopeInfo);
  ...
  private boolean handleApiKeyUpdateEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = getScopeInfo(apiKeyUpdateEvent.getNewApiKey());

    boolean publishedToRedis =
        publishEvent(apiKeyUpdateEvent.getNewApiKey(), EventsFrameworkMetadataConstants.UPDATE_ACTION, scopeInfo);
    AuditEntry auditEntry =
        AuditEntry.builder()
  ...
  private boolean handleApiKeyDeleteEvent(OutboxEvent outboxEvent) throws IOException {
    ...
    ScopeInfo scopeInfo = getScopeInfo(apiKeyDeleteEvent.getApiKey());

    boolean publishedToRedis =
        publishEvent(apiKeyDeleteEvent.getApiKey(), EventsFrameworkMetadataConstants.DELETE_ACTION, scopeInfo);
    AuditEntry auditEntry =
        AuditEntry.builder()
  ...
  private boolean publishEvent(ApiKeyDTO apiKeyDTO, String action, ScopeInfo scopeInfo) {
    try {
      eventProducer.send(
          Message.newBuilder()
              ...
              .setData(ApiKeyEntityChangeDTO.newBuilder()
                           ...
                           .setIdentifier(apiKeyDTO.getIdentifier())
                           .setUniqueId(apiKeyDTO.getUniqueId())
  ...
  private ScopeInfo getScopeInfo(ApiKeyDTO apiKeyDTO) {
    return ScopeInfo.newBuilder()
        .setUniqueId(StringValue.of(apiKeyDTO.getParentUniqueId()))
        .setScope(getScopeType(apiKeyDTO))
        .build();
  }

  private ScopeProtoEnum getScopeType(ApiKeyDTO apiKeyDTO) {
    if (isNotEmpty(apiKeyDTO.getOrgIdentifier())) {
      if (isEmpty(apiKeyDTO.getProjectIdentifier())) {

...
import static io.harness.beans.FeatureName.PL_USE_SCOPE_INFO_FOR_CONNECTOR_ENTITY_V2;
...
public class ProvisionService {
  ...
  private static final String GENERATE_SAMPLE_DELEGATE_CURL_COMMAND_FORMAT_STRING =
      ...
      + "--url "
      + "https://app.harness.io/gateway/api/webhooks/"
      + "WLwBdpY6scP0G9oNsGcX2BHrY4xH44W7r7HWYC94?accountId=gz4oUAlfSgONuOrWmphHif "
  ...
  private Boolean installDelegate(String accountId) {
    ...
    try {
      tokenRequest =
          delegateTokenNgClient.getDelegateTokenValue(accountId, null, null, accountId, DEFAULT_TOKEN).execute();
    } catch (IOException e) {
  ...
  private Boolean installConnector(String accountId) {
    try {
      Optional<ConnectorResponseDTO> connectorResponseDTO =
          featureFlagService.isEnabled(PL_USE_SCOPE_INFO_FOR_CONNECTOR_ENTITY_V2, accountId)
          ? connectorService.get(ScopeInfo.builder()
      ...
      telemetryReporter.sendTrackEvent(
          PROVISION_STARTED, null, accountId, provisionMap, Collections.singletonMap(AMPLITUDE, true), Category.GLOBAL);
      ...
      ConnectorResponseDTO connectorResponse =
          connectorService.create(scopeResolverService.getScopeInfo(accountId, null, null), connectorDTO);
    } catch (Exception e) {
  ...
  public ScmConnectorResponse createDefaultScm(ScopeInfo scopeInfo, ScmConnectorDTO scmConnectorDTO) {
    ...
    if (connectorInfoDTO.getConnectorType() != GITHUB && connectorInfoDTO.getConnectorType() != BITBUCKET
        && connectorInfoDTO.getConnectorType() != GITLAB) {
    ...
    if (secretResponseWrapperOptional.isPresent()) {
      ...
      secretResponseWrapper = ngSecretService.create(scopeInfo, secretDTOV2);
    }

    Optional<ConnectorResponseDTO> connectorResponseDTOOptional =
        connectorService.get(scopeInfo, connectorInfoDTO.getIdentifier());

    if (!connectorResponseDTOOptional.isPresent()) {
      // TODO: remove scopeResolver when CIProvisionResource is handled to get ScopeInfo
      connectorResponseDTO =
          connectorService.create(scopeInfo, ConnectorDTO.builder().connectorInfo(connectorInfoDTO).build());
    } else {
      connectorResponseDTO =
          connectorService.update(scopeInfo, ConnectorDTO.builder().connectorInfo(connectorInfoDTO).build());
    }

    ConnectorValidationResult connectorValidationResult =
        connectorService.testConnection(scopeInfo, connectorInfoDTO.getIdentifier());
  ...
  public String generateYaml(String accountIdentifier, String orgIdentifier, String projectIdentifier,
      String connectorIdentifier, String repo, String version, ScopeInfo scopeInfo) {
    ...
    if (Strings.isBlank(connectorIdentifier)) {
      ...
      repo =
          GitClientHelper.convertToHarnessRepoSlug(accountIdentifier, orgIdentifier, projectIdentifier, repo) + ".git";
    } else {
      autogenInput = buildAutogenInput(ngAccess, connectorIdentifier, scopeInfo);
    }
  ...
  private AutogenInput buildAutogenInputForHarnessCode(String repo, NGAccess ngAccess) {
    ...
    if (principal != null && PrincipalType.SERVICE_ACCOUNT.equals(principal.getType())) {
      uniqueId = ((ServiceAccountPrincipal) principal).getUniqueId();
    }
    String token = harnessCodeConnectorUtils.getTokenWithClaims(
        ngServiceSecret, ngAccess, repo, principal.getName(), principal.getType().name(), uniqueId, 1);
    return AutogenInput.builder().username("admin").password(token).repo(harnessCodeGitUrl).build();
  }

  private AutogenInput buildAutogenInput(BaseNGAccess build, String connectorIdentifier, ScopeInfo scopeInfo) {
    Optional<ConnectorResponseDTO> connectorResponseDTO = scopeInfo != null
        ? connectorService.get(scopeInfo, connectorIdentifier)
        : connectorService.get(build.getAccountIdentifier(), build.getOrgIdentifier(), build.getProjectIdentifier(),
              connectorIdentifier);
    if (!connectorResponseDTO.isPresent()) {
      throw new InvalidRequestException(format("connector %s doesn't exists", connectorIdentifier));
    }

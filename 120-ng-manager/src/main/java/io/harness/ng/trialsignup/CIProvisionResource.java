...
@Slf4j
@ScopeInfoResolutionApi(featureName = FeatureName.PL_USE_SCOPE_INFO_FOR_CONNECTOR_ENTITY_V2)
public class CIProvisionResource {
  ...
  public ResponseDTO<ScmConnectorResponse> createScmConnector(
    ...
    return ResponseDTO.newResponse(provisionService.createDefaultScm(scopeInfo, scmConnectorDTO));
  }
  ...
  public ResponseDTO<String> autogenerateYaml(@Parameter(description = ACCOUNT_PARAM_MESSAGE, required = true) @NotBlank
    ...
    return ResponseDTO.newResponse(provisionService.generateYaml(
        accountIdentifier, orgIdentifier, projectIdentifier, connectorIdentifier, repo, yamlVersion, scopeInfo));
  }
}

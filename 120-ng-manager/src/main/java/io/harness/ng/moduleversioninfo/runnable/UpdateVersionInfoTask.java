...
public class UpdateVersionInfoTask {
  ...
  public static final String DBOPS_API_PATH = "api/";
  ...
  private String getBaseUrl(String moduleName) throws InvalidRequestException {
    ...
    try {
      moduleType = ModuleType.fromString(moduleName);
    } catch (IllegalArgumentException e) {
      String errorMsg = String.format("Invalid moduleName provided: %s", moduleName);
      log.error(errorMsg);
    ...
    switch (moduleType) {
      ...
      case IACM:
        return nextGenConfiguration.getIacmClientConfig().getBaseUrl();
      case CI:
      ...
      case PLATFORM:
        return nextGenConfiguration.getNgManagerClientConfig().getBaseUrl();
      case DBOPS:
        String dbOpsBaseUrl = nextGenConfiguration.getDbOpsServiceClientConfig().getBaseUrl();
        return new StringBuilder(dbOpsBaseUrl)
            .append(dbOpsBaseUrl.endsWith("/") ? "" : "/")
            .append(DBOPS_API_PATH)
            .toString();
      default:
  ...
  private String getCurrentMicroserviceVersions(String serviceName, String serviceVersionUrl)
    ...
    try {
      ModuleType moduleType = ModuleType.fromString(serviceName);
      switch (moduleType) {
        case CF:
  ...
  private void updateModuleVersionInfoCollection(ModuleVersionInfo module) {
    ...
    update.set(ModuleVersionInfoKeys.uuid, module.getUuid())
        ...
        .set(ModuleVersionInfoKeys.lastModifiedAt, LocalDateTime.now().format(DATE_FORMATTER))

...
import static io.harness.eventsframework.EventsFrameworkConstants.CD_BACKFILL_ORCHESTRATION_EVENT_CONSUMER;
...
import static io.harness.eventsframework.EventsFrameworkConstants.GITOPS_UTILIZATION_SNAPSHOT_REDIS_EVENT_CONSUMER;
...
import static io.harness.eventsframework.EventsFrameworkConstants.PIPELINE_BACKFILL_ORCHESTRATION_EVENT_BATCH_SIZE;
...
import static io.harness.eventsframework.EventsFrameworkConstants.TASK_RESPONSE_TOPIC_SUFFIX;
    ...
    if (redisConfig.getRedisUrl().equals("dummyRedisUrl")) {
      ...
      bind(Producer.class)
          ...
          .toInstance(GitAwareRedisProducer.of(EventsFrameworkConstants.SETUP_USAGE, redissonClient,
              EventsFrameworkConstants.SETUP_USAGE_MAX_TOPIC_SIZE, NG_MANAGER.getServiceId(),
              redisConfig.getEnvNamespace()));
      bind(Consumer.class)
          .annotatedWith(Names.named(EventsFrameworkConstants.SETUP_USAGE))
      ...
      bind(Consumer.class)
          .annotatedWith(Names.named(EventsFrameworkConstants.LICENSES_USAGE_REDIS_EVENT_CONSUMER))
          .toInstance(
              RedisConsumer.of(EventsFrameworkConstants.LICENSES_USAGE_REDIS_EVENT_CONSUMER, NG_MANAGER.getServiceId(),
                  redissonClient, EventsFrameworkConstants.LICENSE_USAGE_EVENT_MAX_PROCESSING_TIME,
                  EventsFrameworkConstants.LICENSE_USAGE_EVENT_READ_BATCH_SIZE, redisConfig.getEnvNamespace()));
      ...
      bind(Consumer.class)
          .annotatedWith(Names.named(CDNG_ORCHESTRATION_EVENT_CONSUMER))
          .toInstance(RedisConsumer.of(EventsFrameworkRedisTopicResolver.getPipelineOrchestrationTopic(),
              NG_MANAGER.getServiceId(), redissonClient, java.time.Duration.ofSeconds(MAX_PROCESSING_TIME_SECONDS),
              PIPELINE_ORCHESTRATION_EVENT_BATCH_SIZE, redisConfig.getEnvNamespace()));
      bind(Consumer.class)
          .annotatedWith(Names.named(GITOPS_UTILIZATION_SNAPSHOT_REDIS_EVENT_CONSUMER))
          .toInstance(RedisConsumer.of(debeziumConsumersConfigs.getGitOpsUtilizationSnapshotStreaming().getTopic(),
              NG_MANAGER.getServiceId(), redissonClient, EventsFrameworkConstants.DEFAULT_MAX_PROCESSING_TIME,
              debeziumConsumersConfigs.getGitOpsUtilizationSnapshotStreaming().getBatchSize(),
              redisConfig.getEnvNamespace()));
      bind(Consumer.class)
          .annotatedWith(Names.named(GITOPS_APPLICATIONS_REDIS_EVENT_CONSUMER))
          .toInstance(RedisConsumer.of(debeziumConsumersConfigs.getGitOpsApplicationsStreaming().getTopic(),
              NG_MANAGER.getServiceId(), redissonClient, EventsFrameworkConstants.DEFAULT_MAX_PROCESSING_TIME,
              debeziumConsumersConfigs.getGitOpsApplicationsStreaming().getBatchSize(), redisConfig.getEnvNamespace()));
      bind(Consumer.class)
          .annotatedWith(Names.named(CD_BACKFILL_ORCHESTRATION_EVENT_CONSUMER))
          .toInstance(RedisConsumer.of(EventsFrameworkRedisTopicResolver.getPipelineBackfillOrchestrationTopic(),
              NG_MANAGER.getServiceId(), redissonClient, java.time.Duration.ofSeconds(MAX_PROCESSING_TIME_SECONDS),
              PIPELINE_BACKFILL_ORCHESTRATION_EVENT_BATCH_SIZE, redisConfig.getEnvNamespace()));
      bind(Consumer.class)
          .annotatedWith(Names.named(EventsFrameworkConstants.EXECUTION_RETENTION_CLEANUP_EVENT))
          .toInstance(RedisConsumer.of(EventsFrameworkConstants.EXECUTION_RETENTION_CLEANUP_EVENT,
              NG_MANAGER.getServiceId(), redissonClient,
              EventsFrameworkConstants.EXECUTION_RETENTION_CLEANUP_EVENT_MAX_PROCESSING_TIME,
              EventsFrameworkConstants.EXECUTION_RETENTION_CLEANUP_EVENT_BATCH_SIZE, redisConfig.getEnvNamespace()));

      bind(Consumer.class)
          .annotatedWith(Names.named(SCHEDULED_TASK_DEFAULT_RESPONSE_CONSUMER))
          .toInstance(RedisConsumer.of("scheduled_task_default" + TASK_RESPONSE_TOPIC_SUFFIX, NG_MANAGER.getServiceId(),
              redissonClient, EventsFrameworkConstants.DEFAULT_MAX_PROCESSING_TIME,
              EventsFrameworkConstants.DEFAULT_READ_BATCH_SIZE, redisConfig.getEnvNamespace()));

      bind(Consumer.class)
          .annotatedWith(Names.named(UNIFIED_INSTANCE_SYNC_RESPONSE_CONSUMER))
          .toInstance(RedisConsumer.of("unified_instance_sync" + TASK_RESPONSE_TOPIC_SUFFIX, NG_MANAGER.getServiceId(),
              redissonClient, EventsFrameworkConstants.DEFAULT_MAX_PROCESSING_TIME,
              EventsFrameworkConstants.DEFAULT_READ_BATCH_SIZE, redisConfig.getEnvNamespace()));

      bind(Consumer.class)
          .annotatedWith(Names.named(UNIFIED_ARTIFACT_POLLING_RESPONSE_CONSUMER))
          .toInstance(RedisConsumer.of("unified_artifact_polling" + TASK_RESPONSE_TOPIC_SUFFIX,
              NG_MANAGER.getServiceId(), redissonClient, EventsFrameworkConstants.DEFAULT_MAX_PROCESSING_TIME,
              EventsFrameworkConstants.DEFAULT_READ_BATCH_SIZE, redisConfig.getEnvNamespace()));

      bind(Consumer.class)
          .annotatedWith(Names.named(DRIFT_DETECTION_RESPONSE_CONSUMER))
          .toInstance(RedisConsumer.of("drift_detection" + TASK_RESPONSE_TOPIC_SUFFIX, NG_MANAGER.getServiceId(),
              redissonClient, EventsFrameworkConstants.DEFAULT_MAX_PROCESSING_TIME,
              EventsFrameworkConstants.DEFAULT_READ_BATCH_SIZE, redisConfig.getEnvNamespace()));
    }
  }

...
public class NGScimUserServiceImpl implements ScimUserService {
  ...
  public Response createUser(ScimUser userQuery, String accountId) {
    ...
    log.info("NGSCIM_NPE: Check userInfo {} part of account {} and constructed optionalUser {}",
        userInfoOptional.map(UserInfo::getEmail).orElse(null), accountId,
        userOptional.map(UserMetadataDTO::getEmail).orElse(null));
    if (checkUserPartOfAccount(accountId, userInfoOptional) && userOptional.isPresent()) {
      ...
      ngUserService.addUserToScope(user.getUuid(), Scope.of(accountId, null, null), null, null,
      ...
      log.info("NGSCIM_NPE: Check invite details {}", invite);
      inviteService.create(scopeInfo, invite, true, false);
  ...
  public ScimUser getUser(String userId, String accountId) {
    ...
    if (userInfo.isPresent()) {
      ...
      if (userOptional.isPresent()
          && ngUserService.isUserAtScope(userOptional.get().getUuid(),
              ScopeInfo.builder()
                  .accountIdentifier(accountId)
                  .uniqueId(accountId)
  ...
  private ScimListResponse<ScimUser> removeUsersNotinNG(ScimListResponse<ScimUser> result, String accountId) {
    ...
    for (ScimUser scimUser : result.getResources()) {
      ...
      } else if (getIfUserIsAtScope(accountId, userOptional.get().getUuid())) {
        usersNotinNG.add(scimUser);
      }
    }
  ...
  private boolean getIfUserIsAtScope(String accountId, String userUUID) {
    return !ngUserService.isUserAtScope(userUUID,
        ScopeInfo.builder().accountIdentifier(accountId).uniqueId(accountId).scopeType(ScopeLevel.ACCOUNT).build());
  }
  ...
  public void deleteUser(String userId, String accountId) {
    ...
    ngUserService.removeUserFromScope(userId,
        ScopeInfo.builder().accountIdentifier(accountId).uniqueId(accountId).scopeType(ScopeLevel.ACCOUNT).build(),
  ...
  public ScimUser updateUser(String accountId, String userId, PatchRequest patchRequest) {
    ...
    log.info("NGSCIM: Updating user: Patch Request Logging\nOperations {}\n, Schemas {}\n,External Id {}\n, Meta {}, "
            + "for userId: {}, accountId {}",
  ...
  private void applyUserUpdateOperation(String accountId, String userId, UserMetadataDTO userMetadataDTO,
    ...
    if (isObjectValue(patchOperation)) {
      ScimMultiValuedObject multiValuedObject = patchOperation.getValue(ScimMultiValuedObject.class);
      if (multiValuedObject != null && multiValuedObject.getDisplayName() != null) {
        userMetadataDTO.setName(multiValuedObject.getDisplayName());
        userMetadataDTO.setExternallyManaged(true);
        ngUserService.updateUserMetadata(userMetadataDTO);
      }
    }
    ...
    ngUserService.updateNGUserToCGWithSource(userId, Scope.builder().accountIdentifier(accountId).build(), SCIM);
    if (isObjectValue(patchOperation)) {
      ScimUserValuedObject userValuedObject = patchOperation.getValue(ScimUserValuedObject.class);
      if (userValuedObject != null && !userValuedObject.isActive()) {
        log.info("NGSCIM: Removing user {}, from account: {}", userId, accountId);
        deleteUser(userId, accountId);
      }
    } else {
  ...
  private boolean patchOperationIncludesUserDeletion(PatchRequest patchRequest) {
    for (PatchOperation patchOperation : patchRequest.getOperations()) {
      try {
        ScimUserValuedObject userValuedObject =
            isObjectValue(patchOperation) ? patchOperation.getValue(ScimUserValuedObject.class) : null;
        boolean isActiveFalse = (userValuedObject != null && !userValuedObject.isActive())
  ...
  private boolean isObjectValue(PatchOperation patchOperation) {
    try {
      JsonNode valueNode = patchOperation.getValue(JsonNode.class);
      return valueNode != null && valueNode.isObject();
    } catch (Exception e) {
      return false;
    }
  }

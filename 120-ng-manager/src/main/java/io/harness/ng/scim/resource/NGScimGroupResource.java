...
public class NGScimGroupResource extends ScimResource {
  ...
  @Inject private UserGroupPermissionUtils userGroupPermissionUtils;
  ...
  public Response createGroup(ScimGroup groupQuery, @PathParam("accountIdentifier") String accountIdentifier) {
    String toCheckPermission = userGroupPermissionUtils.getUserGroupManagePermissionForSCIM(accountIdentifier);
    if (!accessControlClient.hasAccess(
            ResourceScope.of(accountIdentifier, null, null), Resource.of(USERGROUP, null), toCheckPermission)) {
      throw new AccessDeniedException(
          String.format(STRING_PERMISSION_FAIL_FORMAT, toCheckPermission, accountIdentifier),
  ...
  public Response deleteGroup(
    ...
    String toCheckPermission = userGroupPermissionUtils.getUserGroupManagePermissionForSCIM(accountIdentifier);
    if (!accessControlClient.hasAccess(ResourceScope.of(accountIdentifier, null, null),
            Resource.of(USERGROUP, groupIdentifier), toCheckPermission)) {
      throw new AccessDeniedException(
          String.format(STRING_PERMISSION_FAIL_FORMAT, toCheckPermission, accountIdentifier),
          ErrorCode.NG_ACCESS_DENIED, WingsException.USER);
    }
  ...
  @GET
  @Path("Groups")
  @ApiOperation(value = "Search groups by their name. Supports pagination. If nothing is passed in filter, all results "
          + "will be returned.",

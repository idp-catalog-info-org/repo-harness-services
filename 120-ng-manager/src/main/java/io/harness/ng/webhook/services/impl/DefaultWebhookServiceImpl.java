...
public class DefaultWebhookServiceImpl implements WebhookEventService {
  ...
  public DefaultWebhookServiceImpl(ScmOrchestratorService scmOrchestratorService,
      ...
      @Named("connectorDecoratorService") ConnectorService connectorService, ConnectorScopeHelper connectorScopeHelper,
  ...
  @VisibleForTesting
  String buildLockKey(UpsertWebhookRequestDTO request) {
    // Lock key format: UPSERT_WEBHOOK_LOCK/{accountId}/{connectorRef}/{repoURL}
    // This ensures webhook upserts for the same connector+repo are serialized
    // Using URL encoding to avoid lock key collisions (e.g., "org/my_repo" vs "org_my/repo")
    String repoKey =
        request.getRepoURL() != null ? URLEncoder.encode(request.getRepoURL(), StandardCharsets.UTF_8) : "default";
    return String.format(
        "UPSERT_WEBHOOK_LOCK/%s/%s/%s", request.getAccountIdentifier(), request.getConnectorIdentifierRef(), repoKey);
  }

  public UpsertWebhookResponseDTO upsertWebhook(UpsertWebhookRequestDTO upsertWebhookRequestDTO) {
    String lockKey = buildLockKey(upsertWebhookRequestDTO);
    log.info("Attempting to acquire lock for webhook upsert with key: {}", lockKey);

    try (AcquiredLock<?> lock = persistentLocker.waitToAcquireLockOptional(lockKey, LOCK_TIMEOUT, WAIT_TIMEOUT)) {
      if (lock == null) {
        log.warn(
            "Could not acquire lock for webhook create/upsert within {} seconds for accountId: {}, connectorRef: {}, "
                + "repoURL: {}. Proceeding without lock for availability.",
            WAIT_TIMEOUT.getSeconds(), upsertWebhookRequestDTO.getAccountIdentifier(),
            upsertWebhookRequestDTO.getConnectorIdentifierRef(), upsertWebhookRequestDTO.getRepoURL());
      } else {
        log.info("Successfully acquired lock for webhook create/upsert with key: {}", lockKey);
      }
      return upsertWebhookInternal(upsertWebhookRequestDTO);
    }
  }

  private UpsertWebhookResponseDTO upsertWebhookInternal(UpsertWebhookRequestDTO upsertWebhookRequestDTO) {
    UpsertWebhookResponseDTO upsertWebhookResponseDTO = null;
    ScmConnector scmConnector = getScmConnector(upsertWebhookRequestDTO.getAccountIdentifier(),
        upsertWebhookRequestDTO.getOrgIdentifier(), upsertWebhookRequestDTO.getProjectIdentifier(),
        upsertWebhookRequestDTO.getParentUniqueId(), upsertWebhookRequestDTO.getConnectorIdentifierRef());
    try (AutoLogContext ignore1 = new NgAutoLogContext(upsertWebhookRequestDTO.getProjectIdentifier(),
             ...
             upsertWebhookRequestDTO.getParentUniqueId(), AutoLogContext.OverrideBehavior.OVERRIDE_ERROR)) {
      String target = getTargetUrl(upsertWebhookRequestDTO.getAccountIdentifier());
      CreateWebhookResponse createWebhookResponse =
          scmOrchestratorService.processScmRequestUsingConnectorSettings(scmClientFacilitatorService
              -> scmClientFacilitatorService.upsertWebhook(
                  upsertWebhookRequestDTO, target, GitWebhookTaskType.UPSERT, scmConnector),
              scmConnector);
      ...
      log.error("Upsert Webhook Error for accountId: {}, orgId:{}, projectId:{} : ",
          upsertWebhookRequestDTO.getAccountIdentifier(), upsertWebhookRequestDTO.getOrgIdentifier(),
          upsertWebhookRequestDTO.getProjectIdentifier(), ex);
      throw new ScmBadRequestException("Unable to connect to Git Provider. Please check if credentials provided are "
          + "correct and the repo url is correct.");
  ...
  ScmConnector getScmConnector(String accountIdentifier, String orgIdentifier, String projectIdentifier,
      String parentUniqueId, String connectorRef) {
    Optional<ConnectorResponseDTO> connectorDTO = isNotEmpty(parentUniqueId)
        ? connectorService.get(
              connectorScopeHelper.getConnectorScopeInfo(
                  Scope.of(accountIdentifier, orgIdentifier, projectIdentifier, parentUniqueId), connectorRef),
              IdentifierRefHelper.getIdentifier(connectorRef))
    ...
    if (connectorDTO.isPresent()) {
      ...
      if (connectorConfigDTO instanceof ScmConnector) {
        ...
        throw new UnexpectedException(
            String.format("The connector with the  identifier [%s], accountIdentifier [%s], orgIdentifier [%s], "
                    + "projectIdentifier [%s] is not an scm connector",
                connectorInfoDTO.getIdentifier(), accountIdentifier, orgIdentifier, projectIdentifier));
      }
    }
    throw new ConnectorNotFoundException(String.format("No connector found for accountIdentifier: [%s], orgIdentifier "
                                                 + ": [%s], projectIdentifier : [%s], connectorRef : [%s]",

...
import io.harness.delegate.beans.polling.DaemonResponseMapper;
import io.harness.delegate.beans.polling.PollingDaemonResponse;
...
import io.harness.perpetualtask.DaemonTaskLogContext;
...
public class PollingResource {
  ...
  @POST
  @Path("daemon-response/{daemonTaskId}")
  @Timed
  @ResponseMetered
  @ApiOperation(hidden = true, value = "Communication APIs for polling framework with daemon servers.",
      nickname = "processDaemonPollingResult")
  public void
  processPollingDaemonResultNg(@PathParam("daemonTaskId") @NotEmpty String daemonTaskId,
      @QueryParam("accountId") @NotEmpty String accountId, PollingDaemonResponse response) {
    try (AutoLogContext ignore1 = new AccountLogContext(accountId, OVERRIDE_ERROR);
         AutoLogContext ignore2 = new DaemonTaskLogContext(daemonTaskId, OVERRIDE_ERROR)) {
      PollingDelegateResponse pollingDelegateResponse = DaemonResponseMapper.mapResponse(response);
      pollingResponseHandler.handlePollingResponse(daemonTaskId, accountId, pollingDelegateResponse);
    }
  }
  ...
  public ResponseDTO<PollingResponseDTO> subscribe(
      byte[] pollingItem, @QueryParam("isUnifiedPipelineFlow") @DefaultValue("false") boolean isUnifiedPipelineFlow) {
    PollingResponseDTO pollingResponseDTO =
        pollingService.subscribe((PollingItem) kryoSerializer.asObject(pollingItem), isUnifiedPipelineFlow);
  ...
  @POST
  @Path("unsubscribe-bulk")
  @Timed
  @ResponseMetered
  @ApiOperation(
      hidden = true, value = "Bulk Unsubscribe API for polling framework.", nickname = "unsubscribePollingBulk")
  public Long
  unsubscribeBulk(List<String> pollingDocIds) {
    return pollingService.bulkDeletePollingDocsAndPerpetualTasks(pollingDocIds);
  }

  @POST
  @Path("reset-perpetual-tasks-for-connector")
  @Timed
  @InternalApi
  @ResponseMetered
  @ApiOperation(hidden = true, value = "Reset perpetual tasks for the polling documents of a connector.",
      nickname = "resetPerpetualTasksForConnector")
  public ResponseDTO<Boolean>
  resetPerpetualTasksForConnector(
      @QueryParam("accountId") @NotEmpty String accountId, @QueryParam("connectorRef") @NotEmpty String connectorRef) {
    try (AutoLogContext ignore = new AccountLogContext(accountId, OVERRIDE_ERROR)) {
      pollingService.resetPerpetualTasksForConnector(accountId, connectorRef);
  ...
  @POST
  @Hidden
  @Path("reset-polling-perpetual-tasks")
  @Timed
  @ResponseMetered
  @ApiOperation(value = "Reset perpetual tasks for polling documents in the given account.",
      nickname = "resetPollingPerpetualTasks")
  public ResponseDTO<Integer>
  resetPollingPerpetualTasks(@QueryParam("accountId") @NotNull String accountId,
      @QueryParam("failedOnly") @DefaultValue("false") boolean failedOnly) {
    try (AutoLogContext ignore = new AccountLogContext(accountId, OVERRIDE_ERROR)) {
      return ResponseDTO.newResponse(pollingService.resetPerpetualTasksForAccount(accountId, failedOnly));
    }
  }
}

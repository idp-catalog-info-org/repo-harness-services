...
public class ModuleLicensesChangeEventHandler extends DebeziumAbstractRedisEventHandler {
  ...
  public Record createRecord(String value, String id) {
    ...
    populateCodeFields(node, record);
    populateSeiFields(node, record);
    populateIacFields(node, record);
    populateIdpFields(node, record);
    populateSscaFields(node, record);
    populateCetFields(node, record);
  ...
  private void populateIdpFields(JsonNode node, Record record) {
    if (node.get("numberOfDevelopers") != null) {
      record.set(Tables.MODULE_LICENSES.IDP_NUMBER_OF_DEVELOPERS, node.get("numberOfDevelopers").asLong());
    }
  }

  private void populateIacFields(JsonNode node, Record record) {
    if (node.get("numberOfDevelopers") != null) {
      record.set(Tables.MODULE_LICENSES.IAC_NUMBER_OF_DEVELOPERS, node.get("numberOfDevelopers").asLong());
    }
    if (node.get("numberOfExecutionApplies") != null) {
      record.set(Tables.MODULE_LICENSES.IAC_NUMBER_OF_EXECUTION_APPLIES, node.get("numberOfExecutionApplies").asLong());
    }
  }

  private void populateCetFields(JsonNode node, Record record) {
    if (node.get("numberOfAgents") != null) {
      record.set(Tables.MODULE_LICENSES.CET_NUMBER_OF_AGENTS, node.get("numberOfAgents").asLong());
    }
  }

  private void populateSeiFields(JsonNode node, Record record) {
    if (node.get("numberOfContributors") != null) {
      record.set(Tables.MODULE_LICENSES.SEI_NUMBER_OF_CONTRIBUTORS, node.get("numberOfContributors").asLong());
    }
  }

  private void populateCodeFields(JsonNode node, Record record) {
    if (node.get("numberOfDevelopers") != null) {
      record.set(Tables.MODULE_LICENSES.CODE_NUMBER_OF_DEVELOPERS, node.get("numberOfDevelopers").asLong());
    }
    if (node.get("numberOfRepositories") != null) {
      record.set(Tables.MODULE_LICENSES.CODE_NUMBER_OF_REPOSITORIES, node.get("numberOfRepositories").asLong());
    }
    if (node.get("maxRepoSizeString") != null) {
      record.set(Tables.MODULE_LICENSES.CODE_MAX_REPO_SIZE_STRING, node.get("maxRepoSizeString").asText());
    }
    if (node.get("maxRepoSizeInBytes") != null) {
      record.set(Tables.MODULE_LICENSES.CODE_MAX_REPO_IN_BYTES, node.get("maxRepoSizeInBytes").asLong());
    }
  }

  private void populateSscaFields(JsonNode node, Record record) {
    if (node.get("numberOfDevelopers") != null) {
      record.set(Tables.MODULE_LICENSES.SSCA_NUMBER_OF_DEVELOPERS, node.get("numberOfDevelopers").asLong());
    }
    if (node.get("numberOfExecutions") != null) {
      record.set(Tables.MODULE_LICENSES.SSCA_NUMBER_OF_EXECUTIONS, node.get("numberOfExecutions").asLong());
    }
  }
}

...
public class PipelineExecutionSummaryCDChangeEventHandler extends DebeziumAbstractRedisEventHandler {
  ...
  public Record createRecord(String value, String id) {
    ...
    if ((moduleInfo == null || moduleInfo.get("cd") == null) && (!hasCDModule(node))) {
      return null;
    }
    ...
    if (node.get(PipelineExecutionSummaryKeys.parentUniqueId) != null) {
      record.set(Tables.PIPELINE_EXECUTION_SUMMARY_CD.PARENT_UNIQUE_ID,
          node.get(PipelineExecutionSummaryKeys.parentUniqueId).asText());
    }
    if (node.get(PipelineExecutionSummaryKeys.uniqueId) != null) {
      record.set(
          Tables.PIPELINE_EXECUTION_SUMMARY_CD.UNIQUE_ID, node.get(PipelineExecutionSummaryKeys.uniqueId).asText());
    }
    return record;
  }
  ...
  private boolean hasCDModule(JsonNode rootNode) {
    JsonNode modules = rootNode.get(PipelineExecutionSummaryKeys.modules);
    boolean hasCD = false;
    if (modules != null && modules.getNodeType().equals(JsonNodeType.OBJECT)) {
      for (JsonNode node : modules) {
        if (node.asText().equals("cd")) {
          hasCD = true;
          break;
        }
      }

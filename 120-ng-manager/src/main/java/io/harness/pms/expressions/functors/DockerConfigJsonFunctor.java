...
public class DockerConfigJsonFunctor implements SdkFunctor {
  public static final String DOCKER_CONFIG_JSON = "dockerConfigJsonSecret";
  ...
  public Object get(Ambiance ambiance, String... args) {
    ...
    if (artifactIdentifier.equals(PRIMARY_ARTIFACT)) {
      ...
      return imagePullSecretUtils.getDockerConfigJson(artifactsOutcome.getPrimary(), ambiance);
    } else if (artifactIdentifier.equals(SIDECAR_ARTIFACTS)) {
      Map<String, Object> sidecarsDockerConfigJsons = new HashMap<>();
      artifactsOutcome.getSidecars().forEach(
          (k, v) -> sidecarsDockerConfigJsons.put(k, imagePullSecretUtils.getDockerConfigJson(v, ambiance)));
      return sidecarsDockerConfigJsons;
    } else {
      return null;
    }
  }

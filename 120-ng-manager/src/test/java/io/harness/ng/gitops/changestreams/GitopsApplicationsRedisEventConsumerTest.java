...
/**
 * Unit tests for {@link GitopsApplicationsRedisEventConsumer}.
 */
@OwnedBy(HarnessTeam.GITOPS)
@RunWith(MockitoJUnitRunner.class)
public class GitopsApplicationsRedisEventConsumerTest extends CategoryTest {
  ...
  private GitopsApplicationsRedisEventConsumer consumer;

  @Before
  public void setUp() {
    MockitoAnnotations.openMocks(this);
    consumer = new GitopsApplicationsRedisEventConsumer(
  ...
  public void processMessage_redisShortCircuitEnabled_skipsProcessingAndReturnsTrue() {
    CdcKafkaConsumerConfig consumerConfig =
        CdcKafkaConsumerConfig.builder().name("applications").redisShortCircuit(true).build();
    when(cdcKafkaConfig.getConsumer(CdcKafkaConfig.APPLICATIONS_CONSUMER)).thenReturn(Optional.of(consumerConfig));
  ...
  public void processMessage_redisShortCircuitDisabled_delegatesToParent() {
    CdcKafkaConsumerConfig consumerConfig =
        CdcKafkaConsumerConfig.builder().name("applications").redisShortCircuit(false).build();
    when(cdcKafkaConfig.getConsumer(CdcKafkaConfig.APPLICATIONS_CONSUMER)).thenReturn(Optional.of(consumerConfig));
  ...
  public void processMessage_consumerConfigNotFound_delegatesToParent() {
    when(cdcKafkaConfig.getConsumer(CdcKafkaConfig.APPLICATIONS_CONSUMER)).thenReturn(Optional.empty());

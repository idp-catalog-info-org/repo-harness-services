...
import static io.harness.delegate.task.artifacts.source.ArtifactSourceType.ARTIFACTORY_REGISTRY;
import static io.harness.delegate.task.artifacts.source.ArtifactSourceType.DOCKER_REGISTRY;
import static io.harness.delegate.task.artifacts.source.ArtifactSourceType.NEXUS3_REGISTRY;
...
public class PollingResponseHandlerTest extends CategoryTest {
  ...
  private PollingResponseHandler spyPollingResponseHandler;
  ...
  public void shouldDeletePerpetualTaskOnHighFailedAttempts() {
    ...
    verify(pollingService, times(99))
        .updateTriggerPollingStatus(ACCOUNT_ID, pollingDocument.getSignatures(), false,
            delegateResponse.getErrorMessage(), Collections.emptyList(),
            Duration
                .ofMinutes(PollingConstants.MANIFEST_COLLECTION_NG_INTERVAL_MINUTES
    ...
    verify(pollingService, times(1))
        .updateTriggerPollingStatus(ACCOUNT_ID, pollingDocument.getSignatures(), false,
            delegateResponse.getErrorMessage(), Collections.emptyList(), null);
  ...
  @Owner(developers = OwnerRule.VINICIUS)
  @Category(UnitTests.class)
  public void testDeletionOfKeysInPollingDocumentFFDisabled() {
    when(ngFeatureFlagHelperService.isEnabled(
             ACCOUNT_ID, FeatureName.CDS_DISABLE_POLLED_KEYS_EVICTION_FOR_ARTIFACT_TRIGGER_POLLING_DOCUMENT))
        .thenReturn(false);
    PollingDocument pollingDocument1 =
        PollingDocument.builder()
            ...
            .build();
    PollingDelegateResponse delegateResponse =
        getPollingDelegateResponse(ArtifactPollingDelegateResponse.builder()
                                       .firstCollectionOnDelegate(false)
                                       .toBeDeletedKeys(Set.of("1.0"))
                                       .unpublishedArtifacts(Collections.emptyList())
                                       .build());

    when(pollingService.get(ACCOUNT_ID, POLLING_DOC_ID)).thenReturn(pollingDocument1);
    pollingResponseHandler.handlePollingResponse(PERPETUAL_TASK_ID, ACCOUNT_ID, delegateResponse);
    assertAndGetArtifactPolledResponse(1, 0);
  ...
  @Owner(developers = OwnerRule.VINICIUS)
  @Category(UnitTests.class)
  public void testDeletionOfKeysInPollingDocumentFFEnabled() {
    when(ngFeatureFlagHelperService.isEnabled(
             ACCOUNT_ID, FeatureName.CDS_DISABLE_POLLED_KEYS_EVICTION_FOR_ARTIFACT_TRIGGER_POLLING_DOCUMENT))
        .thenReturn(true);
    PollingDocument pollingDocument1 =
    ...
    when(pollingService.get(ACCOUNT_ID, POLLING_DOC_ID)).thenReturn(pollingDocument1);
    pollingResponseHandler.handlePollingResponse(PERPETUAL_TASK_ID, ACCOUNT_ID, delegateResponse);
    assertAndGetArtifactPolledResponse(1, 1);
    verify(pollingService, times(1)).get(ACCOUNT_ID, POLLING_DOC_ID);
  }

  @Test
  @Owner(developers = OwnerRule.VASU_AGGARWAL)
  @Category(UnitTests.class)
  public void testDeletionOfGitPollingKeysInPollingDocumentFFDisabled() {
    when(ngFeatureFlagHelperService.isEnabled(
             ACCOUNT_ID, FeatureName.CDS_DISABLE_POLLED_KEYS_EVICTION_FOR_GIT_WEBHOOK_POLLING_DOCUMENT))
        .thenReturn(false);
    PollingDocument pollingDocument1 =
        PollingDocument.builder()
            .uuid(POLLING_DOC_ID)
            ...
            .pollingInfo(GitHubPollingInfo.builder().pollInterval(2).webhookId("123").build())
            .polledResponse(
                GitPollingPolledResponse.builder().allPolledKeys(new HashSet<>(Arrays.asList("1.0"))).build())
            .build();
    PollingDelegateResponse delegateResponse =
        getPollingDelegateResponse(GitPollingDelegateResponse.builder()
                                       .firstCollectionOnDelegate(false)
                                       .toBeDeletedIds(Set.of("1.0"))
                                       .unpublishedEvents(Collections.emptyList())
                                       .build());

    when(pollingService.get(ACCOUNT_ID, POLLING_DOC_ID)).thenReturn(pollingDocument1);
    pollingResponseHandler.handlePollingResponse(PERPETUAL_TASK_ID, ACCOUNT_ID, delegateResponse);
    assertAndGetGitPolledResponse(1, 0);
    verify(pollingService, times(1)).get(ACCOUNT_ID, POLLING_DOC_ID);
  }
  ...
  @Owner(developers = OwnerRule.VASU_AGGARWAL)
  @Category(UnitTests.class)
  public void testDeletionOfGitPollingKeysInPollingDocumentFFEnabled() {
    when(ngFeatureFlagHelperService.isEnabled(
             ACCOUNT_ID, FeatureName.CDS_DISABLE_POLLED_KEYS_EVICTION_FOR_GIT_WEBHOOK_POLLING_DOCUMENT))
        .thenReturn(true);
    PollingDocument pollingDocument1 =
    ...
    when(pollingService.get(ACCOUNT_ID, POLLING_DOC_ID)).thenReturn(pollingDocument1);
    pollingResponseHandler.handlePollingResponse(PERPETUAL_TASK_ID, ACCOUNT_ID, delegateResponse);
    assertAndGetGitPolledResponse(1, 1);
    verify(pollingService, times(1)).get(ACCOUNT_ID, POLLING_DOC_ID);
  }
  ...
  private void testSuccessResponse(Type type, PollingType pollingType) {
    ...
    // Manifest (HelmChart) versions are sorted descending; artifact/git polling versions remain in original order
    List<String> expectedNewVersions = pollingType.equals(PollingType.MANIFEST)
        ? List.of("1011", "1010", "1009", "1008", "1007", "1006")
        : List.of("1006", "1007", "1008", "1009", "1010", "1011");
    verify(pollingService, times(1))
        .updateTriggerPollingStatus(ACCOUNT_ID, pollingDocument.getSignatures(), true, null, expectedNewVersions, null);
    if (pollingResponseHandler.shouldTriggerForAllArtifactsOrManifests(pollingDocument)) {
  ...
  @Category(UnitTests.class)
  public void testHandleArtifactResponse() {
    spyPollingResponseHandler =
        spy(new PollingResponseHandler(pollingService, pollingPerpetualTaskService, polledItemPublisher));
    Map<String, List<String>> signatureLock = new HashMap<>();
    signatureLock.put("signature2", List.of());
    PollingDocument pollingDocument1 =
        PollingDocument.builder()
            .uuid(POLLING_DOC_ID)
            .accountId(ACCOUNT_ID)
    ...
    ArtifactBuildDetailsNG artifactBuildDetailsNG1 = ArtifactBuildDetailsNG.builder()
                                                         .number("build1")
                                                         .buildUrl("url1")
                                                         .metadata(metaData)
                                                         .uiDisplayName("uiDisplayName1")
                                                         .labelsMap(metaData)
                                                         .digest("digest1")
                                                         .build();
    ArtifactBuildDetailsNG artifactBuildDetailsNG2 = ArtifactBuildDetailsNG.builder()
                                                         .number("build2")
                                                         .buildUrl("url2")
                                                         .metadata(metaData)
                                                         .uiDisplayName("uiDisplayName2")
                                                         .labelsMap(metaData)
                                                         .digest("digest2")
                                                         .build();
    GoogleCloudStorageArtifactDelegateResponse googleCloudStorageArtifactDelegateResponse1 =
        GoogleCloudStorageArtifactDelegateResponse.builder()
            ...
            .buildDetails(artifactBuildDetailsNG1)
            .project("project1")
            .build();
    GoogleCloudStorageArtifactDelegateResponse googleCloudStorageArtifactDelegateResponse2 =
        GoogleCloudStorageArtifactDelegateResponse.builder()
            .artifactPath("artifactPath1")
            .bucket("bucket2")
            .sourceType(ArtifactSourceType.GOOGLE_CLOUD_STORAGE_ARTIFACT)
            .buildDetails(artifactBuildDetailsNG2)
            .project("project2")
            .build();
    PollingDelegateResponse delegateResponse = getPollingDelegateResponse(
        ArtifactPollingDelegateResponse.builder()
            ...
            .unpublishedArtifacts(
                List.of(googleCloudStorageArtifactDelegateResponse1, googleCloudStorageArtifactDelegateResponse2))
            .build());

    spyPollingResponseHandler.handleArtifactResponse(pollingDocument1, delegateResponse.getPollingResponseInfc());

    ArgumentCaptor<List<String>> newArtifactKeysCaptor = ArgumentCaptor.forClass(List.class);

    verify(spyPollingResponseHandler, times(1))
        .publishPolledItemToTopic(any(), newArtifactKeysCaptor.capture(), any(), any(), any());

    // Assert that newArtifactKeysToBePublished contains unique keys
    List<String> capturedNewArtifactKeys = newArtifactKeysCaptor.getValue();
    assertThat(capturedNewArtifactKeys.size()).isEqualTo(1);
  }
}

...
public class PollingResourceTest extends CategoryTest {
  ...
  public void testSubscribe() {
    when(pollingService.subscribe(any(), anyBoolean()))
        .thenReturn(PollingResponseDTO.builder().pollingDocId("pollingDocId").build());
    ...
    assertThat(pollingResource.subscribe(bytes, false).getData().getPollingResponse()).isEqualTo(bytes);
  }
  ...
  @Category(UnitTests.class)
  public void testResetPollingPerpetualTasksFailedOnly() {
    when(pollingService.resetPerpetualTasksForAccount(accountId, true)).thenReturn(3);
    assertThat(pollingResource.resetPollingPerpetualTasks(accountId, true).getData()).isEqualTo(3);
    verify(pollingService, times(1)).resetPerpetualTasksForAccount(accountId, true);
  }

  @Test
  @Owner(developers = LOVISH_BANSAL)
  @Category(UnitTests.class)
  public void testResetPollingPerpetualTasksAll() {
    when(pollingService.resetPerpetualTasksForAccount(accountId, false)).thenReturn(5);
    assertThat(pollingResource.resetPollingPerpetualTasks(accountId, false).getData()).isEqualTo(5);
    verify(pollingService, times(1)).resetPerpetualTasksForAccount(accountId, false);
  }
}

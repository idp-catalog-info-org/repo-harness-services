...
public class NGScimUserServiceImplTest extends NgManagerTestBase {
  ...
  private static final String ACCOUNT_UID = generateUuid();
  ...
  public void testScim_IfUserIsPartOfAccountAlready_ItShouldUpdate() {
    ...
    Response response = scimUserService.createUser(scimUser, ACCOUNT_UID);
  ...
  public void testScim_doesNotReturnsFamilyNameAndGivenName_whenFFTurnedOff() {
    ...
    when(ngUserService.getUserByEmail(userInfo.getEmail(), true)).thenReturn(Optional.ofNullable(userMetadataDTO));
    when(ngUserService.getUserById(any())).thenReturn(Optional.ofNullable(userInfo));
    when(ngFeatureFlagHelperService.isEnabled(ACCOUNT_UID, PL_NEW_SCIM_STANDARDS)).thenReturn(false);
    Response response = scimUserService.createUser(scimUser, ACCOUNT_UID);
  ...
  public void testScim_doesReturnsFamilyNameAndGivenName_whenFFTurnedOn() throws IOException {
    ...
    when(ngUserService.getUserByEmail(userInfo.getEmail(), true)).thenReturn(Optional.ofNullable(userMetadataDTO));
    when(ngUserService.getUserById(any())).thenReturn(Optional.ofNullable(userInfo));
    when(ngFeatureFlagHelperService.isEnabled(ACCOUNT_UID, PL_NEW_SCIM_STANDARDS)).thenReturn(true);
    Response response = scimUserService.createUser(scimUser, ACCOUNT_UID);
  ...
  public void testScimGetUser_doesReturnGroups_whenFFTurnedOn() {
    ...
    userGroups.add(UserGroup.builder().name("name1").accountIdentifier(ACCOUNT_UID).identifier("randomId1").build());
    userGroups.add(UserGroup.builder().name("name2").accountIdentifier(ACCOUNT_UID).identifier("randomId2").build());
  ...
  public void testScim_IfUserIsNotPartOfAccountAlready_ItShouldAddToAccount() {
    ...
    accounts.add(GatewayAccountRequestDTO.builder().uuid(ACCOUNT_UID).build());
    UserInfo userInfo =
    ...
    verify(ngUserService, times(1))
        .addUserToScope(userMetadataDTO.getUuid(), Scope.of(ACCOUNT_UID, null, null), null, null,
            UserMembershipUpdateSource.SYSTEM,
            ScopeInfo.builder()
                .accountIdentifier(ACCOUNT_UID)
                .uniqueId(ACCOUNT_UID)
  ...
  public void testNGScimSearchForNGUserButInDifferentAccount() {
    ...
    when(ngUserService.searchScimUsersByEmailQuery(anyString(), anyString(), any(), any()))
        .thenReturn(scimUserScimListResponse);
    when(ngUserService.getUserByEmail(anyString(), anyBoolean())).thenReturn(Optional.ofNullable(userMetadataDTO));
    when(ngUserService.isUserAtScope(anyString(), (ScopeInfo) any())).thenReturn(false);
    ScimListResponse<ScimUser> result = scimUserService.searchUser("accountId", "filter", 1, 0);
    assertThat(result.getTotalResults()).isEqualTo(0);
    assertThat(result.getResources().size()).isEqualTo(0);
  }
  ...
  @Category(UnitTests.class)
  public void testPatchUpdateUser_withStringValue_shouldNotFailDeserialization() {
    String userId = randomAlphabetic(10);
    String accountId = randomAlphabetic(10);

    UserInfo userInfo =
        UserInfo.builder().admin(true).email("user@harness.io").name("display_name").uuid(userId).build();
    UserMetadataDTO userMetadataDTO =
        UserMetadataDTO.builder().uuid(userId).name("display_name").email("user@harness.io").build();

    JsonNode stringValueNode = mapper.valueToTree("User, Test");
    ReplaceOperation replaceOperation = new ReplaceOperation("displayName", stringValueNode);
    PatchRequest patchRequest = new PatchRequest(Collections.singletonList(replaceOperation));

    when(ngUserService.getUserById(userId)).thenReturn(Optional.of(userInfo));
    when(ngUserService.getUserMetadata(userId)).thenReturn(Optional.of(userMetadataDTO));
    when(ngUserService.isUserAtScope(any(), (ScopeInfo) any())).thenReturn(true);
    when(ngUserService.getUserByEmail(any(), anyBoolean())).thenReturn(Optional.of(userMetadataDTO));

    ScimUser result = scimUserService.updateUser(accountId, userId, patchRequest);

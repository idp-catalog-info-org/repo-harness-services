...
public class CDLandingDashboardServiceImplTest extends CategoryTest {
  ...
  public void testGetEnvCount() {
    ...
    Table<Record2<String, String>> orgProjTable = DSL.values(orgProjectRows).as("t", "orgId", "projectId");
    List<String> orgIdentifiers = List.of(ORG_ID);
    ...
    when(timeScaleDAL.buildPermittedScopeCondition(
             Scope.ACCOUNT, orgProjTable, orgIdentifiers, ENVIRONMENTS.ORG_IDENTIFIER, ENVIRONMENTS.PROJECT_IDENTIFIER))
        .thenReturn(condition);

    doReturn(5).when(timeScaleDAL).getTotalEnvCount(ACC_ID, condition);
    doReturn(4).when(timeScaleDAL).getNewEnvCount(ACC_ID, START_TS, END_TS, condition);
    doReturn(2).when(timeScaleDAL).getDeletedEnvCount(ACC_ID, START_TS, END_TS, condition);

    EnvCount envCount = cdLandingDashboardService.getEnvCount(
        ACC_ID, orgProjectIdentifiers, orgIdentifiers, Scope.ACCOUNT, START_TS, END_TS);
    assertThat(envCount.getTotalCount()).isEqualTo(5);
    assertThat(envCount.getNewCount()).isEqualTo(2);

    when(timeScaleDAL.buildPermittedScopeCondition(Scope.PROJECT, orgProjTable, new ArrayList<>(),
             ENVIRONMENTS.ORG_IDENTIFIER, ENVIRONMENTS.PROJECT_IDENTIFIER))
        .thenReturn(condition);
    envCount = cdLandingDashboardService.getEnvCount(
        ACC_ID, orgProjectIdentifiers, new ArrayList<>(), Scope.PROJECT, START_TS, END_TS);
    assertThat(envCount.getTotalCount()).isEqualTo(5);
    assertThat(envCount.getNewCount()).isEqualTo(2);
  }
  ...
  public void testGetServicesCount() {
    ...
    when(timeScaleDAL.buildPermittedScopeCondition(
             Scope.ACCOUNT, orgProjTable, orgIdentifiers, SERVICES.ORG_IDENTIFIER, SERVICES.PROJECT_IDENTIFIER))
        .thenReturn(condition);

    doReturn(5).when(timeScaleDAL).getTotalServicesCount(ACC_ID, condition);
    doReturn(4).when(timeScaleDAL).getNewServicesCount(ACC_ID, START_TS, END_TS, condition);
    doReturn(2).when(timeScaleDAL).getDeletedServiceCount(ACC_ID, START_TS, END_TS, condition);

    ServicesCount serviceCount = cdLandingDashboardService.getServicesCount(
        ACC_ID, orgProjectIdentifiers, orgIdentifiers, Scope.ACCOUNT, START_TS, END_TS);
    assertThat(serviceCount.getTotalCount()).isEqualTo(5);
    assertThat(serviceCount.getNewCount()).isEqualTo(2);

    when(timeScaleDAL.buildPermittedScopeCondition(
             Scope.PROJECT, orgProjTable, new ArrayList<>(), SERVICES.ORG_IDENTIFIER, SERVICES.PROJECT_IDENTIFIER))
        .thenReturn(condition);
    serviceCount = cdLandingDashboardService.getServicesCount(
        ACC_ID, orgProjectIdentifiers, new ArrayList<>(), Scope.PROJECT, START_TS, END_TS);
    assertThat(serviceCount.getTotalCount()).isEqualTo(5);
    assertThat(serviceCount.getNewCount()).isEqualTo(2);
  }
  ...
  @Owner(developers = VAIBHAV_SI)
  @Category(UnitTests.class)
  public void testFilterByStatusesCD() {
    assertThat(cdLandingDashboardService.filterByStatusesCD(ACC_ID, null)).isNotNull();
    assertThat(cdLandingDashboardService.filterByStatusesCD(ACC_ID, Collections.emptyList())).isNotNull();

    List<PipelineExecutionSummaryCd> executionSummaryCdList = getExecutionSummaryListCD();
    PipelinesExecutionDashboardInfo pipelinesExecutionDashboardInfo =
        cdLandingDashboardService.filterByStatusesCD(ACC_ID, executionSummaryCdList);
  ...
  public void testFilterByStatuses() {
    ...
    List<PipelineExecutionSummary> executionSummaryList = getExecutionSummaryList();
    PipelinesExecutionDashboardInfo pipelinesExecutionDashboardInfo =
        cdLandingDashboardService.filterByStatuses(ACC_ID, executionSummaryList);
    assertThat(pipelinesExecutionDashboardInfo).isNotNull();
    assertThat(pipelinesExecutionDashboardInfo.getRunningExecutions()).hasSize(1);
    assertThat(pipelinesExecutionDashboardInfo.getPendingApprovalExecutions()).hasSize(1);
    assertThat(pipelinesExecutionDashboardInfo.getPendingManualInterventionExecutions()).hasSize(1);
  ...
  @NotNull
  private List<PipelineExecutionSummaryCd> getExecutionSummaryListCD() {
  ...
  @NotNull
  private List<PipelineExecutionSummary> getExecutionSummaryList() {
    PipelineExecutionSummary pipelineExecutionSummaryCd1 = mock(PipelineExecutionSummary.class);
    doReturn(ExecutionStatus.APPROVAL_WAITING.name()).when(pipelineExecutionSummaryCd1).getStatus();
    PipelineExecutionSummary pipelineExecutionSummaryCd2 = mock(PipelineExecutionSummary.class);
    doReturn(ExecutionStatus.RUNNING.name()).when(pipelineExecutionSummaryCd2).getStatus();
    PipelineExecutionSummary pipelineExecutionSummaryCd3 = mock(PipelineExecutionSummary.class);
    doReturn(ExecutionStatus.INTERVENTION_WAITING.name()).when(pipelineExecutionSummaryCd3).getStatus();
    return Arrays.asList(pipelineExecutionSummaryCd1, pipelineExecutionSummaryCd2, pipelineExecutionSummaryCd3);
  }
  ...
  @Owner(developers = VAIBHAV_SI)
  @Category(UnitTests.class)
  public void testGetActiveDeploymentStatsCD() {
    assertThat(cdLandingDashboardService.getActiveDeploymentStats(ACC_ID, null)).isNotNull();
    List<PipelineExecutionSummaryCd> executionSummaryList = getExecutionSummaryListCD();

    doReturn(executionSummaryList)
        .when(timeScaleDAL)
        .getPipelineExecutionsForGivenExecutionStatusCD(any(), any(), any());
    ...
    doReturn(pipelinesExecutionDashboardInfo)
        ...
        .filterByStatusesCD(ACC_ID, executionSummaryList);
  ...
  public void testGetActiveDeploymentStats() {
    when(featureFlagService.isEnabled(anyString(), eq(FeatureName.PIPE_SHOW_ALL_EXECUTIONS_ON_ACCOUNT_OVERVIEW_PAGE)))
        .thenReturn(true);
    assertThat(cdLandingDashboardService.getActiveDeploymentStats(ACC_ID, null)).isNotNull();
    List<PipelineExecutionSummary> executionSummaryList = getExecutionSummaryList();
    ...
    PipelinesExecutionDashboardInfo executionDashboardInfo =
        cdLandingDashboardService.getActiveDeploymentStats(ACC_ID, getOrgProjectList());
    verify(timeScaleDAL, times(1)).getPipelineExecutionsForGivenExecutionStatus(any(), any(), any());
    verify(timeScaleDAL, times(0)).getPipelineExecutionsForGivenExecutionStatusCD(any(), any(), any());
    assertThat(executionDashboardInfo).isEqualTo(pipelinesExecutionDashboardInfo);
    assertThat(executionDashboardInfo.getFailed24HrsExecutions()).isEqualTo(failedExecutionList);
  }

...
public class DynamicSecretReferenceHelperTest extends CategoryTest {
  ...
  @Owner(developers = NISHANT)
  @Category(UnitTests.class)
  public void testValidateAndGetSecretRefParsedDataForGcpSmTooManySegments() {
    String secretManagerIdentifier = randomAlphabetic(16);
    String relativePath = "projectId/secretName/extraInfo/5";
    String identifier = "gcpsecretsmanager://" + secretManagerIdentifier + "/" + relativePath;
    exceptionRule.expect(SecretManagementException.class);
    exceptionRule.expectMessage(
        String.format("Reference path [%s] can only have an optional project, secret name and version. "
                + "e.g. [secretName/latest] or [projectId/secretName/latest]",
            relativePath));
    dynamicSecretReferenceHelper.validateAndGetSecretRefParsedData(identifier);
  }
  ...
  @Category(UnitTests.class)
  public void testValidateAndGetSecretRefParsedDataForGcpSmCrossProject() {
    String secretManagerIdentifier = randomAlphabetic(16);
    String projectId = randomAlphabetic(12);
    String secretName = randomAlphabetic(10);
    String version = "7";
    String identifier =
        "gcpsecretsmanager://" + secretManagerIdentifier + "/" + projectId + "/" + secretName + "/" + version;
    ...
    SecretRefParsedData expectedSecretRefParsedData =
        SecretRefParsedData.builder()
            ...
            .additionalMetadata(
                AdditionalMetadata.builder().value("version", version).value("projectId", projectId).build())
            .build();
  ...
  @Owner(developers = SAHIBA)
  @Category(UnitTests.class)
  public void testValidateAndGetSecretRefParsedDataForGcpSmCrossProjectLatest() {
    String secretManagerIdentifier = randomAlphabetic(16);
    String projectId = randomAlphabetic(12);
    String secretName = randomAlphabetic(10);
    String identifier =
        "gcpsecretsmanager://" + secretManagerIdentifier + "/" + projectId + "/" + secretName + "/latest";
    SecretRefParsedData secretRefParsedData =
        dynamicSecretReferenceHelper.validateAndGetSecretRefParsedData(identifier);
    SecretRefParsedData expectedSecretRefParsedData =
        SecretRefParsedData.builder()
            ...
            .relativePath(secretName)
            .additionalMetadata(
                AdditionalMetadata.builder().value("version", "latest").value("projectId", projectId).build())
            .build();
    assertThat(secretRefParsedData).usingRecursiveComparison().isEqualTo(expectedSecretRefParsedData);
  }
  ...
  @Owner(developers = SAHIBA)
  @Category(UnitTests.class)
  public void testValidateAndGetSecretRefParsedDataForGcpSmTwoSegmentTreatedAsSecretNameAndVersion() {
    // A 2-segment path is always interpreted as secretName/version, never as projectId/secretName. Even when the first
    // segment looks like a project ID, it is parsed as the secret name and the second segment as the version (no
    // projectId is added to additionalMetadata). This pins the disambiguation for the otherwise-ambiguous case.
    String secretManagerIdentifier = randomAlphabetic(16);
    String firstSegment = "my-project";
    String secondSegment = "mySecret";
    String identifier = "gcpsecretsmanager://" + secretManagerIdentifier + "/" + firstSegment + "/" + secondSegment;
    SecretRefParsedData secretRefParsedData =
        dynamicSecretReferenceHelper.validateAndGetSecretRefParsedData(identifier);
    SecretRefParsedData expectedSecretRefParsedData =
        SecretRefParsedData.builder()
            ...
            .additionalMetadata(AdditionalMetadata.builder().value("version", secondSegment).build())
            .build();
    assertThat(secretRefParsedData).usingRecursiveComparison().isEqualTo(expectedSecretRefParsedData);
  }
  ...
  @Category(UnitTests.class)
  public void testIsSecretIdentifierAPathReference() {
    String secretIdentifier = "hashicorpvault://vaultSecretManagerProd/abhi-harness-cxe#sample";
    assertThat(dynamicSecretReferenceHelper.isSecretIdentifierAPathReference(secretIdentifier)).isTrue();

    secretIdentifier = "azurevault://account.vaultSecretManagerProd/abhi-harness-cxe#sample";
    assertThat(dynamicSecretReferenceHelper.isSecretIdentifierAPathReference(secretIdentifier)).isTrue();

    secretIdentifier = "awssecretsmanager://org.vaultSecretManagerProd/abhi-harness-cxe#sample";
    assertThat(dynamicSecretReferenceHelper.isSecretIdentifierAPathReference(secretIdentifier)).isTrue();

    secretIdentifier = "gcpsecretsmanager://vaultSecretManagerProd/abhi-harness-cxe#sample";
    assertThat(dynamicSecretReferenceHelper.isSecretIdentifierAPathReference(secretIdentifier)).isTrue();

    secretIdentifier = "safeharness://vaultSecretManagerProd/abhi-harness-cxe#sample";
    assertThat(dynamicSecretReferenceHelper.isSecretIdentifierAPathReference(secretIdentifier)).isFalse();

    secretIdentifier = "custom://vaultSecretManagerProd/abhi-harness-cxe#sample";
    assertThat(dynamicSecretReferenceHelper.isSecretIdentifierAPathReference(secretIdentifier)).isFalse();

    secretIdentifier = "org.someSecretIdentifier";
    assertThat(dynamicSecretReferenceHelper.isSecretIdentifierAPathReference(secretIdentifier)).isFalse();
  }
}

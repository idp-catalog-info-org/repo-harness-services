...
public class InstanceNGCRUDStreamListenerTest extends CategoryTest {
  ...
  @InjectMocks InstanceNGCRUDStreamListener instanceNGCRUDStreamListener;
  ...
  public void testProjectDeleteEvent() {
    ...
    String unique = randomAlphabetic(10);
    Message message =
        Message.newBuilder()
            .setMessage(
                io.harness.eventsframework.producer.Message.newBuilder()
                    ...
                    .setData(getProjectPayload(accountIdentifier, orgIdentifier, identifier, unique))
                    .build())
            .build();
    ScopeInfo scopeInfo = ScopeInfo.builder()
                              ...
                              .build();
    when(instanceService.deleteForProject(scopeInfo)).thenReturn(true);
    boolean result = instanceNGCRUDStreamListener.handleMessage(message);
    verify(instanceService).deleteForProject(scopeInfo);
    assertTrue(result);
  }
  ...
  private ByteString getProjectPayload(String accountIdentifier, String orgIdentifier, String identifier) {
    return getProjectPayload(accountIdentifier, orgIdentifier, identifier, randomAlphabetic(10));
  }

  private ByteString getProjectPayload(
      String accountIdentifier, String orgIdentifier, String identifier, String unique) {

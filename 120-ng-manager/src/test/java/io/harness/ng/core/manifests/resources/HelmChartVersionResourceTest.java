...
public class HelmChartVersionResourceTest extends CategoryTest {
  ...
  public void testGetHelmChartVersionDetailsWithGitStoreYaml() throws IOException {
    ...
    doReturn(false).when(artifactResourceUtils).isRemoteService(any(), any(), any(), any());
    doReturn(helmManifestInternalDTO)
        .when(helmChartService)
        .locateManifestInRemoteService(
            eq(ACCOUNT_ID), eq(ORG_ID), eq(PROJECT_ID), eq(SERVICE_REF), eq(null), eq(FQN_PATH));
    helmChartVersionResource.getHelmChartVersionDetailsWithYaml(ACCOUNT_ID, ORG_ID, PROJECT_ID, PIPELINE_ID,
        SERVICE_REF, FQN_PATH, CONNECTOR_REF, CHART_NAME, REGION, BUCKET, FOLDER_PATH, null, null, null, null, null,
        GIT_ENTITY_FIND_INFO_DTO, "");
    verify(helmChartVersionResourceUtils, times(2))
        .resolveExpressionWithYamlExpressionEvaluator(eq(ACCOUNT_ID), eq(ORG_ID), eq(PROJECT_ID), eq(PIPELINE_ID),
            any(), any(), any(), eq(FQN_PATH), eq(GIT_ENTITY_FIND_INFO_DTO), eq(SERVICE_REF), eq(null));
  }
  ...
  public void testGetHelmChartVersionDetailsWithEcrStoreConfigYaml() throws IOException {
    ...
    doReturn(helmManifestInternalDTO)
        ...
        .locateManifestInRemoteService(
            eq(ACCOUNT_ID), eq(ORG_ID), eq(PROJECT_ID), eq(SERVICE_REF), eq(null), eq(FQN_PATH));
    helmChartVersionResource.getHelmChartVersionDetailsWithYaml(ACCOUNT_ID, ORG_ID, PROJECT_ID, PIPELINE_ID,
        SERVICE_REF, FQN_PATH, CONNECTOR_REF, CHART_NAME, REGION, BUCKET, FOLDER_PATH, null, REGISTRY_ID, null, null,
        null, GIT_ENTITY_FIND_INFO_DTO, "");
    verify(helmChartVersionResourceUtils, times(5))
        .resolveExpressionWithYamlExpressionEvaluator(eq(ACCOUNT_ID), eq(ORG_ID), eq(PROJECT_ID), eq(PIPELINE_ID),
            any(), any(), any(), eq(FQN_PATH), eq(GIT_ENTITY_FIND_INFO_DTO), eq(SERVICE_REF), eq(null));
  }

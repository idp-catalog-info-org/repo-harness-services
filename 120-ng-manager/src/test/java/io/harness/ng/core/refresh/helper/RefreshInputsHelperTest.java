...
public class RefreshInputsHelperTest extends NgManagerTestBase {
  ...
  public void setup() throws IOException {
    ...
    on(refreshInputsHelper).set("chainedPipelineScopeHelper", new ChainedPipelineScopeHelper());
  ...
  public void testRefreshInputsForPipelineYamlWithValidService() {
    ...
    doReturn(null).when(environmentService).createEnvironmentInputsYaml((ScopeInfo) null, "testenv", null);
    doReturn("infrastructureDefinitions:\n"
        + "  - identifier: \"infra2\"\n")
        .when(infrastructureEntityService)
        .createInfrastructureInputsFromYaml(ACCOUNT_ID, ORG_ID, PROJECT_ID, null, "testenv", null,
  ...
  @Category(UnitTests.class)
  public void testRefreshInputsForCrossOrgChainedPipelineUsesChildScope() {
    when(environmentRefreshHelper.isEnvironmentField(anyString(), any(JsonNode.class))).thenReturn(false);
    ...
    String refreshedYaml = refreshInputsHelper.refreshInputs(ACCOUNT_ID, ORG_ID, PROJECT_ID, pipelineYaml, null);
    assertThat(refreshedYaml).isNotNull().contains("serviceRef: iisDealtool").contains("org: WIN");
  ...
  @Owner(developers = THRISHANK)
  @Category(UnitTests.class)
  public void testRefreshInputsForSameOrgChainedPipeline() {
    when(environmentRefreshHelper.isEnvironmentField(anyString(), any(JsonNode.class))).thenReturn(false);
    ...
    String refreshedYaml = refreshInputsHelper.refreshInputs(ACCOUNT_ID, ORG_ID, PROJECT_ID, pipelineYaml, null);
    assertThat(refreshedYaml).isNotNull().contains("serviceRef: serverless");
    verify(serviceEntityService)
        .get(argThat(scope -> matchesScope(scope, ORG_ID, PROJECT_ID)), eq("serverless"), eq(false));
  }
  ...
  @Owner(developers = THRISHANK)
  @Category(UnitTests.class)
  public void testRefreshInputsForNestedChainedPipelineUsesGrandchildScope() {
    when(environmentRefreshHelper.isEnvironmentField(anyString(), any(JsonNode.class))).thenReturn(false);
    ...
    String refreshedYaml = refreshInputsHelper.refreshInputs(ACCOUNT_ID, ORG_ID, PROJECT_ID, pipelineYaml, null);
    assertThat(refreshedYaml).isNotNull().contains("serviceRef: grandchildSvc").contains("org: grandchildOrg");
    verify(serviceEntityService)
        .get(argThat(scope -> matchesScope(scope, "grandchildOrg", "grandchildProject")), eq("grandchildSvc"),
            eq(false));
  }
  ...
  @Owner(developers = THRISHANK)
  @Category(UnitTests.class)
  public void testRefreshInputsPreservesChainedPipelineWhenChildScopeIsUnresolvable() {
    when(environmentRefreshHelper.isEnvironmentField(anyString(), any(JsonNode.class))).thenReturn(false);
    String pipelineYaml = readFile("chained-pipeline-unresolvable-scope.yaml");

    String refreshedYaml = refreshInputsHelper.refreshInputs(ACCOUNT_ID, ORG_ID, PROJECT_ID, pipelineYaml, null);
    assertThat(refreshedYaml).isNotNull().contains("org: <+input>").contains("serviceRef: iisDealtool");

...
public class CIEnableIntelligentDefaultsSettingMigrationTest extends NgManagerTestBase {
  @Mock private NGFeatureFlagHelperService featureFlagService;
  @Mock private SettingsService settingsService;
  @Mock private AccountUtils accountUtils;
  @InjectMocks private CIEnableIntelligentDefaultsSettingMigration migration;
  ...
  public void testMigration() {
    List<String> accountIds = ImmutableList.of(accountId1, accountId2, accountId3);

    when(accountUtils.getAllNGAccountIds()).thenReturn(accountIds);
    when(featureFlagService.isDisabled(accountId1, FeatureName.CI_ENABLE_INTELLIGENT_DEFAULTS)).thenReturn(true);
    when(featureFlagService.isDisabled(accountId2, FeatureName.CI_ENABLE_INTELLIGENT_DEFAULTS)).thenReturn(true);
    when(featureFlagService.isDisabled(accountId3, FeatureName.CI_ENABLE_INTELLIGENT_DEFAULTS)).thenReturn(true);
    ...
    capturedDTOLists.forEach(dtoList -> {
      ...
      assertThat(dto.getIdentifier()).isEqualTo("ci_build_intel_automatic_enable");

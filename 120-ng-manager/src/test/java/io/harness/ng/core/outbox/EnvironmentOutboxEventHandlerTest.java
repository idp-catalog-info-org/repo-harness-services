...
public class EnvironmentOutboxEventHandlerTest extends CategoryTest {
  ...
  public void testCreate() throws JsonProcessingException {
    ...
    Environment environment = Environment.builder()
                                  ...
                                  .parentUniqueId(accountIdentifier + "/" + orgIdentifier)
                                  .uniqueId(accountIdentifier + "/" + orgIdentifier + "/" + projectIdentifier)
                                  .build();
    ...
    String newYaml = getYamlString(EnvironmentMapper.toNGEnvironmentConfig(environment, null));
  ...
  public void testDelete() throws JsonProcessingException {
    ...
    String oldYaml = getYamlString(EnvironmentMapper.toNGEnvironmentConfig(environment, null));
    final ArgumentCaptor<AuditEntry> auditEntryArgumentCaptor = ArgumentCaptor.forClass(AuditEntry.class);
    when(auditClientService.publishAudit(any(), any(), any())).thenReturn(true);
    environmentOutboxEventHandler.handle(outboxEvent);
    verify(auditClientService, times(1)).publishAudit(auditEntryArgumentCaptor.capture(), any(), any());
  ...
  public void testUpdate() throws JsonProcessingException {
    ...
    String newYaml = getYamlString(EnvironmentMapper.toNGEnvironmentConfig(newEnvironment, null));
    String oldYaml = getYamlString(EnvironmentMapper.toNGEnvironmentConfig(oldEnvironment, null));
    final ArgumentCaptor<AuditEntry> auditEntryArgumentCaptor = ArgumentCaptor.forClass(AuditEntry.class);
    when(auditClientService.publishAudit(any(), any(), any())).thenReturn(true);
    environmentOutboxEventHandler.handle(outboxEvent);
    verify(auditClientService, times(1)).publishAudit(auditEntryArgumentCaptor.capture(), any(), any());
  ...
  public void testUpdateEnvCreateServiceOverrideV2() throws IOException {
    ...
    EnvironmentUpdatedEvent environmentUpdatedEvent =
        EnvironmentUpdatedEvent.builder()
            ...
            .newOverrideAuditEventDTO(ServiceOverrideEventDTOMapper.toOverrideAuditEventDTO(
                newServiceOverridesEntity, orgIdentifier, projectIdentifier))
            .build();
    ...
    EnvironmentUpdatedEvent environmentUpdatedEvent =
        EnvironmentUpdatedEvent.builder()
            ...
            .oldOverrideAuditEventDTO(ServiceOverrideEventDTOMapper.toOverrideAuditEventDTO(
                oldServiceOverridesEntity, orgIdentifier, projectIdentifier))
            .build();
  ...
  @Category(UnitTests.class)
  public void testEnvironmentMovedToRemote() throws JsonProcessingException {
    String accountIdentifier = randomAlphabetic(10);
    String orgIdentifier = randomAlphabetic(10);
    String projectIdentifier = randomAlphabetic(10);
    String identifier = randomAlphabetic(10);
    ...
    String oldYaml = "storeType: INLINE";
    String newYaml = "storeType: REMOTE\n"
        + "repo: harness-private\n"
        + "connectorRef: account.ghprivate\n"
        + "filePath: .harness/env_git.yaml";

    Scope scope = Scope.of(accountIdentifier, orgIdentifier, projectIdentifier);
    EnvironmentMoveConfigEvent environmentMoveConfigEvent =
        new EnvironmentMoveConfigEvent(scope, identifier, name, oldYaml, newYaml);

    GlobalContext globalContext = new GlobalContext();
    String eventData = objectMapper.writeValueAsString(environmentMoveConfigEvent);

    OutboxEvent outboxEvent = OutboxEvent.builder()
                                  .id(randomAlphabetic(10))
                                  .blocked(false)
                                  .eventType(OutboxEventConstants.ENVIRONMENT_MOVED_TO_REMOTE)
                                  .eventData(eventData)
                                  .resource(environmentMoveConfigEvent.getResource())
                                  .resourceScope(environmentMoveConfigEvent.getResourceScope())
                                  .globalContext(globalContext)
                                  .createdAt(Long.valueOf(randomNumeric(6)))
                                  .build();
    ...
    assertAuditEntry(accountIdentifier, orgIdentifier, projectIdentifier, identifier, auditEntry, outboxEvent);
    assertEquals(Action.MOVE_TO_GIT, auditEntry.getAction());
  ...
  @Category(UnitTests.class)
  public void testClusterLinkCreated() throws IOException {
    String accountIdentifier = randomAlphabetic(10);
    String orgIdentifier = randomAlphabetic(10);
    String projectIdentifier = randomAlphabetic(10);
    String envRef = randomAlphabetic(10);
    String clusterRef = "account.in-cluster";
    String agentIdentifier = "my-agent";
    ...
    EnvironmentUpdatedEvent environmentUpdatedEvent =
        EnvironmentUpdatedEvent.builder()
            ...
            .projectIdentifier(projectIdentifier)
            .status(CREATED)
            .resourceType(EnvironmentUpdatedEvent.ResourceType.GITOPS_CLUSTER_LINK)
    ...
    AuditEntry auditEntry = auditEntryArgumentCaptor.getValue();
    assertAuditEntry(accountIdentifier, orgIdentifier, projectIdentifier, envRef, auditEntry, outboxEvent);
    assertEquals(Action.UPDATE, auditEntry.getAction());
    assertNotNull(auditEntry.getNewYaml());
    assertThat(auditEntry.getNewYaml()).contains("clusterLinked");
    assertThat(auditEntry.getNewYaml()).contains(clusterRef);
    assertThat(auditEntry.getNewYaml()).contains(agentIdentifier);
    assertThat(auditEntry.getNewYaml()).contains(envRef);
    assertNull(auditEntry.getOldYaml());
  }
  ...
  @Owner(developers = HIMANSHU)
  @Category(UnitTests.class)
  public void testClusterLinkDeleted() throws IOException {
    String accountIdentifier = randomAlphabetic(10);
    String orgIdentifier = randomAlphabetic(10);
    String projectIdentifier = randomAlphabetic(10);
    String envRef = randomAlphabetic(10);
    String clusterRef = "org.staging-cluster";
    String agentIdentifier = "agent-2";
    ...
    assertThat(auditEntry.getOldYaml()).contains("clusterLinked");
    assertThat(auditEntry.getOldYaml()).contains(clusterRef);
    assertThat(auditEntry.getOldYaml()).contains(agentIdentifier);
    assertThat(auditEntry.getOldYaml()).contains(envRef);
    assertNull(auditEntry.getNewYaml());
  }
  ...
  @Owner(developers = HIMANSHU)
  @Category(UnitTests.class)
  public void testClusterLinkEvent_resourceStructure() {
    String accountIdentifier = randomAlphabetic(10);
    String orgIdentifier = randomAlphabetic(10);
    String projectIdentifier = randomAlphabetic(10);
    String envRef = "production";
    String clusterRef = "account.in-cluster";
    String agentIdentifier = "my-agent";
    String parentUniqueId = randomAlphabetic(10);

    EnvironmentUpdatedEvent event = EnvironmentUpdatedEvent.builder()
                                        .accountIdentifier(accountIdentifier)
                                        .orgIdentifier(orgIdentifier)
                                        .projectIdentifier(projectIdentifier)
                                        .status(CREATED)
    ...
    // resource.identifier = envRef
    assertEquals(envRef, event.getResource().getIdentifier());

    // resource.type = ENVIRONMENT
    assertEquals("ENVIRONMENT", event.getResource().getType());

    // resource.uniqueId = agentIdentifier/clusterRef
    assertEquals(agentIdentifier + "/" + clusterRef, event.getResource().getUniqueId());

    // labels contain exactly 5 entries
    assertThat(event.getResource().getLabels()).hasSize(5);
    assertEquals("CREATED", event.getResource().getLabels().get("status"));
    assertEquals("GITOPS_CLUSTER_LINK", event.getResource().getLabels().get("resourceType"));
    assertEquals(clusterRef, event.getResource().getLabels().get("resourceName"));
    assertEquals(clusterRef, event.getResource().getLabels().get("clusterRef"));
    assertEquals(agentIdentifier, event.getResource().getLabels().get("agentIdentifier"));

    // resourceScope uses correct org/project
    ProjectScope projectScope = (ProjectScope) event.getResourceScope();
    assertEquals(accountIdentifier, projectScope.getAccountIdentifier());
    assertEquals(orgIdentifier, projectScope.getOrgIdentifier());
    assertEquals(projectIdentifier, projectScope.getProjectIdentifier());
  }
}

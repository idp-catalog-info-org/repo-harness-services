...
public class ConnectorEntityCRUDStreamListenerTest extends CategoryTest {
  ...
  public void setup() throws IOException {
    ...
    connectorEntityCRUDStreamListener =
        spy(new ConnectorEntityCRUDStreamListener(harnessSMManager, connectorEntityCRUDEventHandler, settingsClient));
  ...
  public void testAccountDeleteEvent() {
    ...
    when(connectorEntityCRUDEventHandler.deleteAssociatedConnectors(any())).thenReturn(true);
    boolean result = connectorEntityCRUDStreamListener.handleMessage(message);
    verify(connectorEntityCRUDEventHandler, times(1)).deleteAssociatedConnectors(idCaptor.capture());
  ...
  public void testAccountChangeEvent_withoutAnyActionSetInMetadata() {
    ...
    Message message = Message.newBuilder()
                          ...
                          .build();
    boolean result = connectorEntityCRUDStreamListener.handleMessage(message);
    verify(connectorEntityCRUDEventHandler, times(0)).deleteAssociatedConnectors(any());
    assertTrue(result);
  }

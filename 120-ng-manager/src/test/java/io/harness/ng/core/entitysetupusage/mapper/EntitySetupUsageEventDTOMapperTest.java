...
public class EntitySetupUsageEventDTOMapperTest extends NgManagerTestBase {
  ...
  public void setUp() throws Exception {
    ...
    GitInfoPopulatorForConnector gitInfoPopulatorForConnector =
        new GitInfoPopulatorForConnector(connectorService, scopeResolverService);
  ...
  public void toEntityDTOWithGitBranchPopulated() {
    ...
    when(connectorService.list(any(), any(), any(), any(), any(), eq(pageable1)))
        .thenReturn(mockedConnectorResponseDTO);
    when(connectorService.list(any(), any(), any(), any(), any(), eq(pageable2))).thenReturn(Page.empty());

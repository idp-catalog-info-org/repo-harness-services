...
if [[ "" != "$GO_CONVERT_SERVICE_GRPC_URL" ]]; then
  export GO_CONVERT_SERVICE_GRPC_URL; yq -i '.goConvertConnectionConfig.url=env(GO_CONVERT_SERVICE_GRPC_URL)' $CONFIG_FILE
fi
...
replace_key_value iteratorsConfig.approvalInstance.threadPoolCount "$APPROVAL_INSTANCE_ITERATOR_THREAD_POOL_SIZE"
...
replace_key_value grpcClientConfigs.pms.target "$INTERNAL_PIPELINE_SERVICE_TARGET"
replace_key_value grpcClientConfigs.pms.authority "$INTERNAL_PIPELINE_SERVICE_AUTHORITY"

#Workload Identity (HarnessID) config - enabled implicitly when target, secret, and REST base URL are set
replace_key_value harnessIdClientConfig.grpcClientConfig.target "$HARNESS_ID_GRPC_TARGET"
replace_key_value harnessIdClientConfig.grpcClientConfig.serviceAuthSecret "$HARNESS_ID_SERVICE_SECRET"
replace_key_value harnessIdClientConfig.restClientConfig.baseUrl "$HARNESS_ID_REST_BASE_URL"

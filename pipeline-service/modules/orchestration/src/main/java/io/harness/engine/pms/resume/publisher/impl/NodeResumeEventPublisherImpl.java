...
package io.harness.engine.pms.resume.publisher.impl;
...
@Singleton
@Slf4j
public class NodeResumeEventPublisherImpl implements NodeResumeEventPublisher {
  ...
  public void publishEvent(ResumeMetadata resumeMetadata, Map<String, ResponseDataProto> responseMap, boolean isError) {
    ...
    var nodeExecutionId = AmbianceUtils.obtainCurrentRuntimeId(resumeMetadata.getAmbiance());
    // markNodesProcessing already retries once on transient Mongo failures. If it is still exhausted, fail the node
    // explicitly instead of publishing the resume event, so the node errors out fast rather than hanging in
    // async-wait until the pipeline timeout. See PIPE-35791.
    try {
      nodeExecutionService.markNodesProcessing(Collections.singletonList(nodeExecutionId), true);
    } catch (Exception ex) {
      log.error("Failed to mark node {} as processing before publishing resume event. Failing the node.",
          nodeExecutionId, ex);
      orchestrationEngine.handleError(resumeMetadata.getAmbiance(), ex);
      return;
    }
    eventSender.sendEvent(resumeEventBuilder.getAmbiance(), resumeEventBuilder.build(), PmsEventCategory.NODE_RESUME,
        resumeMetadata.getModule(), true, false);
  }

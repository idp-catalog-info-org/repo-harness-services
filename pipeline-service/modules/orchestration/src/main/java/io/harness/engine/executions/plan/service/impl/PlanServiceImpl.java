...
import io.harness.repositories.nodeentity.NodeEntityRepository;
import io.harness.repositories.plan.PlanRepository;
...
public class PlanServiceImpl implements PlanService {
  ...
  public Plan save(Plan plan) {
    return transactionHelper.performTransaction(() -> {
      ...
      log.info("[PMS_EXECUTE] Time taken to save nodes is {}ms", System.currentTimeMillis() - start);
  ...
  public Node fetchNode(String planId, String nodeId) {
    ...
    return PlanNode.fromPlanNodeProto(plan.fetchNode(nodeId), plan.getAccountIdentifier());
  }
  ...
  @Override
  public Plan appendNodes(String planId, List<Node> nodes) {
    // Verify plan exists
    Plan plan = fetchPlan(planId);
    if (plan == null) {
      throw new InvalidRequestException("Plan not found for id: " + planId);
    }
    // Convert nodes to NodeEntity objects and save them
    long start = System.currentTimeMillis();
    List<NodeEntity> nodeEntities =
        nodes.stream().map(node -> NodeEntity.fromNode(node, planId)).collect(Collectors.toList());
    nodeEntityRepository.saveAll(nodeEntities);
    log.info("[PMS_EXECUTE] Time taken to append nodes is {}ms", System.currentTimeMillis() - start);

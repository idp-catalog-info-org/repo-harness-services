...
package io.harness.engine.pms.advise.utils;
...
public class NodeAdviserUtils {
  public AdviseEvent createAdviseEvent(
      NodeExecution nodeExecution, FailureInfo failureInfo, Node planNode, Status fromStatus, Ambiance ambiance) {
    // Prioritize nodeExecution's adviserObtainments, append planNode's if nodeExecution's are empty
    List<AdviserObtainment> adviserObtainments = new ArrayList<>();
    if (!EmptyPredicate.isEmpty(nodeExecution.getAdviserObtainments())) {
      adviserObtainments.addAll(nodeExecution.getAdviserObtainments());
    }

    adviserObtainments.addAll(planNode.getAdviserObtainments());

    AdviseEvent.Builder builder =
        AdviseEvent.newBuilder()
            .setAmbiance(ambiance)
            .addAllAdviserObtainments(adviserObtainments)
    ...
    if (failureInfo != null) {
      builder.setFailureInfo(failureInfo);
    }

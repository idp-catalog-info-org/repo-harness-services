...
public class PlanExpansionServiceImpl implements PlanExpansionService {
  ...
  @Override
  public void updateExpansionForRetriedNode(Ambiance ambiance, String originalExecutionId) {
    try {
      // Saving for the stage which is being copied. So all its children entities would be covered.
      if (!StepCategory.STAGE.name().equals(AmbianceUtils.obtainCurrentLevel(ambiance).getGroup())) {
        return;
      }
      String expansionPathUsingLevels = getExpansionPathUsingLevels(ambiance);
      PlanExecutionExpansion planExecutionExpansion = planExecutionExpansionRepository.findWithProjections(
          originalExecutionId, Collections.singleton(expansionPathUsingLevels));
      if (planExecutionExpansion == null || planExecutionExpansion.getExpandedJson() == null) {
        return;
      }

      Object finalVal = JsonUtils.jsonPath(JsonUtils.asJson(planExecutionExpansion), expansionPathUsingLevels);
      Update update = new Update();
      update.set(expansionPathUsingLevels, Document.parse(JsonUtils.asJson(finalVal)));
      planExecutionExpansionRepository.update(
          ambiance.getPlanExecutionId(), update, moduleConfig.getExpandedJsonLockConfig().getLockTimeOutInMinutes());
    } catch (Exception ex) {
      log.error("Exception in copying PlanExecutionExpansion for for retried node", ex);
    }
  }

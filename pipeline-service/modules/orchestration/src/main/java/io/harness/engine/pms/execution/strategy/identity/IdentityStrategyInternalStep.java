...
public class IdentityStrategyInternalStep
  ...
  public ChildExecutableResponse obtainChild(
    ...
    // Handle empty INSERT case - no children were spawned in original execution
    // Check this first to avoid unnecessary DB call to fetch children
    if (StepCategory.INSERT.equals(originalNodeExecution.getStepType().getStepCategory())
        && EmptyPredicate.isEmpty(originalNodeExecution.getExecutableResponses())) {
  ...
  public ChildrenExecutableResponse obtainChildren(
    ...
    try (Stream<NodeExecution> stream =
             // Use original planExecutionId that belongs to the originalNodeExecutionId and not current
             // planExecutionId(ambiance.getPlanExecutionId)
        nodeExecutionService.fetchChildrenNodeExecutionsIterator(originalStrategyNodeExecution.getPlanExecutionId(),
            identityParams.getOriginalNodeExecutionId(), NodeProjectionUtils.fieldsForIdentityStrategyStep)) {
      Iterator<NodeExecution> iterator = stream.iterator();
      while (iterator.hasNext()) {
        NodeExecution next = iterator.next();
  ...
  private List<ChildrenExecutableResponse.Child> getChildrenFromNodeExecutions(
     ...
     */
      ...
      ChildrenExecutableResponse.Child.Builder child =
          ChildrenExecutableResponse.Child.newBuilder().setChildNodeId(node.getUuid());
      if (AmbianceUtils.hasStrategyMetadata(nodeExecution.getCurrentLevel())) {
        child.setStrategyMetadata(nodeExecutionInfoService.getStrategyMetadata(nodeExecution.getCurrentLevel()));
      }

...
public class AbortAllInterruptHandler extends InterruptPropagatorHandler implements InterruptHandler {
  ...
  public Interrupt registerInterrupt(Interrupt interrupt) {
    ...
    if (status == Status.QUEUED || status == Status.QUEUED_EXECUTION_CONCURRENCY_REACHED
        || status == Status.QUEUED_PLAN_CREATION || status == Status.STARTING_PLAN_CREATION) {
      planExecutionService.updateStatus(interrupt.getPlanExecutionId(), ABORTED);
  ...
  public List<String> getTargetIds(List<NodeExecution> finalList) {
    //   Ticket: https://harness.atlassian.net/browse/PIPE-22920
    //   In case of ABORT_ALL interrupt type ( stage abort ), we can mark abort all child nodes at once instead of only
    //   leaf nodes, as failure strategy do not get honoured in abort interrupt.

    return finalList.stream().map(NodeExecution::getUuid).collect(Collectors.toList());
  }
  ...
  protected Interrupt processDiscontinuedInstances(
    ...
    try (AutoLogContext ignore = updatedInterrupt.autoLogContext()) {
      ...
      log.info("Exception occurred while aborting instance. Interrupt Monitor handling it in few minutes.");
      throw ex;
    }

...
public class ExpandedJsonFunctorUtils {
  ...
  public List<String> getExpressions(Ambiance ambiance, Map<String, String> groupAliases, String expression) {
    ...
    if (expression.length() > 500) {
      log.warn("Expression is too long {}. Check if valid or not", expression.length());
    }
    ...
    if (expressionKeys.size() >= 20 && expressionKeys.size() < 30) {
      log.warn("Expression has more 20 parts {}. Check if valid or not", expressionKeys.size());
    } else if (expressionKeys.size() >= 30) {
      log.warn("Expression has more 30 parts {}. Check if valid or not", expressionKeys.size());
      if (AmbianceUtils.checkIfFeatureFlagEnabled(ambiance, FeatureName.PIPE_OPTIMISE_EXPANDED_JSON_FUNCTOR.name())) {
        throw new EngineExpressionEvaluationException("Expression has too many keys in fullyQualifiedName", expression);
      }
    }
    ...
    if (groupAliases.containsKey(group)) {
      finalExpression = getFullyQualifiedExpressionBasedOnGroup(levels, groupAliases.get(group), expressionKeys, true);
    } else {
  ...
  public String createFullQualifiedName(Ambiance ambiance, String expression) {
    ...
    if (GROUP_ALIASES.containsKey(group)) {
      finalExpression =
          getFullyQualifiedExpressionBasedOnGroup(levels, GROUP_ALIASES.get(group), expressionKeys, false);
    } else {
      List<String> fullyQualifiedName = new ArrayList<>();
      for (Level level : levels) {
        if (level.getIdentifier().equals(group)) {
  ...
  private String getFullyQualifiedExpressionBasedOnGroup(
    ...
    postFix.removeIf(EmptyPredicate::isEmpty);

...
public class ExpressionUsagesEventHandler implements OrchestrationEventHandler {
  ...
  @Inject ScopeResolutionHelper scopeInfoService;
  @Override
  public void handleEvent(OrchestrationEvent event) {
    ...
    if (parentUniqueId == null) {
      parentUniqueId = scopeInfoService
                           .getScopeInfo(AmbianceUtils.getAccountId(ambiance), AmbianceUtils.getOrgIdentifier(ambiance),
                               AmbianceUtils.getProjectIdentifier(ambiance))
                           .getUniqueId();
    }

    if (expressionUsageService.doesExpressionUsagesEntityExists(
            AmbianceUtils.getPipelineIdentifier(ambiance), AmbianceUtils.getAccountId(ambiance), parentUniqueId)) {
      return;
    }
    try (AutoLogContext ignore = AmbianceUtils.autoLogContext(ambiance)) {
      PlanExecutionMetadata planExecutionMetadata =
          planExecutionMetadataService.findByPlanExecutionIdWithFieldsIncluded(AmbianceUtils.getAccountId(ambiance),
              ...
              Set.of(PlanExecutionMetadataKeys.yaml, PlanExecutionMetadataKeys.harnessVersion));
      if (planExecutionMetadata == null || isEmpty(planExecutionMetadata.getYaml())) {
        log.error("Could not find yaml for planExecutionId {}. Not storing the expression-usages for this execution.",
            ambiance.getPlanExecutionId());
        return;
      }

      Map<ExpressionCategory, Set<ExpressionMetadata>> expressionUsages = new HashMap<>();
      // Use the pipeline's actual Harness YAML version (defaults to V0 when empty) so that V1 pipelines - whose
      // parallel nodes are objects wrapping stages/steps rather than arrays - are parsed correctly.
      YamlConfig yamlConfig =
          new YamlConfig(planExecutionMetadata.getYaml(), planExecutionMetadata.getHarnessVersion());
      ...
      expressionUsageService.upsertExpressions(AmbianceUtils.getPipelineIdentifier(ambiance),
          ...
          AmbianceUtils.getProjectIdentifier(ambiance), expressionUsages, parentUniqueId, true);
    }
  }

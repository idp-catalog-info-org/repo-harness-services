...
public class WaitForExecutionInputCallback implements OldNotifyCallback {
  ...
  public void notifyTimeout(Map<String, ResponseData> responseMap) {
    ...
    FailureInfo failureInfo = FailureInfo.newBuilder()
                                  ...
                                  .addFailureData(FailureData.newBuilder()
                                                      ...
                                                      .setMessage("Pipeline has passed the time limit to take the user "
                                                          + "input. Please check the timeout configuration")
                                                      .build())
                                  .build();
    ...
    if (CollectionUtils.isEmpty(node.getAdviserObtainments())) {
      engine.endNodeExecution(nodeExecutionService.getAmbiance(nodeExecution));
      return;
    }
  ...
  public void notify(Map<String, ResponseData> response) {
    ...
    executorService.submit(() -> engine.queueOrStartExecution(ambiance));
  }

  ...
  public void handle(Interrupt interrupt) {
    try (AutoLogContext ignore = interrupt.autoLogContext()) {
      ...
      if (planExecution == null) {
        ...
        interruptService.markProcessedForceful(interrupt.getUuid(), PROCESSED_SUCCESSFULLY);
        return;
      }

      List<NodeExecution> nodeExecutions = new LinkedList<>();
      try (Stream<NodeExecution> stream = nodeExecutionService.fetchNodeExecutionsWithoutOldRetriesIterator(
      ...
      if (isEmpty(leaves) && !StatusUtils.isFinalStatus(planExecution.getStatus())) {
        ...
        interruptService.markProcessedForceful(interrupt.getUuid(), PROCESSED_UNSUCCESSFULLY);
        return;
      }
      ...
      if (isNotEmpty(runningNodes)) {
        ...
        if (runningNodes.stream().allMatch(
                ne -> ne.getStatus() == Status.DISCONTINUING || StatusUtils.isQueuedStatus(ne.getStatus()))) {
      ...
      log.error(
          String.format(
              "Unexpected exception while processing interrupt: %s with planExecutionId: %s. Keeping it for next iteration.",
              interrupt.getUuid(), interrupt.getPlanExecutionId()),
          e);
    }
  }

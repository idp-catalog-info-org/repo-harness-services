...
package io.harness.repositories.stepexecution;
...
public class StepExecutionEntityRepositoryCustomImpl implements StepExecutionEntityRepositoryCustom {
  ...
  public UpdateResult update(
      Scope scope, String stepExecutionId, Map<String, Object> updates, boolean parentUniqueIdQueryingEnabled) {
    Criteria criteria = createScopeCriteria(scope, parentUniqueIdQueryingEnabled)
                            .and(StepExecutionEntityKeys.stepExecutionId)
                            .is(stepExecutionId);
  ...
  public UpdateResult updateStatus(
      Scope scope, String stepExecutionId, Status status, boolean parentUniqueIdQueryingEnabled) {
    Criteria criteria = createScopeCriteria(scope, parentUniqueIdQueryingEnabled)
                            .and(StepExecutionEntityKeys.stepExecutionId)
                            .is(stepExecutionId);
  ...
  private Criteria createScopeCriteria(Scope scope, boolean isParentUniqueIdQueryingEnabled) {
    Criteria criteria = new Criteria();
    criteria.and(StepExecutionEntityKeys.accountIdentifier).is(scope.getAccountIdentifier());
    if (isParentUniqueIdQueryingEnabled) {
      criteria.and(StepExecutionEntityKeys.parentUniqueId).is(scope.getParentUniqueId());
    } else {
  ...
  @Override
  public StepExecutionEntity findByStepExecutionId(
      String stepExecutionId, Scope scope, boolean isParentUniqueIdQueryingEnabled) {
    Criteria criteria = new Criteria();
    criteria.and(StepExecutionEntityKeys.accountIdentifier).is(scope.getAccountIdentifier());
    if (isParentUniqueIdQueryingEnabled) {
      criteria.and(StepExecutionEntityKeys.parentUniqueId).is(scope.getParentUniqueId());

...
public interface Resolver {
  ...
  default String consume(@NotNull Ambiance ambiance, @NotNull String name, String value, String groupName) {
    ...
    Ambiance groupAmbiance = processGroupAmbiance(ambiance, groupName);
    return consumeInternal(groupAmbiance, producedBy, name, value, groupName);
  }

  static Ambiance processGroupAmbiance(@NotNull Ambiance ambiance, String groupName) {
    if (EmptyPredicate.isEmpty(groupName)) {
      return ambiance;
    }
    if (groupName.equals(ResolverUtils.GLOBAL_GROUP_SCOPE)) {
      return AmbianceUtils.clone(ambiance, 0);
    }

...
public class OrchestrationUtils {
  ...
  public Status calculateStatusForPlanExecution(List<Status> statuses, String planExecutionId) {
    ...
    if (StatusUtils.isQueuedStatus(calculatedStatus)) {
  ...
  public static boolean isStageNode(NodeExecution nodeExecution) {
    ...
    StepType currentStepType = nodeExecution.getCurrentLevel().getStepType();
    return currentStepType.getStepCategory() == StepCategory.STAGE;
  }

  public static boolean isStepNode(NodeExecution nodeExecution) {
    if (nodeExecution == null) {
      return false;
    }
    return nodeExecution.getCurrentLevel().getStepType().getStepCategory() == StepCategory.STEP
        && AmbianceUtils.STEP.equals(nodeExecution.getGroup());
  }
  ...
  public static boolean isStageOrParallelStageNode(NodeExecution nodeExecution) {
    ...
    StepType parentStepType = NodeExecutionContextUtils.getParentStepType(nodeExecution);
  ...
  public boolean checkAsyncPlanCreation(boolean manualExecutionFF, boolean triggerExecutionFF,
      ExecutionTriggerInfo triggerInfo, boolean isPlanCreationAsync) {
    if (triggerInfo == null || !isPlanCreationAsync) {
      return false;
    }
    if (manualExecutionFF && triggerInfo.getTriggerType() == TriggerType.MANUAL) {
      return true;
    }
    if (triggerExecutionFF && triggerInfo.getTriggerType() != TriggerType.MANUAL) {
      return true;
    }
    return false;
  }

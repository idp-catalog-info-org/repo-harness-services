...
public class NodeExecutionReadHelper {
  ...
  public Optional<NodeExecution> getOneFromSecondary(Query query) {
    validateNodeExecutionProjection(query);
    return Optional.ofNullable(secondaryMongoTemplate.findOne(query, NodeExecution.class));
  }
  ...
  public Stream<NodeExecution> fetchNodeExecutionsWithoutValidation(Query query) {
    query.cursorBatchSize(DEFAULT_BATCH_SIZE);
    return mongoTemplate.stream(query, NodeExecution.class);
  }

  public Stream<NodeExecution> fetchNodeExecutionsWithoutValidationFromSecondary(Query query) {
    query.cursorBatchSize(DEFAULT_BATCH_SIZE);
    return secondaryMongoTemplate.stream(query, NodeExecution.class);
  }
   ...
   * Get approval before using this method
   */
  public Stream<NodeExecution> fetchNodeExecutionsIteratorWithoutProjectionsFromSecondary(Query query) {
    query.cursorBatchSize(MAX_BATCH_SIZE);
    return secondaryMongoTemplate.stream(query, NodeExecution.class);
  }
  ...
  public List<NodeExecution> findNodeExecutionsFromSecondary(Query query) {
    validateNodeExecutionProjection(query);
  ...
  public ExecutionStatistics aggregateRunningExecutionCount() {
    ...
    List<ExecutionStatistics> executionStatisticsList =
        secondaryMongoTemplate.aggregate(aggregation, NodeExecution.class, ExecutionStatistics.class)
            .getMappedResults();
    return executionStatisticsList.size() > 0 ? executionStatisticsList.get(0) : null;
  }

  /**
   * Cluster-wide per-account count of leaf-step nodeExecutions currently occupying a step-concurrency
   * slot (per {@code statuses}). Backing query for the step-concurrency counter rebuild/backfill jobs —
   * not on the hot path.
   */
  public List<ExecutionCountWithAccountResult> aggregateLeafStepCountByAccount(Set<Status> statuses) {
    MatchOperation matchStage = Aggregation.match(Criteria.where(NodeExecutionKeys.mode)
    ...
    ProjectionOperation projectAccount = Aggregation.project()
                                             ...
                                             .as(ExecutionCountWithAccountResultKeys.accountId)
                                             .andInclude(ExecutionCountWithAccountResultKeys.count);
    Aggregation aggregation = Aggregation.newAggregation(matchStage, groupByAccount, projectAccount);
    return secondaryMongoTemplate.aggregate(aggregation, NodeExecution.class, ExecutionCountWithAccountResult.class)
        .getMappedResults();
  }
}

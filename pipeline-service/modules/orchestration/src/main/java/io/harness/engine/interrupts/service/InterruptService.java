...
package io.harness.engine.interrupts.service;
...
public interface InterruptService {
  ...
  Interrupt markProcessedForceful(String interruptId, State interruptState);

  Interrupt markProcessing(String interruptId);

  List<Interrupt> fetchActivePlanLevelInterrupts(List<String> planExecutionId, Ambiance ambiance);

  ExecutionCheck checkInterruptsPreInvocation(List<String> planExecutionIds, String currentNodeExecutionId,
  ...
  List<Interrupt> fetchPlanLevelInterrupt(String planExecutionId, EnumSet<InterruptType> types);
}

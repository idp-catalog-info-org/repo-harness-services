...
@OwnedBy(PIPELINE)
@Slf4j
public class ExpiryHelperImpl implements ExpiryHelper {
  ...
  public void expireDiscontinuedInstance(
    ...
    try {
      progressDataUnitProgress = InterruptHelper.evaluateUnitProgressesFromProgressData(nodeExecution, EXPIRED);
    } catch (Exception ex) {
      log.warn("Error marking progress data status to expired", ex);
    }
        ...
        StepResponseProto.newBuilder()
            ...
            .setFailureInfo(FailureInfo.newBuilder()
                                ...
                                .addFailureData(FailureData.newBuilder()
                                                    ...
                                                    .build())
                                .build())
            .addAllUnitProgress(progressDataUnitProgress.isEmpty() ? unitProgressList : progressDataUnitProgress)
            .build();
    engine.processStepResponse(nodeExecutionService.getAmbiance(nodeExecution), expiredStepResponse);
  }
    ...
    NodeExecution updatedNodeExecution =
        nodeExecutionService.updateStatusWithOps(nodeExecution.getUuid(), Status.EXPIRED, ops -> {
          ...
          ops.set(NodeExecutionKeys.progressData + "." + NodeExecutionKeys.unitProgresses,
              finalProgressDataUnitProgress.isEmpty() ? unitProgresses : finalProgressDataUnitProgress);

...
public class StepExecutionEntityServiceImpl implements StepExecutionEntityService {
  ...
  public StepExecutionEntity createStepExecutionEntity(Ambiance ambiance, Status status) {
    ...
    StepExecutionEntityBuilder stepExecutionEntityBuilder =
        StepExecutionEntity.builder()
            ...
            .status(status);
    StepExecutionEntity stepExecutionEntity = stepExecutionEntityBuilder.build();
    validateUniqueIdAndParentUniqueId(stepExecutionEntity, ambiance);
    return stepExecutionEntityRepository.save(stepExecutionEntity);
  }

  @Override
  public StepExecutionEntity findStepExecutionEntity(String accountIdentifier, String orgIdentifier,
      String projectIdentifier, String stepExecutionId, String parentUniqueId) {
    Scope scope = Scope.of(accountIdentifier, orgIdentifier, projectIdentifier, parentUniqueId);
    return stepExecutionEntityRepository.findByStepExecutionId(stepExecutionId, scope, true);
  }
  ...
  public boolean checkIfStepExecutionEntityExists(Ambiance ambiance) {
    ...
    StepExecutionEntity stepExecutionEntity = findStepExecutionEntity(
        accountIdentifier, orgIdentifier, projectIdentifier, stepExecutionId, parentUniqueIdentifier);
  ...
  public StepExecutionEntity updateStepExecutionEntity(Ambiance ambiance, FailureInfo failureInfo,
    ...
    try {
      ...
      log.error(String.format(
                    "[CustomDashboard]: Unable to update step execution entity, accountIdentifier: %s, orgIdentifier: "
                        + "%s, projectIdentifier: %s, planExecutionId: %s, stageExecutionId: %s, stepExecutionId: %s",
  ...
  public StepExecutionEntity updateStepExecutionEntity(
    ...
    StepExecutionEntity stepExecutionEntity =
        findStepExecutionEntity(accountIdentifier, orgIdentifier, projectIdentifier, stepExecutionId, parentUniqueId);
    Scope scope = Scope.of(accountIdentifier, orgIdentifier, projectIdentifier, parentUniqueId);
    if (stepExecutionEntity == null) {
      log.error(format("StepExecutionEntity should not be null for accountIdentifier %s, orgIdentifier %s, "
              + "projectIdentifier %s and stepExecutionId %s",
      ...
      validateUniqueIdAndParentUniqueId(stepExecution, ambiance);
      stepExecutionEntityRepository.save(stepExecution);
    } else {
  ...
  public void update(Scope scope, String stepExecutionId, Map<String, Object> updates) {
    UpdateResult updateResult = stepExecutionEntityRepository.update(scope, stepExecutionId, updates, true);
    if (!updateResult.wasAcknowledged()) {
      throw new InvalidRequestException(format("Unable to update StepExecutionEntity, accountIdentifier: %s, "
              + "orgIdentifier: %s, projectIdentifier: %s, executionId: %s",
  ...
  public void updateStatus(@NotNull Scope scope, @NotNull final String stepExecutionId, Status status) {
    ...
    UpdateResult updateResult = stepExecutionEntityRepository.updateStatus(scope, stepExecutionId, status, true);
    if (!updateResult.wasAcknowledged()) {
      throw new InvalidRequestException(
          format("Unable to update step execution status, accountIdentifier: %s, orgIdentifier: %s, projectIdentifier: "
                  + "%s, executionId: %s, status: %s",
              scope.getAccountIdentifier(), scope.getOrgIdentifier(), scope.getProjectIdentifier(), stepExecutionId,
              status));
    }
  }

  private void validateUniqueIdAndParentUniqueId(StepExecutionEntity stepExecutionEntity, Ambiance ambiance) {
    if (isEmpty(stepExecutionEntity.getUniqueId())) {
      stepExecutionEntity.setUniqueId(generateUuid());
    }
    if (isEmpty(stepExecutionEntity.getParentUniqueId())) {
      String parentUniqueId = AmbianceUtils.getParentUniqueIdentifier(ambiance);
      if (isEmpty(parentUniqueId)) {
        Optional<ScopeInfo> scopeInfo =
            scopeResolutionHelper.getScopeInfoOptional(stepExecutionEntity.getAccountIdentifier(),
                stepExecutionEntity.getOrgIdentifier(), stepExecutionEntity.getProjectIdentifier());
        if (scopeInfo.isPresent()) {
          parentUniqueId = scopeInfo.get().getUniqueId();
        }
      }
      stepExecutionEntity.setParentUniqueId(parentUniqueId);
    }
  }
}

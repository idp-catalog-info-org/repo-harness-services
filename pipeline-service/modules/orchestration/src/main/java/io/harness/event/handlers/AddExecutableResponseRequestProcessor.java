...
public class AddExecutableResponseRequestProcessor implements SdkResponseProcessor {
  ...
  public void handleEvent(SdkResponseEventProto event) {
    ...
    if (updateNodeExecutionStatus(request)) {
      ...
      NodeExecution nodeExecution =
          nodeExecutionService.updateStatusWithOps(SdkResponseEventUtils.getNodeExecutionId(event), getStatus(request),
              getUpdateOps(request), EnumSet.noneOf(Status.class));
  ...
  private boolean updateNodeExecutionStatus(AddExecutableResponseRequest request) {
    if (request.getExecutableResponse().hasAsync()
      ...
      return true;
    } else if (request.getExecutableResponse().hasFacilitator()) {
      return true;
    }
    return false;
  }

  private Status getStatus(AddExecutableResponseRequest request) {
    if (request.getExecutableResponse().hasFacilitator()) {
      return request.getExecutableResponse().getFacilitator().getStatus();
    }
    return request.getExecutableResponse().getAsync().getStatus();
  }

  private Consumer<Update> getUpdateOps(AddExecutableResponseRequest request) {
    if (request.getExecutableResponse().hasFacilitator()) {
      return ops -> {
        ops.addToSet(NodeExecutionKeys.executableResponses, request.getExecutableResponse());
        setUnset(ops, NodeExecutionKeys.startTs, request.getExecutableResponse().getFacilitator().getStartTs());
      };
    }
    return ops -> ops.addToSet(NodeExecutionKeys.executableResponses, request.getExecutableResponse());
  }
}

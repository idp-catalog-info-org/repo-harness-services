...
import io.harness.expression.SafeJexlEngine;
import io.harness.expression.SafeJexlFactory;
...
public class NodeExecutionValueTest extends OrchestrationTestBase {
  ...
  private SafeJexlEngine engine;
  private HarnessJexlEngine jexlEngine;
  ...
  public void setup() {
    ...
    engine = SafeJexlFactory.createEngine();
    jexlEngine = new HarnessJexlEngine(engine);
    ...
    when(nodeExecutionService.fetchChildrenNodeExecutionsIterator(
        ...
        .thenAnswer((Answer<Stream<NodeExecution>>) invocation
            -> OrchestrationTestHelper.createCloseableIterator(nodeExecutionsList1.iterator()).stream());
    ...
    when(nodeExecutionService.fetchChildrenNodeExecutionsIterator(
        ...
        .thenAnswer((Answer<Stream<NodeExecution>>) invocation
            -> OrchestrationTestHelper.createCloseableIterator(nodeExecutionList2.iterator()).stream());
    ...
    when(nodeExecutionService.fetchChildrenNodeExecutionsIterator(
        ...
        .thenAnswer((Answer<Stream<NodeExecution>>) invocation
            -> OrchestrationTestHelper.createCloseableIterator(nodeExecutionList3.iterator()).stream());
    ...
    when(nodeExecutionService.fetchChildrenNodeExecutionsIterator(
        ...
        .thenAnswer((Answer<Stream<NodeExecution>>) invocation
            -> OrchestrationTestHelper.createCloseableIterator(nodeExecutionList4.iterator()).stream());
    ...
    when(nodeExecutionService.fetchChildrenNodeExecutionsIterator(
        ...
        .thenAnswer((Answer<Stream<NodeExecution>>) invocation
            -> OrchestrationTestHelper.createCloseableIterator(nodeExecutionList5.iterator()).stream());

    List<NodeExecution> nodeExecutionList6 = Collections.EMPTY_LIST;
    when(nodeExecutionService.fetchChildrenNodeExecutionsIterator(
             planExecutionId, nodeExecution2.getUuid(), NodeProjectionUtils.fieldsForExpressionEngine))
        .thenAnswer((Answer<Stream<NodeExecution>>) invocation
            -> OrchestrationTestHelper.createCloseableIterator(nodeExecutionList6.iterator()).stream());
    when(nodeExecutionService.fetchChildrenNodeExecutionsIterator(
  ...
  public void testNodeExecutionAncestorFunctorMatrix() {
    Ambiance newAmbiance = nodeExecution6.getAmbiance();
    doReturn(true).when(pmsOutcomeService).existsOutcomeName(eq(planExecutionId), anyString());
    doReturn("{\"os\": \"test\", \"iteration\": 2}")
        .when(pmsOutcomeService)
        .resolveUsingLevelRuntimeIdx(anyString(), anyList(), any());
    OutcomeMetadata outcomeMetadata = new OutcomeMetadata(pmsOutcomeService, planExecutionId);
    NodeExecutionAncestorFunctor functor =
        NodeExecutionAncestorFunctor.builder()
    ...
    assertThat(engine.getProperty(functor, "stage.matrix.iteration")).isEqualTo(2);
  }
  ...
  public void testNodeExecutionCurrentStatusFailed() {
    ...
    Reflect.on(nodeExecution7).set(NodeExecutionKeys.advisorsProcessed, true);
    Reflect.on(nodeExecution8).set(NodeExecutionKeys.status, Status.QUEUED);

    when(nodeExecutionService.findAllChildrenWithStatusInAndWithoutOldRetriesV2(
             planExecutionId, nodeExecution4.getUuid(), StatusUtils.finalStatuses(), false))
        .thenReturn(Collections.singletonList(nodeExecution7));

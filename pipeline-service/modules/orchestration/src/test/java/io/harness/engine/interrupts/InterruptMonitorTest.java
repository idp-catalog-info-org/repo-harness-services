...
public class InterruptMonitorTest extends OrchestrationTestBase {
  ...
  public void shouldTestMarkingProcessedUnsuccessfullyAfterException() {
    var planExecutionId = generateUuid();
    var planExecution = PlanExecution.builder().uuid(planExecutionId).status(Status.RUNNING).build();
    var interrupt = Interrupt.builder()
                        .uuid(generateUuid())
    ...
    doReturn(planExecution)
        ...
        .getWithFieldsIncluded(planExecutionId, Sets.newHashSet(PlanExecutionKeys.status));
    doThrow(new RuntimeException("unexpected exception"))
        .when(nodeExecutionService)
        .fetchNodeExecutionsWithoutOldRetriesIterator(planExecutionId, NodeProjectionUtils.fieldsForDiscontinuingNodes);

    interruptMonitor.handle(interrupt);

    verify(interruptService, times(0)).markProcessedForceful(any(), any());
  }
  /**
   * Setup for the test
   * pipeline (running)
  ...
  public void shouldTestLeafDiscontinuingException() {
    ...
    interruptMonitor.handle(interrupt);
    verify(interruptService, times(1)).markProcessedForceful(eq(interrupt.getUuid()), eq(PROCESSED_UNSUCCESSFULLY));

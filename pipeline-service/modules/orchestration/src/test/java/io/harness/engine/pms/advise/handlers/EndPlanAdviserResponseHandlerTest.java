...
public class EndPlanAdviserResponseHandlerTest extends OrchestrationTestBase {
  ...
  @Category(UnitTests.class)
  public void shouldTestEndExecutionForAlreadyAbortedNodeExecution() {
    advise = EndPlanAdvise.newBuilder().setIsAbort(true).build();

    ArgumentCaptor<InterruptPackage> interruptPackageArgumentCaptor = ArgumentCaptor.forClass(InterruptPackage.class);
    when(interruptManager.register(interruptPackageArgumentCaptor.capture())).thenReturn(null);

    NodeExecution abortedNodeExecution = getAbortedNodeExecution();
    nodeExecutionService.save(abortedNodeExecution);

    endPlanAdviserResponseHandler.handleAdvise(abortedNodeExecution,
        AdviserResponse.newBuilder().setEndPlanAdvise(advise).setType(AdviseType.END_PLAN).build());
    verify(interruptManager, times(0)).register(any());
    verify(engine, times(1)).endNodeExecution(any());
  }

  private NodeExecution getAbortedNodeExecution() {
    return NodeExecution.builder()
        ...
        .name("DUMMY_2")
        .identifier("dummy2")
        .stepType(StepType.newBuilder().setType("DUMMY_2").setStepCategory(StepCategory.STEP).build())
        .module("CD")
        .startTs(System.currentTimeMillis())

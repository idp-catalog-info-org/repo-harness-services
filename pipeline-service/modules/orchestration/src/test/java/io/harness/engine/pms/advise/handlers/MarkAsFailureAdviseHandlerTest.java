...
public class MarkAsFailureAdviseHandlerTest extends CategoryTest {
  ...
  public void handleAdviseWhenNextNodeIsIsEmpty() {
    ...
    when(planExecutionService.getExecutionMetadataFromPlanExecution(any()))
        .thenReturn(ExecutionMetadata.newBuilder().setExecutionMode(NORMAL).build());
    markAsFailureAdviseHandler.handleAdvise(
        NodeExecution.builder().ambiance(ambiance).status(Status.FAILED).build(), adviserResponse);
  ...
  public void handleAdviseWithoutFailedNode() {
    ...
    markAsFailureAdviseHandler.handleAdvise(
        NodeExecution.builder().status(Status.EXPIRED).ambiance(ambiance).build(), adviserResponse);
    verify(engine).endNodeExecution(any());
  }

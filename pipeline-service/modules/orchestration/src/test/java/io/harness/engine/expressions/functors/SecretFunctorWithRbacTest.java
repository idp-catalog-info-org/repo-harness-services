...
public class SecretFunctorWithRbacTest extends CategoryTest {
  @Mock DynamicSecretReferenceHelper dynamicSecretReferenceHelper;
  ...
  public void testGetValue() {
    ...
    SecretFunctorWithRbac secretFunctorWithRbac = new SecretFunctorWithRbac(
        ambiance, pipelineRbacHelper, secretsRuntimeUsagesSubject, dynamicSecretReferenceHelper);
  ...
  @Category(UnitTests.class)
  public void testGetValue_vaultPathWithDots_shouldNotThrowInvalidIdentifierRefException() {
    // Regression test for CI-23109: vault paths containing dots (e.g. version strings "7.2.0-02")
    // caused InvalidIdentifierRefException at the RBAC check gate for ALL infra types.
    Subject<SecretResolutionObserver> secretsRuntimeUsagesSubject = new Subject<>();
    SecretFunctorWithRbac secretFunctorWithRbac = new SecretFunctorWithRbac(
        ambiance, pipelineRbacHelper, secretsRuntimeUsagesSubject, dynamicSecretReferenceHelper);
    MockedStatic<IdentifierRefProtoDTOHelper> mockedStatic = Mockito.mockStatic(IdentifierRefProtoDTOHelper.class);
    ...
    Object result = secretFunctorWithRbac.getValue(vaultPath);

    assertEquals(
        "${ngSecretManager.obtain(\"" + vaultPath + "\", " + ambiance.getExpressionFunctorToken() + ")}", result);
  }
}

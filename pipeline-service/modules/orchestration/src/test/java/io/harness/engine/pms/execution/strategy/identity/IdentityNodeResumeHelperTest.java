...
public class IdentityNodeResumeHelperTest extends OrchestrationTestBase {
  ...
  public void testResume() {
    ResumeMetadata resumeMetadata = ResumeMetadata.builder().ambiance(Ambiance.newBuilder().build()).build();
    ...
    NodeExecution nodeExecution = NodeExecution.builder().ambiance(Ambiance.newBuilder().build()).build();
    doReturn(Ambiance.newBuilder().build()).when(nodeExecutionService).getAmbiance(nodeExecution);
    identityNodeResumeHelper.resume(nodeExecution, new HashMap<>(), false, "");

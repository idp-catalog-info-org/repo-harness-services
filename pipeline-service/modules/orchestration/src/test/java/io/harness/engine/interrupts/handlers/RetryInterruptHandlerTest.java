...
public class RetryInterruptHandlerTest extends CategoryTest {
  ...
  public void testRegisterInterrupt() {
    ...
    doReturn(NodeExecution.builder()
                 ...
                 .stepType(StepType.newBuilder().setType("step").build())
    ...
    doReturn(NodeExecution.builder()
                 .mode(ExecutionMode.CHILDREN)
                 .status(FAILED)
                 .stepType(StepType.newBuilder().setType("SERVICE_V3").build())
                 .ambiance(
                     Ambiance.newBuilder()
                         .addLevels(Level.newBuilder().setStepType(StepType.newBuilder().setType("SERVICE_V3")).build())
                         .build())
                 .group("STEP_GROUP")
                 .build())
        .when(nodeExecutionService)
        .getWithFieldsIncluded(nodeExecutionId, NodeProjectionUtils.fieldsForRetryInterruptHandler);
    Interrupt returnedInterruptForStepGroup = retryInterruptHandler.registerInterrupt(interruptBuilder.build());

    ArgumentCaptor<Interrupt> interruptArgumentCaptor = ArgumentCaptor.forClass(Interrupt.class);
    verify(interruptService, times(2)).save(interruptArgumentCaptor.capture());
    Interrupt savedInterrupt = interruptArgumentCaptor.getAllValues().get(0);
    Interrupt savedInterruptForStepGroup = interruptArgumentCaptor.getAllValues().get(1);
    assertEquals(savedInterrupt.getState(), Interrupt.State.PROCESSING);
    assertEquals(savedInterruptForStepGroup.getState(), Interrupt.State.PROCESSING);

    verify(retryHelper, times(2))
        .retryNodeExecution(nodeExecutionId, interruptUuid, interruptBuilder.build().getInterruptConfig());
    verify(planExecutionService, times(2)).updateStatus(planExecutionId, RUNNING);
    ...
    assertEquals(returnedInterruptForStepGroup.getInterruptConfig(), interruptBuilder.build().getInterruptConfig());
    assertEquals(returnedInterruptForStepGroup.getState(), Interrupt.State.PROCESSING);
  }

...
public class ApprovalStepStatusUpdateTest extends CategoryTest {
  ...
  public void testHandleNodeStatusUpdate() {
    ...
    verify(nodeExecutionService, times(1))
        .updateStatusWithOps(
            stageNodeExecutionId, APPROVAL_WAITING, null, StatusUtils.planAllowedStartSet(APPROVAL_WAITING));
    verify(planExecutionService, times(1)).updateStatus(planExecutionId, APPROVAL_WAITING);
    verify(pipelineStageStatusHelper, times(1)).updatePipelineAndStageWaitingStatus(nodeUpdateInfo, APPROVAL_WAITING);
  }
}

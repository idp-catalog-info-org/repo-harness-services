...
public class ExpiryHelperTest extends OrchestrationTestBase {
  ...
  @Category(UnitTests.class)
  public void shouldTestEvaluateUnitProgressFromProgressData() {
    List<UnitProgress> unitProgresses = List.of(UnitProgress.newBuilder()
                                                    .setUnitName("Fetch Files")
                                                    .setStatus(UnitStatus.SUCCESS)
                                                    .setEndTime(System.currentTimeMillis())
                                                    .build(),
    ...
    Map<String, Object> unitProgressSerialised = RecastOrchestrationUtils.toMap(unitProgressData);
        ...
        NodeExecution.builder()
            ...
            .executableResponse(ExecutableResponse.newBuilder()
                                    .setTask(TaskExecutableResponse.newBuilder()
                                                 ...
                                                 .build())
                                    .build())
            .progressData(unitProgressSerialised)
            .startTs(123L)
            .build();

    List<UnitProgress> unitProgressList =
        InterruptHelper.evaluateUnitProgressesFromProgressData(nodeExecution, EXPIRED);
  ...
  @Category(UnitTests.class)
  public void shouldExpireNodeExecutionInstanceUseProgressDataInsteadUnitProgress() {
    List<UnitProgress> unitProgresses = List.of(UnitProgress.newBuilder()
                                                    .setUnitName("Fetch Files - progressData")
                                                    .setStatus(UnitStatus.SUCCESS)
                                                    .setEndTime(System.currentTimeMillis())
                                                    .build(),
        UnitProgress.newBuilder().setUnitName("Initialize - progressData").setStatus(UnitStatus.SUCCESS).build(),
        UnitProgress.newBuilder().setUnitName("Prepare - progressData").setStatus(UnitStatus.RUNNING).build());
    UnitProgressData unitProgressData = UnitProgressData.builder().unitProgresses(unitProgresses).build();
    Map<String, Object> unitProgressSerialised = RecastOrchestrationUtils.toMap(unitProgressData);
    ...
    ArgumentCaptor<StepResponseProto> stepResponseArgumentCaptor = ArgumentCaptor.forClass(StepResponseProto.class);
    verify(engine, times(1)).processStepResponse(any(), stepResponseArgumentCaptor.capture());

    StepResponseProto stepResponseProtoCaptor = stepResponseArgumentCaptor.getValue();
    assertThat(stepResponseProtoCaptor.getUnitProgressList())
        .hasSize(3)
        .extracting(UnitProgress::getUnitName)
        .containsExactlyInAnyOrder("Fetch Files - progressData", "Initialize - progressData", "Prepare - progressData");
  }
}

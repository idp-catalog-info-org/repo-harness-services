...
public class EngineProgressCallbackTest extends OrchestrationTestBase {
  ...
  public void shouldTestNotifyWithUnitProgressData() {
    ...
    doNothing().when(progressEventPublisher).publishEvent(nodeExecutionId, binaryResponseData);
  ...
  @Category(UnitTests.class)
  public void shouldTestNotifyWithTimestampedUnitProgressDataUsesTimestampGuard() {
    String correlationId = generateUuid();
    ...
    verify(progressEventPublisher).publishEvent(nodeExecutionId, binaryResponseData);
    verify(nodeExecutionService).updateUnitProgressesIfNewer(nodeExecutionId, unitProgresses, 123L);

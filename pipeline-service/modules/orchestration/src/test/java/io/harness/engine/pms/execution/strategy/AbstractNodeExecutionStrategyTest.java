...
public class AbstractNodeExecutionStrategyTest extends CategoryTest {
  ...
  @Mock StepConcurrencyHelper stepConcurrencyService;
  Ambiance ambiance;
  ...
  public void setUp() throws IllegalAccessException {
    ...
    FieldUtils.writeField(abstractNodeExecutionStrategy, "nodeExecutionService", nodeExecutionService, true);
    ...
    FieldUtils.writeField(abstractNodeExecutionStrategy, "stepConcurrencyService", stepConcurrencyService, true);
  }
  ...
  public void testRunNextNode() {
    ...
    verify(abstractNodeExecutionStrategy, times(1))
        .createAndRunNextNodeExecution(ambiance, node, nodeExecutionMetadata, null, null, null);
  }

  @Test
  @Owner(developers = SHALINI)
  @Category(UnitTests.class)
  public void testCreateAndRunNextNodeExecution() {
    abstractNodeExecutionStrategy.createAndRunNextNodeExecution(ambiance, planNode, nodeExecutionMetadata, "", "", "");
  ...
  @Category(UnitTests.class)
  public void testCreateAndRunNodeExecution() throws IllegalAccessException {
    abstractNodeExecutionStrategy.createAndRunNodeExecution(ambiance, planNode, nodeExecutionMetadata, "", "", "");
    verify(orchestrationEngine, times(1)).queueOrStartExecution(any());
  }

  @Test
  @Owner(developers = SHALINI)
  @Category(UnitTests.class)
  public void testCreateAndRunNextNodeExecutionWithEmptyExecutionInputTemplate() {
    abstractNodeExecutionStrategy.createAndRunNextNodeExecution(ambiance, planNode, nodeExecutionMetadata, "", "", "");
    verify(executorService, times(1)).submit(any(Runnable.class));
  }
  ...
  @Owner(developers = YUVRAJ)
  @Category(UnitTests.class)
  public void testQueueOrStartExecution_leafMode_claimsSlotBeforeStart() throws IllegalAccessException {
    String nodeExecutionId = generateUuid();
    Ambiance testAmbiance =
        Ambiance.newBuilder()
            .setPlanExecutionId("planExecId")
            .putSetupAbstractions("accountId", accountId)
            .addLevels(Level.newBuilder().setRuntimeId(nodeExecutionId).build())
            .setMetadata(
                ExecutionMetadata.newBuilder().putFeatureFlagToValueMap("PIPE_STEP_CONCURRENCY_ENABLED", true).build())
            .build();

    NodeExecution nodeExecution = NodeExecution.builder().uuid(nodeExecutionId).mode(ExecutionMode.TASK).build();
    when(nodeExecutionService.getWithFieldsIncluded(eq(nodeExecutionId), any())).thenReturn(nodeExecution);
    when(stepConcurrencyService.shouldQueue(eq(ExecutionMode.TASK), eq(testAmbiance))).thenReturn(false);
    when(nodeExecutionService.updateStatusWithOps(
             eq(nodeExecutionId), eq(Status.STARTING_QUEUED_STEP), isNull(), any(EnumSet.class)))
        .thenReturn(nodeExecution);

    FieldUtils.writeField(abstractNodeExecutionStrategy, "publishAdviserEventForCustomAdvisers", false, true);
    doNothing().when(abstractNodeExecutionStrategy).startExecution(testAmbiance);
    abstractNodeExecutionStrategy.queueOrStartExecution(testAmbiance);

    verify(nodeExecutionService)
        .updateStatusWithOps(eq(nodeExecutionId), eq(Status.STARTING_QUEUED_STEP), isNull(), any(EnumSet.class));
    verify(abstractNodeExecutionStrategy).startExecution(testAmbiance);
  }

  @Test
  @Owner(developers = YUVRAJ)
  @Category(UnitTests.class)
  public void testQueueOrStartExecution_shouldQueue_marksAsQueued() throws IllegalAccessException {
    String nodeExecutionId = generateUuid();
    Ambiance testAmbiance =
        Ambiance.newBuilder()
            .setPlanExecutionId("planExecId")
    ...
    NodeExecution nodeExecution = NodeExecution.builder().uuid(nodeExecutionId).mode(ExecutionMode.TASK).build();
    when(nodeExecutionService.getWithFieldsIncluded(eq(nodeExecutionId), any())).thenReturn(nodeExecution);
    when(stepConcurrencyService.shouldQueue(eq(ExecutionMode.TASK), eq(testAmbiance))).thenReturn(true);

    FieldUtils.writeField(abstractNodeExecutionStrategy, "publishAdviserEventForCustomAdvisers", false, true);
    abstractNodeExecutionStrategy.queueOrStartExecution(testAmbiance);

    verify(nodeExecutionService)
        .updateStatusWithOps(eq(nodeExecutionId), eq(Status.QUEUED_STEP_LIMIT_REACHED), isNull(), isNull());
    verify(abstractNodeExecutionStrategy, never()).startExecution(any());
  }

  @Test
  @Owner(developers = YUVRAJ)
  @Category(UnitTests.class)
  public void testStartQueuedExecutionIfAny_dequeuesWhenSlotAvailable() throws IllegalAccessException {
    String nodeExecutionId = generateUuid();
    Ambiance testAmbiance =
        Ambiance.newBuilder()
            .setPlanExecutionId("planExecId")
    ...
    NodeExecution nodeExecution = NodeExecution.builder().uuid(nodeExecutionId).mode(ExecutionMode.TASK).build();
    when(nodeExecutionService.getWithFieldsIncluded(eq(nodeExecutionId), any())).thenReturn(nodeExecution);
    when(stepConcurrencyService.shouldStartQueuedStep(eq(ExecutionMode.TASK), eq(testAmbiance))).thenReturn(true);
    when(stepConcurrencyService.findQueuedNode("planExecId")).thenReturn(testAmbiance);

    FieldUtils.writeField(abstractNodeExecutionStrategy, "publishAdviserEventForCustomAdvisers", false, true);
    abstractNodeExecutionStrategy.startQueuedExecutionIfAny(testAmbiance);

...
public class QueueTaskRequestProcessorTest extends OrchestrationTestBase {
  ...
  public void testHandleEvent() {
    ...
    verify(nodeExecutionService).updateV2ForNonFinalStatusNodeExecution(nExIDCaptor.capture(), uCaptor.capture());

    Update update = new Update();

    assertThat(nExIDCaptor.getValue()).isEqualTo(nodeExecutionId);
    uCaptor.getValue().accept(update);
    assertThat(update.getUpdateObject().keySet()).containsExactly("$addToSet", "$set");
    assertThat(update.getUpdateObject().values()).hasSize(2);

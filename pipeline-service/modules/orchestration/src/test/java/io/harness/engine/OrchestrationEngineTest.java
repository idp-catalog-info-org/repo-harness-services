...
public class OrchestrationEngineTest extends OrchestrationTestBase {
  ...
  public void shouldTestEndNodeExecution() {
    Ambiance ambiance =
        Ambiance.newBuilder()
            ...
            .addLevels(Level.newBuilder().setRuntimeId(generateUuid()).setNodeType(PLAN_NODE.toString()).build())
            .build();
    doNothing().when(planNodeExecutionStrategy).endNodeExecution(eq(ambiance), any(), any());
    orchestrationEngine.endNodeExecution(ambiance);
    verify(planNodeExecutionStrategy).endNodeExecution(eq(ambiance), any(), any());
  }
  ...
  @Category(UnitTests.class)
  public void shouldTestInitiateNodes() {
    // Arrange
    String planId = generateUuid();
    String setupId1 = generateUuid();
    String setupId2 = generateUuid();
    String runtimeId1 = generateUuid();
    String runtimeId2 = generateUuid();

    // Create parent ambiance
    Ambiance parentAmbiance = Ambiance.newBuilder().setPlanId(planId).build();

    // Create node requests
    InitiateNodeRequest nodeReq1 =
        InitiateNodeRequest.builder().setupId(setupId1).runtimeId(runtimeId1).strategyMetadata(null).build();
    InitiateNodeRequest nodeReq2 =
        InitiateNodeRequest.builder().setupId(setupId2).runtimeId(runtimeId2).strategyMetadata(null).build();

    // Create batch request
    InitiateNodeBatchRequest batchRequest = InitiateNodeBatchRequest.builder()
                                                .parentAmbiance(parentAmbiance)
                                                .nodes(Arrays.asList(nodeReq1, nodeReq2))
                                                .childCount(2)
                                                .build();
    ...
    // Setup mocks
    when(planService.fetchAllNodes(eq(planId), anySet())).thenReturn(nodes);
    NodeExecution exec1 = NodeExecution.builder().uuid(generateUuid()).build();
    NodeExecution exec2 = NodeExecution.builder().uuid(generateUuid()).build();
    when(planNodeExecutionStrategy.runNodes(org.mockito.ArgumentMatchers.any(), eq(InitiateMode.CREATE)))
        .thenReturn(Arrays.asList(exec1, exec2));

    // Act
    List<NodeExecution> result = orchestrationEngine.initiateNodes(batchRequest, InitiateMode.CREATE);

    // Assert
    assertThat(result).hasSize(2);
    ArgumentCaptor<RunNodeBatchRequest> batchCaptor = ArgumentCaptor.forClass(RunNodeBatchRequest.class);
    verify(planNodeExecutionStrategy).runNodes(batchCaptor.capture(), eq(InitiateMode.CREATE));

    RunNodeBatchRequest capturedBatch = batchCaptor.getValue();
    assertThat(capturedBatch.getNodes()).hasSize(2);
    assertThat(capturedBatch.getNodes().get(0).getRuntimeId()).isEqualTo(runtimeId1);
    assertThat(capturedBatch.getNodes().get(1).getRuntimeId()).isEqualTo(runtimeId2);
    assertThat(capturedBatch.getParentAmbiance()).isEqualTo(parentAmbiance);
  }
}

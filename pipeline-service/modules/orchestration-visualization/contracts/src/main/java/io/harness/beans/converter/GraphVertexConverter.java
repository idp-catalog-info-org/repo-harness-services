...
public class GraphVertexConverter {
  ...
  public GraphVertex convertFrom(NodeExecution nodeExecution, Map<String, PmsOutcome> outcomes,
      NodeExecutionsInfo nodeExecutionsInfo, List<String> logBaseKeys, PmsStepParameters resolvedStepInputs) {
    List<GraphDelegateSelectionLogParams> graphDelegateSelectionLogParamsList =
        delegateInfoHelper.getDelegateInformationForGivenTask(nodeExecution.getExecutableResponses(),
            nodeExecution.getMode(), NodeExecutionContextUtils.getAccountId(nodeExecution));
    Level level = Objects.requireNonNull(NodeExecutionContextUtils.obtainCurrentLevel(nodeExecution));
    GraphVertexBuilder graphVertexBuilder =
        GraphVertex.builder()
            .uuid(nodeExecution.getUuid())
            .currentLevel(NodeExecutionContextUtils.obtainCurrentLevel(nodeExecution))
            .strategyMetadata(nodeExecutionsInfo.getStrategyMetadata())
            .planNodeId(level.getSetupId())
            .identifier(level.getIdentifier())
            .name(nodeExecution.getName())
            .createdAt(nodeExecution.getCreatedAt())
            ...
            .status(nodeExecution.getStatus())
            .failureInfo(nodeExecution.getFailureInfo())
            .stepParameters(resolvedStepInputs)
            .nodeRunInfo(nodeExecution.getNodeRunInfo())
            .mode(nodeExecution.getMode())
            .executableResponses(CollectionUtils.emptyIfNull(nodeExecution.getExecutableResponses()))
            .interruptHistories(nodeExecution.getInterruptHistories())
            ...
            .stepDetails(nodeExecutionsInfo == null
                    ? new HashMap<>()
                    : nodeExecutionsInfo.getNodeExecutionDetailsInfoList().stream().collect(Collectors.toMap(
                          NodeExecutionDetailsInfo::getName, NodeExecutionDetailsInfo::getStepDetails)))
    ...
    if (nodeExecutionsInfo != null && nodeExecutionsInfo.getRetryNodeMetadata() != null) {
      graphVertexBuilder.retryNodeMetadata(nodeExecutionsInfo.getRetryNodeMetadata());
    }

    return graphVertexBuilder.build();
  }
}

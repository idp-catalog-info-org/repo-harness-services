...
public class GraphStatusUpdateHelperTest extends OrchestrationVisualizationTestBase {
  ...
  public void shouldDoNothingIfRuntimeIdIsNull() {
    ...
    verify(graphGenerationService, never()).getCachedOrchestrationGraphFromDB(planExecutionId, "accountId");
  }
  ...
  public void shouldAddRootNodeIdToTheGraphAndAddVertex() {
    ...
    Awaitility.await().atMost(2, TimeUnit.SECONDS).pollInterval(500, TimeUnit.MILLISECONDS).until(() -> {
      OrchestrationGraph graphInternal =
          graphGenerationService.getCachedOrchestrationGraphFromDB(planExecution.getUuid(), "accountId");
  ...
  public void testConvertFromNodeExecution() {
    NodeExecution nodeExecution =
        NodeExecution.builder()
            ...
            .resolvedParamsVersion(1)
            .skipGraphType(SkipType.SKIP_TREE)
            .build();

    GraphVertex prevValue = GraphVertex.builder()
                                .stepParameters(PmsStepParameters.parse(Map.of("a", "b")))
                                .stepParametersVersion(1)
                                .build();
    ...
    NodeExecution newNodeExecution = nodeExecution.withResolvedParams(PmsStepParameters.parse(Map.of("AA", "BB")));
    prevValue.setStepParametersVersion(0);
    newValue = eventHandlerV2.convertFromNodeExecution(prevValue, newNodeExecution);
    assertThat(newValue.getStepParameters()).isNotEqualTo(prevValue.getStepParameters());
    assertThat(newValue.getStepParameters()).isEqualTo(newNodeExecution.getResolvedParams());
    ...
    // Test childrenCount mapping
    NodeExecution nodeExecutionWithChildrenCount =
        NodeExecution.builder()
            .uuid(generateUuid())
            .ambiance(Ambiance.newBuilder()
                          .setPlanExecutionId("plaExecutionId")
                          .addLevels(Level.newBuilder()
                                         .setStepType(StepType.newBuilder().setType("NG_FORK").build())
                                         .setNodeType(NodeType.PLAN_NODE.name())
                                         .setSetupId("planNodeUuid")
                                         .build())
                          .setMetadata(ExecutionMetadata.newBuilder()
            ...
            .nodeId("planNodeUuid")
            .name("parallel")
            .stepType(StepType.newBuilder().setType("NG_FORK").build())
            .childrenCount(5L)
            .build();
    prevValue = GraphVertex.builder().build();
    newValue = eventHandlerV2.convertFromNodeExecution(prevValue, nodeExecutionWithChildrenCount);
    assertThat(newValue.getChildrenCount()).isEqualTo(5L);
  }

  @Test
  @Owner(developers = YUVRAJ)
  @Category(UnitTests.class)
  public void testConvertFromNodeExecutionForFormingResolvedStepInputs() {
    ...
    NodeExecution nodeExecution =
        NodeExecution.builder()
            ...
            .nodeId("planNodeUuid")
            .name("planNodeName")
            .excludedKeysFromStepInputs(new LinkedList<>(Arrays.asList("a", "c.d")))
            .resolvedParams(originalStepParams)
            .stepType(StepType.newBuilder().setType("Http").build())
            .lastUpdatedAt(100L)
            .failureInfo(FailureInfo.newBuilder().build())
            .interruptHistory(InterruptEffect.builder().build())
    ...
    GraphVertex prevValue = GraphVertex.builder().build();
    GraphVertex newValue = eventHandlerV2.convertFromNodeExecution(prevValue, nodeExecution);
    assertThat(newValue.getStepParameters()).isNotEqualTo(prevValue.getStepParameters());
    assertThat(newValue.getStepParameters()).isNotEqualTo(nodeExecution.getResolvedParams());
    assertThat(newValue.getStepParameters().size()).isEqualTo(2);
  }

  @Test
  @Owner(developers = MEENA)
  @Category(UnitTests.class)
  public void testIsOldRetriedStepGroupNode() {
    String planExecutionId = "planExecutionId";
    String nodeExecutionId = "nodeExecutionId";
    GraphVertex node = GraphVertex.builder().uuid(nodeExecutionId).build();
    ...
    OrchestrationGraph cachedGraph = OrchestrationGraph.builder()
                                         .adjacencyList(OrchestrationAdjacencyListInternal.builder()
                                                            .graphVertexMap(new HashMap<>())
                                                            .graphVertexMap(Maps.newHashMap(nodeExecutionId, node))
                                                            .build())
                                         .planExecutionId(planExecutionId)
                                         .build();
    assertThat(eventHandlerV2.isOldRetriedStepGroupNode(nodeExecution, cachedGraph)).isTrue();
    ...
    nodeExecution = NodeExecution.builder()
                        ...
                        .skipGraphType(SkipType.NOOP)
                        .build();
    cachedGraph = OrchestrationGraph.builder()
                      .adjacencyList(OrchestrationAdjacencyListInternal.builder()
                                         .graphVertexMap(new HashMap<>())
                                         .graphVertexMap(Maps.newHashMap(nodeExecutionId, node))
                                         .build())
                      .planExecutionId(planExecutionId)
                      .build();
    assertThat(eventHandlerV2.isOldRetriedStepGroupNode(nodeExecution, cachedGraph)).isFalse();

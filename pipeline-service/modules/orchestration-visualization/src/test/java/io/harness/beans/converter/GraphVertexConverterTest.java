...
public class GraphVertexConverterTest extends OrchestrationVisualizationTestBase {
  ...
  public void testConvertFromNodeExecutionWithNodeExecutionInfoAsNullToGraphVertex() {
    NodeExecution nodeExecution = NodeExecution.builder()
                                      ...
                                      .build();
    GraphVertex graphVertex = graphVertexConverter.convertFrom(nodeExecution, null,
        NodeExecutionsInfo.builder().strategyMetadata(StrategyMetadata.newBuilder().build()).build(), null,
        PmsStepParameters.parse(stepParams));
    assertThat(graphVertex).isNotNull();
    assertThat(graphVertex.getUuid()).isEqualTo(nodeExecution.getUuid());
    assertThat(graphVertex.getCurrentLevel()).isEqualTo(AmbianceUtils.obtainCurrentLevel(ambiance));
    assertThat(graphVertex.getPlanNodeId()).isEqualTo("setUpId");
    ...
    assertThat(graphVertex.getStrategyMetadata()).isEqualTo(StrategyMetadata.newBuilder().build());
  }
  ...
  public void testConvertFromNodeExecutionWithNodeExecutionInfoToGraphVertex() {
    NodeExecution nodeExecution = NodeExecution.builder()
                                      ...
                                      .retryId("retryId")
                                      .executionInputConfigured(true)
                                      .excludedKeysFromStepInputs(Collections.emptyList())
  ...
  @Owner(developers = PRASHANTSHARMA)
  @Category(UnitTests.class)
  public void testConvertFromNodeExecutionWithChildrenCount() {
    // Test that childrenCount is correctly mapped from NodeExecution to GraphVertex
    NodeExecution nodeExecution = NodeExecution.builder()
                                      .uuid("uuid")
                                      .ambiance(ambiance)
                                      .name("parallel")
    ...
    GraphVertex graphVertex = graphVertexConverter.convertFrom(nodeExecution, null);
    assertThat(graphVertex).isNotNull();
    assertThat(graphVertex.getChildrenCount()).isEqualTo(5L);

    // Test with childrenCount = 0 (default)
    NodeExecution nodeExecutionNoChildren = NodeExecution.builder()
                                                .uuid("uuid2")
                                                .ambiance(ambiance)
                                                .name("step")
                                                .startTs(10L)
                                                ...
                                                .mode(ExecutionMode.SYNC)
                                                .build();
    GraphVertex graphVertexNoChildren = graphVertexConverter.convertFrom(nodeExecutionNoChildren, null);
    assertThat(graphVertexNoChildren).isNotNull();
    assertThat(graphVertexNoChildren.getChildrenCount()).isEqualTo(0L);
  }
}

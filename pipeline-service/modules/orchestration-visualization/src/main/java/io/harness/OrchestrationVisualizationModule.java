...
import io.harness.service.impl.PostgreSQLGraphStoreJOOQServiceImpl;
...
public class OrchestrationVisualizationModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    bind(PostgreSQLGraphStoreService.class).to(PostgreSQLGraphStoreJOOQServiceImpl.class);
    ...
    if (redisConfig.getRedisUrl().equals("dummyRedisUrl")) {
      ...
      bind(Consumer.class)
          .annotatedWith(Names.named(ORCHESTRATION_LOG))
          .toInstance(RedisConsumer.of(EventsFrameworkRedisTopicResolver.getOrchestrationLogTopic(),
              redisConsumerGroupId, redissonClient, EventsFrameworkConstants.ORCHESTRATION_LOG_MAX_PROCESSING_TIME,
  ...
  @Provides
  @Singleton
  @Named("ExecutionSummaryBackfillExecutorService")
  public ExecutorService executionSummaryBackfillExecutorService() {
    return ThreadPool.getInstrumentedExecutorService(
        ThreadPoolConfig.builder().build(), "ExecutionSummaryBackfillExecutorService", threadPoolMetricRegistry);
  }
}

...
public class OrchestrationAdjacencyListGenerator {
  ...
  public void addVertex(OrchestrationAdjacencyListInternal adjacencyListInternal, NodeExecution nodeExecution) {
    ...
    List<String> logBaseKeys = StepUtils.generateLogKeys(nodeExecutionService.getAmbiance(nodeExecution), null);
  ...
  private class GraphGeneratorSession {
    ...
    private OrchestrationAdjacencyListInternal generateListStartingFrom(
      ...
      while (!queue.isEmpty()) {
        ...
        if (isOutcomePresent) {
          outcomes = PmsOutcomeMapper.convertJsonToOrchestrationMap(
              pmsOutcomeService.findAllOutcomesMapByRuntimeId(nodeExecution.getPlanExecutionId(), currentNodeId));
        } else {
        ...
        GraphVertex graphVertex = graphVertexConverter.convertFrom(
            nodeExecution, outcomes, stepDetails, logBaseKeys, getResolvedStepInputs(stepDetails, nodeExecution));
        ...
        if (EmptyPredicate.isNotEmpty(nextNodeId)) {
          ...
          // when nextNodeId is parent or next(sibling) to startingNode, do not add to queue
          if (isCurrentNodeUnderParentSubGraph(
                  startingNodeId, currentNodeId, nextNodeId, parentNodeId, nodeExecution.getAccountId())) {
    ...
    private PmsStepParameters getResolvedStepInputs(NodeExecutionsInfo stepDetails, NodeExecution nodeExecution) {
      if (stepDetails != null) {
        if (nodeExecution.getExcludedKeysFromStepInputs() != null) {
          return nodeExecutionService.getResolvedStepInputs(
              nodeExecution.getExcludedKeysFromStepInputs(), nodeExecution.getResolvedStepParameters());
        } else if (stepDetails.getResolvedInputs() != null) {
          return stepDetails.getResolvedInputs();
        }
      }
      return nodeExecution.getResolvedStepParameters();
    }

    private boolean isCurrentNodeUnderParentSubGraph(
        String startingNodeId, String currentNodeId, String nextNodeId, String parentNodeId, String accountId) {
      if (pmsFeatureFlagService.isEnabled(accountId, FeatureName.PIE_RESTRICT_NEXT_OF_STARTING_NODE_IN_NODE_GRAPH)) {
        return !Objects.equals(parentNodeId, nextNodeId) && !Objects.equals(currentNodeId, startingNodeId);
      }
      return !Objects.equals(parentNodeId, nextNodeId);
    }

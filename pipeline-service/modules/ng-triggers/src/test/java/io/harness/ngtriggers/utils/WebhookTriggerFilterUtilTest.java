...
import io.harness.beans.MergeQueueWebhookEvent;
...
import io.harness.beans.TagWebhookEvent;
...
public class WebhookTriggerFilterUtilTest extends CategoryTest {
  ...
  public void setup() {
    initMocks(this);
    doReturn(false).when(pmsFeatureFlagHelper).isEnabled("accountId", FeatureName.CDS_REMOVE_TAG_TRIGGER_FILTER);
  }
  ...
  public void ConvertToLowerCaseTest() {
    ...
    assertThat(WebhookTriggerFilterUtils.sanitiseHeaderConditionsForJexl(
        ...
        .isEqualTo("<+trigger.payload.pull_request.user.login> == \"adwait\" && <+trigger.header['x-github-event']> == "
            + "\"PR\" || <+trigger.header['x-event']> == \"PR\"");
    ...
    assertThat(WebhookTriggerFilterUtils.sanitiseHeaderConditionsForJexl(
        ...
        .isEqualTo("<+trigger.payload.pull_request.user.login> == \"adwait\" && <+trigger.header['xgithubevent']> == "
            + "\"PR\" || <+trigger.header['x-event']> == \"PR\"");
  ...
  public void checkIfEventTypeMatchesTest() {
    ...
    // Create proper WebhookEvent implementation objects based on type
    WebhookEvent prEvent = PRWebhookEvent.builder().build();
    WebhookEvent pushEvent = PushWebhookEvent.builder().build();
    WebhookEvent issueCommentEvent = IssueCommentWebhookEvent.builder().build();
    WebhookEvent releaseEvent = ReleaseWebhookEvent.builder().build();

    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(prEvent, webhookTriggerSpecPR)).isTrue();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(prEvent, webhookTriggerSpecPush)).isFalse();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(pushEvent, webhookTriggerSpecPush)).isTrue();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(pushEvent, webhookTriggerSpecIssueComment)).isFalse();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(issueCommentEvent, webhookTriggerSpecIssueComment))
        .isTrue();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(issueCommentEvent, webhookTriggerSpecRelease))
        .isFalse();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(releaseEvent, webhookTriggerSpecRelease)).isTrue();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(releaseEvent, webhookTriggerSpecPR)).isFalse();
  }

  @Test
  @Owner(developers = VINICIUS)
  @Category(UnitTests.class)
  public void checkIfMergeQueueEventTypeMatchesTest() {
    WebhookTriggerSpecV2 mergeQueueSpec = HarnessSpec.builder()
                                              .type(HarnessTriggerEvent.MERGE_QUEUE)
                                              .spec(HarnessMergeQueueSpec.builder().build())
                                              .build();
    WebhookTriggerSpecV2 pushSpec =
        HarnessSpec.builder().type(HarnessTriggerEvent.PUSH).spec(HarnessPushSpec.builder().build()).build();

    WebhookEvent mergeQueueEvent = MergeQueueWebhookEvent.builder().build();
    WebhookEvent pushEvent = PushWebhookEvent.builder().build();

    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(mergeQueueEvent, mergeQueueSpec)).isTrue();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(mergeQueueEvent, pushSpec)).isFalse();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(pushEvent, mergeQueueSpec)).isFalse();
  }

  @Test
  @Owner(developers = VINICIUS)
  @Category(UnitTests.class)
  public void checkMergeQueueActionAlwaysMatchesTest() {
    WebhookTriggerSpecV2 mergeQueueSpec = HarnessSpec.builder()
                                              .type(HarnessTriggerEvent.MERGE_QUEUE)
                                              .spec(HarnessMergeQueueSpec.builder().build())
                                              .build();

    for (String action : new String[] {"checks_requested", "checks_canceled"}) {
      WebhookPayloadData data =
          WebhookPayloadData.builder()
              .webhookEvent(MergeQueueWebhookEvent.builder()
                                .baseAttributes(WebhookBaseAttributes.builder().action(action).build())
                                .build())
              .build();
      assertThat(checkIfActionMatches(data, mergeQueueSpec)).isTrue();
    }
  }
  ...
  @Owner(developers = VINICIUS)
  @Category(UnitTests.class)
  public void checkIfGithubMergeQueueEventTypeMatchesTest() {
    // GithubMergeQueueSpec and HarnessMergeQueueSpec both use the shared MERGE_QUEUE_EVENT_TYPE ("MergeQueue")
    // string and the SCM client always reports WebhookEvent.Type.MERGE_QUEUE regardless of provider, so the
    // single check added for Harness in checkIfEventTypeMatches already covers GitHub. This test proves it.
    WebhookTriggerSpecV2 githubMergeQueueSpec =
        GithubSpec.builder().type(GithubTriggerEvent.MERGE_QUEUE).spec(GithubMergeQueueSpec.builder().build()).build();
    WebhookTriggerSpecV2 githubPushSpec =
        GithubSpec.builder().type(GithubTriggerEvent.PUSH).spec(GithubPushSpec.builder().build()).build();

    WebhookEvent mergeQueueEvent = MergeQueueWebhookEvent.builder().build();

    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(mergeQueueEvent, githubMergeQueueSpec)).isTrue();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(mergeQueueEvent, githubPushSpec)).isFalse();
  }

  @Test
  @Owner(developers = VINICIUS)
  @Category(UnitTests.class)
  public void checkGithubMergeQueueActionAlwaysMatchesTest() {
    WebhookTriggerSpecV2 githubMergeQueueSpec =
        GithubSpec.builder().type(GithubTriggerEvent.MERGE_QUEUE).spec(GithubMergeQueueSpec.builder().build()).build();

    for (String action : new String[] {"checks_requested", "destroyed"}) {
      WebhookPayloadData data =
          WebhookPayloadData.builder()
              .webhookEvent(MergeQueueWebhookEvent.builder()
                                .baseAttributes(WebhookBaseAttributes.builder().action(action).build())
                                .build())
              .build();
      assertThat(checkIfActionMatches(data, githubMergeQueueSpec)).isTrue();
    }
  }
  ...
  public void evaluateFilterConditionsTest() {
    ...
    WebhookEvent prEvent = PRWebhookEvent.builder().build();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(prEvent, webhookTriggerSpec)).isTrue();
    ...
    assertThat(WebhookTriggerFilterUtils.checkIfPayloadConditionsMatch(
                   webhookPayloadData, webhookTriggerSpec, null, "accountId", pmsFeatureFlagHelper))
        .isTrue();
  }

  // CI-23187: Custom triggers with no conditions have a null inner WebhookTriggerSpecV2.
  // Prior to the fix, checkIfPayloadConditionsMatch would NPE on webhookTriggerSpec.fetchPayloadAware().
  // Expected behavior after fix: return true (no conditions to evaluate = pass).
  @Test
  @Owner(developers = ABHIJEET_GUPTA)
  @Category(UnitTests.class)
  public void checkIfPayloadConditionsMatch_nullSpec_returnsTrue() {
    WebhookPayloadData webhookPayloadData =
        WebhookPayloadData.builder().originalEvent(TriggerWebhookEvent.builder().payload(payload).build()).build();
    assertThat(WebhookTriggerFilterUtils.checkIfPayloadConditionsMatch(
                   webhookPayloadData, null, null, "accountId", pmsFeatureFlagHelper))
        .isTrue();
  }
  ...
  public void checkIfJexlConditionsMatchTest() {
    ...
    assertThat(WebhookTriggerFilterUtils.checkIfJexlConditionsMatch(
                   WebhookTriggerFilterUtils.generatorPMSExpressionEvaluator(
                       webhookPayloadData.getParseWebhookResponse(), Collections.emptyList(), payload),
                   webhookTriggerSpec.fetchPayloadAware().fetchJexlCondition()))
        .isTrue();
  }
  ...
  public void evaluateFilterConditionsForCustomPayloadWebhookTest() {
    ...
    TriggerExpressionEvaluator triggerExpressionEvaluator =
        WebhookTriggerFilterUtils.generatorPMSExpressionEvaluator(null, headerConfigList, "{}");

    assertThat(WebhookTriggerFilterUtils.checkIfCustomHeaderConditionsMatch(triggerExpressionEvaluator,
  ...
  @Category(UnitTests.class)
  public void evaluateTagTest() {
    WebhookTriggerSpecV2 webhookTriggerSpec =
        HarnessSpec.builder()
            .spec(HarnessTagSpec.builder()
                      .actions(Collections.singletonList(HarnessTagAction.CREATE))
                      .payloadConditions(Collections.singletonList(
                          TriggerEventDataCondition.builder().key("tag").operator(EQUALS).value("tagname").build()))
                      .build())
            .type(HarnessTriggerEvent.TAG)
            .build();

    WebhookPayloadData webhookPayloadData =
        WebhookPayloadData.builder()
            .originalEvent(TriggerWebhookEvent.builder().payload(payload).build())
            .webhookEvent(TagWebhookEvent.builder()
                              .baseAttributes(WebhookBaseAttributes.builder().ref("tagname").action("create").build())
                              .build())
            .build();

    WebhookEvent tagWebhookEvent =
        TagWebhookEvent.builder().baseAttributes(WebhookBaseAttributes.builder().ref("tagname").build()).build();
    assertThat(WebhookTriggerFilterUtils.checkIfEventTypeMatches(tagWebhookEvent, webhookTriggerSpec)).isTrue();
    assertThat(checkIfActionMatches(webhookPayloadData, webhookTriggerSpec)).isTrue();
    assertThat(WebhookTriggerFilterUtils.checkIfPayloadConditionsMatch(
                   webhookPayloadData, webhookTriggerSpec, null, "accountId", pmsFeatureFlagHelper))
        .isTrue();
        ...
        .spec(GitlabPRSpec.builder()
                  ...
                  .payloadConditions(Arrays.asList(
                      ...
                      TriggerEventDataCondition.builder()
                          ...
                          .value("secure.gravatar.com")
                          .build()))
                  .jexlCondition("<+trigger.payload.user.name> == \"charles grant\" && <+trigger.payload.user.email> "
                      + "!= \"wrong_email\"")
                  .build())
        .build();
  }

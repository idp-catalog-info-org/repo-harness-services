...
public class TriggerValidationHandlerTest extends CategoryTest {
  ...
  public void testApplyValidationsWebhook() {
    ...
    when(pipelineRefValidator.validate(any(), any(), anyBoolean())).thenReturn(validationResult);
    when(triggerIdentifierRefValidator.validate(any(), any(), anyBoolean())).thenReturn(validationResult);
    when(manifestTriggerValidator.validate(any(), any(), anyBoolean())).thenReturn(validationResult);
    when(artifactTriggerValidator.validate(any(), any(), anyBoolean())).thenReturn(validationResult);

    ValidationResult finalValidationResult =
        triggerValidationHandler.applyValidations(triggerDetailsWebhook, null, false);
    assertThat(finalValidationResult).isEqualToComparingFieldByField(validationResult);
    verify(pipelineRefValidator, times(1)).validate(triggerDetailsWebhook, null, false);
    verify(triggerIdentifierRefValidator, times(1)).validate(triggerDetailsWebhook, null, false);
  }
  ...
  public void testApplyValidationsScheduled() {
    ...
    when(artifactTriggerValidator.validate(any(), any(), anyBoolean())).thenReturn(validationResult);
    ValidationResult finalValidationResult =
        triggerValidationHandler.applyValidations(triggerDetailsScheduled, null, false);
    assertThat(finalValidationResult).isEqualToComparingFieldByField(validationResult);
    verify(pipelineRefValidator, times(1)).validate(triggerDetailsScheduled, null, false);
    verify(triggerIdentifierRefValidator, times(1)).validate(triggerDetailsScheduled, null, false);
  }
  ...
  public void testApplyValidationsArtifact() {
    ...
    when(artifactTriggerValidator.validate(any(), any(), anyBoolean())).thenReturn(validationResult);
    ValidationResult finalValidationResult =
        triggerValidationHandler.applyValidations(triggerDetailsArtifact, null, false);
    assertThat(finalValidationResult).isEqualToComparingFieldByField(validationResult);
    verify(pipelineRefValidator, times(1)).validate(triggerDetailsArtifact, null, false);
    verify(triggerIdentifierRefValidator, times(1)).validate(triggerDetailsArtifact, null, false);
    verify(artifactTriggerValidator, times(1)).validate(triggerDetailsArtifact, null, false);
  }
  ...
  public void testApplyValidationsManifest() {
    ...
    when(artifactTriggerValidator.validate(any(), any(), anyBoolean())).thenReturn(validationResult);
    ValidationResult finalValidationResult =
        triggerValidationHandler.applyValidations(triggerDetailsManifest, null, false);
    assertThat(finalValidationResult).isEqualToComparingFieldByField(validationResult);
    verify(pipelineRefValidator, times(1)).validate(triggerDetailsManifest, null, false);
    verify(triggerIdentifierRefValidator, times(1)).validate(triggerDetailsManifest, null, false);
    verify(manifestTriggerValidator, times(1)).validate(triggerDetailsManifest, null, false);
  }
  ...
  public void testPipelineRefValidator() {
    doReturn(Optional.empty())
        ...
        .fetchPipelineYamlForTrigger(any(), any(), anyBoolean());

    PipelineRefValidator pipelineRefValidator = new PipelineRefValidator(buildTriggerHelper);
    ValidationResult validate =
        pipelineRefValidator.validate(TriggerDetails.builder().ngTriggerEntity(ngTriggerEntity).build(), null, false);
  ...
  public void testPipelineRefValidatorWhenPipelineBranchNameIsNull() {
    ...
    PipelineRefValidator pipelineRefValidator = new PipelineRefValidator(buildTriggerHelper);
    ValidationResult validate = pipelineRefValidator.validate(triggerDetails1, null, false);
    assertThat(validate.isSuccess()).isTrue();
  }
  ...
  public void testTriggerIdentifierRefValidator() {
    ...
    ValidationResult validate = triggerIdentifierRefValidator.validate(
        TriggerDetails.builder().ngTriggerEntity(NGTriggerEntity.builder().build()).build(), null, false);
    ...
    validate = triggerIdentifierRefValidator.validate(triggerDetails, null, false);
    assertThat(validate.isSuccess()).isTrue();
  }
  ...
  public void testArtifactTriggerValidatorWithRemotePipeline() throws IOException {
    ...
    TriggerDetails artifactTriggerDetails =
        ngTriggerElementMapper.toTriggerDetails("accountId", "orgId", "projectId", null, artifactTriggerYaml, true);
    String pipelineYaml =
    ...
    doNothing().when(artifactTriggerValidator).validateBasedOnArtifactType(any(), any(), anyBoolean());
    when(generatorFactory.retrievePollingItemGenerator(any()))
        .thenReturn(new AcrPollingItemGenerator(validationHelper));
    ValidationResult validate = artifactTriggerValidator.validate(artifactTriggerDetails, null, false);
    assertThat(validate.isSuccess()).isTrue();
  }
}

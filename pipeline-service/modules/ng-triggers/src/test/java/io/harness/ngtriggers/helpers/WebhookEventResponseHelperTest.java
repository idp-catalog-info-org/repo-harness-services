...
public class WebhookEventResponseHelperTest extends CategoryTest {
  ...
  public void toResponse() {
    ...
    // A null execution DTO paired with a genuine failure status still marks exceptionOccurred.
    webhookEventResponse = TriggerEventResponseHelper.toResponse(
        INVALID_RUNTIME_INPUT_YAML, triggerWebhookEvent, null, ngTriggerEntity, message, summary);
    assertThat(webhookEventResponse.isExceptionOccurred()).isTrue();

    // A null execution DTO paired with a status that is itself a successful outcome (e.g. a merge queue cancel,
    // where not starting an execution IS success) must not be flagged as an exception, otherwise it would flip the
    // trigger's lastExecutionSuccessful() flag to false even though nothing failed.
    webhookEventResponse = TriggerEventResponseHelper.toResponse(
        MERGE_QUEUE_CHECKS_CANCELED, triggerWebhookEvent, null, ngTriggerEntity, message, summary);
    assertThat(webhookEventResponse.isExceptionOccurred()).isFalse();

    // Same for a skipped status. A suppressed merge queue redelivery starts nothing by design, so it is neither a
    // success nor an exception - only the genuine failure statuses above may set the flag.
    webhookEventResponse = TriggerEventResponseHelper.toResponse(
        MERGE_QUEUE_CHECKS_ALREADY_RUNNING, triggerWebhookEvent, null, ngTriggerEntity, message, summary);
    assertThat(webhookEventResponse.isExceptionOccurred()).isFalse();

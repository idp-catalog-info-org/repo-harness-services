...
public class TriggerHelperTest extends CategoryTest {
  ...
  @Category(UnitTests.class)
  public void testBuildJsonObjectFromAmbianceWebhookPushWithCommitSinglePath() {
    User sender = User.newBuilder().setLogin("user").build();
    Repository repo = Repository.newBuilder().setLink("repo_link").build();
    PushHook push =
        PushHook.newBuilder().setRepo(repo).setRef("refs/heads/branch").setSender(sender).setAfter("sha").build();
    ParsedPayload pushParsedPayload = ParsedPayload.newBuilder().setPush(push).build();
    TriggerPayload pushPayload = TriggerPayload.newBuilder()
                                     .setParsedPayload(pushParsedPayload)
                                     .setSourceType(GITHUB_REPO)
                                     .addAllChangedFiles(List.of("README.md"))
                                     .build();
    Map<String, Object> jsonObject = TriggerHelper.buildJsonObjectFromAmbiance(pushPayload);
    assertThat(jsonObject.get(BRANCH)).isEqualTo("branch");
    assertThat(jsonObject.get(TARGET_BRANCH)).isEqualTo("branch");
    ...
    assertThat(jsonObject.get(GIT_USER)).isEqualTo("user");
    assertThat(jsonObject.get(SOURCE_TYPE)).isEqualTo(GITHUB.getValue());
    assertThat(jsonObject.get(CHANGED_FILES)).isEqualTo(List.of("README.md"));
  }

  @Test
  @Owner(developers = EDGAR_GARCIA)
  @Category(UnitTests.class)
  public void testBuildJsonObjectFromAmbianceWebhookPushWithCommitMultiplePath() {
    User sender = User.newBuilder().setLogin("user").build();
    Repository repo = Repository.newBuilder().setLink("repo_link").build();
    PushHook push =
        PushHook.newBuilder().setRepo(repo).setRef("refs/heads/branch").setSender(sender).setAfter("sha").build();
    ...
    TriggerPayload pushPayload = TriggerPayload.newBuilder()
                                     .setParsedPayload(pushParsedPayload)
                                     .setSourceType(GITHUB_REPO)
                                     .addAllChangedFiles(List.of("src/infra/module/cloud/existing-infra.js",
                                         "src/infra/module/cloud/new-infra.js", "src/infra/module/cloud/new-infra2.js",
                                         "src/infra/module/cloud/old-infra.js"))
                                     .build();
    Map<String, Object> jsonObject = TriggerHelper.buildJsonObjectFromAmbiance(pushPayload);
    assertThat(jsonObject.get(BRANCH)).isEqualTo("branch");
    assertThat(jsonObject.get(TARGET_BRANCH)).isEqualTo("branch");
    ...
    assertThat(jsonObject.get(GIT_USER)).isEqualTo("user");
    assertThat(jsonObject.get(SOURCE_TYPE)).isEqualTo(GITHUB.getValue());
    assertThat(jsonObject.get(CHANGED_FILES))
        .isEqualTo(List.of("src/infra/module/cloud/existing-infra.js", "src/infra/module/cloud/new-infra.js",
            "src/infra/module/cloud/new-infra2.js", "src/infra/module/cloud/old-infra.js"));
  }

  @Test
  @Owner(developers = EDGAR_GARCIA)
  @Category(UnitTests.class)
  public void testBuildJsonObjectFromAmbianceWebhookPushWithCommitMultiplePathInMultipleLevel() {
    User sender = User.newBuilder().setLogin("user").build();
    Repository repo = Repository.newBuilder().setLink("repo_link").build();
    PushHook push =
        PushHook.newBuilder().setRepo(repo).setRef("refs/heads/branch").setSender(sender).setAfter("sha").build();
    ...
    TriggerPayload pushPayload =
        TriggerPayload.newBuilder()
            ...
            .setSourceType(GITHUB_REPO)
            .addAllChangedFiles(List.of("src/infra/module/cloud/existing-infra.js",
                "pipeline-service/service/source/main/java/io/harness/events/existing-event.java", "README.md",
                "src/infra/module/cloud/new-infra.js", "src/infra/module/cloud/new-infra2.js",
                "pipeline-service/service/source/main/java/io/harness/health/existing-healthcheck.java"))
            .build();
    Map<String, Object> jsonObject = TriggerHelper.buildJsonObjectFromAmbiance(pushPayload);
    assertThat(jsonObject.get(BRANCH)).isEqualTo("branch");
    assertThat(jsonObject.get(TARGET_BRANCH)).isEqualTo("branch");
    ...
    assertThat(jsonObject.get(SOURCE_TYPE)).isEqualTo(GITHUB.getValue());
    assertThat(jsonObject.get(CHANGED_FILES))
        .isEqualTo(List.of("src/infra/module/cloud/existing-infra.js",
            "pipeline-service/service/source/main/java/io/harness/events/existing-event.java", "README.md",
            "src/infra/module/cloud/new-infra.js", "src/infra/module/cloud/new-infra2.js",
            "pipeline-service/service/source/main/java/io/harness/health/existing-healthcheck.java"));
  }
  ...
  @Owner(developers = SAHITHI)
  @Category(UnitTests.class)
  public void testBuildJsonObjectFromAmbianceWebhookMergeQueue() {
    User sender = User.newBuilder().setLogin("user").build();
    Repository repo = Repository.newBuilder().setLink("repo_link").build();
    MergeQueueHook mergeQueueHook = MergeQueueHook.newBuilder()
                                        ...
                                        .setSha("abc123")
                                        .build();
    ParsedPayload mergeQueueParsedPayload = ParsedPayload.newBuilder().setMergeQueue(mergeQueueHook).build();
    TriggerPayload mergeQueuePayload =
        TriggerPayload.newBuilder().setParsedPayload(mergeQueueParsedPayload).setSourceType(GITHUB_REPO).build();
    Map<String, Object> jsonObject = TriggerHelper.buildJsonObjectFromAmbiance(mergeQueuePayload);
    assertThat(jsonObject.get(BRANCH)).isEqualTo("main");
    assertThat(jsonObject.get(TARGET_BRANCH)).isEqualTo("main");
    assertThat(jsonObject.get(COMMIT_SHA)).isEqualTo("abc123");
    assertThat(jsonObject.get(EVENT)).isEqualTo(MERGE_QUEUE_EVENT_TYPE);
    // regression guard: if the MERGE_QUEUE case is ever removed, the switch falls to `default`
    // which sets CUSTOM_TYPE instead, and this assertion catches it.
    assertThat(jsonObject.get(TYPE)).isEqualTo(WEBHOOK_TYPE);
    assertThat(jsonObject.get(REPO_URL)).isEqualTo("repo_link");
    assertThat(jsonObject.get(GIT_USER)).isEqualTo("user");
    // A merge queue commit is synthesized on the queue ref from a batch of pull requests, so there is no single source
    // branch to report. Absent rather than empty, matching the PUSH case.
    assertThat(jsonObject).doesNotContainKey(SOURCE_BRANCH);
  }

  @Test
  @Owner(developers = SAHITHI)
  @Category(UnitTests.class)
  public void testBuildJsonObjectFromAmbianceWebhookMergeQueueFallsBackToSenderName() {
    // Harness Code senders carry a name but frequently no login, which would otherwise resolve
    // <+trigger.gitUser> to an empty string.
    MergeQueueHook mergeQueueHook = MergeQueueHook.newBuilder()
                                        .setAction(Action.CHECKS_REQUESTED)
                                        .setRepo(Repository.newBuilder().setLink("repo_link").build())
                                        .setSender(User.newBuilder().setName("Real Name").build())
                                        .setBranch("main")
                                        .setSha("abc123")
                                        .build();
    Map<String, Object> jsonObject = TriggerHelper.buildJsonObjectFromAmbiance(
        TriggerPayload.newBuilder()
            .setParsedPayload(ParsedPayload.newBuilder().setMergeQueue(mergeQueueHook).build())
            .setSourceType(HARNESS_REPO)
            .build());

    assertThat(jsonObject.get(GIT_USER)).isEqualTo("Real Name");
  }
  ...
  @Category(UnitTests.class)
  public void testGetAllTriggerExpressions() {
    List<String> triggerExpressions = TriggerHelper.getAllTriggerExpressions();

    assertThat(triggerExpressions)
        .containsExactlyInAnyOrder("trigger.targetBranch", "trigger.sourceBranch", "trigger.gitUser", "trigger.repoUrl",
            "trigger.commitSha", "trigger.baseCommitSha", "trigger.event", "trigger.prNumber", "trigger.prTitle",
            "trigger.artifact.build", "trigger.manifest.version", "lastPublished.tag", "trigger.tag",

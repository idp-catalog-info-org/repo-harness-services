...
public class BuildTriggerSignatureFilterTest {
  ...
  public void testApplyFilter_MatchingTrigger() {
    NGTriggerEntity ngTriggerEntity = NGTriggerEntity.builder().parentUniqueId(PARENT_UNIQUE_ID).build();
    ...
    // Mock the behavior of ngTriggerElementMapper.toTriggerDetails with resolved ScopeInfo
    when(ngTriggerElementMapper.toTriggerDetails(any(), eq(scopeInfo), anyBoolean())).thenReturn(triggerDetails);

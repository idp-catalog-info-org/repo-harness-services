...
public class HelmHttpPollingItemGeneratorTest extends CategoryTest {
  ...
  public void testHelmS3PollingItemGeneration_pipelineContainsFixedValuesExceptTag() throws Exception {
    TriggerDetails triggerDetails =
        ngTriggerElementMapper.toTriggerDetails("acc", "org", "proj", null, ng_trigger_manifest_helm_http, false);
    ...
    PollingItem pollingItem = httpHelmPollingItemGenerator.generatePollingItem(buildTriggerOpsData, null, false);
  ...
  @Category(UnitTests.class)
  public void testHelmHttpPollingItemGenerationWithHelmV4() throws Exception {
    String triggerYaml = ng_trigger_manifest_helm_http.replace("helmVersion: V3", "helmVersion: V4");
    TriggerDetails triggerDetails =
        ngTriggerElementMapper.toTriggerDetails("acc", "org", "proj", null, triggerYaml, false);
    triggerDetails.getNgTriggerEntity().getMetadata().getBuildMetadata().setPollingConfig(
        PollingConfig.builder().signature("sig1").build());
    BuildTriggerOpsData buildTriggerOpsData = buildTriggerHelper.generateBuildTriggerOpsDataForManifest(
        triggerDetails, helm_http_pipeline_manifest_snippet_runtime_all);
    ...
    assertThat(pollingItem.getPollingPayloadData().getHttpHelmPayload().getHelmVersion().name()).isEqualTo("V4");
  }
}

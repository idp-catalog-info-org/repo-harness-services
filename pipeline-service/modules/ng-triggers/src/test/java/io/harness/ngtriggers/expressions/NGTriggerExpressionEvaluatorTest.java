...
public class NGTriggerExpressionEvaluatorTest extends CategoryTest {
        ...
        .thenReturn(Optional.of(
            PlanExecutionMetadata.builder()
                ...
                .triggerPayload(TriggerPayload.newBuilder()
                                    ...
                                    .build())
                .build()));
    NGTriggerExpressionEvaluator expressionEvaluator =
        new NGTriggerExpressionEvaluator(ambiance, planExecutionMetadataService, planExecutionService);
    assertExpressionEvaluations(expressionEvaluator);
    when(AmbianceUtils.checkIfFeatureFlagEnabled(
             eq(ambiance), eq(FeatureName.PIPE_EXECUTION_SWITCH_FIELD_SOURCE.name())))
        .thenReturn(true);
    when(planExecutionService.getWithFieldsIncluded(
             ambiance.getPlanExecutionId(), Set.of(PlanExecutionKeys.triggerHeader)))
  ...
  private static void assertExpressionEvaluations(NGTriggerExpressionEvaluator expressionEvaluator) {
    ...
    assertThat(expressionEvaluator.renderExpression("<+abc.prTitle>")).isEqualTo("null");
  }
}

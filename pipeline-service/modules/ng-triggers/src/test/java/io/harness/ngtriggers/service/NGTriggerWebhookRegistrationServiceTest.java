...
public class NGTriggerWebhookRegistrationServiceTest extends CategoryTest {
  ...
  public void shouldRegisterWebhook() throws IOException {
    ...
    WebhookRegistrationStatusData webhookRegistrationStatus =
        ngTriggerWebhookRegistrationService.registerWebhook(ngTriggerEntity, null, false);
  ...
  @Category(UnitTests.class)
  public void shouldRegisterWebhookHarnessArtifactRegistry() throws IOException {
    ClassLoader classLoader = getClass().getClassLoader();
    String ngTriggerYaml_azurerepo_pr =
        Resources.toString(Objects.requireNonNull(classLoader.getResource("ng-trigger-harness_artifact_registry.yaml")),
    ...
    Call<ResponseDTO<UpsertRegistryWebhookResponseDTO>> requestCall = mock(Call.class);
    when(requestCall.execute())
        .thenReturn(
            Response.success(ResponseDTO.newResponse(UpsertRegistryWebhookResponseDTO.builder().status(200).build())));
    when(webhookEventClient.upsertRegistryWebhook(any())).thenReturn(requestCall);
    NGTriggerEntity ngTriggerEntity =
        NGTriggerEntity.builder()
            .projectIdentifier("proj")
            .orgIdentifier("org")
            ...
            .metadata(NGTriggerMetadata.builder()
                          .webhook(WebhookMetadata.builder()
                                       .type(WebhookTriggerType.HARNESS_ARTIFACT_REGISTRY.getEntityMetadataName())
                                       .harMetadata(HarnessArtifactRegistryMetadata.builder()
    ...
    assertThat(webhookRegistrationStatus.getWebhookAutoRegistrationStatus().getRegistrationResult())
        .isEqualTo(WebhookRegistrationStatus.SUCCESS);
    verify(webhookEventClient, times(1)).upsertRegistryWebhook(any());
  }
}

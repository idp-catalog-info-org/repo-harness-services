...
public class BuildTriggerHelper {
  ...
  public Optional<String> fetchPipelineYamlForTrigger(
      TriggerDetails triggerDetails, ScopeInfo scopeInfo, boolean isParentIdQueryingEnabled) {
    PMSPipelineResponseDTO response = fetchPipelineForTrigger(triggerDetails, scopeInfo, isParentIdQueryingEnabled);
    return response != null ? Optional.of(response.getYamlPipeline()) : Optional.empty();
  }

  public PMSPipelineResponseDTO fetchPipelineForTrigger(
      TriggerDetails triggerDetails, ScopeInfo scopeInfo, boolean isParentIdQueryingEnabled) {
  ...
  public Optional<String> fetchResolvedTemplatesPipelineForTrigger(
      TriggerDetails triggerDetails, ScopeInfo scopeInfo, boolean isParentIdQueryingEnabled) {
    NGTriggerEntity ngTriggerEntity = triggerDetails.getNgTriggerEntity();
    NGTriggerConfigV2 ngTriggerConfigV2 = triggerDetails.getNgTriggerConfigV2();
    TemplatesResolvedPipelineResponseDTO response =
        NGRestUtils.getResponse(pipelineServiceClient.getResolvedTemplatesPipelineByIdentifier(
            ngTriggerEntity.getTargetIdentifier(), ngTriggerEntity.getAccountId(),
            isParentIdQueryingEnabled ? scopeInfo.getOrgIdentifier() : ngTriggerEntity.getOrgIdentifier(),
            isParentIdQueryingEnabled ? scopeInfo.getProjectIdentifier() : ngTriggerEntity.getProjectIdentifier(),
            ngTriggerConfigV2.getPipelineBranchName(), null, false, "true"));
  ...
  public Map<String, JsonNode> fetchTriggerBuildSpecMap(NGTriggerEntity ngTriggerEntity) throws IOException {
    ...
    return JsonNodeUtils.getMap(jsonNode.get("spec"), "spec");
  }
  ...
  public BuildTriggerOpsData generateBuildTriggerOpsDataForManifest(TriggerDetails triggerDetails, String pipelineYml)
    ...
    if (triggerManifestSpecMap.containsKey(STAGE_IDENTIFIER) && triggerManifestSpecMap.containsKey("manifestRef")) {
      ...
      List<String> keys = Arrays.asList("pipeline.stages.stage[identifier:STAGE_REF].spec.serviceConfig."
              + "serviceDefinition.spec.manifests.manifest[identifier:BUILD_REF]",
          "pipeline.stages.stage[identifier:STAGE_REF].spec.serviceConfig.stageOverrides.manifests.manifest[identifier:"
              + "BUILD_REF]",
          "pipeline.stages.PARALLEL.stage[identifier:STAGE_REF].spec.serviceConfig.serviceDefinition.spec.manifests."
              + "manifest[identifier:BUILD_REF]",
          "pipeline.stages.PARALLEL.stage[identifier:STAGE_REF].spec.serviceConfig.stageOverrides.manifests.manifest["
              + "identifier:BUILD_REF]");
  ...
  public BuildTriggerOpsData generateBuildTriggerOpsDataForArtifact(TriggerDetails triggerDetails, String pipelineYml)
    ...
    if (triggerArtifactSpecMap.containsKey(STAGE_IDENTIFIER) && triggerArtifactSpecMap.containsKey("artifactRef")) {
      ...
      if (buildRef.equals("primary")) {
        ...
        keys.add("pipeline.stages.parallel.stage[identifier:STAGE_REF].spec.serviceConfig.serviceDefinition.spec."
            + "artifacts.primary");
        keys.add(
            "pipeline.stages.parallel.stage[identifier:STAGE_REF].spec.serviceConfig.stageOverrides.artifacts.primary");
      } else {
        keys.add("pipeline.stages.stage[identifier:STAGE_REF].spec.serviceConfig.serviceDefinition.spec.artifacts."
            + "sidecars.sidecar[identifier:BUILD_REF]");
        keys.add("pipeline.stages.stage[identifier:STAGE_REF].spec.serviceConfig.stageOverrides.artifacts.sidecars."
            + "sidecar[identifier:BUILD_REF]");
        keys.add("pipeline.stages.parallel.stage[identifier:STAGE_REF].spec.serviceConfig.serviceDefinition.spec."
            + "artifacts.sidecars.sidecar[identifier:BUILD_REF]");
        keys.add("pipeline.stages.parallel.stage[identifier:STAGE_REF].spec.serviceConfig.stageOverrides.artifacts."
            + "sidecars.sidecar[identifier:BUILD_REF]");
      }
      pipelineBuildSpecMap = generateFinalMapWithBuildSpecFromPipeline(pipelineYml, stageRef, buildRef, keys);
    }
  ...
  public List<BuildTriggerOpsData> generateBuildTriggerOpsDataForMultiArtifact(TriggerDetails triggerDetails)
    ...
    if (HarnessYamlVersion.V0.equals(triggerDetails.getNgTriggerEntity().getHarnessVersion())) {
      ...
      sources = (ArrayNode) jsonNode.get("spec").get("spec").get("sources");
    }
  ...
  public String fetchValueFromJsonNode(String path, Map<String, Object> map) {
    try {
      ...
      return ((TextNode) evaluateExpression).asText();
    } catch (HintException | EngineExpressionEvaluationException e) {
      // JEXL 3.5 throws on missing properties; pre-3.5 returned null. EngineExpressionEvaluator wraps the
      // JexlException as HintException/EngineExpressionEvaluationException. Treat missing-path probes as EMPTY
      // to preserve the legacy probe-then-fallback contract used by CustomPollingItemGenerator and others.
      return EMPTY;
    }
  }
  ...
  public void validatePollingItemForArtifact(PollingItem pollingItem) {
    ...
    } else if (pollingPayloadData.hasGceImagePayload()) {
      validatePollingItemForGCEImage(pollingItem);
  ...
  private void validatePollingItemForGCEImage(PollingItem pollingItem) {
    GCEImagePayload gceImagePayload = pollingItem.getPollingPayloadData().getGceImagePayload();

    String error = checkFiledValueError("project", gceImagePayload.getProject());
  ...
  public List<GCEImageLabel> validateAndFetchGCEImageLabelsListFromJsonNode(
      BuildTriggerOpsData buildTriggerOpsData, String key) {
    List<GCEImageLabel> labels = buildTriggerOpsData.getPipelineBuildSpecMap().containsKey(key)
        ? JsonUtils.asList(((JsonNode) buildTriggerOpsData.getPipelineBuildSpecMap().get(key)).asText(),
              new TypeReference<List<GCEImageLabel>>() {})
        : Collections.emptyList();
    ...
    if (isEmpty(labels)) {
      ...
      labels = JsonUtils.asList(evaluateExpression.toString(), new TypeReference<List<GCEImageLabel>>() {});
    }

    return labels;
  }

  public List<GCEImageFilter> validateAndFetchGCEImageFiltersListFromJsonNode(
      BuildTriggerOpsData buildTriggerOpsData, String key) {
    List<GCEImageFilter> filters = buildTriggerOpsData.getPipelineBuildSpecMap().containsKey(key)
        ? JsonUtils.asList(((JsonNode) buildTriggerOpsData.getPipelineBuildSpecMap().get(key)).asText(),
              new TypeReference<List<GCEImageFilter>>() {})
        : Collections.emptyList();
    ...
    if (isEmpty(filters)) {
      ...
      filters = JsonUtils.asList(evaluateExpression.toString(), new TypeReference<List<GCEImageFilter>>() {});
    }
  ...
  public HelmVersion getHelmVersionFromString(String helmVersion) {
    if (helmVersion.equals("v2")) {
      return HelmVersion.V2;
    } else if (helmVersion.equals("v3")) {
      return HelmVersion.V3;
    } else if (helmVersion.equals("v4")) {
  ...
  public Pair<String, String> getImageAndTagFromLocation(String location) {
    String[] strings = location.split(":");
    return Pair.of(strings[0], strings[1]);
  }

  public Pair<String, String> getChartNameAndVersionFromLocation(String location) {
    String[] strings = location.split("@");
    return Pair.of(strings[0], strings[1]);
  }
}

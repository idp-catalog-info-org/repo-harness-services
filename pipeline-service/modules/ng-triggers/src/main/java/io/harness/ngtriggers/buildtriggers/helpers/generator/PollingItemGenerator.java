...
public interface PollingItemGenerator {
  ...
  default PollingItem.Builder getBaseInitializedPollingItem(NGTriggerEntity ngTriggerEntity,
    ...
    // We need to pass updated organizationId and projectId so when pollingId is saved, it is saved with updated scope
    // Adding parentUniqueId is un-needed because the scopeInfo is recalculated when pollingDoc is saved.
    pollingItem.setCategory(category).setQualifier(
        Qualifier.newBuilder()
            ...
            .setParentUniqueId(
                emptyIfNull(isParentIdQueryingEnabled ? scopeInfo.getUniqueId() : ngTriggerEntity.getParentUniqueId()))
            .build());

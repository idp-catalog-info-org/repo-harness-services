...
public class WebhookEventPayloadParser {
  ...
  public WebhookPayloadData convertWebhookResponse(
    ...
    boolean isGitlabTriggerTagEventEnabled =
        pmsFeatureFlagHelper.isEnabled(triggerWebhookEvent.getAccountId(), FeatureName.CDS_GITLAB_TRIGGER_TAG_EVENT);
    GitProvider gitProvider = webhookParserSCMService.obtainWebhookSource(triggerWebhookEvent.getHeaders());
    WebhookPayload parsedPayload =
        webhookParserSCMService.parseWebhookPayload(parseWebhookResponse, gitProvider, isGitlabTriggerTagEventEnabled);

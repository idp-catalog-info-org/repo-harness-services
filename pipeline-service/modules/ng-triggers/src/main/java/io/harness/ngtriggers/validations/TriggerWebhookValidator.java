...
public class TriggerWebhookValidator {
  ...
  private ValidationResult validateTriggers(TriggerWebhookEvent triggerWebhookEvent) {
    ...
    List<NGTriggerEntity> triggersForAccount = ngTriggerService.findTriggersForCustomWehbook(triggerWebhookEvent, true);
    ...
    if (isEmpty(triggersForAccount)) {
      ...
      if (isNotBlank(triggerWebhookEvent.getParentUniqueId())) {
        errorMsg.append(", ParentUniqueId: ").append(triggerWebhookEvent.getParentUniqueId());
      }
      builder.success(false).message(errorMsg.toString());
    } else if (!isEmpty(triggersForAccount.get(0).getCustomWebhookToken())) {
      String errorMsg = "This webhook url is no longer supported for newly created custom webhook triggers. We are "
          + "migrating to new webhook urls using custom webhook tokens in the url.";

...
public class UrlHelper {
  ...
  public String buildApiExecutionUrlV2(String uuid, String accountIdentifier) {
    return format("%sgateway/pipeline/api/webhook/triggerExecutionDetailsV2/%s?accountIdentifier=%s",
        getBaseUrl(accountIdentifier), uuid, accountIdentifier);
  }

  public String buildUiUrl(
      String accountIdentifier, String orgIdentifier, String projectIdentifier, String pipelineIdentifier) {
    return format("%s%s#/account/%s/all/orgs/%s/projects/%s/deployments?pipelineIdentifier=%s&page=0",
  ...
  public String buildUiSetupUrl(
      String accountIdentifier, String orgIdentifier, String projectIdentifier, String pipelineIdentifier) {
    return format("%s%s#/account/%s/all/orgs/%s/projects/%s/pipelines/%s/pipeline-studio/",
        getBaseUrl(accountIdentifier), NG_UI_PATH_PREFIX, accountIdentifier, orgIdentifier, projectIdentifier,
        pipelineIdentifier);
  }
}

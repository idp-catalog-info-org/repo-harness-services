...
import io.harness.gitsync.common.utils.GitProviderUtils;
...
public class SCMFilePathEvaluatorOnManager extends SCMFilePathEvaluator {
  ...
  private PmsFeatureFlagEvaluator pmsFeatureFlagEvaluator;
  ...
  public List<TriggerFilepathResponse> execute(FilterRequestData filterRequestData,
    ...
    try {
      ...
      List<TriggerFilepathResponse> responses = new ArrayList<>();
      for (TriggerCondition triggerCondition : params.getTriggerCondition()) {
        boolean matched = false;
        for (String filepath : changedFiles) {
          if (ConditionEvaluator.evaluate(filepath, triggerCondition.getValue(), triggerCondition.getOperator())) {
            responses.add(TriggerFilepathResponse.builder()
                              .triggerEntityId(triggerCondition.getTriggerEntityId())
                              .scmPathFilterEvaluationTaskResponse(
                                  ScmPathFilterEvaluationTaskResponse.builder().matched(true).build())
                              .build());
            matched = true;
            break;
          }
  ...
  public Set<String> getChangedFileset(
    ...
    if (params.getPrNumber() > 0) {
      ...
      CompareCommitsResponse compareCommitsResponse = null;
      if (GitProviderUtils.isBitbucketOnPrem(params.getScmConnector())
          && !params.isBBOnpremCompareCommitsIssueFixFFEnabled()) {

...
import io.harness.ngtriggers.validations.result.ValidationResult.ValidationResultBuilder;
...
public class ArtifactTriggerValidator implements TriggerValidator {
  ...
  private static final List<String> v1ArtifactTypesSupported = new ArrayList<>();

  static {
    for (ArtifactType artifactType : ArtifactType.values()) {
      artifactTypesSupported.add(artifactType.getValue());
    }
    for (io.harness.ngtriggers.beans.source.v1.artifact.ArtifactType artifactType :
        io.harness.ngtriggers.beans.source.v1.artifact.ArtifactType.values()) {
      v1ArtifactTypesSupported.add(artifactType.getValue());
    }
  }
  ...
  public ValidationResult validate(
    ...
    try {
      Optional<String> pipelineYmlOptional = validationHelper.fetchResolvedTemplatesPipelineForTrigger(
      ...
      validateBasedOnArtifactType(buildTriggerOpsData, scopeInfo, isParentIdQueryingEnabled);
    } catch (Exception e) {
  ...
  public void validateBasedOnArtifactType(
      BuildTriggerOpsData buildTriggerOpsData, ScopeInfo scopeInfo, boolean isParentIdQueryingEnabled) {
    String version = buildTriggerOpsData.getTriggerDetails().getNgTriggerEntity().getHarnessVersion();
    String typeFromTrigger = validationHelper.fetchBuildType(buildTriggerOpsData.getTriggerSpecMap());
    if ((HarnessYamlVersion.isV1(version) && !v1ArtifactTypesSupported.contains(typeFromTrigger))
        || (!HarnessYamlVersion.isV1(version) && !artifactTypesSupported.contains(typeFromTrigger))) {
      throw new InvalidRequestException(
    ...
    PollingItem pollingItem =
        pollingItemGenerator.generatePollingItem(buildTriggerOpsData, scopeInfo, isParentIdQueryingEnabled);

...
@Singleton
@OwnedBy(PIPELINE)
public class SourceRepoTypeTriggerFilter implements TriggerFilter, TriggerCriteriaFilter {
  @Override
  public void applyCriteria(Criteria criteria, FilterRequestData filterRequestData) {
    criteria.and("metadata.webhook.type")
        .regex(filterRequestData.getWebhookPayloadData().getOriginalEvent().getSourceRepoType(),
            CASE_INSENSITIVE_MONGO_OPTIONS);
  }
  ...
  public WebhookEventMappingResponse applyFilter(FilterRequestData filterRequestData) {
    ...
    if (isEmpty(filteredList)) {
      String msg = String.format("No enabled trigger found for sourceRepoType %s for project: %s",

...
import io.harness.ngtriggers.beans.source.webhook.WebhookTriggerConfigV2.WebhookTriggerConfigV2Builder;
...
import io.harness.ngtriggers.beans.source.webhook.v2.github.event.GithubPullRequestReviewSpec;
...
import io.harness.ngtriggers.beans.source.webhook.v2.gitlab.event.GitlabPipelineSpec;
...
import io.harness.ngtriggers.beans.source.webhook.v2.spec.AwsCodeCommitSpec.AwsCodeCommitSpecBuilder;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.BitbucketSpec;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.BitbucketSpec.BitbucketSpecBuilder;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.CustomTriggerSpec;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.CustomTriggerSpec.CustomTriggerSpecBuilder;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.GithubSpec;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.GithubSpec.GithubSpecBuilder;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.GitlabSpec;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.GitlabSpec.GitlabSpecBuilder;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.HarnessSpec;
import io.harness.ngtriggers.beans.source.webhook.v2.spec.HarnessSpec.HarnessSpecBuilder;
...
public class NgTriggerConfigAdaptor {
    ...
    if (githubTriggerSpec.getEvent() == WebhookEvent.PULL_REQUEST) {
      ...
      githubSpecBuilder.spec(
          GithubPRSpec.builder()
              ...
              .build());
    } else if (githubTriggerSpec.getEvent() == WebhookEvent.PULL_REQUEST_REVIEW) {
      githubSpecBuilder.type(GithubTriggerEvent.PULL_REQUEST_REVIEW);
      githubSpecBuilder.spec(
          GithubPullRequestReviewSpec.builder()
              .connectorRef(githubTriggerSpec.getGitRepoSpec().getIdentifier())
              .repoName(githubTriggerSpec.getGitRepoSpec().getRepoName())
              .headerConditions(mapToTriggerEventDataCondition(githubTriggerSpec.getHeaderConditions()))
              .payloadConditions(mapToTriggerEventDataCondition(githubTriggerSpec.getPayloadConditions()))
              .jexlCondition(githubTriggerSpec.getJexlCondition())
              .autoAbortPreviousExecutions(false)
              .actions(getGithubPullRequestReviewActions(githubTriggerSpec))
              .build());
    ...
    if (gitlabTriggerSpec.getEvent() == WebhookEvent.MERGE_REQUEST) {
      ...
      gitlabSpecBuilder.spec(
          GitlabPushSpec.builder()
              ...
              .autoAbortPreviousExecutions(false)
              .build());
    } else if (gitlabTriggerSpec.getEvent() == WebhookEvent.PIPELINE_HOOK) {
      gitlabSpecBuilder.type(GitlabTriggerEvent.PIPELINE_HOOK);
      gitlabSpecBuilder.spec(
          GitlabPipelineSpec.builder()
              .connectorRef(gitlabTriggerSpec.getGitRepoSpec().getIdentifier())
              .repoName(gitlabTriggerSpec.getGitRepoSpec().getRepoName())
              .headerConditions(mapToTriggerEventDataCondition(gitlabTriggerSpec.getHeaderConditions()))
              .payloadConditions(mapToTriggerEventDataCondition(gitlabTriggerSpec.getPayloadConditions()))
  ...
  private List<GithubPullRequestReviewAction> getGithubPullRequestReviewActions(GithubTriggerSpec githubTriggerSpec) {
    if (isEmpty(githubTriggerSpec.getActions())) {
      return emptyList();
    }

    List<GitAction> gitActions =
        getGitActions(githubTriggerSpec.getActions(), Arrays.asList(GithubPullRequestReviewAction.values()));
    return gitActions.stream().map(gitAction -> (GithubPullRequestReviewAction) gitAction).collect(toList());
  }

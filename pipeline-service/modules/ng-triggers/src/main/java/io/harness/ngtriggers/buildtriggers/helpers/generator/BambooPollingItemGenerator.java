...
public class BambooPollingItemGenerator implements PollingItemGenerator {
  ...
  public PollingItem generatePollingItem(
    ...
    if (HarnessYamlVersion.isV1(ngTriggerEntity.getHarnessVersion())) {
      ...
      artifactPathKey = Constants.PATHS;
      planKeyKey = Constants.PLAN_KEY;
    } else {
      connectorRefKey = "spec.connectorRef";
      artifactPathKey = "spec.artifactPaths";
      planKeyKey = "spec.planKey";
    }
    String connectorRef = buildTriggerHelper.validateAndFetchFromJsonNode(buildTriggerOpsData, connectorRefKey);
    String planKey = buildTriggerHelper.validateAndFetchFromJsonNode(buildTriggerOpsData, planKeyKey);
    List<String> artifactPath =
        buildTriggerHelper.validateAndFetchStringListFromJsonNode(buildTriggerOpsData, artifactPathKey);
    return builder

...
public class TriggerSetupUsageHelper {
  ...
  public void publishSetupUsageEvent(NGTriggerEntity ngTriggerEntity, List<EntityDetailProtoDTO> referredEntities,
    ...
    deleteExistingSetupUsages(ngTriggerEntity, scopeInfo, isParentIdQueryingEnabled);
    if (EmptyPredicate.isEmpty(referredEntities)) {
      return;
    }

    String accountId = ngTriggerEntity.getAccountId();
    TriggerReferenceProtoDTO.Builder triggerReferenceProtoDTOBuilder =
        TriggerReferenceProtoDTO.newBuilder()
    ...
    if (isParentIdQueryingEnabled) {
      triggerReferenceProtoDTOBuilder.setParentUniqueId(StringValue.of(emptyIfNull(scopeInfo.getUniqueId())));
    }
    EntityDetailProtoDTO referredEntity = EntityDetailProtoDTO.newBuilder()
                                              .setTriggerRef(triggerReferenceProtoDTOBuilder.build())
  ...
  public void deleteExistingSetupUsages(
    ...
    EntityDetailProtoDTO triggerDetails = EntityDetailProtoDTO.newBuilder()
                                              .setTriggerRef(triggerReferenceProtoDTOBuilder.build())
                                              .setType(EntityTypeProtoEnum.TRIGGERS)
                                              .build();
    EntitySetupUsageCreateV2DTO entityReferenceDTO = EntitySetupUsageCreateV2DTO.newBuilder()

  ...
  public WebhookEventMappingResponse applyFilter(FilterRequestData filterRequestData) {
    ...
    if (isParentIdQueryingEnabled) {
      List<String> parentUniqueIds =
          ngTriggerEntities.stream().map(NGTriggerEntity::getParentUniqueId).collect(Collectors.toList());
    ...
    if (isEmpty(ngTriggerEntities)) {
      ...
      prepareTriggerDetails(
          matchedTriggers, ngTriggerEntities, parentUniqueIdToScopeInfoMap, isParentIdQueryingEnabled);

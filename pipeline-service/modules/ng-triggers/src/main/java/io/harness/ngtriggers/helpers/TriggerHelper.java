...
public class TriggerHelper {
  public Map<String, Object> buildJsonObjectFromAmbiance(TriggerPayload triggerPayload) {
    ...
    switch (parsedPayload.getPayloadCase()) {
      ...
      case PUSH:
        ...
        jsonObject.put(CHANGED_FILES, new ArrayList<>(triggerPayload.getChangedFilesList()));
        break;
      case RELEASE:
      ...
      case BRANCH:
        String branchAction;
        if ((Action.CREATE).equals(parsedPayload.getBranch().getAction())) {
          branchAction = CREATE_EVENT_TYPE;
        } else {
          branchAction = DELETE_EVENT_TYPE;
        }
        jsonObject.put(BRANCH, parsedPayload.getBranch().getRef().getName().replaceFirst("^refs/heads/", ""));
        jsonObject.put(EVENT, branchAction);
        jsonObject.put(TYPE, WEBHOOK_TYPE);
      ...
      case TAG:
        String tagAction;
        if ((Action.CREATE).equals(parsedPayload.getBranch().getAction())) {
          tagAction = CREATE_EVENT_TYPE;
        } else {
          tagAction = DELETE_EVENT_TYPE;
        }
        jsonObject.put(EVENT, tagAction);
        jsonObject.put(TYPE, WEBHOOK_TYPE);
      ...
      case MERGEQUEUE:
        ...
        jsonObject.put(BRANCH, mergeQueueHook.getBranch());
        jsonObject.put(TARGET_BRANCH, mergeQueueHook.getBranch());
        jsonObject.put(COMMIT_SHA, mergeQueueHook.getSha());
        jsonObject.put(EVENT, MERGE_QUEUE_EVENT_TYPE);
        jsonObject.put(TYPE, WEBHOOK_TYPE);
        jsonObject.put(REPO_URL, mergeQueueHook.getRepo().getLink());
        jsonObject.put(GIT_USER, gitUserOf(mergeQueueHook.getSender()));
        // SOURCE_BRANCH is deliberately absent, same as PUSH above: a merge queue commit has no single source branch.
        break;
      default:
    ...
    jsonObject.put(HEADER, new HashMap<>(triggerPayload.getHeadersMap()));
  ...
  private String getGitUser(ParsedPayload parsedPayload) {
    return gitUserOf(parsedPayload.getPr().getSender());
  }

  /**
   * Harness Code and Azure senders often carry a name but no login, so a bare getLogin() would leave
   * &lt;+trigger.gitUser&gt; empty on those providers.
   */
  private String gitUserOf(User sender) {

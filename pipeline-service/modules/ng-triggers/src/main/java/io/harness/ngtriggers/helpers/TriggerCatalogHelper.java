  ...
  public List<TriggerCatalogItem> getTriggerTypeToCategoryMapping(String accountIdentifier) {
    final Map<NGTriggerType, List<TriggerCatalogType>> triggerCategoryListMap =
        Arrays.stream(TriggerCatalogType.values())
            ...
            .filter(enumFilter.filter(accountIdentifier, FeatureName.CDS_EVENT_BRIDGE_WEBHOOK))
            .filter(enumFilter.filter(accountIdentifier, FeatureName.HAR_TRIGGERS))
            .filter(enumFilter.filter(accountIdentifier, FeatureName.SYSTEM_EVENTS_TRIGGERS))

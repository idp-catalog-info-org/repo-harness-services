...
import static io.harness.ngtriggers.beans.source.artifact.ArtifactType.GCE_IMAGE;
...
import io.harness.ngtriggers.beans.source.v1.artifact.ManifestType;
...
public class GeneratorFactory {
  ...
  private final GCEImagePollingItemGenerator gceImagePollingItemGenerator;
  ...
  private PollingItemGenerator retrievePollingItemGeneratorForArtifact(BuildTriggerOpsData buildTriggerOpsData) {
    ...
    if (GCR.getValue().equals(buildType) || ArtifactType.GCR.getValue().equals(buildType)) {
      return gcrPollingItemGenerator;
    } else if (ECR.getValue().equals(buildType) || ArtifactType.ECR.getValue().equals(buildType)) {
      return ecrPollingItemGenerator;
    } else if (DOCKER_REGISTRY.getValue().equals(buildType)
        || ArtifactType.DOCKER_REGISTRY.getValue().equals(buildType)) {
      return dockerRegistryPollingItemGenerator;
    } else if (ARTIFACTORY_REGISTRY.getValue().equals(buildType)
        || ArtifactType.ARTIFACTORY_REGISTRY.getValue().equals(buildType)) {
      return artifactoryRegistryPollingItemGenerator;
    } else if (ACR.getValue().equals(buildType) || ArtifactType.ACR.getValue().equals(buildType)) {
      return acrPollingItemGenerator;
    } else if (AMAZON_S3.getValue().equals(buildType) || ArtifactType.AMAZON_S3.getValue().equals(buildType)) {
      return s3PollingItemGenerator;
    } else if (JENKINS.getValue().equals(buildType) || ArtifactType.JENKINS.getValue().equals(buildType)) {
      return jenkinsPollingItemGenerator;
    } else if (CUSTOM_ARTIFACT.getValue().equals(buildType)
        || ArtifactType.CUSTOM_ARTIFACT.getValue().equals(buildType)) {
      return customPollingItemGenerator;
    } else if (GoogleArtifactRegistry.getValue().equals(buildType)
        || ArtifactType.GoogleArtifactRegistry.getValue().equals(buildType)) {
      return garPollingItemGenerator;
    } else if (GITHUB_PACKAGES.getValue().equals(buildType)
        || ArtifactType.GITHUB_PACKAGES.getValue().equals(buildType)) {
      return githubPackagesPollingItemGenerator;
    } else if (NEXUS2_REGISTRY.getValue().equals(buildType)
        || ArtifactType.NEXUS2_REGISTRY.getValue().equals(buildType)) {
      return nexus2RegistryPollingItemGenerator;
    } else if (NEXUS3_REGISTRY.getValue().equals(buildType)
        || ArtifactType.NEXUS3_REGISTRY.getValue().equals(buildType)) {
      return nexus3PollingItemGenerator;
    } else if (AZURE_ARTIFACTS.getValue().equals(buildType)
        || ArtifactType.AZURE_ARTIFACTS.getValue().equals(buildType)) {
      return azureArtifactsPollingItemGenerator;
    } else if (AMI.getValue().equals(buildType) || ArtifactType.AMI.getValue().equals(buildType)) {
      return amiPollingItemGenerator;
    } else if (GCE_IMAGE.getValue().equals(buildType)) {
      return gceImagePollingItemGenerator;
    } else if (GOOGLE_CLOUD_STORAGE.getValue().equals(buildType)
        || ArtifactType.GOOGLE_CLOUD_STORAGE.getValue().equals(buildType)) {
      return googleCloudStoragePollingItemGenerator;
    } else if (BAMBOO.getValue().equals(buildType) || ArtifactType.BAMBOO.getValue().equals(buildType)) {
  ...
  private PollingItemGenerator retrievePollingItemGeneratorForManifest(BuildTriggerOpsData buildTriggerOpsData) {
    String buildType = buildTriggerHelper.fetchBuildType(buildTriggerOpsData.getTriggerSpecMap());
    if (HELM_MANIFEST.getValue().equals(buildType) || ManifestType.HELM_MANIFEST.getValue().equals(buildType)) {
  ...
  private PollingItemGenerator retrievePollingItemGeneratorForHelmChart(BuildTriggerOpsData buildTriggerOpsData) {
    ...
    if (BuildStoreType.HTTP.getValue().equals(storeTypeFromTrigger)
        || io.harness.ngtriggers.beans.source.v1.artifact.store.BuildStoreType.HTTP.getValue().equals(
            storeTypeFromTrigger)) {
      return httpHelmPollingItemGenerator;
    } else if (BuildStoreType.S3.getValue().equals(storeTypeFromTrigger)
        || io.harness.ngtriggers.beans.source.v1.artifact.store.BuildStoreType.S3.getValue().equals(
            storeTypeFromTrigger)) {
      return s3HelmPollingItemGenerator;
    } else if (BuildStoreType.GCS.getValue().equals(storeTypeFromTrigger)
        || io.harness.ngtriggers.beans.source.v1.artifact.store.BuildStoreType.GCS.getValue().equals(
            storeTypeFromTrigger)) {

...
@OwnedBy(PIPELINE)
@Schema(name = "ExecutionStatus",
    description = "Execution Status of the entity. Valid values (PascalCase): Running, AsyncWaiting, TaskWaiting,"
        + " TimedWaiting, Failed, Errored, IgnoreFailed, NotStarted, Expired, Aborted, Discontinuing, Queued,"
...
public enum ExecutionStatus {
  ...
  @JsonProperty("UploadWaiting") UPLOADWAITING(Status.UPLOAD_WAITING, "UploadWaiting"),
  ...
  @JsonIgnore QUEUED_STEP_CONCURRENCY(Status.QUEUED_STEP_LIMIT_REACHED, "Queued"),
  @JsonIgnore STARTING(Status.STARTING_QUEUED_STEP, "Queued"),
  @JsonIgnore STARTING_PLAN_CREATION(Status.STARTING_PLAN_CREATION, "Queued");
  ...
  public static ExecutionStatus getExecutionStatus(@JsonProperty("type") String displayName) {
    for (ExecutionStatus executionStatus : ExecutionStatus.values()) {
      if (executionStatus.displayName != null && executionStatus.displayName.equalsIgnoreCase(displayName)) {
        return executionStatus;
      }
      if (executionStatus.name().equalsIgnoreCase(displayName)) {
        return executionStatus;
      }
    }
  ...
  public static List<ExecutionStatus> getCompleteListWithInternalStatuses(List<ExecutionStatus> statusList) {
    if (statusList.contains(ExecutionStatus.QUEUED)) {
      List<ExecutionStatus> mutableStatusList = new ArrayList<>(statusList);
      mutableStatusList.add(ExecutionStatus.QUEUED_PLAN_CREATION);
      mutableStatusList.add(ExecutionStatus.STARTING_PLAN_CREATION);

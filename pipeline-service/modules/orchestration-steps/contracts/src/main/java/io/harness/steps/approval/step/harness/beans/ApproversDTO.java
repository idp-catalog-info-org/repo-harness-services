/*
 * Copyright 2021 Harness Inc. All rights reserved.
 * Use of this source code is governed by the PolyForm Free Trial 1.0.0 license
 * that can be found in the licenses directory at the root of this repository, also available at
 * https://polyformproject.org/wp-content/uploads/2020/05/PolyForm-Free-Trial-1.0.0.txt.
 */

package io.harness.steps.approval.step.harness.beans;

import static io.harness.annotations.dev.HarnessTeam.CDC;

import io.harness.annotations.dev.CodePulse;
import io.harness.annotations.dev.HarnessModuleComponent;
import io.harness.annotations.dev.OwnedBy;
import io.harness.annotations.dev.ProductModule;
import io.harness.common.ParameterFieldHelper;
import io.harness.exception.InvalidRequestException;

import io.swagger.v3.oas.annotations.media.Schema;
import java.util.ArrayList;
import java.util.List;
import lombok.AccessLevel;
import lombok.Builder;
import lombok.Data;
import lombok.experimental.FieldDefaults;

@OwnedBy(CDC)
@Data
@Builder
@FieldDefaults(level = AccessLevel.PRIVATE)
@Schema(name = "Approvers", description = "This contains details of the Approvers")
@CodePulse(
    module = ProductModule.CDS, unitCoverageRequired = false, components = {HarnessModuleComponent.CDS_APPROVALS})
public class ApproversDTO {
  List<String> userGroups;
  List<String> serviceAccounts;
  List<String> disallowedUserEmails;
  int minimumCount;
  boolean disallowPipelineExecutor;

  public static ApproversDTO fromApprovers(Approvers approvers) {
    if (approvers == null) {
      return null;
    }

    Object userGroupsObj = new ArrayList<>();
    if (approvers.getUserGroups() != null && approvers.getUserGroups().fetchFinalValue() != null) {
      userGroupsObj = approvers.getUserGroups().fetchFinalValue();
    }
    if (userGroupsObj instanceof String) {
      throw new InvalidRequestException(String.format(
          "User groups should be a list of user group identifiers, got a single string value '%s'", userGroupsObj));
    }
    if (!(userGroupsObj instanceof List)) {
      throw new InvalidRequestException(
          String.format("User groups should be a list of user group identifiers, got value %s of type %s",
              userGroupsObj, userGroupsObj.getClass().getSimpleName()));
    }
    List<String> userGroups = (List<String>) userGroupsObj;

    Object serviceAccountsObj = new ArrayList<>();
    if (approvers.getServiceAccounts() != null && approvers.getServiceAccounts().fetchFinalValue() != null) {
      serviceAccountsObj = approvers.getServiceAccounts().fetchFinalValue();
    }
    if (serviceAccountsObj instanceof String) {
      throw new InvalidRequestException(String.format(
          "Service Accounts should be a list of service account identifiers, got a single string value '%s'",
          serviceAccountsObj));
    }
    if (!(serviceAccountsObj instanceof List)) {
      throw new InvalidRequestException(
          String.format("Service Accounts should be a list of service account identifiers, got value %s of type %s",
              serviceAccountsObj, serviceAccountsObj.getClass().getSimpleName()));
    }
    List<String> serviceAccounts = (List<String>) serviceAccountsObj;

    Object disallowedUserEmailsObj = new ArrayList<>();
    if (approvers.getDisallowedUserEmails() != null && approvers.getDisallowedUserEmails().fetchFinalValue() != null) {
      disallowedUserEmailsObj = approvers.getDisallowedUserEmails().fetchFinalValue();
    }
    if (!(disallowedUserEmailsObj instanceof List)) {
      throw new InvalidRequestException(
          String.format("Disallowed User Emails should be a list of emails, got value %s of type %s",
              disallowedUserEmailsObj, disallowedUserEmailsObj.getClass().getSimpleName()));
    }
    List<String> disallowedUserEmails = (List<String>) disallowedUserEmailsObj;

    if (userGroups.size() + serviceAccounts.size() < 1) {
      throw new InvalidRequestException(
          "At least 1 User Group or Service Account should be configured. Both cannot be empty.");
    }

    Object minCountObj = approvers.getMinimumCount().fetchFinalValue();
    int minimumCount;
    if (minCountObj instanceof String) {
      try {
        minimumCount = Integer.parseInt((String) minCountObj);
      } catch (NumberFormatException ne) {
        throw new InvalidRequestException("The minimum count value (" + minCountObj + ") must be a valid integer");
      }
    } else if (minCountObj instanceof Double && (int) ((double) minCountObj) == (double) minCountObj) {
      minimumCount = (int) ((double) minCountObj);
    } else if (minCountObj instanceof Integer) {
      minimumCount = (int) minCountObj;
    } else {
      throw new InvalidRequestException("The minimum count value (" + minCountObj + ") must be a valid integer");
    }
    if (minimumCount < 1) {
      throw new InvalidRequestException("Minimum count should be > 0");
    }

    return ApproversDTO.builder()
        .userGroups(userGroups)
        .serviceAccounts(serviceAccounts)
        .disallowedUserEmails(disallowedUserEmails)
        .minimumCount(minimumCount)
        .disallowPipelineExecutor(
            (Boolean) ParameterFieldHelper.getBooleanParameterFieldValue(approvers.getDisallowPipelineExecutor()))
        .build();
  }
}

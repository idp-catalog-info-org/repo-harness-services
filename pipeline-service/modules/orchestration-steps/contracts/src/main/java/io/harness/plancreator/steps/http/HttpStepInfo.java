...
public class HttpStepInfo
    extends HttpBaseStepInfo implements PMSStepInfo, Visitable, WithDelegateSelector, WithSecretRef {
  ...
  public Map<String, ParameterField<String>> extractSecretRefs() {
    ...
    if (ParameterField.isNotBlank(certificate)) {
      secretRefMap.put(YAMLFieldNameConstants.CERTIFICATE, certificate);
    }
    if (ParameterField.isNotBlank(certificateKey)) {
      secretRefMap.put(YAMLFieldNameConstants.CERTIFICATE_KEY, certificateKey);
    }
    return secretRefMap;
  }
}

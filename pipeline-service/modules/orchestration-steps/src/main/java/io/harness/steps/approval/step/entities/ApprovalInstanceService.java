...
public interface ApprovalInstanceService {
  ...
  /**
   * This method is fetch approval instances from object store, if FF disabled
   ...
   */
  ApprovalInstance fetchFromObjectStoreWithFallback(
  ...
  /**
   * Fetches all HarnessApprovalInstances for a plan execution, sorted by createdAt ascending (oldest first).
   * Used to aggregate approval activities across all Manual Approval steps run in the same plan execution.
   * @param planExecutionId planExecutionId
   * @return list of all Harness approval instances in chronological order
   */
  List<HarnessApprovalInstance> findAllHarnessApprovalInstancesByPlanExecutionId(@NotEmpty String planExecutionId);

  /**
   * This method is fetch approval instances by planExecutionId
   * Uses - planExecutionId_status_type_nodeExecutionId index
   * @param planExecutionId planExecutionId
   * @return list of approvals
   */
  List<ApprovalInstance> getApprovalInstancesByExecutionId(String planExecutionId);
  ...
  void updateOutputVariablesInApprovalInstance(
      @NotNull ServiceNowApprovalInstance approvalInstance, Map<String, String> outVars);
  void updateOutputVariablesInApprovalInstance(
      @NotNull JiraApprovalInstance approvalInstance, Map<String, String> outVars);
  ...
  void updateLastManualRunTimestamp(String approvalInstanceId, Long timestamp);

  List<ApprovalInstance> getApprovalInstancesByApprovalInstanceIds(String planExecutionId,
      ApprovalStatus approvalStatus, ApprovalType approvalType, String nodeExecutionId, String callbackId,
      List<String> approvalInstanceIds);

  List<ApprovalInstance> getWaitingApprovalsByAccountAndType(

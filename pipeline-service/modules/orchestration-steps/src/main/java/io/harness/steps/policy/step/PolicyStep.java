...
@Slf4j
@Singleton
public class PolicyStep implements SyncExecutable<StepBaseParameters> {
  ...
  public StepResponse executeSync(Ambiance ambiance, StepBaseParameters stepParameters, StepInputPackage inputPackage,
    ...
    stepsInstrumentationHelper.publishStepEvent(
        ambiance, StepExecutionTelemetryEventDTO.builder().stepType(STEP_TYPE.getType()).build());
    ...
    try {
      ...
      opaEvaluationResponseHolder = SafeHttpCall.executeWithErrorMessage(opaServiceClient.evaluateWithCredentialsByID(
          accountId, orgIdentifier, projectIdentifier, policySetsQueryParam,
          PolicyEvalUtils.getEntityMetadataString(
              stepParameters.getName(), AmbianceUtils.getPipelineExecutionIdentifier(ambiance)),
          payloadObject));

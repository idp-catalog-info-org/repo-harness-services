...
import io.harness.steps.approval.step.servicenow.v1.ServiceNowApprovalStepParameters;
...
public class ApprovalUtils {
  ...
  public void handleApprovalExpiryEvent(Ambiance ambiance, ApprovalInstanceService approvalInstanceService,
    ...
    NGLogCallback logCallback =
        new NGLogCallback(logStreamingStepClientFactory, ambiance, ShellScriptTaskNG.COMMAND_UNIT, false);
    logCallback.saveExecutionLog(
        LogHelper.color("Approval instance has expired", LogColor.Red), LogLevel.ERROR, CommandExecutionStatus.FAILURE);
  }

  /**
   * calculates timeout of approvalInstance defaulting to 1d in case of exceptions
   * returns: timeout in seconds
   * */
  public long getTimeoutInSeconds(@NotNull ApprovalInstance approvalInstance) {
    try {
      if (isNull(approvalInstance) || isNull(approvalInstance.getDeadline())
          || approvalInstance.getDeadline() < System.currentTimeMillis()) {
        log.warn("Valid deadline missing in approval instance, defaulting to 1d");
        return SECONDS_IN_A_DAY;
      }

      return Duration.ofMillis(approvalInstance.getDeadline() - System.currentTimeMillis()).get(ChronoUnit.SECONDS);
    } catch (Exception ex) {
  ...
  public ServiceNowApprovalSpecParameters getServiceNowApprovalStepParameters(StepBaseParameters stepBaseParameters) {
    String version = stepBaseParameters.getSpec().getVersion();
    if (HarnessYamlVersion.isV1(version)) {
      return ((ServiceNowApprovalStepParameters) stepBaseParameters.getSpec()).toServiceNowApprovalSPecParametersV0();
    }
    return (ServiceNowApprovalSpecParameters) stepBaseParameters.getSpec();
  }
}

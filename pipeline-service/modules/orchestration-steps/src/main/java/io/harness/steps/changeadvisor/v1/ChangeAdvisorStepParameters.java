/*
 * Copyright 2026 Harness Inc. All rights reserved.
 * Use of this source code is governed by the PolyForm Free Trial 1.0.0 license
 * that can be found in the licenses directory at the root of this repository, also available at
 * https://polyformproject.org/wp-content/uploads/2020/05/PolyForm-Free-Trial-1.0.0.txt.
 */

package io.harness.steps.changeadvisor.v1;

import io.harness.annotation.RecasterAlias;
import io.harness.annotations.dev.HarnessTeam;
import io.harness.annotations.dev.OwnedBy;
import io.harness.plancreator.steps.common.SpecParameters;
import io.harness.pms.yaml.HarnessYamlVersion;
import io.harness.pms.yaml.ParameterField;
import io.harness.steps.approval.step.harness.beans.Approvers;
import io.harness.steps.changeadvisor.ChangeAdvisorStepSpecParameters;

import java.util.List;
import lombok.Builder;
import lombok.Value;

@OwnedBy(HarnessTeam.PIPELINE)
@Value
@Builder
@RecasterAlias("io.harness.steps.changeadvisor.v1.ChangeAdvisorStepParameters")
public class ChangeAdvisorStepParameters implements SpecParameters {
  ParameterField<String> mode;
  ParameterField<String> policyPack;
  ParameterField<Integer> timeoutMinutes;
  ParameterField<List<String>> presets;
  ParameterField<String> env;
  Approvers approvers;

  @Override
  public String getVersion() {
    return HarnessYamlVersion.V1;
  }

  public ChangeAdvisorStepSpecParameters toChangeAdvisorStepSpecParameters() {
    return ChangeAdvisorStepSpecParameters.builder()
        .mode(mode)
        .policyPack(policyPack)
        .timeoutMinutes(timeoutMinutes)
        .presets(presets)
        .env(env)
        .approvers(approvers)
        .build();
  }
}

...
package io.harness.steps.approval.step.jira.step;
...
import io.harness.execution.step.JiraApprovalStepExecutionDetails;
...
public class JiraApprovalStep extends PipelineAsyncExecutable {
  ...
  public AsyncExecutableResponse executeAsyncAfterRbac(
    ...
    JiraApprovalInstance approvalInstance = HarnessYamlVersion.isV1(AmbianceUtils.getPipelineVersion(ambiance))
        ? JiraApprovalInstance.fromUnifiedStepParameters(ambiance, stepParameters)
    ...
    try {
      ...
      if (instance.getStatus() == ApprovalStatus.FAILED) {
        ...
        try {
          ...
          log.error("Error while updating jira approval step execution details", e);
        }
      ...
      Outcome outcome = HarnessYamlVersion.isV1(AmbianceUtils.getPipelineVersion(ambiance))
          ? instance.toUnifiedJiraApprovalOutcome(hasApprovalCriteriaMet, hasRejectionCriteriaMet)
          : instance.toJiraApprovalOutcome();

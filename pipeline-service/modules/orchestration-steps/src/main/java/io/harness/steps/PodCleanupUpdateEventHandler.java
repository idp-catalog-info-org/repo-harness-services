...
public class PodCleanupUpdateEventHandler implements NodeStatusUpdateObserver, AsyncInformObserver {
  ...
  public void onNodeStatusUpdate(NodeUpdateInfo nodeUpdateInfo) {
    if (StepGroupStep.STEP_TYPE.getType().equals(
            NodeExecutionContextUtils.getCurrentStepType(nodeUpdateInfo.getNodeExecution()).getType())
        && StatusUtils.isFinalStatus(nodeUpdateInfo.getStatus())) {
      containerStepCleanupHelper.sendCleanupRequest(
          nodeExecutionService.getAmbiance(nodeUpdateInfo.getNodeExecution()));
    }
  }

...
public class CfExecutionPMSPlanCreator extends ChildrenPlanCreator<ExecutionElementConfig> {
    ...
    for (YamlField stepYamlField : stepYamlFields) {
      ...
      responseMap.put(stepYamlField.getNode().getUuid(),
          PlanCreationResponse.builder()
              .dependencies(DependenciesUtils.toDependenciesProtoWithMetadataMap(stepYamlFieldMap,
                  InjectUtils.getDependencyMetadataMapWithInjectType(stepYamlField, ctx, InjectTypes.STEP.toString())))
              .build());
    }
    // Add Steps Node
  ...
  private List<YamlField> getStepYamlFields(PlanCreationContext planCreationContext) {
    ...
    yamlNodes.forEach(yamlNode -> {
      ...
      YamlField injectField = yamlNode.getField(YAMLFieldNameConstants.INSERT);
      if (stepField != null) {
      ...
      } else if (injectField != null) {
        stepFields.add(injectField);
      }
    });
    return stepFields;
  }

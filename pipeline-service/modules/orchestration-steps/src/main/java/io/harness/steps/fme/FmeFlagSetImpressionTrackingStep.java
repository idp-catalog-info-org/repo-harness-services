...
@Slf4j
public class FmeFlagSetImpressionTrackingStep extends FmeBaseStep {
  public static final StepType STEP_TYPE = StepSpecTypeConstants.FME_FLAG_SET_IMPRESSION_TRACKING_STEP_TYPE;
  ...
  protected StepResponse executeFmeStep(
    ...
    log.info("Executing FME Flag Set Impression Tracking step...");
    logCallback.saveExecutionLog("Starting Flag Set Impression Tracking", LogLevel.INFO);

    Scope scope = getScope(ambiance);
    FmeFlagSetImpressionTrackingParameters p = (FmeFlagSetImpressionTrackingParameters) stepParameters.getSpec();
    ...
    Boolean enabled = p.getEnabled() != null ? p.getEnabled().obtainValue() : null;
    ...
    if (enabled == null) {
      throw new FmeInvalidParameterException("Enabled is required");
    }

    logCallback.saveExecutionLog(
        format("Setting impression tracking to '%s' for flag '%s' in environment '%s'", enabled, flagName, environment),
        LogLevel.INFO);
    ...
    ExecutionContext context = ExecutionContext.builder()
                                   ...
                                   .operationName("set impression tracking")
                                   .build();
    ...
    FmeApiExecutor.executeWrapped(
        fmePipelineClient.patchFeatureFlagDefinition(scope.getAccountIdentifier(), scope.getOrgIdentifier(),
            ...
            Collections.singletonList(FmePatchOperation.replace("/impressionsDisabled", !enabled))),
        context, NotFoundBehavior.THROW_DEFINITION_NOT_FOUND,
        (flag, env, errorBody)
            -> format("Error setting impression tracking for flag '%s' in environment '%s'. Error: %s", flag, env,
                errorBody));

    logCallback.saveExecutionLog(
        format("Successfully set impression tracking to '%s' for flag '%s' in environment '%s'", enabled, flagName,
            environment),

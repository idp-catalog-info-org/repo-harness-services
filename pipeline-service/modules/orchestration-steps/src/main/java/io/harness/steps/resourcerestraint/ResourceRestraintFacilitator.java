...
import static io.harness.pms.sdk.core.execution.events.node.facilitate.response.FacilitatorResponse.FacilitatorResponseBuilder;
...
public class ResourceRestraintFacilitator implements Facilitator {
  ...
  public FacilitatorResponse facilitate(
    ...
    IResourceRestraintSpecParameters specParameters =
        ResourceRestraintUtils.getIResourceRestraintSpecParameters((StepBaseParameters) stepParameters);
    ...
    try (AcquiredLock<?> lock = persistentLocker.waitToAcquireLockOptional(
      ...
      Map<String, Object> constraintContext =
          populateConstraintContext(resourceRestraint, specParameters, releaseEntityId, ambiance);
      ...
      try {
        ...
        if (AmbianceUtils.checkIfFeatureFlagEnabled(
                ambiance, FeatureName.PIPE_FIX_RESOURCE_RESTRAINTS_FOR_RETRY_STEPS.name())
            && AmbianceUtils.isUnderRetry(ambiance)) {
          ResourceRestraintInstance instance = resourceRestraintInstanceService.findByReleaseEntityIdAndFqn(
              releaseEntityId, AmbianceUtils.getFQNUsingLevels(ambiance.getLevelsList()));
          if (instance != null) {
            consumerId = instance.getUuid();
            state = instance.getState();
          } else {
            state = constraint.registerConsumerNg(renderedResourceUnit, new ConsumerId(consumerId), permits,
                constraintContext, resourceRestraintRegistry, false);
          }
  ...
  private Map<String, Object> populateConstraintContext(ResourceRestraint resourceRestraint,
      IResourceRestraintSpecParameters stepParameters, String releaseEntityId, Ambiance ambiance) {
    ...
    constraintContext.put(ResourceRestraintInstanceKeys.fqn, AmbianceUtils.getFQNUsingLevels(ambiance.getLevelsList()));

...
import io.harness.execution.step.CustomApprovalStepExecutionDetails;
...
public class CustomApprovalStep extends PipelineAsyncExecutable {
  ...
  public AsyncExecutableResponse executeAsyncAfterRbac(
    ...
    CustomApprovalInstance approvalInstance = HarnessYamlVersion.isV1(AmbianceUtils.getPipelineVersion(ambiance))
        ? CustomApprovalInstance.fromUnifiedStepParameters(ambiance, stepParameters)
        : CustomApprovalInstance.fromStepParameters(ambiance, stepParameters);
    ...
    try {
      ...
      if (instance.getStatus() == ApprovalStatus.FAILED) {
        ...
        try {
          ...
          log.error("Error while updating step execution entity for Custom Approval Step", e);
        }
        throw new ApprovalStepNGException(errorMsg);
      }
      CustomApprovalBaseOutcome outcome = instance.toCustomApprovalOutcome(
          customApprovalResponseData.getTicket(), stepParameters.getSpec().getVersion());
  ...
  private CustomApprovalStepExecutionDetails createCustomApprovalStepExecutionDetailsFromCustomApprovalOutcome(
      CustomApprovalBaseOutcome outcome) {
    if (outcome != null) {

...
public class ResourceRestraintRegistryImpl implements ResourceRestraintRegistry {
  ...
  public List<Consumer> loadConsumers(ConstraintId id, ConstraintUnit unit, boolean hitSecondaryNode) {
    ...
    Iterator<ResourceRestraintInstance> instances = instancesIterators.iterator();
    while (instances.hasNext()) {
      ResourceRestraintInstance instance = instances.next();
      consumers.add(
          Consumer.builder()
              .id(new ConsumerId(instance.getUuid()))
              .state(instance.getState())
              .permits(instance.getPermits())
              .context(ImmutableMap.of(ResourceRestraintInstanceKeys.releaseEntityType, instance.getReleaseEntityType(),
                  ResourceRestraintInstanceKeys.releaseEntityId, instance.getReleaseEntityId()))
              .build());
    }
    return consumers;
  }
  ...
  public boolean registerConsumer(ConstraintId id, ConstraintUnit unit, Consumer consumer, int currentlyRunning) {
    ...
    builder.fqn((String) consumer.getContext().get(ResourceRestraintInstanceKeys.fqn));

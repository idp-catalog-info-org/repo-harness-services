...
import io.harness.execution.step.ServiceNowImportSetStepExecutionDetails;
...
public class ServiceNowImportSetStep extends PipelineTaskExecutable<ServiceNowTaskNGResponse> {
  ...
  public StepResponse handleTaskResultWithSecurityContext(Ambiance ambiance, StepBaseParameters stepParameters,
    ...
    try {
      ...
      StepResponse stepResponse = serviceNowStepHelperService.prepareImportSetStepResponse(() -> taskResponse);
      updateServiceNowImportSetStepExecutionDetails(
          ambiance, taskResponse, stepParameters.getName(), stepResponse.getStatus());
      return stepResponse;
    } catch (Exception ex) {
  ...
  private void updateServiceNowImportSetStepExecutionDetails(
      Ambiance ambiance, ServiceNowTaskNGResponse taskResponse, String stepName, Status status) {
    try {
      if (taskResponse != null && taskResponse.getServiceNowImportSetResponseNG() != null) {
        ServiceNowImportSetResponseNG importSetResponse = taskResponse.getServiceNowImportSetResponseNG();
        String transformMapOutcome = null;
        if (importSetResponse.getServiceNowImportSetTransformMapResultList() != null
            && !importSetResponse.getServiceNowImportSetTransformMapResultList().isEmpty()) {
          transformMapOutcome = importSetResponse.getServiceNowImportSetTransformMapResultList().get(0).getStatus();
        }
        ServiceNowImportSetStepExecutionDetails executionDetails =
            ServiceNowImportSetStepExecutionDetails.builder()
                .stagingTableName(importSetResponse.getStagingTable())
                .importSetNumber(importSetResponse.getImportSet())
                .transformMapOutcome(transformMapOutcome)
                .build();
        stepExecutionEntityService.updateStepExecutionEntity(ambiance, null, executionDetails, stepName, status);
      }
    } catch (Exception ex) {
      log.warn("Failed to update step execution entity for ServiceNow ImportSet step", ex);
    }
  }
  ...
  private void updateStepExecutionEntityOnFailure(Ambiance ambiance, String stepName, String errorMessage) {
    try {
      ...
      log.warn("Failed to update step execution entity on failure for ServiceNow ImportSet step", ex);
    }
  }

...
import io.harness.steps.approval.step.servicenow.ServiceNowApprovalOutCome.ServiceNowApprovalOutComeBuilder;
import io.harness.steps.approval.step.servicenow.ServiceNowApprovalSpecParameters;
import io.harness.steps.approval.step.servicenow.UnifiedServiceNowApprovalOutcome;
...
public class ServiceNowApprovalInstance extends ApprovalInstance implements PersistentIrregularIterable {
  ...
  public static ServiceNowApprovalInstance fromStepParameters(Ambiance ambiance, StepBaseParameters stepParameters) {
    ...
    ServiceNowApprovalSpecParameters specParameters = ApprovalUtils.getServiceNowApprovalStepParameters(stepParameters);
  ...
  public static ServiceNowApprovalInstance fromUnifiedStepParameters(
    ...
    if (!(spec instanceof ServiceNowApprovalSpecParameters)) {
      throw new InvalidRequestException("Invalid spec parameters type for ServiceNow approval");
    }
    ...
    if (specParameters.getRunStepInfo() == null) {
      throw new InvalidRequestException("ServiceNow approval run step is not set");
    }
    ...
    if (ParameterField.isNull(instance.getRetryInterval())) {
      throw new InvalidRequestException("retry interval field for ServiceNow approval cannot be empty");
    }

    if (instance.getRetryInterval().getValue().getTimeoutInMillis() < 10000L) {
      throw new InvalidRequestException("retry interval field for ServiceNow approval cannot be less than 10s");
    }
  ...
  public ServiceNowApprovalOutCome toServiceNowApprovalOutcome(
      boolean hasApprovalCriteriaMet, boolean hasRejectionCriteriaMet) {
    ServiceNowApprovalOutComeBuilder builder = ServiceNowApprovalOutCome.builder()
                                                   .ticketType(ticketType)
                                                   .ticketNumber(ticketNumber)
                                                   .approvalCriteria(approvalCriteria)
                                                   .hasApprovalCriteriaMet(hasApprovalCriteriaMet)
                                                   .hasRejectionCriteriaMet(hasRejectionCriteriaMet);
    if (rejectionCriteria != null) {
      builder.rejectionCriteria(rejectionCriteria);
    }

    return builder.build();
  }

  public UnifiedServiceNowApprovalOutcome toUnifiedServiceNowApprovalOutcome(
      boolean hasApprovalCriteriaMet, boolean hasRejectionCriteriaMet) {
    UnifiedServiceNowApprovalOutcome outcome = new UnifiedServiceNowApprovalOutcome(outputVariables);
    outcome.put("hasApprovalCriteriaMet", hasApprovalCriteriaMet);
    outcome.put("hasRejectionCriteriaMet", hasRejectionCriteriaMet);
    outcome.put("approvalCriteria", approvalCriteria);

    if (rejectionCriteria != null) {
      outcome.put("rejectionCriteria", rejectionCriteria);
    }

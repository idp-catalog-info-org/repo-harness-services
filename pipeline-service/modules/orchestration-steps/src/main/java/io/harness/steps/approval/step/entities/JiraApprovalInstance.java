...
import io.harness.steps.approval.step.jira.UnifiedJiraApprovalOutcome;
...
public class JiraApprovalInstance extends ApprovalInstance implements PersistentIrregularIterable {
  ...
  public static JiraApprovalInstance fromUnifiedStepParameters(Ambiance ambiance, StepBaseParameters stepParameters) {
    if (stepParameters == null) {
      return null;
    }
    ...
    if (specParameters.getRunStepInfo() == null) {
      throw new InvalidRequestException("Jira approval run step configuration is not set");
    }
    if (ParameterField.isNotNull(specParameters.getRetryInterval())
        && specParameters.getRetryInterval().isExpression()) {
      throw new InvalidRequestException(String.format("Could not resolve expression %s for retry interval field",
    ...
    if (instance.getRetryInterval().getValue().getTimeoutInMillis() < 10000L) {
      throw new InvalidRequestException("retry interval field for Jira approval cannot be less than 10s");
    }
  ...
  public UnifiedJiraApprovalOutcome toUnifiedJiraApprovalOutcome(
      boolean hasApprovalCriteriaMet, boolean hasRejectionCriteriaMet) {
    UnifiedJiraApprovalOutcome outcome = new UnifiedJiraApprovalOutcome(outputVariables);

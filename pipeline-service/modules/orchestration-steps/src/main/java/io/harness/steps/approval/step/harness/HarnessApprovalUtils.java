...
import io.harness.steps.approval.step.harness.v1.HarnessApprovalStepParameters;
...
public class HarnessApprovalUtils {
  ...
  private static void validateAutoApprovalTimeDuration(Long autoApprovalTimeout) {
    if (autoApprovalTimeout <= MINIMUM_TIME_REQUIRED_FOR_AUTO_APPROVAL) {
      throw new InvalidYamlRuntimeException(
          "Time given for auto approval in approval step should be greater than 15 minutes with respect to current time");
    }
  }
  ...
  public static void checkForNullOrThrowAutoApproval(@NotNull AutoApprovalParams autoApprovalParams) {
  ...
  public static HarnessApprovalSpecParameters getHarnessApprovalStepParameters(StepBaseParameters stepParameters) {
    String version = stepParameters.getSpec().getVersion();
    switch (version) {
      case HarnessYamlVersion.V0:
        return (HarnessApprovalSpecParameters) stepParameters.getSpec();
      case HarnessYamlVersion.V1:
        return ((HarnessApprovalStepParameters) stepParameters.getSpec()).toHarnessApprovalStepParameterV0();
      default:

...
public class ContainerInitCpuMemHelper extends InitMemoryCalculatorService {
  ...
  public Pair<Integer, Integer> getStepGroupRequest(
      InitContainerV2StepInfo initializeStepInfo, String accountId, boolean flexibleTemplateEnabled) {
    return this.getRequest(initializeStepInfo.getStepsExecutionConfig().getSteps(), accountId,
        initializeStepInfo.getStrategyExpansionMap(), initializeStepInfo.getPluginsData(), flexibleTemplateEnabled);
  }
  ...
  public Integer getStepCpuLimit(
    ...
    // Empty response lists are valid (e.g. a DBOPS wrapper that reuses another step's container and
    // skips its own Clone). Contribute 0 so step-group init does not allocate for this wrapper.

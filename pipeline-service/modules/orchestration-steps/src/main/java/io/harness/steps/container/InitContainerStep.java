...
public class InitContainerStep implements TaskExecutableWithRbac<StepElementParameters, K8sTaskExecutionResponse> {
  ...
  public TaskRequest obtainTaskAfterRbac(
    ...
    final List<TaskSelector> delegateSelectors = getDelegateSelectors(ambiance, containerStepSpec);
    CIInitializeTaskParams buildSetupTaskParams =
        containerStepInitHelper.getK8InitializeTaskParams(containerStepSpec, ambiance, logPrefix, delegateSelectors);
  ...
  public TaskData getTaskData(
    ...
    if (buildSetupTaskParams != null && CIInitializeTaskParams.Type.GCP_K8.equals(buildSetupTaskParams.getType())
        && buildSetupTaskParams instanceof CIK8InitializeTaskParams) {
      CIK8InitializeTaskParams cik8InitializeTaskParams = (CIK8InitializeTaskParams) buildSetupTaskParams;
      CIK8PodParams<CIK8ContainerParams> cik8PodParams = cik8InitializeTaskParams.getCik8PodParams();
      if (cik8PodParams != null && cik8PodParams.getVolumes() != null
          && cik8PodParams.getVolumes().stream().anyMatch(podVolume
              -> (podVolume.getType().equals(PodVolume.Type.CONFIG_MAP)
                  || podVolume.getType().equals(PodVolume.Type.SECRET)))) {
        taskType = TaskType.CONTAINER_INITIALIZATION_V2.name();
      }
    }
  ...
  private List<TaskSelector> getDelegateSelectors(Ambiance ambiance, ContainerStepSpec containerStepSpec) {
    return AmbianceUtils.checkIfFeatureFlagEnabled(
               ambiance, FeatureName.CDS_CONTAINER_STEP_DELEGATE_SELECTOR_PRECEDENCE.name())
        ? pluginUtils.getContainerStepDelegateSelectors(ambiance, containerStepSpec)

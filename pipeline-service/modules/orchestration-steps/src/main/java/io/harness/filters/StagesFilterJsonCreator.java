...
public class StagesFilterJsonCreator {
  ...
  public List<YamlField> getStageYamlFields(YamlField stagesYamlField) {
    ...
    yamlNodes.forEach(yamlNode -> {
      ...
      YamlField insertStageField = yamlNode.getField("insert");
      if (stageField != null) {
        stageFields.add(stageField);
      } else if (parallelStageField != null) {
        stageFields.add(parallelStageField);
      } else if (insertStageField != null) {
        stageFields.add(insertStageField);
      } else {
        // V1 YAML: the array element itself is the stage (no nested "stage:" key).

...
public interface ShellScriptHelperServiceOld {
  String EXIT_CODE_KEY = "exit_code";
  ...
  void exportOutputVariablesUsingAlias(
  ...
  static ShellScriptBaseOutcome prepareShellScriptOutcome(Map<String, String> sweepingOutputEnvVariables,
    ...
    return prepareShellScriptOutcome(sweepingOutputEnvVariables, outputVariables, secretOutputVariables, version, null);
  }
  ...
  static ShellScriptBaseOutcome prepareShellScriptOutcome(Map<String, String> sweepingOutputEnvVariables,
    ...
    // Always create resolvedOutputVariables map, even if no explicit output variables are defined
    Map<String, String> resolvedOutputVariables = new HashMap<>();

    // Process explicit output variables if they exist
    ...
    // Add exit code as output variable if available and is 0 or 1
    if (shellExecutionData != null && shellExecutionData.getExitCode() != null) {
      Integer exitCodeInt = shellExecutionData.getExitCode();
      exitCode = exitCodeInt.toString();
    }
    return getShellScriptOutcome(resolvedOutputVariables, version, exitCode);
  }

  // We have separate POJO for step outcome for v1 because we also need to support expressions of outcomes following v1
  // rfc
  static ShellScriptBaseOutcome getShellScriptOutcome(
      Map<String, String> resolvedOutputVariables, String version, String exitCode) {
    switch (version) {
      ...
      case HarnessYamlVersion.V0:
        io.harness.steps.shellscript.ShellScriptOutcome.ShellScriptOutcomeBuilder builder =
            io.harness.steps.shellscript.ShellScriptOutcome.builder().outputVariables(resolvedOutputVariables);
        if (exitCode != null) {

...
public class PluginUtils {
  ...
  public List<TaskSelector> getContainerStepDelegateSelectors(Ambiance ambiance, ContainerStepSpec containerStepInfo) {
    // Step > Step Group > Stage > Pipeline
    List<TaskSelector> highestPriorityDelegateSelectors =
        ContainerSpecUtils.getStepDelegateSelectors(containerStepInfo);

    // Step > Step Group > Stage > Pipeline > Connector
    if (isEmpty(highestPriorityDelegateSelectors)) {
      ConnectorDetails connectorDetails = connectorUtils.getConnectorDetails(
          AmbianceUtils.getNgAccess(ambiance), getContainerStepK8sInfraConnectorRef(containerStepInfo));
      return ContainerSpecUtils.getConnectorDelegateSelectors(connectorDetails);
    }

    return highestPriorityDelegateSelectors;
  }

  public String getContainerStepK8sInfraConnectorRef(ContainerStepSpec containerStepInfo) {

...
import static io.harness.rule.OwnerRule.HARSHIL;
...
/**
 * Unit tests for FmeFlagRestoreStep - tests the restore/unkill feature flag functionality.
 * Covers success scenarios, failure handling, edge cases (404, false response), and parameter validation.
 */
@OwnedBy(HarnessTeam.FME)
@RunWith(MockitoJUnitRunner.class)
public class FmeFlagRestoreStepTest extends CategoryTest {
  @InjectMocks FmeFlagRestoreStep fmeFlagRestoreStep;
  ...
  @Test
  @Owner(developers = HARSHIL)
  @Category(UnitTests.class)
  public void testExecuteSyncAfterRbacSuccess() throws Exception {
    // Arrange
    FmeFlagRestoreStepParameters restoreStepParameters = FmeFlagRestoreStepParameters.builder()
    ...
    when(stepBaseParameters.getSpec()).thenReturn(restoreStepParameters);
    ...
    when(fmePipelineClient.restoreFeatureFlag(
             eq(FLAG_NAME), eq(ACCOUNT_ID), eq(ORG_ID), eq(PROJECT_ID), eq(ENVIRONMENT)))
        .thenReturn(mockCall);

    // Act
    StepResponse stepResponse =
        fmeFlagRestoreStep.executeSyncAfterRbac(ambiance, stepBaseParameters, StepInputPackage.builder().build(), null);

    // Assert
    assertThat(stepResponse.getStatus()).isEqualTo(Status.SUCCEEDED);
    verify(fmePipelineClient, times(1))
        .restoreFeatureFlag(eq(FLAG_NAME), eq(ACCOUNT_ID), eq(ORG_ID), eq(PROJECT_ID), eq(ENVIRONMENT));
  }
  ...
  public void testExecuteSyncAfterRbacWithFalseResponse() throws Exception {
    // Arrange - test when API returns response with entity (flag restored successfully)
    FmeFlagRestoreStepParameters restoreStepParameters = FmeFlagRestoreStepParameters.builder()
                                                             .flagName(ParameterField.createValueField(FLAG_NAME))
                                                             .environment(ParameterField.createValueField(ENVIRONMENT))
                                                             .build();
  ...
  public void testGetStepType() {
    assertThat(FmeFlagRestoreStep.STEP_TYPE).isNotNull();
  }

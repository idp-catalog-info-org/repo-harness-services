...
public class FmeFlagsetDeleteStepTest extends CategoryTest {
  @InjectMocks FmeFlagsetDeleteStep fmeFlagsetDeleteStep;
  ...
  public void testExecuteSyncAfterRbacSuccess() throws Exception {
    // Arrange
    FmeFlagsetDeleteParameters params =
        FmeFlagsetDeleteParameters.builder().name(ParameterField.createValueField(FLAGSET_NAME)).build();
    ...
    when(fmePipelineClient.deleteFlagset(eq(FLAGSET_NAME), eq(ACCOUNT_ID), eq(ORG_ID), eq(PROJECT_ID)))
        .thenReturn(mockCall);

    // Act
    StepResponse response = fmeFlagsetDeleteStep.executeSyncAfterRbac(
        ambiance, stepBaseParameters, StepInputPackage.builder().build(), null);

    // Assert
    assertThat(response.getStatus()).isEqualTo(Status.SUCCEEDED);
    verify(fmePipelineClient, times(1)).deleteFlagset(eq(FLAGSET_NAME), eq(ACCOUNT_ID), eq(ORG_ID), eq(PROJECT_ID));
  }
  ...
  public void testExecuteSyncAfterRbacMissingName() {
    // Arrange
    FmeFlagsetDeleteParameters params =
        FmeFlagsetDeleteParameters.builder().name(ParameterField.createValueField(null)).build();
    StepBaseParameters stepBaseParameters = mock(StepBaseParameters.class);
    when(stepBaseParameters.getSpec()).thenReturn(params);
  ...
  public void testStepType() {
    assertThat(FmeFlagsetDeleteStep.STEP_TYPE).isEqualTo(StepSpecTypeConstants.FME_FLAGSET_DELETE_STEP_TYPE);
  }
  ...
  public void testGetStepParametersClass() {
    assertThat(fmeFlagsetDeleteStep.getStepParametersClass()).isEqualTo(StepBaseParameters.class);
  }
}

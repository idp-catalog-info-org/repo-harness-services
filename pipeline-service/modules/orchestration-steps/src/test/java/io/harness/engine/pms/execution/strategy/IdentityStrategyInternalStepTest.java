...
public class IdentityStrategyInternalStepTest extends CategoryTest {
  ...
  public void testObtainChildren() {
    ...
    Level childLevel2 = Level.newBuilder()
    ...
    childrenNodeExecutions.add(NodeExecution.builder()
                                   .uuid("uuid1")
                                   .ambiance(oldAmbianceBuilder.addLevels(childLevel2).build())
                                   .nodeId(node1.getUuid())
                                   .build());
    Level childLevel3 = Level.newBuilder()
                            .setNodeType(NodeType.PLAN_NODE.toString())
                            .setStrategyMetadata(StrategyMetadata.newBuilder().build())
                            .build();
    ...
    childrenNodeExecutions.add(NodeExecution.builder()
                                   .uuid("uuid2")
                                   .ambiance(oldAmbianceBuilder.addLevels(childLevel3).build())
                                   .nodeId(node2.getUuid())
                                   .build());
    IdentityPlanNode iNode1 = IdentityPlanNode.builder().uuid("identityUuid1").build();
    Level childLevel4 = Level.newBuilder()
                            .setNodeType(NodeType.IDENTITY_PLAN_NODE.toString())
                            .setStrategyMetadata(StrategyMetadata.newBuilder().build())
                            .build();
    childrenNodeExecutions.add(NodeExecution.builder()
                                   .uuid("uuid3")
                                   .ambiance(oldAmbianceBuilder.addLevels(childLevel4).build())
                                   .nodeId(iNode1.getUuid())
                                   .build());
    IdentityPlanNode iNode2 = IdentityPlanNode.builder().uuid("identityUuid2").build();
    Level childLevel1 = Level.newBuilder()
                            .setNodeType(NodeType.IDENTITY_PLAN_NODE.toString())
                            .setStrategyMetadata(StrategyMetadata.newBuilder().build())
                            .build();
    childrenNodeExecutions.add(NodeExecution.builder()
                                   .uuid("uuid4")
                                   .ambiance(oldAmbianceBuilder.addLevels(childLevel1).build())
                                   .nodeId(iNode2.getUuid())
                                   .build());

    when(nodeExecutionInfoService.getStrategyMetadata(childLevel1)).thenReturn(StrategyMetadata.newBuilder().build());
    when(nodeExecutionInfoService.getStrategyMetadata(childLevel2)).thenReturn(StrategyMetadata.newBuilder().build());
    when(nodeExecutionInfoService.getStrategyMetadata(childLevel3)).thenReturn(StrategyMetadata.newBuilder().build());
    when(nodeExecutionInfoService.getStrategyMetadata(childLevel4)).thenReturn(StrategyMetadata.newBuilder().build());
    ...
    Stream<NodeExecution> iterator =
        OrchestrationStepsTestHelper.createCloseableIterator(childrenNodeExecutions.listIterator()).stream();
    ...
    verify(pmsSweepingOutputService, times(1))
        .cloneForRetryExecution(newAmbiance, stepParameters.getOriginalNodeExecutionId());
    ...
    doReturn(strategyNodeExecution)
        ...
        .getWithFieldsIncluded(
            stepParameters.getOriginalNodeExecutionId(), NodeProjectionUtils.fieldsForIdentityStrategyStep);
    iterator = OrchestrationStepsTestHelper.createCloseableIterator(childrenNodeExecutions.listIterator()).stream();
    doReturn(iterator)
        .when(nodeExecutionService)
    ...
    verify(pmsSweepingOutputService, times(2))
        .cloneForRetryExecution(newAmbiance, stepParameters.getOriginalNodeExecutionId());
    assertEquals(response.getChildrenCount(), childrenNodeExecutions.size());
    ...
    response = identityStrategyInternalStep.obtainChildren(newAmbiance, stepParameters, null);
    verify(pmsSweepingOutputService, times(3))
        .cloneForRetryExecution(newAmbiance, stepParameters.getOriginalNodeExecutionId());
    assertEquals(response.getChildrenCount(), childrenNodeExecutions.size());
  ...
  public void testHandleChildrenResponse() {
    ...
    doReturn(StepResponse.builder().status(Status.ABORTED).build())
        ...
        .handleChildrenResponse(any(), any(), anyMap());
  ...
  public void testObtainChild() {
    ...
    NodeExecution originalNodeExecution =
        NodeExecution.builder()
            ...
            .createdAt(50L)
            .stepType(io.harness.pms.contracts.steps.StepType.newBuilder().setStepCategory(StepCategory.STEP).build())
            .build();
    ...
    Stream<NodeExecution> iterator = OrchestrationStepsTestHelper
                                         .createCloseableIterator(Arrays
    ...
    PlanNode node1 = PlanNode.builder().uuid("planUuid1").build();
    PlanNode node2 = PlanNode.builder().uuid("planUuid2").build();
    iterator = OrchestrationStepsTestHelper
                   .createCloseableIterator(Arrays
                                                .asList(NodeExecution.builder()
                                                            .uuid("uuid1")
                                                            .ambiance(oldAmbiance)
  ...
  @Category(UnitTests.class)
  public void testObtainChildWithEmptyInsertExecutableResponses() {
    // Test case for empty INSERT node inside a matrix - should return skip=true instead of throwing exception
    String originalNodeExecutionId = "originalNodeExecutionId";
    String ORIGINAL_PLAN_EXECUTION_ID = "originalPlanExecutionId";
    Ambiance newAmbiance =
        Ambiance.newBuilder().setPlanId(generateUuid()).setPlanExecutionId("PLAN_EXECUTION_ID").build();
    ...
    // NodeExecution with empty executableResponses and INSERT stepCategory (empty INSERT case)
    NodeExecution originalNodeExecution =
        NodeExecution.builder()
            .uuid("originalNodeExecutionId")
            .ambiance(Ambiance.newBuilder().setPlanExecutionId(ORIGINAL_PLAN_EXECUTION_ID).build())
            .stepType(io.harness.pms.contracts.steps.StepType.newBuilder()
                          .setStepCategory(io.harness.pms.contracts.steps.StepCategory.INSERT)
                          .setType("INSERT")
                          .build())
    ...
    // Should return skip=true instead of throwing exception
    assertThat(response.getSkip()).isTrue();
    verify(pmsSweepingOutputService, times(1))
        .cloneForRetryExecution(newAmbiance, stepParameters.getOriginalNodeExecutionId());
    // Should not call fetchChildrenNodeExecutionsIterator since we check executableResponses first
    verify(nodeExecutionService, times(0))
        .fetchChildrenNodeExecutionsIterator(anyString(), anyString(), any(Direction.class), anySet());
  }
}

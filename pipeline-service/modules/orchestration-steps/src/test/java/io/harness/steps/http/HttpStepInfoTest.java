...
public class HttpStepInfoTest extends CategoryTest {
  ...
  public void testExtractSecretRefs() {
    HttpStepInfo httpStepInfo = HttpStepInfo.infoBuilder()
                                    ...
                                    .build();
    Map<String, ParameterField<String>> secretRefs = httpStepInfo.extractSecretRefs();
    assertThat(secretRefs)
        .hasSize(2)
        .containsEntry(YAMLFieldNameConstants.CERTIFICATE, ParameterField.createValueField(certRef))
        .containsEntry(YAMLFieldNameConstants.CERTIFICATE_KEY, ParameterField.createValueField(certKeyRef));
    httpStepInfo.setCertificateKey(null);
    secretRefs = httpStepInfo.extractSecretRefs();
    assertThat(secretRefs)
        .hasSize(1)
        .containsEntry(YAMLFieldNameConstants.CERTIFICATE, ParameterField.createValueField(certRef));
  }

...
public class FmeFlagSetTargetsTest extends CategoryTest {
  @InjectMocks FmeFlagSetTargets fmeFlagSetTargets;
  ...
  public void testGetStepType() {
    assertThat(FmeFlagSetTargets.STEP_TYPE).isEqualTo(StepSpecTypeConstants.FME_FLAG_SET_TARGETS_STEP_TYPE);
  }

  @Test
  @Owner(developers = GONZALO)
  @Category(UnitTests.class)
  public void testSetTargetsWithExistingDefinition() throws Exception {
    Scope scope = ambianceToScope(ambiance);

    Target target = Target.builder()
                        .treatment(ParameterField.createValueField("on"))
                        .keys(ParameterField.createValueField(Arrays.asList("user1", "user2")))
                        .segments(ParameterField.createValueField(Arrays.asList("segment1")))
                        .build();
    ...
    FeatureFlagDefinition existingDefinition =
        FeatureFlagDefinition.builder()
            ...
            .environment(envUrn)
            .treatments(
                Arrays.asList(Treatment.builder().name("on").keys(new ArrayList<>(Arrays.asList("oldUser"))).build(),
                    Treatment.builder().name("off").keys(new ArrayList<>()).description("OFF treatment").build()))
            .build();
    ...
    fmeFlagSetTargets.setTargets(ngLogCallback, scope, FLAG_NAME, ENVIRONMENT, targets);
    ...
    assertThat(capturedPatch.get(0).getPath()).isEqualTo("/treatments");
    List<Treatment> patchedTreatments = (List<Treatment>) capturedPatch.get(0).getValue();
  ...
  @Owner(developers = GONZALO)
  @Category(UnitTests.class)
  public void testSetTargetsPreservesConfigurationAndDescription() throws Exception {
    Scope scope = ambianceToScope(ambiance);

    // User wants to set only keys and segments for the "on" treatment
    Target target = Target.builder()
                        .treatment(ParameterField.createValueField("on"))
                        .keys(ParameterField.createValueField(Arrays.asList("user1", "user2")))
                        .segments(ParameterField.createValueField(Arrays.asList("beta-testers")))
                        .build();
    ...
    // Existing definition has configuration and description that should be preserved
    String originalConfiguration = "{\"theme\": \"dark\", \"features\": [\"advanced\"]}";
    String originalDescription = "ON treatment with premium features";
    ...
    FeatureFlagDefinition existingDefinition =
        FeatureFlagDefinition.builder()
            ...
            .treatments(Arrays.asList(Treatment.builder()
                                          .name("on")
                                          .keys(new ArrayList<>(Arrays.asList("oldUser1", "oldUser2")))
                                          .segments(new ArrayList<>(Arrays.asList("old-segment")))
                                          .configurations(originalConfiguration)
                                          .description(originalDescription)
                                          .largeSegments(Arrays.asList("largeSegment1", "largeSegment2"))
  ...
  @Owner(developers = GONZALO)
  @Category(UnitTests.class)
  public void testSetTargetsWithNoExistingDefinition() throws Exception {
    Scope scope = ambianceToScope(ambiance);

    Target target = Target.builder()
                        .treatment(ParameterField.createValueField("on"))
                        .keys(ParameterField.createValueField(Arrays.asList("user1")))
                        .build();
  ...
  public void testExecuteSyncAfterRbacSuccess() {
    ...
    FmeFlagSetTargetsParameters stepParams = FmeFlagSetTargetsParameters.builder()
                                                 .environment(ParameterField.createValueField(ENVIRONMENT))
                                                 .flagName(ParameterField.createValueField(FLAG_NAME))
                                                 .treatments(ParameterField.createValueField(Arrays.asList(target)))
                                                 .build();
    ...
    StepResponse response =
        fmeFlagSetTargets.executeSyncAfterRbac(ambiance, stepBaseParameters, StepInputPackage.builder().build(), null);
  ...
  @Owner(developers = GONZALO)
  @Category(UnitTests.class)
  public void testSetTargetsUpdatesMultipleTreatments() throws Exception {
    Scope scope = ambianceToScope(ambiance);

    Target onTarget = Target.builder()
                          .treatment(ParameterField.createValueField("on"))
                          .keys(ParameterField.createValueField(Arrays.asList("user1", "user2")))
                          .segments(ParameterField.createValueField(Arrays.asList("beta")))
                          .build();

    Target offTarget = Target.builder()
                           .treatment(ParameterField.createValueField("off"))
                           .keys(ParameterField.createValueField(Arrays.asList("user3")))
                           .build();

    List<Target> targets = Arrays.asList(onTarget, offTarget);
    ...
    FeatureFlagDefinition existingDefinition =
        FeatureFlagDefinition.builder()
            .name(FLAG_NAME)
            .environment(envUrn)
            .treatments(Arrays.asList(Treatment.builder().name("on").configurations("{\"config\": 1}").build(),
                Treatment.builder().name("off").configurations("{\"config\": 2}").build(),
                Treatment.builder().name("control").configurations("{\"config\": 3}").build()))
            .build();

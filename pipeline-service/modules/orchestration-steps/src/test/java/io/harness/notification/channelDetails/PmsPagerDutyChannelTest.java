...
public class PmsPagerDutyChannelTest extends CategoryTest {
  ...
  PmsPagerDutyChannel pmsPagerDutyChannelWithDelegateSelectors;
  PmsPagerDutyChannel pmsPagerDutyChannelWithDelegateExpression;
  ...
  public void setUp() {
    ...
    pmsPagerDutyChannel = spy(new PmsPagerDutyChannel(userGroups, integrationKey, null, false));
    pmsPagerDutyChannelWithDelegateSelectors = spy(new PmsPagerDutyChannel(
        userGroups, integrationKey, ParameterField.<List<String>>builder().value(delegateSelectors).build(), true));
    pmsPagerDutyChannelWithDelegateExpression = spy(new PmsPagerDutyChannel(userGroups, integrationKey,
  ...
  public void testToNotificationChannelWithDelegateSelectors() {
    ...
    String templateId = "templateId";
    Map<String, String> templateData = new HashMap<>();
    NotificationChannel notificationChannel = pmsPagerDutyChannelWithDelegateSelectors.toNotificationChannel(accountId,
  ...
  public void testToNotificationChannelWithDelegateExpression() {
    ...
    String templateId = "templateId";
    Map<String, String> templateData = new HashMap<>();
    NotificationChannel notificationChannel = pmsPagerDutyChannelWithDelegateExpression.toNotificationChannel(accountId,
        orgId, projectId, templateId, templateData, Ambiance.newBuilder().setExpressionFunctorToken(123L).build());
    assertEquals(notificationChannel.getAccountId(), accountId);
    assertEquals(((PagerDutyChannel) notificationChannel).getIntegrationKeys().get(0), integrationKey);
    assertEquals(notificationChannel.getUserGroups().get(0).getOrgIdentifier(), orgId);

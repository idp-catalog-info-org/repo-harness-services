...
public class BarrierWithinStrategyExpanderTest extends OrchestrationStepsTestBase {
  ...
  private static final String parentPipelineExecutionId = "parentPipelineExecutionId";
  ...
  @Category(UnitTests.class)
  public void testOnInitializeRequestWithNestedStrategyAndParentPipeline() {
    when(persistentLocker.waitToAcquireLock(any(), any(), any())).thenReturn(AcquiredNoopLock.builder().build());
    when(barrierService.existsByPlanExecutionIdAndStrategySetupId(eq("planExecutionId"), eq("stage1SetupId")))
        .thenReturn(false);
    when(barrierService.existsByPlanExecutionIdAndStrategySetupId(eq("parentPipelineExecutionId"), eq("stage1SetupId")))
        .thenReturn(true);
    when(barrierService.findManyByPlanExecutionIdAndStrategySetupId(any(), any()))
        .thenReturn(List.of(obtainBarrierExecutionInstanceWithStrategyAndParentPipeline()));
    doNothing()
        .when(barrierService)
        .updateBarrierPositionInfoListAndStrategyConcurrency(any(), any(), any(), any(), anyInt());
    ...
    BarrierExpandRequest barrierExpandRequest1 = BarrierExpandRequest.builder()
                                                     ...
                                                     .childrenRuntimeIds(stage1RuntimeIds)
                                                     .maxConcurrency(10)
                                                     .parentPlanExecutionId(parentPipelineExecutionId)
                                                     .build();
    ArgumentCaptor<List<BarrierPosition>> updatedPositionsCaptor1 = ArgumentCaptor.forClass(List.class);
    barrierWithinStrategyExpander.onInitializeRequest(barrierExpandRequest1);
    verify(barrierService, times(1))
        .updateBarrierPositionInfoListAndStrategyConcurrency(eq(barrierIdentifier), eq(parentPipelineExecutionId),
            updatedPositionsCaptor1.capture(), eq(stage1SetupId), eq(2));
    List<BarrierPosition> updatedPositions1 = updatedPositionsCaptor1.getValue();
    assertThat(updatedPositions1.size()).isEqualTo(2);
    assertThat(updatedPositions1.stream().filter(position -> position.getStepSetupId().equals(step2SetupId)).count())
        .isEqualTo(1);
    ...
    BarrierExecutionInstance barrierExecutionInstanceWithUpdatedConcurrencyMap =
        obtainBarrierExecutionInstanceWithStrategyAndParentPipeline();
    barrierExecutionInstanceWithUpdatedConcurrencyMap.getSetupInfo().setStrategyConcurrencyMap(
        new HashMap<>(Map.of(stage1SetupId, 2)));
    when(barrierService.findManyByPlanExecutionIdAndStrategySetupId(any(), any()))
        .thenReturn(List.of(obtainBarrierExecutionInstanceWithStrategyAndParentPipeline()));
    when(barrierService.existsByPlanExecutionIdAndStrategySetupId(eq("planExecutionId"), eq("stepGroup1SetupId")))
        .thenReturn(false);
    when(barrierService.existsByPlanExecutionIdAndStrategySetupId(
             eq("parentPipelineExecutionId"), eq("stepGroup1SetupId")))
        .thenReturn(true);
  ...
  private BarrierExecutionInstance obtainBarrierExecutionInstanceWithStrategyAndParentPipeline() {
    return BarrierExecutionInstance.builder()
        .uuid(generateUuid())
        .identifier(barrierIdentifier)

...
public interface ApprovalResource {
  ...
  getApprovalInstance(@Parameter(description = APPROVAL_PARAM_MESSAGE) @NotEmpty @PathParam(
      ...
      @QueryParam("refresh") @Parameter(description = "Force a refresh of the approval status") boolean refresh);
  ...
  @GET
  @ApiOperation(value = "Lists pending approvals for the current user", nickname = "listPendingApprovals")
  @Operation(operationId = "listPendingApprovals",
      summary = "Lists pending approval instances awaiting the current user's action",
      responses =
      {
        @io.swagger.v3.oas.annotations.responses.
        ApiResponse(responseCode = "default", description = "Returns list of pending approvals")
      })
  @Hidden
  ResponseDTO<PageResponse<PendingApprovalSummaryDTO>>
  listPendingApprovals(
      @QueryParam(NGCommonEntityConstants.ACCOUNT_KEY) @Parameter(
          description = PipelineResourceConstants.ACCOUNT_PARAM_MESSAGE) @AccountIdentifier @NotNull String accountId,
      @QueryParam("size") @DefaultValue("20") @Parameter(
          description = "Number of results to return, max 100") int size);
}

...
import io.harness.pms.preflight.utils.PreflightCommonUtils;
...
public class ConnectorPreflightHandler {
  ...
  public ConnectorsTestRegisterResponse registerConnectorsForTest(@NotBlank String accountId, @NotBlank String orgId,
      @NotBlank String projectId, @NotBlank String preflightId, List<EntityDetail> connectorEntityDetails) {
    final List<String> connectorRefs = connectorEntityDetails.stream()
                                           .map(EntityDetail::getEntityRef)
                                           .filter(IdentifierRef.class ::isInstance)
                                           .map(IdentifierRef.class ::cast)
                                           .map(connectorCheckResponse
                                               -> IdentifierRef.builder()
                                                      .identifier(connectorCheckResponse.getIdentifier())
                                                      .scope(connectorCheckResponse.getScope())
                                                      .build())
                                           .map(IdentifierRef::buildScopedIdentifier)
                                           .distinct()
                                           .collect(Collectors.toList());
    return NGRestUtils.getResponse(connectorResourceClient.registerForTest(accountId, orgId, projectId,
        ConnectorsTestRegisterRequest.builder().preFlightRequestId(preflightId).connectorRefs(connectorRefs).build()));
  }
    ...
    return entityDetails.stream()
        .map(entityDetail
            -> ConnectorCheckResponse.builder()
                   ...
                   .scope(entityDetail.getEntityRef().getScope())
                   .build())
        .collect(Collectors.toList());
  }
  ...
  public List<ConnectorCheckResponse> getConnectorCheckResponse(Map<String, Object> fqnToObjectMapMergedYaml,
    ...
    for (ConnectorResponseDTO connectorResponse : connectorResponses) {
      ...
      ConnectorCheckResponseBuilder checkResponse =
          ConnectorCheckResponse.builder()
              ...
              .scope(Scope.of(connectorInfoDTO.getAccountIdentifier(), connectorInfoDTO.getOrgIdentifier(),
      ...
      if (isNotEmpty(connectorConnectivityDetails.getErrorSummary())
          && isNotEmpty(connectorConnectivityDetails.getErrors())) {
    ...
    List<String> availableConnectors = connectorResponses.stream().map(c -> c.getConnector().getIdentifier()).toList();

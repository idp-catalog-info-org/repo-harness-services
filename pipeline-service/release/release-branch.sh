...
if [[ "$EXECUTE_NEW_CODE" == "true" ]]; then
    ...
    if [ $pipeline_curl_result -eq 0 ]; then
        ...
        echo "Pipeline V0 file was updated"
    else
    ...
    if [ $pipeline_v1_curl_result -eq 0 ]; then
        git add ${PIPELINE_JSON_V1}
        echo "Pipeline V1 file was updated"
    else
        echo "Pipeline V1 file was not updated"
    fi

    # Updating static-schema for v1 trigger.json
    TRIGGER_JSON_V1="pipeline-service/service/src/main/resources/static-schema/v1/trigger.json"
    perform_curl_with_retry "https://raw.githubusercontent.com/harness/harness-schema/${head_static_commit}/v1/trigger.json" ${TRIGGER_JSON_V1}
    trigger_v1_curl_result=$?


    # Check the return values in if-else conditions
    if [ $trigger_v1_curl_result -eq 0 ]; then
        git add ${TRIGGER_JSON_V1}
        echo "Trigger V1 file was updated"
    else
        echo "Trigger V1 file was not updated"
    fi

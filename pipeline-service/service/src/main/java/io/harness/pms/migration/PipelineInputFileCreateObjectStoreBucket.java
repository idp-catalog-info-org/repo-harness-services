...
@NoArgsConstructor
@Slf4j
public class PipelineInputFileCreateObjectStoreBucket implements NGMigration {
  ...
  public void migrate() {
    if (objectStoreClient != null) {
      try {
        ...
        log.error("Unable to create bucket for data retention in object store", ex);
      }
    } else {
      log.error(String.format("[Migration]: Migration {%s} failed - OBJECTSTORE NOT AVAILABLE", getClass()));
    }
  }
}

...
import io.harness.pms.merger.yaml.YamlMapGenerator;
...
public class InputSetSanitizer {
  ...
  public YamlConfig trimValues(YamlConfig config) {
    return new YamlConfig(trimValues(config.getYamlMap(), HarnessYamlVersion.V0));
  }

  public JsonNode trimValues(JsonNode config, String version) {
    Map<FQN, Object> fqnToValueMap = FQNMapGenerator.generateFQNMap(config, version);
    ...
    return YamlMapGenerator.generateYamlMap(fqnToValueMap, config, false, false, version);
  }
}

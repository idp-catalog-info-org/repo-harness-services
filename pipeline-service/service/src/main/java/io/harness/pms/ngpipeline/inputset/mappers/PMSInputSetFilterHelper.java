...
public class PMSInputSetFilterHelper {
  public Criteria createCriteriaForGetListForBranchAndRepo(String accountId, String orgIdentifier,
      String projectIdentifier, String pipelineIdentifier, InputSetListTypePMS type, String uniqueIdentifier,
      boolean isParentIdQueryingEnabled) {
    Criteria criteria = createCriteriaForGetList(accountId, orgIdentifier, projectIdentifier, pipelineIdentifier, type,
        null, false, uniqueIdentifier, isParentIdQueryingEnabled);
  ...
  // TODO: Find all reference and pass correctly.
  public Criteria createCriteriaForGetList(String accountId, String orgIdentifier, String projectIdentifier,
      String pipelineIdentifier, InputSetListTypePMS type, String searchTerm, boolean deleted, String uniqueIdentifier,
      boolean isParentIdQueryingEnabled) {
    Criteria criteria = new Criteria();

    if (isParentIdQueryingEnabled) {
      if (isNotEmpty(uniqueIdentifier)) {
        criteria.and(InputSetEntityKeys.parentUniqueId).is(uniqueIdentifier);
      }
    } else {
  ...
  public Criteria listInputSetsForProjectCriteria(String accountId, String orgIdentifier, String projectIdentifier,
      ...
      InputSetFilterPropertiesDto inputSetFilterPropertiesDto, ScopeInfo scopeInfo, boolean isParentIdQueryingEnabled) {
    Criteria criteria = new Criteria();
    if (isParentIdQueryingEnabled) {
      if (isNotEmpty(scopeInfo.getUniqueId())) {
        criteria.and(InputSetEntityKeys.parentUniqueId).is(scopeInfo.getUniqueId());
      }
    } else {
      if (isNotEmpty(accountId)) {
        criteria.and(InputSetEntityKeys.accountId).is(accountId);
  ...
  public Criteria getCriteriaForFind(
      ScopeInfo scopeInfo, String pipelineIdentifier, String identifier, boolean notDeleted) {
    return Criteria.where(InputSetEntityKeys.deleted)
        .is(!notDeleted)
  ...
  public Criteria buildCriteriaForRepoListing(ScopeInfo scopeInfo, String pipelineIdentifier) {
    return Criteria.where(InputSetEntityKeys.parentUniqueId)
        .is(scopeInfo.getUniqueId())
        .and(InputSetEntityKeys.pipelineIdentifier)
        .is(pipelineIdentifier);
  }
  ...
  public Criteria getCriteriaForFindByPipelineIdentifiers(String accountIdentifier, String orgIdentifier,
      String projectIdentifier, ScopeInfo scopeInfo, InputSetEntityType inputSetEntityType, boolean isDeleted,
      List<String> pipelineIdentifiers, boolean isParentIdQueryingEnabled, String searchTerm) {
    Criteria criteria;
    if (isParentIdQueryingEnabled) {
      criteria = Criteria.where(InputSetEntityKeys.parentUniqueId).is(scopeInfo.getUniqueId());
    } else {
      criteria = Criteria.where(InputSetEntityKeys.accountId)
  ...
  public Criteria getCriteriaForFindByInputSetIdentifiers(String accountIdentifier, String orgIdentifier,
      String projectIdentifier, ScopeInfo scopeInfo, boolean isDeleted, String pipelineIdentifier,
      List<String> inputSetIdentifiers, boolean isParentIdQueryingEnabled) {
    Criteria criteria;
    if (isParentIdQueryingEnabled) {
      criteria = Criteria.where(InputSetEntityKeys.parentUniqueId).is(scopeInfo.getUniqueId());
    } else {
  ...
  public Criteria getCriteriaForAllInputSetsInProject(String accountIdentifier, String orgIdentifier,
      String projectIdentifier, ScopeInfo scopeInfo, InputSetEntityType inputSetEntityType, boolean isDeleted,
      boolean isParentIdQueryingEnabled, String searchTerm) {
    Criteria criteria;
    if (isParentIdQueryingEnabled) {
      criteria = Criteria.where(InputSetEntityKeys.parentUniqueId).is(scopeInfo.getUniqueId());
    } else {
    ...
    criteria.and(InputSetEntityKeys.inputSetEntityType)
        .is(inputSetEntityType)
        .and(InputSetEntityKeys.deleted)
        .is(isDeleted);

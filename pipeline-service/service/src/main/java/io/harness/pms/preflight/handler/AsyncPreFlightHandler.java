...
public class AsyncPreFlightHandler implements Runnable {
  ...
  private final PmsFeatureFlagService ngFeatureFlagHelperService;
  ...
  public void run() {
    try (AutoLogContext ignore = entity.autoLogContext()) {
      ...
      preflightService.updateStatus(entity.getUuid(), PreFlightStatus.IN_PROGRESS, null, null);
      ...
      List<ConnectorCheckResponse> connectorCheckResponses = null;
      ...
      connectorCheckResponses =
          preflightService.updateConnectorCheckResponses(scopeInfo.getAccountIdentifier(), scopeInfo.getOrgIdentifier(),
              scopeInfo.getProjectIdentifier(), entity.getUuid(), fqnToObjectMapMergedYaml, connectorUsages);
      ...
      preflightService.updateStatus(entity.getUuid(), overallStatus, null, null);
      ...
      preflightService.updateStatus(
          entity.getUuid(), PreFlightStatus.FAILURE, PreflightCommonUtils.getInternalIssueErrorInfo(e), null);
    }
  }
}

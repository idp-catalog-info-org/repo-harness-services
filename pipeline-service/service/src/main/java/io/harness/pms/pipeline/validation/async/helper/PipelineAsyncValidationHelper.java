...
public class PipelineAsyncValidationHelper {
  ...
  public String buildFQN(PipelineEntity pipelineEntity, String branch, boolean isParentIdQueryingEnabled) {
    String accountId = pipelineEntity.getAccountIdentifier();
    String identifier = pipelineEntity.getIdentifier();
    String parentUniqueId = pipelineEntity.getParentUniqueId();
    if (EmptyPredicate.isEmpty(branch)) {
      return accountId + "/" + parentUniqueId + "/" + identifier;
    }
    return accountId + "/" + parentUniqueId + "/" + identifier + "/" + branch;
  }

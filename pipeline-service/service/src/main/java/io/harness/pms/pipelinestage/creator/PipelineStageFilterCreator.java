...
package io.harness.pms.pipelinestage.creator;
...
public class PipelineStageFilterCreator extends GenericStageFilterJsonCreatorV2<PipelineStageNode> {
  ...
  public FilterCreationResponse handleNode(FilterCreationContext filterCreationContext, PipelineStageNode stageNode) {
    ...
    pipelineEnforcementService.validatePipelineChainingEnforcement(
    ...
    // If stage specifies a branch for the child pipeline, scope a EntityGitDetailsGuard to that branch for child fetch
    String childBranch =
        pipelineStageConfig.getGitBranch() != null ? pipelineStageConfig.getGitBranch().getValue() : null;

    boolean isParentIdQueryingEnabled = true;
    ScopeInfo scopeInfo = scopeResolutionHelper.getScopeInfo(
        setupMetadata.getAccountId(), pipelineStageConfig.getOrg(), pipelineStageConfig.getProject());
    Optional<PipelineEntity> childPipelineEntity;
    childPipelineEntity = GitxBranchContextUtils.withBranch(gitRequestParamsInfo, childBranch,
        ()
    ...
    pipelineStageHelper.validateNestedChainedPipeline(childPipelineEntity.get(), stageNode.getName(), scopeInfo);

...
@Singleton
public class PipelineOutboxEventHandler implements OutboxEventHandler, MoveConfigAuditHandler {
  ...
  private void asyncFilterCreation(PipelineEntity pipelineEntity) {
    ...
    if (null == isDraftPipeline || !isDraftPipeline) {
      // pipelineEntity has the updated value as the entity is getting picked from audit event
  ...
  private void sendFilterCreationEvent(
    ...
    if (featureFlagService.isEnabled(accountId, FeatureName.PIE_ASYNC_FILTER_CREATION)) {
      try {
        ...
        log.error("Error while the creating event for async filter creation event for pipeline identifier {} in "
                + "project {} org {} account {}",
  ...
  private boolean handleStepEndEvent(OutboxEvent outboxEvent) throws JsonProcessingException {
    StepEndEvent stepEndEvent = objectMapper.readValue(outboxEvent.getEventData(), StepEndEvent.class);
    return stepEndEventKafkaSender.sendEvent(stepEndEvent);
  }
  ...
  private boolean handlePipelineEndEventForKafka(OutboxEvent outboxEvent) throws JsonProcessingException {
    PipelineKafkaEvent pipelineEndEvent = objectMapper.readValue(outboxEvent.getEventData(), PipelineKafkaEvent.class);
    return pipelineEndEventKafkaSender.sendEvent(pipelineEndEvent);
  }

  private boolean handleStageEndEventForKafka(OutboxEvent outboxEvent) throws JsonProcessingException {
    StageKafkaEvent stageEndEvent = objectMapper.readValue(outboxEvent.getEventData(), StageKafkaEvent.class);
    return stageEndEventKafkaSender.sendEvent(stageEndEvent);
  }

  private boolean handleStepEndEventForKafka(OutboxEvent outboxEvent) throws JsonProcessingException {
    StepEndEvent stepEndEvent = objectMapper.readValue(outboxEvent.getEventData(), StepEndEvent.class);
    return stepEndEventKafkaSender.sendEvent(stepEndEvent);
  }
    ...
    try (AutoLogContext ignore = new OutboxLogContext(outboxEvent.getId(), OVERRIDE_NESTS)) {
      try {
        switch (outboxEvent.getEventType()) {
          ...
          case PipelineOutboxEvents.PIPELINE_MOVED_TO_REMOTE:
          ...
          case NodeExecutionOutboxEventConstants.STEP_END:
            return handleStepEndEvent(outboxEvent);
          case NodeExecutionOutboxEventConstants.PIPELINE_END_FOR_KAFKA:
            return handlePipelineEndEventForKafka(outboxEvent);
          case NodeExecutionOutboxEventConstants.STAGE_END_FOR_KAFKA:
            return handleStageEndEventForKafka(outboxEvent);
          case NodeExecutionOutboxEventConstants.STEP_END_FOR_KAFKA:
            return handleStepEndEventForKafka(outboxEvent);
          default:
  ...
  public ModuleType getModuleType() {
    return ModuleType.PMS;
  }

  @Override
  public Class<? extends MoveConfigEvent> getMoveEventClass() {
    return PipelineMoveConfigEvent.class;
  }

...
public class CustomApprovalCallback extends AbstractApprovalCallback implements PushThroughNotifyCallback {
  ...
  private void pushInternal(Map<String, ResponseData> response, CustomApprovalInstance instance) {
    ...
    try {
      TicketNG ticketNG = handleCustomApprovalResponse(data, instance);
  ...
  private CustomApprovalTicketNG handleCustomApprovalResponseV1(
      ResponseData responseData, CustomApprovalInstance instance) {
    var ticketNG = CustomApprovalTicketNG.builder();
    Map<String, String> outputVars = approvalUtils.handleUnifiedApprovalResponse(responseData, instance.getAccountId());
    ticketNG.fields(outputVars);
    return ticketNG.build();
  }

  private CustomApprovalTicketNG handleCustomApprovalResponse(
      ResponseData responseData, CustomApprovalInstance instance) {
    Ambiance ambiance = instance.getAmbiance();
    if (HarnessYamlVersion.isV1(AmbianceUtils.getPipelineVersion(ambiance))) {
      return handleCustomApprovalResponseV1(responseData, instance);
    } else {
      if (responseData instanceof ShellScriptTaskResponseNG scriptTaskResponse) {
        ...
        return CustomApprovalTicketNG.builder()
            .fields(shellScriptOutcome != null ? shellScriptOutcome.getOutputVariables() : new HashMap<>())
            .build();
      }
    }
    return CustomApprovalTicketNG.builder().build();
  }
}

    ...
    try (AutoLogContext ignore1 = createAutoLogContext(accountId, resourceUnit)) {
      ...
      Stream<ResourceRestraintInstance> instancesIterators =
          resourceRestraintInstanceService.getAllByRestraintIdAndResourceUnitAndStates(
              resourceConstraint.getUuid(), resourceUnit, new ArrayList<>(Arrays.asList(ACTIVE, BLOCKED)));
      Iterator<ResourceRestraintInstance> instanceIterator = instancesIterators.iterator();
      while (instanceIterator.hasNext()) {
        ResourceRestraintInstance instance = instanceIterator.next();
  ...
  private String getPlanExecutionId(ResourceRestraintInstance instance) {
    if (isScopeStage(instance)) {
      try {
        ...
        return nodeExecution.getPlanExecutionId();
  ...
  private PipelineEntity getPipelineEntityName(
    ...
    try (AutoLogContext ignore1 =
      ...
      return pipelineService
          .getPipeline(accountId, orgIdentifier, projectIdentifier, pipelineIdentifier, false, true, false, false,
              scopeInfo, true)

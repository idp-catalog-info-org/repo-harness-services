...
public class RecentExecutionsInfoHelper {
  ...
  public void onExecutionStart(
      ScopeInfo scopeInfo, String pipelineIdentifier, PlanExecution planExecution, boolean isParentIdQueryingEnabled) {
    ...
    Informant0<List<RecentExecutionInfo>> subject = (List<RecentExecutionInfo> recentExecutionInfoList) -> {
      ...
      Criteria criteria =
          getCriteriaForPipelineMetadata(scopeInfo.getAccountIdentifier(), scopeInfo.getUniqueId(), pipelineIdentifier);
    ...
    updateMetadata(scopeInfo, pipelineIdentifier, subject, planExecution, isParentIdQueryingEnabled);
  }
   ...
   */
  public void onExecutionUpdate(Ambiance ambiance, PlanExecution planExecution, ScopeInfo scopeInfo,
      boolean isParentIdQueryingEnabled, FailureInfo failureInfo) {
    ...
    Informant0<List<RecentExecutionInfo>> subject = (List<RecentExecutionInfo> recentExecutionInfoList) -> {
      ...
      for (RecentExecutionInfo recentExecutionInfo : recentExecutionInfoList) {
        if (recentExecutionInfo.getPlanExecutionId().equals(planExecutionId)) {
          ...
          if (StatusUtils.brokeStatuses().contains(planExecution.getStatus())) {
            recentExecutionInfo.setFailureInfo(failureInfo);
          } else {
            recentExecutionInfo.setFailureInfo(null);
          }

          Criteria criteria = getCriteriaForPipelineMetadata(
              scopeInfo.getAccountIdentifier(), scopeInfo.getUniqueId(), pipelineIdentifier);
          Update update = getUpdateOperationForRecentExecutionInfo(recentExecutionInfoList);
          pipelineMetadataService.update(criteria, update);
          return;
        }
  ...
  void updateMetadata(ScopeInfo scopeInfo, String pipelineIdentifier, Informant0<List<RecentExecutionInfo>> subject,
      PlanExecution planExecution, boolean isParentIdQueryingEnabled) {
    ...
    try (AcquiredLock<?> lock =
      ...
      if (lock == null) {
        log.error(String.format("Unable to acquire lock while updating Pipeline Metadata for Pipeline [%s] in Project "
                + "[%s], Org [%s], Account [%s]",
      ...
      Optional<PipelineMetadataV2> optionalPipelineMetadata =
          pipelineMetadataService.getMetadata(accountId, scopeInfo.getUniqueId(), pipelineIdentifier);
      ...
      log.error("RecentExecutionInfo could not be updated for the planExecution: {} to status {} in account: {}, org: "
              + "{}, project: {}",
          planExecution.getUuid(), planExecution.getStatus(), accountId, orgIdentifier, projectIdentifier, ex);
    } catch (Exception ex) {
      log.error("Could not update recentExecutions in PipelineMetadata for Pipeline {} and planExecutionId {} having "
              + "account: {} org: {} project {}",
  ...
  Criteria getCriteriaForPipelineMetadata(String accountId, String parentUniqueId, String pipelineIdentifier) {
    return Criteria.where(PipelineMetadataV2Keys.accountIdentifier)
        .is(accountId)

...
public class SchemaFetcher {
  private JsonNode pipelineStaticSchemaV0;
  private JsonNode pipelineStaticSchemaV1;

  private JsonNode triggerStaticSchemaV0;
  private JsonNode triggerStaticSchemaV1;
  ...
  private final String TRIGGER_JSON_PATH_V1 = "static-schema/v1/trigger.json";
  ...
  public JsonNode fetchTriggerStaticYamlSchema(String version) {
    log.info("[PMS] Fetching static schema");
    try (ResponseTimeRecorder ignore = new ResponseTimeRecorder("Fetching Trigger Static Schema")) {
      ...
      */
      return getTriggerStaticSchema(version);
    } catch (InvalidRequestException exception) {
      throw exception;
    } catch (Exception e) {
  ...
  private JsonNode getTriggerStaticSchema(String version) throws IOException {
    if (useSchemaFromHarnessSchemaRepo()) {
      return fetchSchemaFromRepo(EntityType.TRIGGERS, version);
    }
    if (version.equals(VERSION_V0)) {
      if (null == triggerStaticSchemaV0) {
        triggerStaticSchemaV0 = fetchFile(TRIGGER_JSON_PATH_V0);
      }
      return triggerStaticSchemaV0;
    } else if (version.equals(VERSION_V1)) {
      if (null == triggerStaticSchemaV1) {
        triggerStaticSchemaV1 = fetchFile(TRIGGER_JSON_PATH_V1);
      }
      return triggerStaticSchemaV1;
    } else {
      throw new InvalidRequestException(
          String.format("[PMS] Incorrect version [%s] of Trigger Schema passed, Valid values are [v0, v1]", version));
    }
  }

...
import io.harness.pms.helpers.ResolvedInputSetYamlHelper;
...
public class ResolvedInputSetYamlUpdateEventHandler implements OrchestrationEventHandler {
      ...
      try {
        PlanExecutionMetadata planExecutionMetadata =
            planExecutionMetadataService.findByPlanExecutionIdWithFieldsIncluded(AmbianceUtils.getAccountId(ambiance),
                planExecutionId,
                Sets.newHashSet(PlanExecutionMetadataKeys.inputSetYaml, PlanExecutionMetadataKeys.harnessVersion));
        String resolvedInputSetYaml = resolveExpressionsInYaml(
            planExecutionMetadata.getInputSetYaml(), ambiance, planExecutionMetadata.getHarnessVersion());
        // Persist portable <+secrets.getValue> form so Inputs / re-run UIs never see deferred
        // ${ngSecretManager.obtain(...)} functors from RESOLVE_ALL_EXPRESSIONS.
        resolvedInputSetYaml = ResolvedInputSetYamlHelper.revertSecretExpressions(resolvedInputSetYaml);
        pmsExecutionSummaryService.updateResolvedUserInputSetYaml(
            planExecutionId, resolvedInputSetYaml, planExecutionMetadata.getHarnessVersion());
      } catch (Exception ex) {
        log.error(
  ...
  private String resolveExpressionsInYaml(String inputSetYaml, Ambiance ambiance, String harnessVersion) {
    if (EmptyPredicate.isNotEmpty(inputSetYaml)) {
      inputSetYaml = yamlExpressionResolveHelper.resolveExpressionsInYaml(
          inputSetYaml, ResolveInputYamlType.RESOLVE_ALL_EXPRESSIONS, ambiance, harnessVersion);
    }
    return inputSetYaml;
  }
}
...
import static io.harness.pms.approval.UnifiedApprovalConstants.SERVICENOW_TICKET_PLUGIN_ENV;
...
public class ServiceNowApprovalCallback extends AbstractApprovalCallback implements PushThroughNotifyCallback {
  ...
  @Inject private UnifiedApprovalUtils approvalUtils;
  ...
  private void pushInternal(Map<String, ResponseData> response, ServiceNowApprovalInstance instance) {
    ...
    Map<String, String> pluginOutVars = Collections.emptyMap();
    try {
      ...
      if (HarnessYamlVersion.isV1(AmbianceUtils.getPipelineVersion(ambiance))) {
        pluginOutVars = approvalUtils.handleUnifiedApprovalResponse(responseData, instance.getAccountId());
        String ticketJson = pluginOutVars.get(SERVICENOW_TICKET_PLUGIN_ENV);
        ServiceNowTicketNG ticket = isNotEmpty(ticketJson)
            ? approvalUtils.deserializeOutputVariable(ticketJson, ServiceNowTicketNG.class)
            : ServiceNowTicketNG.builder().build();

        serviceNowTaskNGResponse = ServiceNowTaskNGResponse.builder().ticket(ticket).build();
      } else {
        if (!(responseData instanceof ServiceNowTaskNGResponse)) {
          throw new InvalidRequestException("Invalid response data type for ServiceNow approval");
        }
    ...
    if (HarnessYamlVersion.isV1(AmbianceUtils.getPipelineVersion(ambiance))) {
      try {
        approvalUtils.sanitizeOutputVariables(pluginOutVars);
        updateOutputVariablesInApprovalInstance(pluginOutVars, instance);
      } catch (Exception ex) {
        logCallback.saveExecutionLog(
            LogHelper.color(
                String.format("Error while updating approval with plugin output variables: %s. Ignoring it...",
                    ExceptionUtils.getMessage(ex)),
                LogColor.Red),
            LogLevel.WARN);
        log.warn("Error while updating approval instance with plugin output variables", ex);
      }
    }
  ...
  @Override
  protected boolean evaluateCriteria(
      TicketNG ticket, CriteriaSpecDTO criteriaSpec, NGLogCallback logCallback, ApprovalInstance instance) {
    return ServiceNowCriteriaEvaluator.evaluateCriteria((ServiceNowTicketNG) ticket, criteriaSpec, logCallback);
  }
  ...
  @Override
  protected void updateOutputVariablesInApprovalInstance(Map<String, String> outVars, ApprovalInstance instance) {
    approvalInstanceService.updateOutputVariablesInApprovalInstance((ServiceNowApprovalInstance) instance, outVars);
  }

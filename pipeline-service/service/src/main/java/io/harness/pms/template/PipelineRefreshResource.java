...
public class PipelineRefreshResource {
  ...
  refreshAndUpdateTemplate(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
      ...
      @BeanParam GitEntityUpdateInfoDTO gitEntityBasicInfo, @Context ScopeInfo scopeInfo) {
    return ResponseDTO.newResponse(pipelineRefreshService.refreshTemplateInputsInPipeline(
        accountId, orgId, projectId, pipelineIdentifier, loadFromCache, scopeInfo));
  }
  ...
  @Operation(operationId = "validateTemplateInputs",
      description = "Validates the template inputs in a pipeline's YAML specification. If the template inputs are "
  ...
  public ResponseDTO<YamlDiffResponseDTO> getYamlDiff(
    ...
    return ResponseDTO.newResponse(
        pipelineRefreshService.getYamlDiff(accountId, orgId, projectId, pipelineIdentifier, loadFromCache, scopeInfo));
  }

  @POST
  @Path("refresh-all")
  @ApiOperation(value = "This recursively refresh and update template inputs in pipeline", nickname = "refreshAll")
  @Operation(operationId = "refreshAllTemplatesInputsInPipeline",
      description = "This recursively refresh and update template inputs in pipeline",
      summary = "This recursively refresh and update template inputs in pipeline",
      responses =
      {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "default", description = "This recursively refresh and update template inputs in pipeline")
      })
  @Timed
  @ResponseMetered
  public ResponseDTO<Boolean>
  refreshAll(@Parameter(description = NGCommonEntityConstants.ACCOUNT_PARAM_MESSAGE) @NotNull @QueryParam(
    ...
    return ResponseDTO.newResponse(pipelineRefreshService.recursivelyRefreshAllTemplateInputsInPipeline(
        accountId, orgId, projectId, pipelineIdentifier, gitEntityBasicInfo, loadFromCache, scopeInfo));
  }
}

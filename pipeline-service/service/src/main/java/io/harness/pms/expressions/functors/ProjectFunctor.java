  ...
  public Object bind() {
    ...
    try {
      Optional<ProjectResponse> resp =
          NGRestUtils.getResponse(projectClient.getProject(projectIdentifier, accountId, orgIdentifier));
      if (HarnessYamlVersion.isV1(ambiance.getMetadata().getHarnessVersion())) {
        return MapperUtils.toMapViaJsonString(resp.map(ProjectResponse::getProject).orElse(null));
      } else {

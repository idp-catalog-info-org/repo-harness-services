  ...
  public WebhookEventProcessingResult handleTriggerWebhookConfirmationEvent(TriggerWebhookEvent event) {
    try {
      ...
      if (awsCodeCommitApiTaskResponse.getCommandExecutionStatus() == CommandExecutionStatus.SUCCESS) {
        log.info("Successfully confirmed trigger subscription event for accountId:{}, orgId: {}, projectId: {}, "
                + "sourceRepoType: {}",
        ...
        log.error(format("Failed to confirm trigger subscription event for accountId: %s, orgId: %s, projectId: %s, "
                + "sourceRepoType: %s, with error message: %s",
            event.getAccountId(), event.getOrgIdentifier(), event.getProjectIdentifier(), event.getSourceRepoType(),
      ...
      log.error(
          format("Failed to confirm trigger subscription event for accountId:%s, orgId:%s, projectId:%s, "
                  + "sourceRepoType: %s",
              event.getAccountId(), event.getOrgIdentifier(), event.getProjectIdentifier(), event.getSourceRepoType()),
          e);
      return WebhookEventProcessingResult.builder()
          .mappedToTriggers(false)
          .responses(Collections.singletonList(TriggerEventResponseHelper.toResponse(
              TRIGGER_CONFIRMATION_FAILED, event, null, TriggerEventResponseHelper.extractErrorMessage(e), null)))
          .build();
    }
  }

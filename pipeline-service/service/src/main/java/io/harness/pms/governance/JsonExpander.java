...
public class JsonExpander {
  ...
  public Set<ExpansionResponseBatch> fetchExpansionResponses(
    ...
    List<String> failedToExpandServices = Collections.synchronizedList(new ArrayList<>());
      ...
      for (ExpansionRequestBatch expansionRequestBatch : expansionRequestBatches.get(module)) {
        completableFutures.supplyAsync(() -> {
          try {
            ...
            failedToExpandServices.add(module.toString());
            log.error("Error fetching JSON expansion for module: {}", module);
            throw ex;
          }
        });
      }
    ...
    try {
      ...
      String servicesWithError = failedToExpandServices.toString();
      log.error("Error fetching JSON expansion responses from services: " + servicesWithError + " "
              + ExceptionUtils.getMessage(ex),
          ex);
      throw new UnexpectedException("Error fetching JSON expansion responses from services: " + servicesWithError, ex);
    }
  }

...
import static io.harness.pms.approval.UnifiedApprovalConstants.JIRA_TICKET_PLUGIN_ENV;
...
public class JiraApprovalCallback extends AbstractApprovalCallback implements PushThroughNotifyCallback {
  ...
  protected void updateOutputVariablesInApprovalInstance(Map<String, String> outVars, ApprovalInstance instance) {
    approvalInstanceService.updateOutputVariablesInApprovalInstance((JiraApprovalInstance) instance, outVars);
  }
  ...
  private void pushInternal(Map<String, ResponseData> response) {
    ...
    try {
      ...
      if (HarnessYamlVersion.isV1(AmbianceUtils.getPipelineVersion(ambiance))) {
        ...
        String ticketJson = pluginOutVars.get(JIRA_TICKET_PLUGIN_ENV);
        JiraIssueNG ticket = isNotEmpty(ticketJson)
            ? approvalUtils.deserializeOutputVariable(ticketJson, JiraIssueNG.class)
            : JiraIssueNG.builder().build();
        jiraTaskNGResponse = JiraTaskNGResponse.builder().issue(ticket).build();
      } else {
        if (!(responseData instanceof JiraTaskNGResponse)) {
          throw new InvalidRequestException("Invalid response data type for Jira Approval.");
        }
      ...
      if (isNull(jiraTaskNGResponse.getIssue())) {
        ...
        String errorMessage = String.format(
            "Invalid issue key: %s. This could also be caused by expired or invalid Jira connector credentials",
            instance.getIssueKey());

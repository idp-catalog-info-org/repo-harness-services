...
public class OrchestrationEndTagsResolveHandler implements OrchestrationEndObserver, AsyncInformObserver {
  ...
  public void onEnd(Ambiance ambiance, Status endStatus) {
    ...
    if (pipelineExecutionSummaryEntity != null
      ...
      List<NGTag> filteredTags =
          resolvedTags.stream().filter(tag -> !"null".equals(tag.getKey())).collect(Collectors.toList());
      Update update = new Update().set(PlanExecutionSummaryKeys.tags, filteredTags);

...
import io.harness.expression.celcustomfunctor.ConfigFileCelFunctor;
...
@AllArgsConstructor
@Slf4j
public class UnifiedConfigFileFunctor extends LateBindingMap implements ExpressionFunctor, ConfigFileCelFunctor {
  private static final String CONFIG_FILE_IDENTIFIER_REFERENCE_DELIMITER = ":";
  public static final String CONFIG_FILES_OUTPUT = "configFiles";
  ...
  private String getConfigFileContent(Ambiance ambiance, ConfigFileOutcomeLite configFileOutcome, String reference,
    ...
    if (ParameterField.isNotNull(configFileOutcome.getFiles())
      ...
      if (FUNCTOR_BASE64_METHOD_NAME.equals(methodName)) {
        String configFileIdentifierWithReference = configFileIdentifier;
        if (isNotEmpty(reference)) {
          configFileIdentifierWithReference = configFileIdentifierWithReference + ":" + reference;
        }
        return "${{configFile.getAsBase64Internal(${{configFile.getAsString('" + configFileIdentifierWithReference
            + "')}})}}";
      }
  ...
  private String getSecretFileContentAsString(Ambiance ambiance, final String ref) {
    return "${ngSecretManager.obtain(\"" + ref + "\", " + ambiance.getExpressionFunctorToken() + ")}";
  }
  ...
  private void validateGitStoreConfigFilesWithoutReference(
    ...
    if (isNotEmpty(configGitFileList) && configFilePathIndex != null
      ...
      throw new InvalidArgumentsException(format("Found config file Path reference out of bound for, "
              + "configFileIdentifier: %s. Expected between 0 and %d. but found %d",
  ...
  private ConfigFileOutcomeLite getConfigFileOutcome(Ambiance ambiance, String configFileIdentifier) {
    ...
    return ConfigFileOutcomeLite.convertToLiteUnified(objectNode.get(configFileIdentifier));
  }

  public Optional<String> getConfigFilesOutcome(Ambiance ambiance) {
    OptionalOutcome configFilesOutcome =
        outcomeService.resolveOptional(ambiance, RefObjectUtils.getOutcomeRefObject(CONFIG_FILES_OUTPUT));

  ...
  public Object bind() {
    ...
    try {
      Optional<OrganizationResponse> resp =
          NGRestUtils.getResponse(organizationClient.getOrganization(orgIdentifier, accountId));
      if (HarnessYamlVersion.isV1(ambiance.getMetadata().getHarnessVersion())) {
        return MapperUtils.toMapViaJsonString(resp.map(OrganizationResponse::getOrganization).orElse(null));
      } else {

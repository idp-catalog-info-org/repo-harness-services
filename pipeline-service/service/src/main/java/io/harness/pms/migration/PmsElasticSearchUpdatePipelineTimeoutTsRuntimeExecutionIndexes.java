...
@Slf4j
public class PmsElasticSearchUpdatePipelineTimeoutTsRuntimeExecutionIndexes implements NGMigration {
    ...
    if (elasticsearchClient != null) {
      try {
        PutMappingResponse response = elasticsearchClient.putMapping(
            PipelineSearchUtils.buildPutMappingRequestForDateType(PMS_RUNNING_EXECUTIONS_INDEX, FIELD_NAME));
        ...
        throw new InternalServerErrorException(
            String.format("Unable to update the index %s in elasticsearch with %s field", PMS_RUNNING_EXECUTIONS_INDEX,
                FIELD_NAME),
            exception);
      }
    } else {

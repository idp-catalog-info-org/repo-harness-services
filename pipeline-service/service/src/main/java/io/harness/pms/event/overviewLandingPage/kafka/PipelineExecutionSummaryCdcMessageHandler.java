...
/**
 * CDC Kafka {@link MessageHandler} that bridges Avro change events from Kafka Connect to
 * the existing idempotent {@link PipelineExecutionSummaryChangeEventHandler}.
 *
 ...
 * <p>The downstream handler is idempotent (INSERT … ON CONFLICT DO UPDATE + DELETE by id), so
 ...
 * on a poison pill. This matches the behavior of the prior (deleted) {@code CdcKafkaAvroConsumer}.
 *
 ...
 * For {@code planExecutionsSummary}, {@code _id} is the {@code planExecutionId} string (UUID),
 * so the synthesized key matches what Debezium's JsonConverter would have produced.
 */
@OwnedBy(PIPELINE)
@Singleton
@Slf4j
public class PipelineExecutionSummaryCdcMessageHandler implements MessageHandler<GenericRecord> {
  ...
  private final PipelineExecutionSummaryChangeEventHandler eventHandler;
  private final FeatureFlagService featureFlagService;

  @Inject
  public PipelineExecutionSummaryCdcMessageHandler(
      PipelineExecutionSummaryChangeEventHandler eventHandler, FeatureFlagService featureFlagService) {
  ...
  public void onMessage(GenericRecord message, Map<String, String> metadata, Map<String, Object> metricInfo) {
    if (!isFeatureFlagEnabled()) {
      log.info("[CDC-Kafka] FF PIPE_CDC_KAFKA_PLAN_EXECUTIONS_SUMMARY is OFF; committing offset without processing");
      return;
    }
    if (message == null) {
      log.warn("[CDC-Kafka] Received null Avro record for planExecutionsSummary; skipping");
      return;
    }
    ...
    if (id.isEmpty()) {
      log.warn(
          "[CDC-Kafka] planExecutionsSummary event has empty _id; optype={} — skipping to avoid bad DB write", optype);
      return;
    }
    ...
    log.info("[CDC-Kafka] planExecutionsSummary event received: id={}, optype={}, ts={}ms", id, optype, timestamp);
    ...
    for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        boolean ok = eventHandler.handleEvent(event);
        if (ok) {
          log.info("[CDC-Kafka] planExecutionsSummary event handled successfully: id={}, optype={}", id, optype);
        } else {
          log.warn("[CDC-Kafka] planExecutionsSummary event handler returned false for id={}, optype={}", id, optype);
        }
        return;
      } catch (Exception e) {
        if (attempt < MAX_RETRIES) {
          log.warn("[CDC-Kafka] planExecutionsSummary event processing failed (attempt {}/{}), retrying: "
                  + "id={}, optype={}, error={}",
              attempt, MAX_RETRIES, id, optype, e.getMessage());
          try {
            Thread.sleep(RETRY_BACKOFF_MS * attempt);
          } catch (InterruptedException ie) {
            Thread.currentThread().interrupt();
            log.error("[CDC-Kafka] Retry interrupted for planExecutionsSummary event id={}, optype={}", id, optype);
            return;
          }
        } else {
          log.error("[CDC-Kafka] planExecutionsSummary event processing failed after {} attempts: "
                  + "id={}, optype={}, error={}",
              MAX_RETRIES, id, optype, e.getMessage(), e);
        }
      }
    }
  }
  ...
  boolean isFeatureFlagEnabled() {
    try {
      return featureFlagService != null
          && featureFlagService.isGlobalEnabled(FeatureName.PIPE_CDC_KAFKA_PLAN_EXECUTIONS_SUMMARY);
    } catch (Exception e) {
      log.warn("[CDC-Kafka] Failed to evaluate feature flag {}, defaulting to disabled",
          FeatureName.PIPE_CDC_KAFKA_PLAN_EXECUTIONS_SUMMARY, e);
      return false;
    }
  }
  ...
  static String syntheticKey(String id) {
    try {
      ...
      log.warn("[CDC-Kafka] Failed to serialize synthetic key for id={}", id, e);
      return "{\"id\":\"\"}";
    }
  }
   ...
   * {@code ObjectMapper.readTree()} used by {@link PipelineExecutionSummaryChangeEventHandler#createRecord}.
   *
  ...
  static String convertValueToJson(Object value) {
    ...
    try {
      ...
      log.warn("[CDC-Kafka] Failed to convert Avro value to JSON, falling back to toString()", e);
  ...
  private static Object toPlainObject(Object value) {
    ...
    if (value instanceof Collection) {
      return ((Collection<?>) value)
          .stream()
          .map(PipelineExecutionSummaryCdcMessageHandler::toPlainObject)

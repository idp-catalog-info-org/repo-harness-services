...
public class PipelineRollbackStagePlanCreator implements PartialPlanCreator<PipelineRollbackStageNode> {
  ...
  public PipelineRollbackStageNode getFieldObject(YamlField field) {
    try {
      return YamlUtils.read(field.getNode().toString(), PipelineRollbackStageNode.class);
    } catch (IOException e) {
      throw new InvalidYamlException(
          "Unable to parse pipeline rollback stage yaml. Please ensure that it is in correct format", e);
    }
  }

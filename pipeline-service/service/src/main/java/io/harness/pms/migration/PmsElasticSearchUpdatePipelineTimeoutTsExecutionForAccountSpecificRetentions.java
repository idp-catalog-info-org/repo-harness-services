...
@Slf4j
public class PmsElasticSearchUpdatePipelineTimeoutTsExecutionForAccountSpecificRetentions implements NGMigration {
  ...
  public final String FIELD_NAME = "pipelineTimeoutTs";
      ...
      try (Stream<DataRetentionEntity> stream = pipelineRetentionService.getAllWithSearchSettingsFromSecondary()) {
        ...
        while (iterator.hasNext()) {
          ...
          if (dataRetentionEntity.getDataRetentionSettings().getDataRetentionPeriod() != null) {
            ...
            pipelineSearchService.updateIndexAlias(
                accountIdentifier, FIELD_NAME, pipelineSearchIndexRetentionPeriods, PipelineSearchFieldType.DATE);
          }
        }
      } catch (Exception ex) {
        throw new InternalServerErrorException(String.format("[ELASTIC_SEARCH]: Could not update the index alias for "
                                                       + "account: %s specific retention %s in elasticsearch",

...
public class AsyncFilterCreationDispatcher implements Runnable {
  ...
  public AsyncFilterCreationDispatcher(PMSPipelineServiceHelper pmsPipelineServiceHelper,
      ...
      PmsFeatureFlagHelper pmsFeatureFlagHelper, ScopeInfoClient scopeInfoClient) {
          ...
          ScopeInfo scopeInfo =
              NGRestUtils
                  .getResponse(scopeInfoClient.getScopeInfos(
                      pipelineEntity.get().getAccountIdentifier(), Set.of(pipelineEntity.get().getParentUniqueId())))
                  .get(pipelineEntity.get().getParentUniqueId())
                  .orElseThrow();

          try {
            SecurityContextBuilder.setContext(new ServicePrincipal(PIPELINE_SERVICE.getServiceId()));
            SourcePrincipalContextBuilder.setSourcePrincipal(new ServicePrincipal(PIPELINE_SERVICE.getServiceId()));
            PipelineGitXHelper.setupGitParentEntityDetails(scopeInfo.getAccountIdentifier(),
                scopeInfo.getOrgIdentifier(), scopeInfo.getProjectIdentifier(), pipelineEntity.get().getConnectorRef(),
                pipelineEntity.get().getRepo());
            updatedPipelineEntity = pmsPipelineServiceHelper.updatePipelineInfo(
                pipelineEntity.get(), pipelineEntity.get().getHarnessVersion(), scopeInfo, true);
            ...
            if (updatedResult == null) {
              throw new InvalidRequestException(String.format(
                  ...
                  pipelineEntity.get().getIdentifier(), scopeInfo.getProjectIdentifier(),
                  scopeInfo.getOrgIdentifier()));
            }
          } catch (Exception e) {
            log.error(
                String.format("Async Filter creation failed for Pipeline %s in account - %s, org - %s, project - %s",
                    pipelineEntity.get().getIdentifier(), scopeInfo.getAccountIdentifier(),

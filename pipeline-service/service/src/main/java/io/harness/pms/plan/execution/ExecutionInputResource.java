...
@Tag(name = "Execution Input", description = "APIs for runtime execution input on pipeline steps and stages.")
...
public class ExecutionInputResource {
  ...
  listVariablesDuringExecution(
    ...
    if (planExecutionMetadataOptional.isPresent()) {
      VariableMergeServiceResponse variablesResponse = variableCreatorMergeService.createVariablesResponses(
          planExecutionMetadataOptional.get().getYaml(), false, accountId);

...
public class PopulateAmbianceFieldsAtFirstLevelInApprovalInstancesMigration implements NGMigration {
  ...
  public void migrate() {
    ...
    try {
      ...
      try (Stream<ApprovalInstance> stream = mongoTemplate.stream(query, ApprovalInstance.class)) {
        Iterator<ApprovalInstance> approvalInstancesIterator = stream.iterator();
        while (approvalInstancesIterator.hasNext()) {
          ApprovalInstance approvalInstance = approvalInstancesIterator.next();
          // projectId and orgId fields are not removed, this job should be entirely removed and moved to NOOP.
          // as part of deprecation of project and orgId
          if (!isEmpty(approvalInstance.getAccountId()) && !isEmpty(approvalInstance.getOrgIdentifier())
            ...
            log.info(String.format("%s Skipping since approval instance with identifier %s already has ambiance fields "
                    + "populated at first-level",

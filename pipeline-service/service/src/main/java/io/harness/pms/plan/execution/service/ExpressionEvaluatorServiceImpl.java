...
public class ExpressionEvaluatorServiceImpl implements ExpressionEvaluatorService {
  ...
  public ExpressionEvaluationDetailDTO evaluateExpression(String planExecutionId, String yaml) {
    try {
      ...
      log.info(String.format(
          "Unresolved Expression FQNs for planExecutionId: %s, %s", planExecutionId, unresolvedAmbianceFqns));
      // This fetches all the leaf node executions for the given plan execution id.
      Stream<NodeExecution> nodeExecutionsStream =
      ...
      Map<String, Ambiance> fqnToAmbianceMap =
          getFQNToAmbianceMap(nodeExecutionsStream.iterator(), unresolvedAmbianceFqns);
      ...
      log.error("Exception occurred while evaluating expression for planExecutionId: " + planExecutionId, ex);
      throw new InvalidRequestException("Exception occurred while evaluating expression", ex);
    }
  }
  ...
  private String fqnForAmbiance(String fqn) {
    ...
    int end = indexToNearestGroup + 1;
    if (qualifiedName.get(indexToNearestGroup).equals("pipeline")) {
      end--;
    }
    // Add back the

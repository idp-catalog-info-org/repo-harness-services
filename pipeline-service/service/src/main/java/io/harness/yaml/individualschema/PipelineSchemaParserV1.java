...
public class PipelineSchemaParserV1 extends BasePipelineSchemaParser {
  ...
  static final Set<String> STEP_TYPES_THAT_NEED_NODE_GROUP_DIFFERENTIATOR = Set.of(YAMLFieldNameConstants.GROUP);
    ...
    if (currentFqn.endsWith(STEP_NODE_REF_SUFFIX_UNDER_EXECUTIONS_WRAPPER_CONFIG)) {
      ...
      if (EmptyPredicate.isNotEmpty(stepType)) {
        return ObjectNodeWithMetadata.builder()
            ...
            .nodeType(stepType)
            .objectNode(objectNode)
            .nodeGroupDifferentiator(getNodeGroupDifferentiatorForStep(currentFqn, stepType))
            .build();
      }
    }
    return null;
  ...
  private String getNodeGroupDifferentiatorForStep(String currentFqn, String stepType) {
    String nodeGroupDifferentiator = "";
    if (STEP_TYPES_THAT_NEED_NODE_GROUP_DIFFERENTIATOR.contains(stepType)) {
      String[] fqnComponents = currentFqn.split("/");
      if (fqnComponents.length > 5) {
        nodeGroupDifferentiator = fqnComponents[4];
      }
    }
    return nodeGroupDifferentiator;
  }
}

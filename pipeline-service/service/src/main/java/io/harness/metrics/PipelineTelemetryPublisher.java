  ...
  public void recordTelemetry() {
    ...
    try {
      ...
      if (EmptyPredicate.isNotEmpty(accountId) || !accountId.equals(GLOBAL_ACCOUNT_ID)) {
        ...
        // Community devs only
        if (pmsFeatureFlagHelper.isEnabled(accountId, FeatureName.PIPE_ENABLE_ELASTIC_SEARCH)) {
          // Passing null to include all entries under the account scope
          List<Query> matchQueries = PipelineSearchHelper.getScopeQuery(accountId, null);
          totalPipelinesExecuted = pipelineSearchService.getExecutionSummariesCount(accountId,
              ElasticSearchQueryBuilder.buildNestedQuery(
                  CONSTANT_SCORE, null, ElasticSearchQueryBuilder.buildCombinedQuery(MUST_MATCH_ALL, matchQueries)));

          matchQueries.add(ElasticSearchQueryBuilder.buildSingleValueComparisonQuery(GREATER_THAN,
              PipelineSearchExecutionSummaryDTOKeys.createdAt, System.currentTimeMillis() - MILLISECONDS_IN_A_DAY));
          pipelinesExecutedInADay = pipelineSearchService.getExecutionSummariesCount(accountId,
              ElasticSearchQueryBuilder.buildNestedQuery(
                  CONSTANT_SCORE, null, ElasticSearchQueryBuilder.buildCombinedQuery(MUST_MATCH_ALL, matchQueries)));
        } else {

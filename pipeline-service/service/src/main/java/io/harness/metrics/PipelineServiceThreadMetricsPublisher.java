...
public class PipelineServiceThreadMetricsPublisher implements MetricsPublisher {
  ...
  private static final Set<String> metricFilters =
      Set.of("Pipeline", "EngineExecutor", "SdkResponse", "VariableCreator", "FilterCreator", "ObserverExecutor",
          "OrchestrationVisualization", "Cd", "Ci", "Cv", "Sto", "CustomWebhook");
  ...
  private void recordGauge(String metricName, Gauge gauge) {
    try (DwThreadMetricContext ignore =
             new DwThreadMetricContext(NAMESPACE, CONTAINER_NAME, SERVICE_NAME, extractThreadName(metricName))) {
      ...
      recordMetric(extractMetricName(metricName), value);
    }
  }

  // metric name from module will be of format threadName_tasks_queued_count hence part before first _ is thread name
  // and after that is real metric name
  private String extractThreadName(String metricName) {
    return metricName.substring(0, metricName.indexOf('_'));
  }

  private String extractMetricName(String metricName) {
    return metricName.substring(metricName.indexOf('_') + 1);
  }

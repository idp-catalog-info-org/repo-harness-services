...
public class InputSetEntityReadHelperTest extends CategoryTest {
  ...
  public void testAggregateDelegatesToSecondaryMongoTemplate() {
    ...
    when(secondaryMongoTemplate.aggregate(same(aggregation), eq(InputSetEntity.class), eq(TestProjection.class)))
    ...
    verify(secondaryMongoTemplate, times(1))
        .aggregate(same(aggregation), eq(InputSetEntity.class), eq(TestProjection.class));
  }
  ...
  public void testAggregateUsesCallerProvidedOutputType() {
    ...
    when(secondaryMongoTemplate.aggregate(same(aggregation), eq(InputSetEntity.class), eq(String.class)))
        .thenReturn(mockedResults);

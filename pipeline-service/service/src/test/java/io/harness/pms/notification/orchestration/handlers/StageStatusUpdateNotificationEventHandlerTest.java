...
public class StageStatusUpdateNotificationEventHandlerTest extends CategoryTest {
  ...
  public void setUp() {
    ...
    stageStatusUpdateNotificationEventHandler.nodeExecutionService = nodeExecutionService;
  }
  ...
  public void testOnNodeStatusUpdate() {
    ...
    when(nodeExecutionService.getAmbiance(nodeExecution)).thenReturn(stageAmbiance);
    ...
    when(nodeExecutionService.getAmbiance(nodeExecution)).thenReturn(stepAmbiance);
    stageStatusUpdateNotificationEventHandler.onNodeStatusUpdate(nodeUpdateInfo);
  ...
  @Owner(developers = BRIJESH)
  @Category(UnitTests.class)
  public void testOnNodeStatusUpdateForUserActionRequired() {
    PlanNode stagePlanNode = PlanNode.builder()
                                 .uuid(generateUuid())
                                 .stepType(StepType.newBuilder().setStepCategory(StepCategory.STAGE).build())
                                 .identifier("dummyIdentifier")
    ...
    NodeExecution nodeExecution = NodeExecution.builder().ambiance(stageAmbiance).status(Status.INPUT_WAITING).build();
    NodeUpdateInfo nodeUpdateInfo = NodeUpdateInfo.builder().nodeExecution(nodeExecution).build();
    when(nodeExecutionService.getAmbiance(nodeExecution)).thenReturn(stageAmbiance);
    when(notificationHelper.getEventTypeForStage(nodeExecution)).thenReturn(Optional.empty());
    doNothing().when(notificationHelper).sendNotification(any(), any(), any(), any());
    ...
    verify(notificationHelper, times(1))
        .sendNotification(any(), pipelineEventTypeArgumentCaptor.capture(), any(), any());
    assertEquals(pipelineEventTypeArgumentCaptor.getValue(), PipelineEventType.WAITING_FOR_USER_ACTION);
  }
}

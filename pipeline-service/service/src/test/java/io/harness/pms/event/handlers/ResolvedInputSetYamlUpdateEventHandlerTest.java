  ...
  public void shouldTestHandleEvent() {
    ...
    OrchestrationEvent event =
        OrchestrationEvent.builder()
            ...
            .build();
    when(planExecutionMetadataService.findByPlanExecutionIdWithFieldsIncluded(any(), eq(planExecutionId),
             eq(Sets.newHashSet(PlanExecutionMetadataKeys.inputSetYaml, PlanExecutionMetadataKeys.harnessVersion))))
    ...
    when(yamlExpressionResolveHelper.resolveExpressionsInYaml(
             anyString(), any(ResolveInputYamlType.class), any(), anyString()))
    ...
    verify(pmsExecutionSummaryService, times(1))
        .updateResolvedUserInputSetYaml(planExecutionId, resolvedInputSetYaml, HarnessYamlVersion.V0);
  }

  @Test
  @Owner(developers = AVEESHA_JINDAL)
  @Category(UnitTests.class)
  public void shouldRevertSecretFunctorsBeforePersistingResolvedInputSetYaml() {
    String planExecutionId = UUIDGenerator.generateUuid();
    String inputSetYaml = "input-set-yaml";
    String resolvedWithSecretFunctor =
        "pipeline:\n  variables:\n    - name: key\n      value: ${ngSecretManager.obtain(\"account.secret\", 123)}\n";
    String expectedPersistedYaml =
        "pipeline:\n  variables:\n    - name: key\n      value: <+secrets.getValue('account.secret')>\n";
    OrchestrationEvent event =
        OrchestrationEvent.builder()
            .ambiance(
                Ambiance.newBuilder()
    ...
    when(yamlExpressionResolveHelper.resolveExpressionsInYaml(
             anyString(), any(ResolveInputYamlType.class), any(), anyString()))
        .thenReturn(resolvedWithSecretFunctor);
    resolvedInputSetYamlUpdateEventHandler.handleEvent(event);
    verify(pmsExecutionSummaryService, times(1))
        .updateResolvedUserInputSetYaml(planExecutionId, expectedPersistedYaml, HarnessYamlVersion.V0);
  }
}
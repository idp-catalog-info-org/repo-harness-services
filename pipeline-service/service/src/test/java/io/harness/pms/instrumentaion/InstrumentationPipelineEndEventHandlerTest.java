...
public class InstrumentationPipelineEndEventHandlerTest extends CategoryTest {
  ...
  public void testOnEnd() {
    ...
    Stream<NodeExecution> iterator =
        PipelineServiceTestHelper.createCloseableIterator(nodeExecutionList.iterator()).stream();
    ...
    doReturn(pipelineExecutionSummaryEntity)
        ...
        .getPipelineExecutionSummaryEntity("accountId", "planExecutionId", false);
    doReturn(notificationRulesList).when(notificationInstrumentationHelper).getNotificationRules(any());
    ...
    doReturn(null).when(planExecutionMetadataService).getWithFieldsIncludedFromSecondary(any(), any(), any());
    ...
    assertEquals(propertiesMap.get(PipelineInstrumentationConstants.ORGANIZATION_ID), "orgId");

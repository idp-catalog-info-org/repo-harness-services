...
public class PipelineOutboxEventHandlerTest extends CategoryTest {
  ...
  @Category(UnitTests.class)
  public void testPipelineMovedToRemote() throws JsonProcessingException {
    String accountIdentifier = randomAlphabetic(10);
    String orgIdentifier = randomAlphabetic(10);
    String projectIdentifier = randomAlphabetic(10);
    String identifier = randomAlphabetic(10);
    ...
    Scope scope = Scope.builder()
                      ...
                      .build();
    PipelineMoveConfigEvent pipelineMoveEvent =
        new PipelineMoveConfigEvent(scope, identifier, "test-pipe", oldYaml, newYaml);
    String eventData = objectMapper.writeValueAsString(pipelineMoveEvent);

    OutboxEvent outboxEvent = OutboxEvent.builder()
                                  .resource(pipelineMoveEvent.getResource())
                                  .resourceScope(pipelineMoveEvent.getResourceScope())
                                  .eventType(PipelineOutboxEvents.PIPELINE_MOVED_TO_REMOTE)
                                  .blocked(false)
  ...
  public void testAbort() throws JsonProcessingException {
    ...
    PipelineAbortEvent pipelineAbortEvent = new PipelineAbortEvent(accountIdentifier, orgIdentifier, projectIdentifier,
        parentUniqueId, identifier, "planExecutionId", triggeredInfo, 0);
    String eventData = objectMapper.writeValueAsString(pipelineAbortEvent);

    ExecutionOutboxEvent outboxEvent = ExecutionOutboxEvent.builder()
  ...
  public void testStartPipeline() throws JsonProcessingException {
    ...
    PipelineStartEvent pipelineStartEvent = new PipelineStartEvent(accountIdentifier, orgIdentifier, projectIdentifier,
        parentUniqueId, identifier, "planExecutionId", triggeredInfo, Instant.now().toEpochMilli(), 0);
  ...
  public void testEndPipeline() throws JsonProcessingException {
    ...
    PipelineEndEvent pipelineEndEvent = new PipelineEndEvent(accountIdentifier, orgIdentifier, projectIdentifier,
        parentUniqueId, identifier, "planExecutionId", triggeredInfo, "RUNNING", Instant.now().toEpochMilli() - 60,
  ...
  public void testStartStage() throws JsonProcessingException {
    ...
    StageStartEvent stageStartEvent = new StageStartEvent(accountIdentifier, orgIdentifier, projectIdentifier,
        parentUniqueId, identifier, "planExecutionId", "stageIdentifier", "Deploy", Instant.now().toEpochMilli(),
        "nodeExecutionId", 1, triggeredInfo);
  ...
  public void testEndStage() throws JsonProcessingException {
    ...
    StageEndEvent stageEndEvent = new StageEndEvent(accountIdentifier, orgIdentifier, projectIdentifier, parentUniqueId,
        identifier, "planExecutionId", "stageIdentifier", "Deploy", Instant.now().toEpochMilli() - 10,
        "nodeExecutionId", Instant.now().toEpochMilli(), "RUNNING", 1, triggeredInfo);

...
public class RollbackGraphGeneratorTest extends CategoryTest {
  ...
  public void setUp() {
    ...
    rollbackGraphGenerator = new RollbackGraphGenerator(
        executionService, retryExecutionHelper, pmsExecutionSummaryDtoUpdateHelper, scopeResolutionHelper);
  ...
  public void testCheckAndBuildRollbackGraph() {
    ...
    doReturn(childExecutionSummary).when(executionService).fetchExecutionSummary(accountId, rollbackId);
    ...
    when(PipelineExecutionSummaryDtoMapper.toDto(childExecutionSummary, null, false, false, null, null))
        .thenReturn(rollbackExecutionDTO);

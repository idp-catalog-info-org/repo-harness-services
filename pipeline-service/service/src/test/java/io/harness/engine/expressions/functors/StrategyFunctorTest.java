...
public class StrategyFunctorTest extends PipelineServiceTestBase {
  ...
  public void testMatrixFunctorWithStrategyMetadata() {
    ...
    for (String key : expected.keySet()) {
      assertThat(new StrategyFunctor(ambiance, nodeExecutionsCache, nodeExecutionInfoService, null).get(key))
  ...
  public void testMatrixFunctorWithoutStrategyMetadata() {
    ...
    Map<String, Object> expected = new HashMap<>();
    assertThat(new StrategyFunctor(ambiance, nodeExecutionsCache, nodeExecutionInfoService, null).get("matrix"))
        .isNotNull();
  }
  ...
  public void testCurrentStatus() {
    ...
    Ambiance ambiance = buildAmbiance(false);
    assertThat(new StrategyFunctor(ambiance, nodeExecutionsCache, nodeExecutionInfoService, null)
                   .get(StrategyFunctor.NODE_KEY))
        .isNotNull();
    StrategyNodeFunctor strategyNodeFunctor =
        (StrategyNodeFunctor) new StrategyFunctor(ambiance, nodeExecutionsCache, nodeExecutionInfoService, null)
            .get(StrategyFunctor.NODE_KEY);
    ...
    doReturn(Collections.singletonList(Status.SUCCEEDED))
        ...
        .findAllTerminalChildrenStatusOnly(any(), eq(true), eq(false));
    assertThat(new StrategyFunctor(ambiance, nodeExecutionsCache1, nodeExecutionInfoService, null)
